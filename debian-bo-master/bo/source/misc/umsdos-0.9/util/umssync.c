		...
		*/
		...
		if (strncmp(name,"__link",6)!=0){
			...
			fprintf (stderr
				...
				,fpath,newname);
			if (ioc.dosrename (name,newname)!=-1){
				ret = umssync_create (ioc,path,opt,newname,largest);
		...
		if (ret != -1){
			...
			ret = ioc.create (name,mode,fstat.st_mtime,fstat.st_mtime
	...
	struct umsdos_dirent &ent)
{
	...
	return is_dir ? ioc.urmdir (name) : ioc.uunlink (name);
}	
...
{
	UMSSYNC_INFO *i1 = (UMSSYNC_INFO *)p1;
	UMSSYNC_INFO *i2 = (UMSSYNC_INFO *)p2;
	...
	int &nbsub)
{
	int ret = -1;
	struct stat fstat;
	if (ioc.dosstat(dos_name,fstat)==-1){
		fprintf (stderr,"dosstat failed for entry %s\n",ums_name);
		ret = 0;
	}else if (S_ISDIR(fstat.st_mode)){
		if (nbsub < maxsub){
			ret = 0;
			UMS_DIRINFO *ptsub = tbsub + nbsub++;
					...
					// tbsub[]
{
	...
	if (!opt.condit || ioc.ispromoted()){
		hasemd = -1;
		if (opt.verbose) printf ("%s\n",path);

		ioc.initemd();
		
		UMSSYNC_INFO tb_udir[4000];
		int nb_udir = 0;
		/* First read the EMD file */
		UMSSYNC_INFO *pt_udir = tb_udir;
		while (ioc.ureaddir(pt_udir->uent,pt_udir->ment)!=-1){
		...
		pt_udir = tb_udir;
		pt_ddir = tb_ddir;
		UMSSYNC_INFO  *last_udir = tb_udir + nb_udir;
		struct dirent *last_ddir = tb_ddir + nb_ddir;
		ret = 0;
		...
		for (; pt_udir < last_udir && ret != -1; pt_udir++){
			if (pt_ddir == last_ddir){
				...
				if (cmp == 0){
					...
					ret = umssync_create (ioc,path,opt,pt_ddir->d_name
						,largest);
					if (ret != -1){
						toomanysub |= umssync_addsdir (ioc
		...
		for (; pt_ddir < last_ddir && ret != -1; pt_ddir++){
			ret = umssync_create (ioc,path,opt,pt_ddir->d_name,largest);
			if (ret != -1){
				toomanysub |= umssync_addsdir (ioc
					,pt_ddir->d_name,pt_ddir->d_name
					,opt.uid,opt.gid,tbsub,maxsub,nbsub);
		...
		if (opt.full_recurse > 0) {
			...
			while (ioc.readdir(cf)!=-1 && !toomanysub) {
				if (cf.d_name[0] != '.'){
					toomanysub |= umssync_addsdir (ioc
						,cf.d_name,cf.d_name
						,opt.uid,opt.gid
...
static int umssync_dodirs (
	...
	UMSSYNC_OPT & opt,
	int depth,
	UMS_DIRINFO tbsub[],	// Accumulator for subdirectories
							// This table is shared by all instance
							// of this function.
	int maxsub)
{
	int nbsub;
	UMSSYNC_OPT new_opt = opt;
	if (opt.inherit){
		new_opt.uid = dir_stat.st_uid;
		new_opt.gid = dir_stat.st_gid;
	}
	int hasemd;
	int ret = umssync_dodir (path,new_opt,tbsub,maxsub,nbsub,hasemd);
	if (ret != -1 && depth > 0){
		depth--;
		if (!hasemd)
		...
		// tbsub[].
		UMS_DIRINFO *endsub = tbsub + nbsub;
		int new_maxsub = maxsub - nbsub;
		UMS_DIRINFO *ptsub = tbsub;
		for (int i=0; i<nbsub && ret != -1; i++,ptsub++){
			char spath[PATH_MAX];
