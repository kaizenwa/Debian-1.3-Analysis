...
struct device **makedev(struct device **current, char *name)
{
  ...
  *current=malloc(sizeof(struct device));
  ...
  if (!(*current)->name) oops("malloc()");
  (*current)->fd=fd; (*current)->next=NULL;
  devcount++;
  return &((*current)->next);
}
...
int eventlist(int fd, char *buff, int buflen, int test, int readstep)
{
  ...
  while(1)
    ...
    if (pending==0 && !active) {active++; to.tv_sec=10; continue;}
    if (pending==0) return got; /* timeout */

    if (FD_ISSET(0,&selSet))
      { getchar(); if (active) return got; }
...
#define BAUD(i) (baudtab[(i)%4])
  ...
  if (argc==1) /* no cmdline, get all devices */
    ...
    f=popen("ls -l /dev/* | "
	    "awk '/^c/ && $5==\"10,\" || $NF ~ \"ttyS[0-3]$\" {print $NF}'",
	    "r");
    if (!f) oops("popen()");
    while (fgets(s,64,f))
      {
...
/*====================================== First of all, detect the device */
  ...
  while (devcount>1)
	...
	continue;
      if (pending==0 || FD_ISSET(fileno(stdin),&devSet)) {getchar(); break;}
    ...
    if (gotthem)
      for (nextdev=&devlist; *nextdev; /* nothing */ )
	{
	cur=*nextdev;
...
/*====================================== Use that info to discard protocols */
  ...
  for (nextitem=&list; *nextitem; /* nothing */)
    ...
    if (match<=0)
      { REMOVETYPE(cur,"incorrect button detected"); continue;}
    if (packetheads<got/(packetsize+2))
      { REMOVETYPE(cur,"too few events got decoded"); continue; }
 ...
 */
  ...
  for (nextitem=&list; *nextitem; /* nothing */)
    ...
    if (pending<0) { REMOVETYPE(cur,"spurious button reported"); continue; }
    printf("** type '%s' still possible\r\n",cur->this->name);
    nextitem=&(cur->next);
    }
  CHECKFAIL(typecount);
  ...
  for (nextitem=&list; *nextitem; /* nothing */)
    ...
    for (i=0;i<got;i++)
      ...
      if (   ((buf[i]  &(cur->this->proto)[0]) == (cur->this->proto)[1])
        ...
        if ((*(cur->this->fun))(&event,buf+i)==-1) continue;
        i+=packetsize-1;
        if (event.buttons && event.buttons!=GPM_B_MIDDLE)
          pending--;
	...
	if (pending<0)
	  { REMOVETYPE(cur,"spurious button reported"); continue; }
	if (pending>3)
	  {
	  noraw();
