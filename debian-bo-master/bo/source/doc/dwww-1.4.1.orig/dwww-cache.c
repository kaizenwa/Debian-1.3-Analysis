 ...
 * Version:	"(#)dwww:$Id: dwww-cache.c,v 1.1.1.1 1996/12/12 06:41:08 jim Exp $"
 ...
 */
#define SPOOL_DIR "/var/spool/dwww/"
...
static int help(char *type, char *location) { puts("help"); return 0; }
...
int main(int argc, char **argv) {
	...
	set_progname(argv[0], "dwww-cache");
...
static int list(char *type, char *location) {
	struct dochash hash;
	...
	data = hash.tab.data;
	sprintf(orig, "%s%s", prefs[data[i].pref], data[i].original);
	sprintf(buf, "%s%s", SPOOL_DIR, data[i].converted);
...
static int list_all(char *type, char *location) {
	struct dochash hash;
	struct cache_entry *data;
	int i, db;
	char buf[1024];
...
static int lookup(char *type, char *location) {
	struct dochash hash;
	struct cache_entry *data;
	int i, db;
	char buf[1024];
...
static int store(char *type, char *location) {
	...
	fd = create_new_file(SPOOL_DIR, "", buf);
	if (fd == -1) {
		errormsg(0, -1, "can't create cache file");
		return -1;
	}
	sprintf(full, "%s/%s", SPOOL_DIR, buf);
	
	if (copy_fd_to_fd(0, fd) == -1) {
		errormsg(0, -1, "can't copy stdin to %s", full);
		(void) unlink(full);
		return -1;
	}
	...
	if (i != -1) {
		data =  hash.tab.data;
		sprintf(oldname, "%s%s", SPOOL_DIR, data[i].converted);
		(void) unlink(oldname);
	}
...
static int clean(char *type, char *location) {
	struct dochash hash;
	struct cache_entry *data;
	char buf[1024];
	char orig[1024];
	...
	for (i = 0; i < hash.tab.used; ++i) {
		sprintf(orig, "%s%s", prefs[data[i].pref], data[i].original);
		sprintf(buf, "%s%s", SPOOL_DIR, data[i].converted);
		if (file_exists(orig) && file_exists(buf))
			data[j++] = data[i];
		else
			(void) unlink(buf);
	}
...
static int open_db_reading(void) {
	...
	db = open(CACHE_DB, O_RDONLY, 0);
	if (db == -1 && errno != ENOENT)
		errormsg(0, -1, "can't check cache database");
	
	/* fixme: create shared lock here */	
...
static int open_db_writing(void) {
	...
	/* fixme: lock the database exclusively */
...
static int write_db(int db, struct dochash *hash) {
	...
	if (ftruncate(db, 0) == -1) {
		errormsg(0, -1, "can't update truncate database");
		return -1;
	}
...
static int copy_fd_to_fd(int from, int to) {
	char buf[1024];
	int n;
...
static int create_new_file(char *prefix, char *suffix, char *full) {
	char buf[1024];
	int i, fd;
	...
	} while (fd == -1 && errno == EEXIST && i++ < INT_MAX-1);

	if (fd != -1)
		sprintf(full, "%d%s", i, suffix);
	return fd;
}
