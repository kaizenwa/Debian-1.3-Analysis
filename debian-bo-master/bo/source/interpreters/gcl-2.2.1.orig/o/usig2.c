	 ...
	 {SS1(p->free1[i],ad->tm_free);
	  if (p->free1[i])
	    { char *beg =  (char *) (p->free1[i]);
	      object x = (object)beg;
	      int amt = ad->tm_size;
	      SS1(p->free2[i],F_LINK(p->free1[i]));
	      ad->tm_nfree --;
		...
		{ x = (object) p->free2[i];
		  ...
		  x->d.m = 0;
		  bzero(beg+8,amt-8);
		  SS1(ad->tm_free,F_LINK(p->free2[i]));
		  ad->tm_nfree --;
		}
	      else
		{ SS1(ad->tm_free,F_LINK(p->free1[i]));
		}}
	}}
   }
 SS1(p->fcall,fcall);
...
#define MINN(a,b) (a<b?a :b)
 ...
 else { p->token_bufp= OUR_ALLOCA(p->token_st_dim);}
	 ...
	 {RS1(p->free1[i],ad->tm_free);
	  if (p->free1[i])
	    { char *beg =  (char *) (p->free1[i]);
	      object x = (object)beg;
	      int amt = ad->tm_size;
	      RS1(p->free2[i],F_LINK(p->free1[i]));
		...
		{ x = (object) p->free2[i];
		  if (x->d.m) error("should not be free");
		  x->d.m = FREE;
		  F_LINK(F_LINK(ad->tm_free)) = current_fl;
		  ad->tm_nfree += 2;
		}
	      else
		ad->tm_nfree =1;
