    ...
    Tk_Window tkwin;
{
	...
	(ClientData)cubiPtr);
#if (TK_MAJOR_VERSION == 3)
    Tk_ManageGeometry(tkwin, SlaveGeometryProc, (ClientData)cubiPtr);
#else
    ...
    char **argv;
{
	...
	if (partNum > rowColPtr->length) {
	    sprintf(tablePtr->interp->result,
		"%s index \"%c%d\" is out of range",
		rowColPtr->type, rowColPtr->type[0], partNum);
	    return TCL_ERROR;
	}
    ...
    char **argv;
{
    ...
    sprintf(interp->result, "%d,%d", rowIndex, colIndex);
    return TCL_OK;
}
    ...
    Table *tablePtr;		/* Table widget structure */
{
...
#if (TK_MAJOR_VERSION == 3)
	/*
	 * If the slave's parent window is not the master window, translate
	 * the master window's x and y coordinates to the coordinate system
	 * of the slave's parent.
	 */
	for (ancestor = tablePtr->tkwin; ancestor != Tk_Parent(cubiPtr->tkwin);
	...
	if (tablePtr->tkwin != Tk_Parent(cubiPtr->tkwin)) {
	    ...
	    /* XXXX Tk_MoveResizeWindow(cubiPtr->tkwin, x, y, winWidth, winHeight); */
	    if (!Tk_IsMapped(cubiPtr->tkwin)) {
...
Blt_TableInit(interp)
    Tcl_Interp *interp;
{
    static Blt_CmdSpec cmdSpec = { "table", TABLE_VERSION, TableCmd, };
