...
/*ARGSUSED*/
static int
NaturalOper(tkwin, interp, xPtr, yPtr, splXPtr, splYPtr, argc, argv)
    Tk_Window tkwin;		/* Main window of the interpreter. Used to
				 * process options using Tk_ConfigureWidget */
    Tcl_Interp *interp;
    Blt_Vector *xPtr, *yPtr, *splXPtr, *splYPtr;
    int argc;			/* Not used */
    char **argv;		/* Not used */
{
    double *work;
    ...
    work = (double *)malloc(sizeof(double) * xPtr->numValues * 9);
    if (work == NULL) {
	interp->result = "can't allocate working storage for spline";
	return TCL_ERROR;
    }
    result = NaturalSpline(xPtr->valueArr, yPtr->valueArr, xPtr->numValues,
	splXPtr->valueArr, splYPtr->valueArr, splXPtr->numValues, work);
    free((char *)work);
    if (result != TCL_OK) {
...
/* ARGSUSED */
static int
QuadraticOper(tkwin, interp, xPtr, yPtr, splXPtr, splYPtr, argc, argv)
    Tk_Window tkwin;		/* Main window of the interpreter. Used to
				 * process options using Tk_ConfigureWidget */
    Tcl_Interp *interp;
    Blt_Vector *xPtr, *yPtr, *splXPtr, *splYPtr;
    ...
    char **argv;
{
    ...
    /* allocate space for vectors used in calculation */
    work = (double *)malloc(xPtr->numValues * sizeof(double));
    if (work == NULL) {
	interp->result = "can't allocate work array";
	return TCL_ERROR;
    }
    result = QuadraticSpline(xPtr->valueArr, yPtr->valueArr, xPtr->numValues,
	splXPtr->valueArr, splYPtr->valueArr, splXPtr->numValues, work,
	epsilon);
    free((char *)work);
    if (result != 0) {
	sprintf(interp->result, "error #%d in \"%s\"", result, argv[2]);
	return TCL_ERROR;
    }
    return TCL_OK;
}
...
{
    {"natural", 1, (Blt_OperProc) NaturalOper, 6, 0,
	"x y splx sply ?option value?...",},
    {"quadratic", 1, (Blt_OperProc) QuadraticOper, 6, 0,
	"x y splx sply ?option value?...",},
};
    ...
    char **argv;
{
	 ...
	 */
	if (Blt_CreateVector(interp, argv[5], splX.numValues,
		&splY) != TCL_OK) {
	    return TCL_ERROR;
	}
    } else if (splX.numValues != splY.numValues) {
	/*
	 * The x and y vectors differ in size. Make the number of ordinates
	 * the same as the number of abscissas.
	 */

	if (Blt_ResizeVector(interp, argv[5], splX.numValues) != TCL_OK) {
	    return TCL_ERROR;
	}
	/* Re-get the vector, since its size has now changed */
	if (Blt_GetVector(interp, argv[5], &splY) != TCL_OK) {
	    return TCL_ERROR;
	}
    }
     ...
     */
    if (Blt_ResetVector(interp, argv[5], &splY, TCL_STATIC) != TCL_OK) {
	return TCL_ERROR;
    }
    return TCL_OK;
}
...
Blt_SplineInit(interp)
    Tcl_Interp *interp;
{
    static Blt_CmdSpec cmdSpec = { "spline", SPLINE_VERSION, SplineCmd, };
