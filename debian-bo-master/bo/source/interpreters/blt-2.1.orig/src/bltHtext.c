...
typedef struct HText {
    ...
    XFontStruct *fontPtr;	/* Font for normal text. May affect the size
    ...
    Position viewOrigin;	/* Position of viewport in virtual coordinates */
...
extern int TkMeasureChars _ANSI_ARGS_((XFontStruct *fontStructPtr,
	char *source, int maxChars, int startX, int maxX, int tabOrigin,
    ...
    int *indexPtr;
{
    ...
    charPos = TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
	textLength, curX, x, NO_FLAGS, &dummy);
#else
    charPos = TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
	textLength, curX, x, 0, NO_FLAGS, &dummy);
    ...
    int *indexPtr;		/* Where to store converted relief. */
{
  ...
  badIndex:
    ...
    Tcl_AppendResult(interp, "invalid index \"", string, "\": \
...
#if (TK_MAJOR_VERSION == 3)
    Tk_ManageGeometry(tkwin, SlaveGeometryProc, (ClientData)slavePtr);
#else
...
#if (TK_MAJOR_VERSION == 3)
	    cmdName = Tk_PathName(textPtr->tkwin);
#else
 ...
 */
static int
ReadFile(interp, fileName, bufferPtr)
    Tcl_Interp *interp;
    char *fileName;
    char **bufferPtr;
{
    ...
    int numBytes;
{
	...
	interp->result = "can't reallocate array of lines";
	return TCL_ERROR;
    }
    ...
    if (ResizeArray(&(textPtr->charArr), sizeof(char), count,
	    numBytes) != TCL_OK) {
	interp->result = "can't reallocate text character buffer";
	return TCL_ERROR;
    }
    ...
    char *fileName;
{
    ...
    if (fileName != NULL) {
	numBytes = ReadFile(interp, fileName, &buffer);
	if (numBytes < 0) {
	    return TCL_ERROR;
	}
    } else {
			         ...
			         * already have values for some fields. */
{
	...
	textPtr->reqWidth =
	    textPtr->numColumns * XTextWidth(textPtr->fontPtr, "0", 1);
    }
    ...
    Line *linePtr;
{
	...
	case JUSTIFY_TOP:
	    ascent = textPtr->fontPtr->ascent + slavePtr->padTop;
	    descent = slavePtr->cavityHeight - textPtr->fontPtr->ascent;
	    break;
	case JUSTIFY_CENTER:
	    ...
	    break;
	case JUSTIFY_BOTTOM:
	    ascent = slavePtr->cavityHeight - textPtr->fontPtr->descent;
	...
	if (textLength > 0) {
#if (TK_MAJOR_VERSION == 3)
	    TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
		textLength, x, 10000, TK_AT_LEAST_ONE, &newX);
#else
	    TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
		textLength, x, 10000, 0, TK_AT_LEAST_ONE, &newX);
#endif /* TK_MAJOR_VERSION == 3 */
	    slavePtr->precedingTextWidth = newX - x;
	    x = newX;
	}
	switch (slavePtr->justify) {
	case JUSTIFY_TOP:
	    y = maxAscent - textPtr->fontPtr->ascent;
	    break;
	case JUSTIFY_CENTER:
	    y = maxAscent - (slavePtr->cavityHeight + median) / 2;
	    break;
	case JUSTIFY_BOTTOM:
	    y = maxAscent + textPtr->fontPtr->descent - slavePtr->cavityHeight;
	    break;
	}
...
#if (TK_MAJOR_VERSION == 3)
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
	    textLength, x, 10000, NO_FLAGS, &newX);
#else
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + textStart,
	    textLength, x, 10000, 0, NO_FLAGS, &newX);
#endif /* TK_MAJOR_VERSION == 3 */
	x = newX;
    }
    ...
    TextSegment *segPtr;
{
...
#if (TK_MAJOR_VERSION == 3)
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + segPtr->textStart, textLength,
	    x, y + linePtr->baseline, NO_FLAGS);
#else
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + segPtr->textStart, textLength,
	    x, y + linePtr->baseline, tabOrigin, NO_FLAGS);
#endif /* TK_MAJOR_VERSION == 3 */
	return;
    }
    /*
...
#if (TK_MAJOR_VERSION == 3)
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + segPtr->textStart,
	    numChars, x, 10000, NO_FLAGS, &lastX);
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + segPtr->textStart, numChars, x,
	    y + linePtr->baseline, NO_FLAGS);
#else
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + segPtr->textStart,
	    numChars, x, 10000, tabOrigin, NO_FLAGS, &lastX);
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + segPtr->textStart, numChars, x,
	    y + linePtr->baseline, tabOrigin, NO_FLAGS);
#endif /* TK_MAJOR_VERSION == 3 */
	curPos = selectStart;
    }
...
#if (TK_MAJOR_VERSION == 3)
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + selectStart,
	    selectLength, lastX, 10000, NO_FLAGS, &nextX);
#else
	TkMeasureChars(textPtr->fontPtr, textPtr->charArr + selectStart,
	    selectLength, lastX, 10000, tabOrigin, NO_FLAGS, &nextX);
#endif /* TK_MAJOR_VERSION == 3 */
	width = (selectEnd == linePtr->textEnd)
	    ? textPtr->virtText.width - textPtr->viewOrigin.x - lastX :
	    nextX - lastX;
#if (TK_MAJOR_VERSION == 3)
	Tk_Fill3DRectangle(textPtr->display, draw, textPtr->selectBorder,
	    lastX, y + linePtr->baseline - textPtr->fontPtr->ascent,
	    width, TEXTHEIGHT(textPtr->fontPtr),
	    textPtr->selectBW, TK_RELIEF_RAISED);
	TkDisplayChars(textPtr->display, draw, textPtr->selectGC,
	    textPtr->fontPtr, textPtr->charArr + selectStart, selectLength,
	    lastX, y + linePtr->baseline, NO_FLAGS);
#else
	...
	TkDisplayChars(textPtr->display, draw, textPtr->selectGC,
	    textPtr->fontPtr, textPtr->charArr + selectStart, selectLength,
	    lastX, y + linePtr->baseline, tabOrigin, NO_FLAGS);
#endif /* TK_MAJOR_VERSION == 3 */
	lastX = nextX;
...
#if (TK_MAJOR_VERSION == 3)
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + curPos, numChars, lastX, y + linePtr->baseline,
	    NO_FLAGS);
#else
	TkDisplayChars(textPtr->display, draw, textPtr->drawGC, textPtr->fontPtr,
	    textPtr->charArr + curPos, numChars, lastX, y + linePtr->baseline,
...
#if (TK_MAJOR_VERSION == 3)
	Tk_OwnSelection(textPtr->tkwin, TextLostSelection, (ClientData)textPtr);
#else
    ...
    char **argv;
{
    ...
    sprintf(textPtr->interp->result, "%d", line);
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    sprintf(interp->result, "%d", x / textPtr->scrollX);
    return TCL_OK;
}
...
#else
    first = IndexSearch(textPtr, searchFirst, 0, textPtr->numLines - 1);
    last = IndexSearch(textPtr, searchLast, 0, textPtr->numLines - 1);
    for (i = first; i <= last; i++) {
	startPtr = textPtr->charArr + textPtr->lineArr[i].textStart;
	endPtr = textPtr->charArr + textPtr->lineArr[i].textEnd + 1;
	if (i == first) {
	    startPtr = textPtr->charArr + searchFirst;
	}
	if (i == last) {
	    endPtr = textPtr->charArr + searchLast;
	}
	saved = *endPtr;
	*endPtr = '\0';		/* Make the line a string by changing the
				 * '\n' into a NUL byte before searching */
	match = Tcl_RegExpMatch(interp, startPtr, argv[2]);
	*endPtr = saved;
	if (match < 0) {
	    return TCL_ERROR;
	} else if (match > 0) {
	    matchStart = startPtr - textPtr->charArr;
	    matchEnd = endPtr - textPtr->charArr;
	    break;
	}
    }
#endif /* (TCL_MAJOR_VERSION >= 7) && (TCL_MINOR_VERSION >= 4) */
    if (match > 0) {
	sprintf(interp->result, "%d %d", matchStart, matchEnd);
    } else {
	Tcl_ResetResult(interp);
    }
    return TCL_OK;
    ...
    char **argv;
{
    ...
    sprintf(interp->result, "%d", textIndex);
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    sprintf(interp->result, "%d.%d", lineNum, charPos);
    return TCL_OK;
}
...
static Blt_OperSpec operSpecs[] =
{
    ...
    {"xview", 1, (Blt_OperProc) XViewOper, 2, 3, "?pos?",},
    {"yview", 1, (Blt_OperProc) YViewOper, 2, 3, "?pos?",},
};
...
#if (TK_MAJOR_VERSION == 3)
    Tk_CreateSelHandler(tkwin, XA_STRING, TextSelectionProc,
...
Blt_HtextInit(interp)
    Tcl_Interp *interp;
{
    static Blt_CmdSpec cmdSpec = { "htext", HTEXT_VERSION, TextCmd, };
