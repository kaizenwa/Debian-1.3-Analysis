    ...
    int size;
{
	...
	free((char *)vPtr->vector.valueArr);
    }
	...
	newArr = (double *)calloc(sizeof(double), size);
	if (newArr == NULL) {
    ...
    double **dataPtrPtr;
{
	...
	if (dataArr == NULL) {
	    interp->result = "can't allocate new vector";
	    goto badList;
	}
	dataPtr = dataArr;
	for (i = 0; i < numElem; i++) {
    ...
    int offset;			/* not used */
{
    ...
    if (numElem & 1) {
	interp->result = "odd number of data points";
	free((char *)newArr);
	return TCL_ERROR;
    }
    newSize = numElem / 2;
    ...
    char **argv;		/* List of element names */
{
	...
	activeArr = (int *)malloc(sizeof(int) * numActiveIndices);
	if (activeArr == NULL) {
	...
	free((char *)elemPtr->activeIndexArr);
    }
    ...
    char *argv[];
{
    ...
    if (Blt_ConfigModified(elemPtr->configSpecs, "-*data", "-map*",
	    (char *)NULL)) {
	/*
	 * Data points or axes have changed: Reset the axes (may
    ...
    char **argv;		/* Element name */
{
    ...
    interp->result = NameOfElementType(elemPtr);
    return TCL_OK;
}
