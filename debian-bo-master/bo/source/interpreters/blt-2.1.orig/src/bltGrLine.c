...
none, circle, square, diamond, plus, cross, splus, scross, or triangle",
	    (char *)NULL);
	return TCL_ERROR;
    }
    return TCL_OK;
    ...
    ClosestInfo *infoPtr;	/* Info about closest point in element */
{
	...
	d1 = hypot((double)(infoPtr->x - p1.x), (double)(infoPtr->y - p1.y));
	d2 = hypot((double)(infoPtr->x - p2.x), (double)(infoPtr->y - p2.y));
    ...
    AxisPair *pairPtr;
{
	...
	if ((linePtr->flags & LINE_INIT_SYMBOL_SCALE) ||
	    ...
	    symbolSize *= BLT_MIN(scaleX, scaleY);
	}
    }
    /*
    ...
    int length;			/* Number of points forming the trace */
{
    ...
    size = sizeof(Trace) + (sizeof(XPoint) * (length - 1));
    tracePtr = (Trace *)calloc(1, size);
    if (tracePtr == NULL) {
    ...
    Element *elemPtr;		/* Element component record */
{
    ...
    coordArr = (Coordinate *) malloc(numDataPoints * sizeof(Coordinate));
    if (coordArr == NULL) {
...
#define S_RATIO		0.886226925452758
    ...
    case SYMBOL_SQUARE:
	{
	    ...
	    s = r2 + r2;
	    rectArr = (XRectangle *)calloc(numPoints, sizeof(XRectangle));
	    rectPtr = rectArr;
    ...
    case SYMBOL_SCROSS:
	{
	    ...
	    segArr = (XSegment *)calloc(numPoints * 2, sizeof(XSegment));
	    segPtr = segArr;
	    for (i = 0; i < numPoints; i++) {
    ...
    case SYMBOL_CROSS:
	{
	    ...
	    coordArr = (XPoint *)calloc(numPoints * 13, sizeof(XPoint));
	    coordPtr = coordArr;
	    for (i = 0; i < numPoints; i++) {
		for (n = 0; n < 13; n++) {
    ...
    case SYMBOL_DIAMOND:
	{
	    ...
	    coordArr = (XPoint *)calloc(numPoints * 5, sizeof(XPoint));
	    coordPtr = coordArr;
	    for (i = 0; i < numPoints; i++) {
		for (n = 0; n < 5; n++) {
		    coordPtr->x = template[n].x + pointArr[i].x;
...
#define COS30		0.86602540378443871
	    ...
	    coordArr = (XPoint *)calloc(numPoints * 4, sizeof(XPoint));
	    coordPtr = coordArr;
	    for (i = 0; i < numPoints; i++) {
		for (n = 0; n < 4; n++) {
		    coordPtr->x = template[n].x + pointArr[i].x;
    ...
    XPoint **pointPtrPtr;
{
    ...
    pointArr = (XPoint *)malloc(sizeof(XPoint) * linePtr->numActiveIndices);
    if (pointArr == NULL) {
...
Blt_LineElement()
{
    ...
    linePtr->drawNormalProc = DrawNormalLine;
    linePtr->drawActiveProc = DrawActiveLine;
    linePtr->limitsProc = GetLineLimits;
    linePtr->closestProc = ClosestLine;
    linePtr->coordsProc = ComputeLineCoordinates;
    linePtr->printNormalProc = PrintNormalLine;
    linePtr->printActiveProc = PrintActiveLine;
