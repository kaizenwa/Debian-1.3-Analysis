...
/* #define DEF_LEGEND_BG_COLOR	   	STD_NORMAL_BG_COLOR */
/* #define DEF_LEGEND_BG_MONO		STD_NORMAL_BG_MONO */
    ...
    Graph *graphPtr;
{
	...
	Tk_Draw3DRectangle(graphPtr->display, graphPtr->pixwin,
	    graphPtr->border, x, y, width, height, legendPtr->borderWidth,
	    legendPtr->relief);
#else
...
#if (TK_MAJOR_VERSION == 3)
	    Tk_Fill3DRectangle(graphPtr->display, graphPtr->pixwin,
    ...
    char *argv[];
{
	...
	if (graphPtr->flags & REDRAW_PENDING) {
		...
		Tk_DoWhenIdle((Tk_IdleProc *) DrawLegend, (ClientData)graphPtr);
		graphPtr->flags |= DRAW_LEGEND;
	    }
	}
    }
