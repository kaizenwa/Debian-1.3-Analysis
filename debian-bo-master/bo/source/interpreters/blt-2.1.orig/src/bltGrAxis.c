...
typedef struct {
    ...
    XFontStruct *titleFontPtr;	/* Font used to the axis title. */
    ...
    XFontStruct *tickFontPtr;	/* Font used to draw tick labels. */
...
#define DEF_AXIS_TICK_FONT	"*-Courier-Bold-R-Normal-*-100-*"
    ...
    char **exprArr;
{
	...
	valueArr = (double *)malloc(sizeof(double) * numExprs);
	if (valueArr == NULL) {
    ...
    Tcl_FreeProc **freeProcPtr;
{
    ...
    *freeProcPtr = (Tcl_FreeProc *)free;
    result = strdup(Tcl_DStringValue(&valueList));
				 ...
				 * containing the formatted label */
{
	 ...
	 */
	strncpy(string, interp->result, MAX_TICK_LABEL);
    ...
    double step;
{
	...
	if (ticksPtr->numTicks > STATIC_TICK_SPACE) {
	    valueArr = (double *)malloc(sizeof(double) * ticksPtr->numTicks);
	    if (valueArr == NULL) {
    ...
    int flags;
{
	...
	sprintf(graphPtr->interp->result,
	    ...
	    axisNames[axisPtr->type], axisPtr->minLimit, axisPtr->maxLimit);
	return TCL_ERROR;
    }
    ...
    GraphAxis *axis;
{
	...
	free((char *)axisPtr->majorTicks.valueArr);
    }
    if (axisPtr->minorTicks.valueArr != axisPtr->minorTicks.defSpace) {
	free((char *)axisPtr->minorTicks.valueArr);
    }
    free((char *)axisPtr);
}
    ...
    Axis *axisPtr;
{
	...
	axisPtr->titleY = Round(graphPtr->lly + (double)(axisPtr->height + TITLE_PAD));
	...
	axisPtr->titleX = Round(graphPtr->urx + (double)(axisPtr->width + TITLE_PAD));
	axisPtr->titleY = Round((graphPtr->lly + graphPtr->ury) * 0.5);
    ...
    Axis *axisPtr;
{
    ...
    newArr = (TickLabel *)malloc(arraySize + poolSize);
    if (newArr == NULL) {
	...
	if (poolSize <= (used + length)) {
	    ...
	    newArr = (TickLabel *)realloc((char *)newArr, arraySize + newSize);
	    if (newArr == NULL) {
    ...
    char **argv;
{
    ...
    coord = Transform((GraphAxis *)axisPtr, value);
    sprintf(graphPtr->interp->result, "%d", coord);
    return TCL_OK;
}
    ...
    int flags;			/* Configuration flags */
{
    ...
    axisPtr->drawProc = DrawAxis;
    axisPtr->coordsProc = ComputeAxisCoordinates;
