    ...
    register XEvent *eventPtr;	/* Event which triggered call to routine */
{
	...
	if (graphPtr->tkwin != NULL) {
	    ...
	    cmdName = Tk_PathName(graphPtr->tkwin);
#else
    ...
    Graph *graphPtr;		/* Graph widget record */
{
    ...
    if (Blt_ConfigModified(configSpecs, "-invertxy", "-title", "-font",
	    "-*margin", "-*width", "-height", "-barmode", (char *)NULL)) {
...
\"rightmargin\", \"topmargin\", or \"bottommargin\"", (char *)NULL);
	return TCL_ERROR;
    }
    ...
    Graph *graphPtr;
{
	...
	Tk_Draw3DRectangle(graphPtr->display, graphPtr->pixwin,
	    graphPtr->border, x, y, width, height, graphPtr->plotBW,
	    graphPtr->plotRelief);
#else
    ...
    Tcl_Interp *interp;
{
    ...
    {
	{"graph", GRAPH_VERSION, GraphCmd,},
	{"barchart", GRAPH_VERSION, BarchartCmd,},
    };
