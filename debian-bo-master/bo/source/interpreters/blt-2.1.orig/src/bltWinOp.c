...
static Tk_Window
GetWindow(interp, pathName, mainWindow)
    Tcl_Interp *interp;
    char *pathName;
    ...
    char **argv;
{
    ...
    for (i = 2; i < argc; i++) {
	tkwin = GetWindow(interp, argv[i], (Tk_Window)clientData);
	if (tkwin == NULL) {
	    return TCL_ERROR;
	}
    ...
    char **argv;
{
	...
	if (argv[2][0] == '@') {
	    ...
	    tkwin = GetWindow(interp, argv[2], (Tk_Window)clientData);
	    if (tkwin == NULL) {
		return TCL_ERROR;
	    }
