...
#if TCL_MAJOR_VERSION == 7 && TCL_MINOR_VERSION == 0
		a = TclWordEnd(a,0)+1;
#else
...
#ifndef HAVE_TCLREGERROR
			if (tclRegexpError) {
			    exp_error(interp,"-re failed: %s",tclRegexpError);
#else
			if (TclGetRegError()) {
...
char **argv;
{
 ...
 indirect_update_abort:
	...
	if (eg.ecd.count) {
		...
		if (ecmd->ecd.count) {
			/* append to end */
			ecmd->ecd.cases = (struct ecase **)ckrealloc(ecmd->ecd.cases, count * sizeof(struct ecase *));
...
/* handle background errors - a skeletal version of Tk_BackgroundError */
/* If we are in Tk, we skip this and use Tk's version */
/* If Tcl ever gets a background error handler, we can dump this */
void
	...
	if (cc == -1) {
		...
		if (i_read_errno == EINTR) {
			if (alarm_fired) {
				...
				if (!f->valid) {
					exp_error("spawn_id %d no longer valid",f-exp_fs);
					return EXP_TCLERROR;
				}
				goto restart;
			}
...
{
	switch (cmdtype) {
	case EXP_CMD_UNUSED: return("expect");
...
char **argv;
{
	...
	exp_cmd_init(&eg,EXP_CMD_UNUSED,EXP_TEMPORARY);
	fd_list = 0;
	masters = 0;
...
cmd_data[]  = {
{"expect",	Exp_ExpectCmd,	(ClientData)&spawn_id_bad,	0},
...
{"expect_user",	Exp_ExpectCmd,	(ClientData)&spawn_id_user,	0},
{"expect_tty",	Exp_ExpectCmd,	(ClientData)&exp_dev_tty,	0},
