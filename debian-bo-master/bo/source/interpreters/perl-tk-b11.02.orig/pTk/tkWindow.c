 ...
 *	error message is left in Tcl_GetResult(interp).  The location at
    ...
    int *screenPtr;		/* Where to store screen number. */
{
	...
	if (screenName == NULL) {
	    Tcl_SetResult(interp, "no display name and no $DISPLAY environment variable",TCL_STATIC);
	...
	Tcl_SprintfResult(interp, "bad screen number \"%d\"", screenId);
	return (TkDisplay *) NULL;
    }
    *screenPtr = screenId;
    return dispPtr;
				 ...
				 * for main window). */
{
    ...
    Tcl_SetVar(interp, variable, libDir, TCL_GLOBAL_ONLY);
    LangFreeVar(variable);
    
    variable = LangFindVar( interp, NULL,  "tk_patchLevel");
    Tcl_SetVar(interp, variable, TK_PATCH_LEVEL, TCL_GLOBAL_ONLY);
    LangFreeVar(variable);

    variable = LangFindVar( interp, NULL,  "tk_version");
    Tcl_SetVar(interp, variable, TK_VERSION, TCL_GLOBAL_ONLY);
    LangFreeVar(variable);
    ...
    if (TkInitFrame(interp, tkwin, 1, 0, args) == NULL) {
	return NULL;
    }
 ...
 *	screen), then an error message is left in Tcl_GetResult(interp) and
 *	NULL is returned.
 *
 * Side effects:
 *	A new window structure is allocated locally.  An X
    ...
    Tk_Window tkwin;		/* Window to destroy. */
{
	...
	if (winPtr->pathName != NULL) {
	    Tk_DeleteAllBindings(winPtr->mainPtr->bindingTable,
            ...
            LangDeadWindow(winPtr->mainPtr->interp, (Tk_Window) winPtr); 
 ...
 *	window.  In this case, an error message is left in Tcl_GetResult(interp).
 *
 * Side effects:
 *	None.
 *
 ...
 *
 * Results:
 *	Returns a standard Tcl completion code and sets Tcl_GetResult(interp)
 ...
 *	returned and an error message is left in Tcl_GetResult(interp).
 *
 * Side effects:
 *	None.
 *
