...
static void ProtocolFree _ANSI_ARGS_((ClientData clientData));
 ...
 */
	...
	/* ARGSUSED */
int
Tk_WmCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " option window ?arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[1])[0];
    length = strlen(LangString(args[1]));
    if ((c == 't') && (strncmp(LangString(args[1]), "tracing", length) == 0)
	    && (length >= 3)) {
	if ((argc != 2) && (argc != 3)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " tracing ?boolean?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 2) {
	...
	return Tcl_GetBoolean(interp, args[2], &wmTracing);
    }

    if (argc < 3) {
	goto wrongNumArgs;
    }
    winPtr = (TkWindow *) Tk_NameToWindow(interp, LangString(args[2]), tkwin);
    if (winPtr == NULL) {
	return TCL_ERROR;
    }
	...
	if ((argc != 3) && (argc != 7)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " aspect window ?minNumer minDenom ",
		    "maxNumer maxDenom?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    if ((Tcl_GetInt(interp, args[3], &numer1) != TCL_OK)
		    || (Tcl_GetInt(interp, args[4], &denom1) != TCL_OK)
		    || (Tcl_GetInt(interp, args[5], &numer2) != TCL_OK)
		    || (Tcl_GetInt(interp, args[6], &denom2) != TCL_OK)) {
		return TCL_ERROR;
	    }
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " client window ?name?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	wmPtr->clientMachine = (char *)
		ckalloc((unsigned) (strlen(LangString(args[3])) + 1));
	strcpy(wmPtr->clientMachine, LangString(args[3]));
	if (!(wmPtr->flags & WM_NEVER_MAPPED)) {
	    XTextProperty textProp;
	    if (XStringListToTextProperty(&wmPtr->clientMachine, 1, &textProp)
		    != 0) {
    ...
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "colormapwindows", length) == 0)
	    && (length >= 3)) {
	Window *cmapList;
	TkWindow *winPtr2;
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " colormapwindows window ?windowList?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	ckfree((char *) cmapList);
        if (freeProc)
         (*freeProc)(windowArgc,windowArgs); 
	return TCL_OK;
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "command", length) == 0)
	    && (length >= 3)) {
	int cmdArgc;
	Arg *cmdArgs = NULL;
        LangFreeProc *freeProc = NULL;
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " command window ?value?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
    ...
    } else if ((c == 'd') && (strncmp(LangString(args[1]), "deiconify", length) == 0)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " deiconify window\"",          NULL);
	    return TCL_ERROR;
	}
	if (wmPtr->iconFor != NULL) {
	    Tcl_AppendResult(interp, "can't deiconify ", LangString(args[2]),
		    ": it is an icon for ", winPtr->pathName,          NULL);
	    return TCL_ERROR;
	}
	wmPtr->hints.initial_state = NormalState;
	wmPtr->withdrawn = 0;
    ...
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "focusmodel", length) == 0)
	    && (length >= 2)) {
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " focusmodel window ?active|passive?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    Tcl_SetResult(interp, wmPtr->hints.input ? "passive" : "active",TCL_STATIC);
	    return TCL_OK;
	}
	c = LangString(args[3])[0];
	length = strlen(LangString(args[3]));
	if ((c == 'a') && (strncmp(LangString(args[3]), "active", length) == 0)) {
	    wmPtr->hints.input = False;
	} else if ((c == 'p') && (strncmp(LangString(args[3]), "passive", length) == 0)) {
	    wmPtr->hints.input = True;
	} else {
	    Tcl_AppendResult(interp, "bad argument \"", LangString(args[3]),
		    "\": must be active or passive",          NULL);
	    return TCL_ERROR;
	}
	UpdateHints(winPtr);
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "frame", length) == 0)
	    && (length >= 2)) {
	Window window;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " frame window\"",          NULL);
	    return TCL_ERROR;
	}
	...
	Tcl_SprintfResult(interp, "0x%x", (unsigned int) window);
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " geometry window ?newGeometry?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    ...
	    Tcl_SprintfResult(interp, "%dx%d%c%d%c%d", width, height,
	...
	return ParseGeometry(interp, LangString(args[3]), winPtr);
    } else if ((c == 'g') && (strncmp(LangString(args[1]), "grid", length) == 0)
	    && (length >= 3)) {
	int reqWidth, reqHeight, widthInc, heightInc;

	if ((argc != 3) && (argc != 7)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " grid window ?baseWidth baseHeight ",
		    "widthInc heightInc?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    if ((Tcl_GetInt(interp, args[3], &reqWidth) != TCL_OK)
		    || (Tcl_GetInt(interp, args[4], &reqHeight) != TCL_OK)
		    || (Tcl_GetInt(interp, args[5], &widthInc) != TCL_OK)
		    || (Tcl_GetInt(interp, args[6], &heightInc) != TCL_OK)) {
		return TCL_ERROR;
	    }
	    if (reqWidth < 0) {
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " group window ?pathName?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    tkwin2 = Tk_NameToWindow(interp, LangString(args[3]), tkwin);
	    if (tkwin2 == NULL) {
		return TCL_ERROR;
	    }
	...
	UpdateHints(winPtr);
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconbitmap", length) == 0)
	    && (length >= 5)) {
	Pixmap pixmap;

	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconbitmap window ?bitmap?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    if (wmPtr->hints.flags & IconPixmapHint) {
		Tcl_SetResult(interp, Tk_NameOfBitmap(winPtr->display, wmPtr->hints.icon_pixmap),TCL_STATIC);
	...
	UpdateHints(winPtr);
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconify", length) == 0)
	    && (length >= 5)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconify window\"",          NULL);
	    return TCL_ERROR;
	}
	...
	if (wmPtr->iconFor != NULL) {
	    Tcl_AppendResult(interp, "can't iconify ", LangString(args[2]),
		    ": it is an icon for ", winPtr->pathName,          NULL);
	    return TCL_ERROR;
	}
    ...
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconmask", length) == 0)
	    && (length >= 5)) {
	Pixmap pixmap;

	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconmask window ?bitmap?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    if (wmPtr->hints.flags & IconMaskHint) {
		Tcl_SetResult(interp, Tk_NameOfBitmap(winPtr->display, wmPtr->hints.icon_mask),TCL_STATIC);
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    pixmap = Tk_GetBitmap(interp, tkwin, Tk_GetUid(LangString(args[3])));
	    if (pixmap == None) {
		return TCL_ERROR;
	    }
	...
	UpdateHints(winPtr);
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconname", length) == 0)
	    && (length >= 5)) {
	if (argc > 4) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconname window ?newName?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    Tcl_SetResult(interp, (wmPtr->iconName != NULL) ? wmPtr->iconName : "",TCL_STATIC);
	    return TCL_OK;
	} else {
	    wmPtr->iconName = Tk_GetUid(LangString(args[3]));
	    if (!(wmPtr->flags & WM_NEVER_MAPPED)) {
		XSetIconName(winPtr->display, winPtr->window, wmPtr->iconName);
	    }
	}
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconposition", length) == 0)
	    && (length >= 5)) {
	int x, y;

	if ((argc != 3) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconposition window ?x y?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    if ((Tcl_GetInt(interp, args[3], &x) != TCL_OK)
		    || (Tcl_GetInt(interp, args[4], &y) != TCL_OK)){
		return TCL_ERROR;
	    }
	...
	UpdateHints(winPtr);
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "iconwindow", length) == 0)
	    && (length >= 5)) {
	Tk_Window tkwin2;
	WmInfo *wmPtr2;
	XSetWindowAttributes atts;
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " iconwindow window ?pathName?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    if (wmPtr->icon != NULL) {
		Tcl_ArgResult(interp,LangWidgetArg(interp,wmPtr->icon));
	    }
	    return TCL_OK;
	}
	if (LangString(args[3])[0] == '\0') {
		...
		Tcl_AppendResult(interp, "can't use ", LangString(args[3]),
			" as icon window: not at top level",          NULL);
		return TCL_ERROR;
	    }
	    wmPtr2 = ((TkWindow *) tkwin2)->wmInfoPtr;
	    if (wmPtr2->iconFor != NULL) {
		Tcl_AppendResult(interp, LangString(args[3]), " is already an icon for ",
		...
		if (XWithdrawWindow(Tk_Display(tkwin2), Tk_WindowId(tkwin2),
		    ...
		    Tcl_SetResult(interp, "couldn't send withdraw message to window manager",TCL_STATIC);
	...
	UpdateHints(winPtr);
    } else if ((c == 'm') && (strncmp(LangString(args[1]), "maxsize", length) == 0)
	    && (length >= 2)) {
	int width, height;
	if ((argc != 3) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " maxsize window ?width height?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if ((Tcl_GetInt(interp, args[3], &width) != TCL_OK)
		|| (Tcl_GetInt(interp, args[4], &height) != TCL_OK)) {
	    return TCL_ERROR;
	}
	...
	wmPtr->flags |= WM_UPDATE_SIZE_HINTS;
	goto updateGeom;
    } else if ((c == 'm') && (strncmp(LangString(args[1]), "minsize", length) == 0)
	    && (length >= 2)) {
	int width, height;
	if ((argc != 3) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " minsize window ?width height?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    Tcl_IntResults(interp,2,0, wmPtr->minWidth,
		    wmPtr->minHeight);
	    return TCL_OK;
	}
	if ((Tcl_GetInt(interp, args[3], &width) != TCL_OK)
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " overrideredirect window ?boolean?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (Tcl_GetBoolean(interp, args[3], &boolean) != TCL_OK) {
	    return TCL_ERROR;
	}
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " saveunder window ?boolean?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	Tk_ChangeWindowAttributes((Tk_Window) winPtr, CWSaveUnder,
		&atts);
    } else if ((c == 'p') && (strncmp(LangString(args[1]), "positionfrom", length) == 0)
	    && (length >= 2)) {
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " positionfrom window ?user/program?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if (LangString(args[3])[0] == '\0') {
		...
		Tcl_AppendResult(interp, "bad argument \"", LangString(args[3]),
			"\": must be program or user",          NULL);
		return TCL_ERROR;
	    }
	}
	wmPtr->flags |= WM_UPDATE_SIZE_HINTS;
	goto updateGeom;
    } else if ((c == 'p') && (strncmp(LangString(args[1]), "protocol", length) == 0)
	    && (length >= 2)) {
	...
	if ((argc < 3) || (argc > 5)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " protocol window ?name? ?command?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	protocol = Tk_InternAtom((Tk_Window) winPtr, LangString(args[3]));
	if (argc == 4) {
	    /*
	...
	cmdLength = strlen(LangString(args[4]));
	if (cmdLength > 0) {
	...
	if ((argc != 3) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " resizable window ?width height?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	if ((Tcl_GetBoolean(interp, args[3], &width) != TCL_OK)
		|| (Tcl_GetBoolean(interp, args[4], &height) != TCL_OK)) {
	    return TCL_ERROR;
	}
	if (width) {
	...
	wmPtr->flags |= WM_UPDATE_SIZE_HINTS;
	goto updateGeom;
    } else if ((c == 's') && (strncmp(LangString(args[1]), "sizefrom", length) == 0)
	    && (length >= 2)) {
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " sizefrom window ?user|program?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	...
	wmPtr->flags |= WM_UPDATE_SIZE_HINTS;
	goto updateGeom;
    } else if ((c == 's') && (strncmp(LangString(args[1]), "state", length) == 0)
	    && (length >= 2)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " state window\"",          NULL);
	    return TCL_ERROR;
	}
	if (wmPtr->iconFor != NULL) {
    ...
    } else if ((c == 't') && (strncmp(LangString(args[1]), "title", length) == 0)
	    && (length >= 2)) {
	if (argc > 4) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " title window ?newTitle?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    Tcl_SetResult(interp, (wmPtr->titleUid != NULL) ? wmPtr->titleUid : winPtr->nameUid,TCL_STATIC);

	    return TCL_OK;
	} else {
	    wmPtr->titleUid = Tk_GetUid(LangString(args[3]));
	    if (!(wmPtr->flags & WM_NEVER_MAPPED)) {
		XTextProperty textProp;
    ...
    } else if ((c == 't') && (strncmp(LangString(args[1]), "transient", length) == 0)
	    && (length >= 3)) {
	Tk_Window master;

	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " transient window ?master?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    if (wmPtr->master != None) {
	...
	if (LangString(args[3])[0] == '\0') {
	    ...
	    master = Tk_NameToWindow(interp, LangString(args[3]), tkwin);
	    if (master == NULL) {
		return TCL_ERROR;
	    }
    ...
    } else if ((c == 'w') && (strncmp(LangString(args[1]), "withdraw", length) == 0)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    LangString(args[0]), " withdraw window\"",          NULL);
	    return TCL_ERROR;
	}
	if (wmPtr->iconFor != NULL) {
	    Tcl_AppendResult(interp, "can't withdraw ", LangString(args[2]),
	...
	Tcl_AppendResult(interp, "unknown or ambiguous option \"", LangString(args[1]),
 ...
 *	Tcl_GetResult(interp) if an error occurs.
 *
 * Side effects:
