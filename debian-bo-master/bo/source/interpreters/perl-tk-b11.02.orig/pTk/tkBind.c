 ...
 *	left in Tcl_GetResult(interp).  If all went well then the return
 ...
 *	occurs then Tcl_GetResult(interp) will contain an error message.
 *
 * Side effects:
 ...
 *	Tcl_GetResult(interp).  The return value is semi-static:  it
					 ...
					 * to check for a matching binding. */
{
     ...
     *    where Tcl_GetResult(interp) is significant (for example, a widget
 ...
 *	and Tcl_GetResult(interp) contains a message describing the problem.
