...
/*VARARGS*/
void
exp_error EXP_VARARGS_DEF(Tcl_Interp *,arg1)
/*exp_error(va_alist)*/
/*va_dcl*/
{
	Tcl_Interp *interp;
	...
	interp = EXP_VARARGS_START(Tcl_Interp *,arg1,args);
	/*va_start(args);*/
...
{ FILE *fp = fopen("/tmp/error","w");
  fprintf(fp,"error occurred in execl\n");
...
char **argv;
{
	static char *logfile_tcl = 0;	/* logfile from Tcl's open */
...
#define NO_CHILD -2
	...
	if (result == -1) {
		sprintf(interp->result,"%d %d -1 %d POSIX %s %s",
			f->pid,m,errno,Tcl_ErrnoId(),strerror(errno));
		result = TCL_OK;
...
/* old code - this code used to remove spawn id from Expect's memory */
/* in anticipation of passing to Tcl - now we let Expect remember */
#if 0
