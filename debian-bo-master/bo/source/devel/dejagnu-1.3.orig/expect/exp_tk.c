	...
	for (;;) {
		...
		for (;;) {
			...
			if (old_configure_count != exp_configure_count) {
				if (timer_created)
					Tk_DeleteTimerHandler(timetoken);
...
double sec;
{
	...
	Tk_CreateTimerHandler((int)(sec*1000),exp_timehandler,(ClientData)&timer_fired);
