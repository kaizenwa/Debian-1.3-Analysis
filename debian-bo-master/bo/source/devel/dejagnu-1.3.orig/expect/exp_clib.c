...
#if 0 /* This is now handled in expect_comm.h.  */
...
/*VARARGS*/
int
exp_spawnl EXP_VARARGS_DEF(char *,arg1)
/*exp_spawnl(va_alist)*/
/*va_dcl*/
{
	va_list args;
	...
	arg = EXP_VARARGS_START(char *,arg1,args);
	/*va_start(args);*/
	for (i=1;;i++) {
	...
	argv[0] = EXP_VARARGS_START(char *,arg1,args);
	/*va_start(args);*/
	for (i=1;;i++) {
...
#define return_errno(x)	{sys_error = x; goto cleanup;}
	...
	*exp_buffer_end = '\0';
	...
	if (f->msize != bufsiz) {
		...
		exp_buffer = ckrealloc(exp_buffer,bufsiz+1);
...
/*VARARGS*/
int
exp_expectl EXP_VARARGS_DEF(int,arg1)
...
/*va_dcl*/
{
	...
	fd = EXP_VARARGS_START(int,arg1,args);
	/* va_start(args);*/
	/* fd = va_arg(args,int);*/
...
int
exp_fexpectl EXP_VARARGS_DEF(FILE *,arg1)
...
/*va_dcl*/
{
	...
	fp = EXP_VARARGS_START(FILE *,arg1,args);
	/*va_start(args);*/
	/*fp = va_arg(args,FILE *);*/
	...
	EXP_VARARGS_START(FILE *,arg1,args);	/*COMPUTED, BUT NOT USED*/
	for (ec=ecases;;ec++) {
		ec->type = va_arg(args,enum exp_type);
		if (ec->type == exp_end) break;
		ec->pattern = va_arg(args,char *);
