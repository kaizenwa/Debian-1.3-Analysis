...
/* FIXME: A copy of this table also lives in winsup/path.cc.  */
static struct {
  const char *dev;
  const char *path;
  int flags;
...
usage (void)
{
  fprintf (stderr, "Usage %s [-m] [-b] <dospath> <unixpath>\n", progname);
  fprintf (stderr, "-m = add mixed case support [currently not recommended]\n");
