#!/bin/sh
# Submit a problem report to a PRMS site.
...
# Where the PRMS directory lives, if at all.
[ -z "$PRMS_ROOT" ] && 
PRMS_ROOT=xPRMS_ROOTx

# The default mail address for PR submissions. 
PRMS_ADDR=xPRMS_ADDRx

# Where the prms category tree lives.
DATADIR=xDATADIRx

# If we've been moved around, try using GCC_EXEC_PREFIX.
[ ! -d $DATADIR/prms -a -d "$GCC_EXEC_PREFIX" ] && 
...
PRMS_SITE=xPRMS_SITEx

# Newer config information?
[ -f ${PRMS_ROOT}/prms-adm/config ] && . ${PRMS_ROOT}/prms-adm/config
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*) echo "$USAGE" ; exit 1 ;;
    *) if [ -z "$USER_PRMS_SITE" ]; then
	 if [ ! -r "$DATADIR/prms/$1" ]; then
	   echo "$COMMAND: the PRMS site $1 does not have a categories list."
	   exit 1
	 else
...
if [ -n "$USER_PRMS_SITE" ] && [ "$USER_PRMS_SITE" != "$PRMS_SITE" ]; then
  PRMS_SITE=$USER_PRMS_SITE
  PRMS_ADDR=$USER_PRMS_SITE-prms
fi
...
if [ -r "$DATADIR/prms/$PRMS_SITE" ]; then
  CATEGORIES=`grep -v '^#' $DATADIR/prms/$PRMS_SITE | sort`
else
  echo "$COMMAND: could not read $DATADIR/prms/$PRMS_SITE for categories list."
  exit 1
fi

if [ -z "$CATEGORIES" ]; then
  echo "$COMMAND: the categories list for $PRMS_SITE was empty!"
  exit 1
fi
...
if [ -n "$IN_FILE" ]; then
  if [ "$IN_FILE" = "-" ]; then
    ...
    if [ -n "$EXPLICIT_PRMS_ADDR" ]; then
      sed -e "s;^[Tt][Oo]:.*;To: $PRMS_ADDR;" > $TEMP
    else
      cat > $TEMP
    fi
  else
    ...
    if [ -n "$EXPLICIT_PRMS_ADDR" ]; then
      sed -e "s;^[Tt][Oo]:.*;To: $PRMS_ADDR;" $IN_FILE > $TEMP
    else
      cat $IN_FILE > $TEMP
    fi
  fi
