     ...
     register struct file *file;
{
  ...
  for (i = 0; i < cmds->ncommand_lines; ++i)
	...
	strcpy (out, in);
      ...
      /* CYGNUS LOCAL cygwin32 */
#if 0 && (defined(__MSDOS__) || defined(_WIN32))
      /* This converts all /'s to \'s, and then doubles all \'s.
	 The current MSDOS FSF make does not provide such support and this
	 approach seems a bit too heavy handed.  */

      /* convert all forward slashes to single backslashes */
      slash = cmds->command_lines[i];
      while ((slash = (char *) index (out, '/')) != NULL)
	*slash = '\\';

      /* convert all backslashes to double backslashes */
      /* the shell make invokes will then strip these back down to one */
      nslash = 0;
      slash = cmds->command_lines[i];
      while (*slash)
	{
	...
	{
	  char *newline, *curptr;
	  len = strlen (cmds->command_lines[i]);
	  newline = curptr = (char *) xmalloc (len + nslash + 1);
	  slash = cmds->command_lines[i];
	  while (*slash)
	    {
	      /* Add another slash if we see one.  */
	      if (*slash == '\\')
		*curptr++ = '\\';
	      *curptr++ = *slash++;
	    }
