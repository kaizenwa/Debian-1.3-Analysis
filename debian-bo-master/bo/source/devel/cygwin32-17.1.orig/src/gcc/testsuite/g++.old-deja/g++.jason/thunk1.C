...
// excess errors test - XFAIL mips*-*-* alpha*-*-*
...
class MMixin {
public:
   virtual void* MixinFunc(int arg, ...) = 0;
};

class CExample : public CBase, public MMixin {
public:
   void* MixinFunc(int arg, ...);
};
...
void* CExample::MixinFunc(int arg, ...)
{
  va_list ap;
  va_start (ap, arg);
...
{
  return anExample.MixinFunc(1,2,3,4,5,6,7,8,9);
}
