...
// $Id: Compiler.C,v 1.19 1997/03/02 22:53:42 geppetto Exp $
...
CCompiler::~CCompiler()
{
  ...
  delete fCurrentFunction;
  delete fCurrentClass;
}
...
{
  assert(fCurrentClass == 0);
  assert(fCurrentFunction == 0);
  unicode_string fullName;
  if (fPackageName.size() > 0) {
  ...
  fCurrentClass = new CIntermediateClass(fFileName, fullName, adoptModifiers,
					 adoptExtends, adoptInterfaces);
}
...
{
  StartClass(className, adoptModifiers, 0, adoptInterfaceExtends);
}
...
CCompiler::EndClass()
{
  assert(fCurrentFunction == 0);
  assert(fCurrentClass != 0);
  if (fCurrentClass->fStaticInitializer != 0 ||
      fCurrentClass->fStaticDeclarations.size() > 0) {
    ...
    CIntermediateFunction* staticFunction = 
      new CIntermediateFunction(staticSignature, modifiers, 0, 0);
    if (fCurrentClass->fStaticInitializer != 0) {
      staticFunction->fBlock = fCurrentClass->fStaticInitializer;
      fCurrentClass->fStaticInitializer = 0;
    } else {
      staticFunction->fBlock = new CCompoundStatement(new StatementList);
    }
    staticFunction->fMaxLocalVariables = fCurrentClass->fStaticLocalVariables;
    fCurrentClass->fFunctions.push_back(staticFunction);
  }
  fIntermediateClasses.push_back(fCurrentClass);
  fCurrentClass = 0;
}
			    ...
			    deque<unicode_string>* adoptThrows)
{
  ...
  StartFunction(type, kConstructorName, modifiers,
		adoptParameters, adoptThrows);
}
			 ...
			 deque<unicode_string>* adoptThrows)
{
  ...
  if (adoptParameters != 0) {
    for (deque<CJavaFieldSignature>::iterator i = adoptParameters->begin();
  ...
  fCurrentFunction = new CIntermediateFunction(signature, newModifiers,
					       adoptParameters, adoptThrows);
}
...
CCompiler::EndFunction(CCompoundStatement* statementBlock)
{
  ...
  PopLocalScope();
  // assert(fVariableScopes.size() == 0);
  fVariableScopes.erase(fVariableScopes.begin(), fVariableScopes.end());
  // assert(fLocalVariables.size() == 0);
  fLocalVariables.erase(fLocalVariables.begin(), fLocalVariables.end());
  fCurrentFunction->fMaxLocalVariables = fCurrentLocalVariable;
  fCurrentFunction->fBlock = statementBlock;
  fCurrentClass->fFunctions.push_back(fCurrentFunction);
  fCurrentFunction = 0;
  fCurrentLocalVariable = 0;
}
...
{
  fVariableScopes.push_front(deque<unicode_string>());
}
...
{
  assert(fVariableScopes.size() > 0);
  deque<unicode_string>::iterator end = fVariableScopes.front().end();
  for (deque<unicode_string>::iterator i = fVariableScopes.front().begin();
       i != end; i++) {
    LocalVariableTable::iterator variable = fLocalVariables.find(*i);
    assert(variable != fLocalVariables.end());
    fLocalVariables.erase(variable);
  }
  fVariableScopes.pop_front();
}
...
//
unsigned short
CCompiler::AddLocalVariable(const CJavaFieldSignature& signature)
{
  unicode_string name = signature.GetFieldName();
  (fVariableScopes.front()).push_front(name);
  fLocalVariables.insert(LocalVariableTable::value_type(name,
		   ::make_pair(signature, fCurrentLocalVariable)));
  unsigned short index = fCurrentLocalVariable;
  fCurrentLocalVariable += signature.GetType().GetWidth();
  return index;
}
...
//
const pair<CJavaFieldSignature, CCompiler::LocalVariableIndex>*
CCompiler::LookupLocalVariable(const unicode_string& name) const
{
  LocalVariableTable::const_iterator match = fLocalVariables.find(name);
  if (match != fLocalVariables.end()) {
    return &((*match).second);
  } else {
    return 0;
  }
...
CCompiler::AddField(CDeclarationStatement* declaration)
{
  ...
  if (declaration->fModifiers->fStatic != 0) {
    fCurrentClass->fStaticDeclarations.push_back(declaration);
  } else {
    fCurrentClass->fNonStaticDeclarations.push_back(declaration);
  }
}
...
CCompiler::AddStaticCode(CCompoundStatement* statement)
{
  ...
  if (statement->fChildren->size() > 0) {
    if (fCurrentClass->fStaticInitializer != 0) {
      fCurrentClass->fStaticInitializer->fChildren->push_back(statement);
      fCurrentClass->fStaticLocalVariables += fCurrentLocalVariable;
    } else {
      fCurrentClass->fStaticInitializer = statement;
      fCurrentClass->fStaticLocalVariables = fCurrentLocalVariable;
    }
  } else {
    delete statement;
...
{
  delete intermediate.fRealClass;
  intermediate.fRealClass = new CJavaClassFile;
  intermediate.fRealClass->fVersion = kVersionID;
  intermediate.fRealClass->fAccessFlags = intermediate.fAccessFlags;
  intermediate.fRealClass->fSourceFile =
    new CJavaSourceFileAttribute(fileName);
  intermediate.fRealClass->fThisClassName = intermediate.fName;
...
CCompiler::PrepareClassDeclarations(CIntermediateClass& intermediate)
{
  ...
  for (StatementList::iterator i = intermediate.fStaticDeclarations.begin();
    ...
    for (; !(field == declaration->fDeclarations->end()); ++field) {
      ...
      CJavaFieldSignature newSignature(FixType(oldSignature.GetType()),
				       oldSignature.GetFieldName());
      CJavaFieldInfo* fieldInfo =
	new CJavaFieldInfo(*declaration->fModifiers, newSignature);
  ...
  for (; functions != intermediate.fFunctions.end(); ++functions) {
    ...
    if (info->fSignature.GetName() == kConstructorName) {
  ...
  if (!foundConstructor && intermediate.fAccessFlags.fInterface == 0) {
    deque<CJavaTypeSignature> noArguments;
    CJavaMethodSignature constructorSignature(CJavaTypeSignature::kVoid,
			 kConstructorName, noArguments);
    CJavaAccessFlags flags;
    flags.fPrivate = intermediate.fAccessFlags.fPrivate;
    flags.fPublic = intermediate.fAccessFlags.fPublic;
    flags.fProtected = intermediate.fAccessFlags.fProtected;
    CIntermediateFunction* defaultConstructor =
      new CIntermediateFunction(constructorSignature, flags, 0, 0);
    defaultConstructor->fMaxLocalVariables = 1;
    defaultConstructor->fBlock = new CCompoundStatement(0);
    intermediate.fFunctions.push_back(defaultConstructor);
    CJavaMethodInfo* info = new CJavaMethodInfo;
    info->fSignature = constructorSignature;
  ...
  if (realClass->fSuperclassName.length() > 0) {
	...
	realClass->fSuperclassName = parent->GetClassName();
	  ...
	  realClass->GetAccessFlags().fInterface == 0) {
  ...
  for (deque<unicode_string>::iterator i = realClass->fInterfaces.begin();
	...
	<< ::UnicodeToString(intermediate.fName) << endl;
      result = false;
    } else if (!(*i == interface->GetClassName())) {
...
CCompiler::GenerateClass(CIntermediateClass& intermediate)
{
  ...
  assert(realClass != 0);
  bool result = true;
  if (intermediate.fAccessFlags.fPublic) {
    unicode_string baseFile(fileName, 0, fileName.find_last_of('.'));
    if (!(baseFile == intermediate.GetShortName())) {
      cerr << ::UnicodeToString(intermediate.fName)
	<< " may not be declared public in a file named "
	  << ::UnicodeToString(fileName) << endl;
      result = false;
    }
  }
  if (result) {
  ...
  if (result) {
	...
	LookupClass(intermediate.fRealClass->GetSuperclassName());
      if (parentClass != 0 &&
	  !((*function)->fSignature.GetName() == kConstructorName)) {
			  ...
			  CIntermediateFunction& intermediateMethod)
{
  ...
  CJavaMethodInfo* method = (CJavaMethodInfo*)((*methodPosition).second);
  ...
  if (error == 0) {
    for (deque<unicode_string>::iterator i= intermediateMethod.fThrows.begin();
	 i != intermediateMethod.fThrows.end(); ++i) {
      const CJavaClassFile* thrown = LookupClass(*i);
      if (thrown == 0) {
	method = 0;
  ...
  if (error == 0) {
    ...
    if (intermediateMethod.fSignature.GetName() == kStaticName &&
	method->fCodeAttribute == 0) {
	  ...
	  intermediateMethod.fAccessFlags.fStatic != 0 ||
			...
			CJavaMethodInfo& method)
{
  ...
  if (methodFlags.fAbstract != 0) {
    ...
    if (body == 0) {
      ...
      CCompileContext context(this, intermediate.fRealClass, &method,
	...
	if (!callsThisConstructor &&
	    intermediate.fNonStaticDeclarations.size() > 0) {
	  fInClassInitializers = true;
	  for (StatementList::iterator i =
		 intermediate.fNonStaticDeclarations.begin();
	       error == 0 && !(i == intermediate.fNonStaticDeclarations.end());
	       ++i) {
...
//
const CJavaClassFile*
CCompiler::LookupClass(const unicode_string& name) const
{
  if (name.length() == 0) {
    return 0;
  }
  const CIntermediateClass* intermediate = LookupIntermediateClass(name);
  if (intermediate != 0 && intermediate->fRealClass != 0) {
...
CCompiler::FindConstantField(CExpression*& result,
			     ...
			     const CJavaFieldSignature& fieldSignature) const
{
  CCompileError* error = 0;
  result = 0;
  const CJavaClassFile* classFile = 0;
  ...
  if (intermediate != 0 && intermediate->fRealClass != 0) {
    ...
    classFile = LookupClass(className);
    if (classFile != 0) {
      const CJavaFieldInfo* fieldInfo = classFile->LookupField(fieldSignature);
      if (fieldInfo != 0 && fieldInfo->GetModifiers().fFinal != 0) {
				       ...
				       const CJavaFieldInfo& fieldInfo) const
{
  ...
  for (StatementList::const_iterator i = declarations->begin();
	...
	CCompileContext context(nonConstThis, (CJavaClassFile*)classFile,
				&dummyInfo, 0);
			      ...
			      const CIntermediateFunction& method)
{
  ...
  if (match != 0) {
	...
	::StringToUnicode("Invalid override of final method ");
      ...
      error = new CCompileError(message);
