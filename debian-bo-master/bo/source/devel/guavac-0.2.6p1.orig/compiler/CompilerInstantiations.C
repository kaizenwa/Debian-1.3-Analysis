...
// $Id: CompilerInstantiations.C,v 1.3 1996/01/14 03:27:58 geppetto Exp $
...
template class pair<CJavaFieldSignature, unsigned short>;
template class pair<const CJavaClassFile*, const CJavaMethodInfo*>;
typedef pair<CJavaFieldSignature, unsigned short> LocalVariableEntry;
template class map<unicode_string, LocalVariableEntry,less<unicode_string> >;
typedef map<unicode_string, LocalVariableEntry,less<unicode_string> > LVarMap;
template class pair<unicode_string const, LocalVariableEntry>;
...
template LocalVariableEntry make_pair(CJavaFieldSignature const&,
