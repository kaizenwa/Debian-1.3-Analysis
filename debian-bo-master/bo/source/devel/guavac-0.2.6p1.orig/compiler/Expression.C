...
// $Id: Expression.C,v 1.21 1997/03/02 22:53:43 geppetto Exp $
	  ...
	  CCompileContext& context, CJavaTypeSignature& setType, bool asLValue)
{
  ...
  if (error == 0 && expressionPointer->fType != CJavaTypeSignature::kNullType
    ...
    const CJavaClassFile* classFile = context.GetCompiler().LookupClass(className);
    if (classFile == 0) {
		   ...
		   bool isStatement)
{
  ...
  for (ExpressionList::iterator sizeIterator = fArraySizes->begin();
	...
	error =
	  (*sizeIterator)->GenerateCode(code, context, stackUsed);
      }
    }
  }
  if (error == 0) {
		   ...
		   bool isStatement)
{
  ...
  const CJavaClassFile* javaClass =
    context.GetCompiler().LookupClass(className);
  if (javaClass == 0) {
  ...
  if (error == 0) {
    ...
    pair<const CJavaClassFile*, const CJavaMethodInfo*>* match =
      context.GetCompiler().MatchMethod(CCompiler::kConstructorName,
				*javaClass, argumentTypes, matchErrorString);
		   ...
		   bool isStatement)
{
  ...
  if (literal != 0) {
    if (literal->GetBoolean()) {
      error = fLeft->GenerateCode(code, context, stackUsed);
    } else {
		   ...
		   bool isStatement)
{
  ...
  case INCR:
	...
	unsigned short localVariableIndex = localVariable->GetVariableIndex();
	CCodeSequence::Instruction instruction;
	...
	if (fSide == kPostfix && !isStatement && stackUsed > 0) {
	  if (fChild->GetType().GetWidth() == 1) {
	    code.Append(CJavaCodeAttribute::dup_x1, fLineNumber);
	  } else {
	    assert(fChild->GetType().GetWidth() == 2);
...
//
unsigned short
CLocalVariableExpression::GetVariableIndex() const
{
  return fLocalVariableIndex;
}
		   ...
		   bool isStatement)
{
  assert(!isStatement);
  CCompileError* error = 0;
  if (!context.IsVariableInitialized(fLocalVariableIndex)) {
    ...
    if (GetType().IsReference()) {
      code.Append(CJavaCodeAttribute::aload, fLocalVariableIndex,
		  fLineNumber);
    } else {
      switch (GetType().GetBaseType()) {
      case CJavaTypeSignature::Double:
	code.Append(CJavaCodeAttribute::dload, fLocalVariableIndex,
		    fLineNumber);
	break;
      case CJavaTypeSignature::LongInteger:
	code.Append(CJavaCodeAttribute::lload, fLocalVariableIndex,
		    fLineNumber);
	break;
      case CJavaTypeSignature::Float:
	code.Append(CJavaCodeAttribute::fload, fLocalVariableIndex,
		    fLineNumber);
	break;
      case CJavaTypeSignature::Integer:
      case CJavaTypeSignature::Byte:
      ...
      case CJavaTypeSignature::Short:
      case CJavaTypeSignature::Boolean:
	code.Append(CJavaCodeAttribute::iload, fLocalVariableIndex,
		    fLineNumber);
	break;
      default:
	assert(0);
		   ...
		   bool isStatement)
{
  ...
  if (GetType().IsReference()) {
    code.Append(CJavaCodeAttribute::astore, fLocalVariableIndex,
		fLineNumber);
  } else {
    switch (GetType().GetBaseType()) {
    case CJavaTypeSignature::Double:
      code.Append(CJavaCodeAttribute::dstore, fLocalVariableIndex,
		  fLineNumber);
      break;
    case CJavaTypeSignature::LongInteger:
      code.Append(CJavaCodeAttribute::lstore, fLocalVariableIndex,
		  fLineNumber);
      break;
    case CJavaTypeSignature::Float:
      code.Append(CJavaCodeAttribute::fstore, fLocalVariableIndex,
		  fLineNumber);
      break;
    case CJavaTypeSignature::Integer:
    case CJavaTypeSignature::Byte:
    ...
    case CJavaTypeSignature::Short:
    case CJavaTypeSignature::Boolean:
      code.Append(CJavaCodeAttribute::istore, fLocalVariableIndex,
		  fLineNumber);
      break;
    default:
      assert(0);
  ...
  context.InitializeVariable(fLocalVariableIndex);
  return 0;
}
					  ...
					  CJavaTypeSignature& setType)
{
  ...
  if (fBase != 0) {
	...
	context.GetCompiler().LookupClass(baseName);
      if (explicitClass != 0) {
	delete error;
	error = 0;
	delete fBase;
						...
						CCompileContext& context) const
{
  ...
  if (!IsArrayLength()) {
    error = context.GetCompiler().FindConstantField(result,
						 fClassAlias->GetClassName(),
						 fFieldInfo.GetSignature());
  }
  return error;
}
				...
				CJavaTypeSignature& setType)
{
  ...
  if (error == 0) {
	...
	const CJavaClassFile* javaClass =
	  context.GetCompiler().LookupClass(prefix);
	if (javaClass != 0) {
	  string* matchErrorString = 0;
	  pair<const CJavaClassFile*, const CJavaMethodInfo*>* match =
	...
	if (targetType.GetArrayBounds() > 0) {
	  javaClass =
	    context.GetCompiler().LookupClass(CCompiler::kObjectName);
	} else {
	  unicode_string className;
	  bool isClass = targetType.GetBaseClassName(className);
	  assert(isClass);
	  javaClass = context.GetCompiler().LookupClass(className);
	}
	if (javaClass == 0) {
	  unicode_string errorString = ::StringToUnicode("Method ");
	  errorString += fMethodName;
