...
--  In both these cases, Replace_Substitute_Tree or Rewrite_Substitute_Tree
--  to achieve the actual replacement of the node, since it is only passed
--  the Id of the node to be expanded, and the resulting expanded node Id
...
--  compile in semantics checks only mode. Thus Replace_Substitute_Tree may
--  be freely used in such instances.
...
--  on expanded trees. This means that Replace_Substitute_Tree should not be
--  used on subexpression nodes. Instead use Rewrite_Substitute_Tree.
...
package Expander is
   ...
   --  Expander_Mode_Save_And_Set procedure. These routines should *always*
   --  be used to change and subsequently restore the value of this flag.
   --
   ...
   --        or declaration
   --        (see routines Start_Generic_Processing and End_Generic_Processing
   --         in Sem_Ch12)
   --
