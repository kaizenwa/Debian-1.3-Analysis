...
package body Exp_Ch7 is
   ...
   begin
      ...
      --    Initialize (Ref);
         ...
         Append_To (Res,
           Make_Attach_Call (Cref, Flist_Ref, With_Attach));

         Append_To (Res,
           Make_Procedure_Call_Statement (Loc,
             Name => New_Reference_To (Proc, Loc),
             Parameter_Associations => New_List (New_Copy_Tree (Cref))));
   ...
   begin
         ...
         Append_To (Res,
           Make_Procedure_Call_Statement (Loc,
             ...
             Parameter_Associations =>
               New_List (Flist_Ref, Cref, With_Attach)));
      ...
      --    Adjust (Ref);
         ...
         Cref := Convert_View (Proc, Cref);
         Append_To (Res, Make_Attach_Call (Cref, Flist_Ref, With_Attach));
   ...
   begin
      ...
      if Is_Abstract (Proc) and then Is_Tagged_Type (Ftyp) then
        ...
        and then
          ((Is_Private_Type (Ftyp) and then Atyp = Full_View (Ftyp))
             or else
           (Is_Private_Type (Atyp) and then Ftyp = Full_View (Atyp)))
      then
   ...
   begin
      --  Functions returning array with controlled components don't need
      --  any processing
      ...
      --  linked final list:
      --
      --    Resxxx := F (X)'reference;
      --    Attach_To_Final_List (_Lxxx, Resxxx.all, 1);
      --  or when there are controlled components
      --    Attach_To_Final_List (_Lxxx, Resxxx._controller, 1);

      if Has_Controlled_Component (Rtype) then
         if Rtype /= Utype then
            Ref := Unchecked_Convert_To (Utype, Ref);
         end if;
   ...
   begin
      --  Optimization: If the number of linkl is statically '0', don't
   ...
   begin
      ...
      --  If the construct is a protected subprogram, then the call to the
      --  corresponding unprotected program appears in a block which is the
      --  last statement in the body, and it is this block that must be
      --  covered by the At_End handler.

      if Is_Protected then
         Stats := Handled_Statement_Sequence
           (Last (Statements (Handled_Statement_Sequence (N))));
      else
         Stats := Handled_Statement_Sequence (N);
      end if;

      Set_Identifier (Stats, New_Occurrence_Of (Clean, Loc));
   ...
   begin
      ...
      Rewrite_Substitute_Tree (N, New_Reference_To (E, Loc));
   ...
   begin
      Rewrite_Substitute_Tree (N, Make_Transient_Block (Loc, New_Statement));
   ...
   --  If the only cleanup required is the releasing the secondary stack
   --  we don't use the cleanup mechanism which is too costly but rather
   ...
   --  later catch the leak. Here is the expansion for the latter case:
   ...
   --  Furthermore, even this limited wrapping is unnecessary if the
   --  action being wrapped is a procedure call to a procedure for which
   --  the No_Return pragma has been specified. Ir cannot be used either
   --  when the return statement is to be wrapped because the SS_Release
   --  would never occur.
   ...
   begin
      ...
      then
         --  Add mark release unless call to No_Return procedure

         if Nkind (Action) /= N_Procedure_Call_Statement
           or else not Is_Entity_Name (Name (Action))
           or else not No_Return (Entity (Name (Action)))
         then
   ...
   begin
      if Is_Task then
         ...
         --  Task entry call
            ...
            --  Cancel_Task_Entry_Call (C);  ??? what is this ???
