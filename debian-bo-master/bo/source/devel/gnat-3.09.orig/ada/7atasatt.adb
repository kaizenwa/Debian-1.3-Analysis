...
--  exclusion. That is provided by the lock System.Tasking.Task_-
--  Attributes.All_Attrs_L.
...
--  Gigi abort ???:
--      (To_Access_Dummy_Wrapper (Wrapper'Access), Local'Unchecked_Access);
--      (Access_Dummy_Wrapper' (To_Access_Dummy_Wrapper (Wrapper'Access)),
--          Local'Unchecked_Access);
   ...
   exception
      when others =>
         null;
         pragma Assert (Shutdown ("Exception in Deallocate"));
   end Deallocate;
   ...
   begin
      ...
      begin
         Defer_Abortion;
         POP.Write_Lock (TT);
         --  Why do we want to lock this? if TT is not me then possible
         --  deadlock. No? Dong-Ik???
         ...
         if Local.Index /= 0 then
            ...
            return To_Attribute_Handle
               (TT.Direct_Attributes (Local.Index)'Access);
            ...
            begin
               while P /= null loop
--  Gigi bug: ???
--                  if P.Instance = Local'Unchecked_Access then
   ...
   begin
      ...
      if Local.Index = 0 then
         ...
         begin
            Defer_Abortion; POP.Write_Lock (TT);

            Q := To_Access_Node (TT.Indirect_Attributes);
            while Q /= null loop
--  Gigi bug: ???
--               if Q.Instance = Local'Unchecked_Access then
...
begin
   ...
   begin
      ...
      POP.Write_Lock (Initialization.All_Tasks_L'Access);
      ...
      if Local.Index /= 0 then
         ...
         declare
            C : System.Tasking.Task_ID := Initialization.All_Tasks_List;
            P : Access_Node;
      ...
      POP.Unlock (Initialization.All_Tasks_L'Access);
   ...
   exception
      when others => null;
         ...
         --  If we later decide to allow exceptions to propagate,
         --  we need to not only release locks and undefer abortion,
         --  we also need to undo any initializations that
         --  succeeded up to this point, or we will risk a
         --  dangling reference when the task terminates.
   end;
