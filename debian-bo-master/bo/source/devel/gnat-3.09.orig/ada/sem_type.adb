...
package body Sem_Type is
   ...
   procedure Add_One_Interp (N : Node_Id; E : Entity_Id; T : Entity_Id) is
      ...
      function Has_Universal_Interpretation (Opnd : Node_Id) return Boolean;
      --   True if there is one interpretation with a universal type.
         ...
         while Present (It.Nam) loop
            ...
            then
               ...
               --  predefined operator in any case.
                 ...
                 or else Scope_Depth (Name) <= Scope_Depth (It.Nam)
               then
                  return;
      ...
      begin
         if Ekind (Op) /= E_Operator then
            ...
            return Has_Universal_Interpretation (Left_Opnd (N))
              and then Has_Universal_Interpretation (Right_Opnd (N));

         elsif Nkind (N) in N_Unary_Op then
            return Has_Universal_Interpretation (Right_Opnd (N));
            return True;
            ...
            while Present (Arg) loop
               ...
               if not Has_Universal_Interpretation (Arg) then
                  return False;
               end if;
      ...
      begin
         if not Is_Overloaded (Opnd) then
            return Etype (Opnd) = Universal_Integer
      ...
      end Has_Universal_Interpretation;
   ...
   begin
      ...
      --  whether the operator is visible.
         ...
         while Present (H) loop
            ...
            then
               ...
               for J in First_Interp .. All_Interp.Last - 1 loop
                  ...
                  elsif Type_Conformant (H, All_Interp.Table (J).Nam)
                    and then Scope (H) /= Standard_Standard
                  then
                     goto Next_Homograph;
                  end if;
               end loop;
   ...
   begin
      ...
      --  They are however compatible with their own subtypes.
      --  More precise check needed for subtypes ???
      ...
      then
        ...
        and then Ekind (Typ2) = E_Anonymous_Access_Type
        and then Covers (Designated_Type (Typ1), Designated_Type (Typ2))
      then
         return True;
   ...
   begin
      ...
      if Chars (Nam1) in  Any_Operator_Name then
         ...
         then
            ...
            begin
               ...
               if Present (Arg2)
                 and then Etype (Arg1) = Etype (Arg2)
                 and then (Etype (Arg1) = Universal_Integer
                   or else Etype (Arg1) = Universal_Real)
               then
                  Get_First_Interp (N, I, It);
      ...
      --  results from two formals types with the same actual.
         ...
         --  in the original generic.
            ...
            then
               ...
               if Etype (Left_Opnd (N)) = Etype (First_Formal (Nam1))
                 and then Etype (Right_Opnd (N)) =
                   Etype (Next_Formal (First_Formal (Nam1)))
               then
                  return It1;
               else
                  return It2;
      ...
      --  scope, it hides the predefined operator for the type.
