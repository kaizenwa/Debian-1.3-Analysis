...
package body Inline is
   ...
   --  scope of the containing function anyway.
   ...
   begin
      ...
      if not Is_Abstract (E) and then not Is_Nested (E) then
         Pack := Scope (E);
   ...
   begin
      ...
      while Present (Elmt) loop
         ...
         if Ekind (Scop) = E_Block then
            ...
            Decl := Get_Declaration_Node (Scop);

            if Nkind (Decl) = N_Subprogram_Declaration
              or else Nkind (Decl) = N_Task_Type_Declaration
            then
               Decl := Get_Declaration_Node (Corresponding_Body (Decl));
            end if;
         end if;
      ...
      if Errors_Detected = 0 then
         ...
         loop
            ...
            if Nkind (Decl) = N_Package_Declaration then
               ...
               Add_Scope_To_Clean (Defining_Entity (Decl));
