...
--        or else Shutdown ("GNULLI failure---pthread_mutexattr_settype_np"));
...
--        Shutdown ("GNULLI failure---pthread_mutexattr_setprioceiling"));
   ...
   begin
      ...
      pragma Assert (Result = 0 or else Result = EINVAL
          or else Shutdown ("GNULLI failure---pthread_mutex_lock1"));
   end Write_Lock;
   ...
   begin
      Result := pthread_mutex_lock (L);
      pragma Assert (Result = 0
          or else Shutdown ("GNULLI failure---pthread_mutex_lock2"));
   end Write_Lock;
   ...
   begin
      ...
      pragma Assert (Result = 0
          or else Shutdown
            ("GNULLI failure---pthread_mutex_lock3: "
   ...
   procedure Sleep_For (Self_ID : Task_ID; Rel_Time : Duration) is
      Result : Interfaces.C.int := ETIMEDOUT;
   ...
   begin
      ...
      if Rel_Time > 0.0 then
         ...
         pragma Assert
           (Result = 0 or else Result = ETIMEDOUT
   ...
   begin
      ...
      if Abs_Time > Clock + Clock_Delay_Correction then
   ...
   function Get_Exc_Stack return Address;
   function Get_Exc_Stack return Address is
   begin
      return Self.LL.Exc_Stack_Addr;
   end Get_Exc_Stack;
...
--        Shutdown ("GNULLI failure---pthread_mutexattr_setprioceiling"));
      ...
      Self_ID.LL.Exc_Stack_Ptr := new Exc_Stack_T;
      Self_ID.LL.Exc_Stack_Addr
        := Self_ID.LL.Exc_Stack_Ptr (Exc_Stack_T'Last)'Address;
   ...
   begin
      ...
      pragma Assert (Result = 0
        or else Shutdown ("GNULLI failure---pthread_attr_setinheritsched"));

      System.Tasking_Soft_Links.Get_Exc_Stack_Addr := Get_Exc_Stack'Access;
      ...
      --  ENOMEM is a valid runtime error.  Don't shut down.
