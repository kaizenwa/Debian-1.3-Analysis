...
package body Ch5 is
   ...
   --  to indicate acceptable termination conditions for the sequenece:
   ...
   begin
      ...
      loop
         ...
         begin
            ...
            --  If we are checking columns, then if the next statement is at
            --  the start of the line, it must start to the right of the opener
            --  of the enclosing construct. If not, then we terminate the scan
            --  for statements, and the caller will give an error message if
            --  we are not at an appropriate terminating token.
            ...
            then
               ...
               --  For THEN ABORT, we are supposed to be positioned on ABORT

               if Token = Tok_Then  and SS_Flags.Tatm then
                  Save_Scan_State (Scan_State); -- at THEN
                  Scan; -- past THEN
                  exit when Token = Tok_Abort;
            ...
            case Token is
               ...
               when Tok_Identifier =>
                  ...
                  then
                     ...
                     --  expression token, then assume the colon was a
                     --  malformed attempt at an assignment symbol.

                     elsif Token not in Token_Class_Eterm then
                        Error_Msg_SP ("assignment symbol is "":=""");
                        Append_To (Statement_List,
                          P_Assignment_Statement (Id_Node));
   ...
   begin
      ...
      --  Also carry out this check if we are checking for Ada 95 features

      if Ada_83 or Features_On then
         Decl := First (Decls);
         ...
         Outer : while Present (Decl) loop
            ...
            --  items. The inner loop checks the rest of the list.
               ...
               Inner : while Present (Decl) loop
                  ...
                  then
                     Note_Feature (Later_Declaration_Ordering, Sloc (Decl));
      ...
      --  also reset the expected column for the END to match the BEGIN.
         ...
         --  No BEGIN present
            ...
            --  Otherwise BEGIN is not required for a package body.
            ...
            then
               null; -- BEGIN not required for these cases
