...
package Exp_TSS is
   ...
   --  argument is the entity for a newly created TSS. The tree for this TSS
   --  has not yet been inserted into the main tree or analyzed. Set_TSS finds
   --  the appropriate place to install the subprogram tree, and then analyzes
   --  it. The resulting analyzed entity is then placed in the TSS list for the
   --  type whose entity is given as the first argument, replacing an old entry
   --  of the same name if there was one present.
