...
package Ttypes is
   ...
   System_Max_Priority : constant Pos := 30;
   --  System.Priority'Last. At the moment this is a fixed constant, but
   --  later we should add this value to the GCC configuration file so that
   --  its value can be configuration dependent, in which case this constant
   --  will become dependent on the value in this file.

   System_Max_Interrupt_Priority : constant Pos := 31;
   --  System.Any_Priority'Last. At the moment this is a fixed constant, but
   --  later we should add this value to the GCC configuration file so that
   --  its value can be configuration dependent, in which case this constant
   --  will become dependent on the value in this file.
