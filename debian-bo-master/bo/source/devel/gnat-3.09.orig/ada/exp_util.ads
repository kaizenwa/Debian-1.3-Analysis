...
package Exp_Util is
   ...
   procedure Make_Default_Expr_Functions
     (N : Node_Id;
      E : Entity_Id);
   --  Build parameterless functions to be used for complex default
   --  expressions for formals. This is done for subprograms and entries.
   ...
   --  if necessary. If the expression is already of the appropriate actual
   --  type, then it is unchanged.
   ...
   --  side effect (used of implementing Force_Evaluation).  Note: after a
   ...
   --  to Remove_Side_Effect when Variable_Ref is set to TRUE. That is to
