...
--  xeinfo.ads and xeinfo.adb files must meet certain requirements and be laid
...
--  Ada 95 has two important points with respect to an entity. The point at
--  which it is declared, and the point at which it is frozen. In the case
--  of an incomplete or private type, there are three points, since there
--  are two declaration points, but still only one freeze point.

--  In the backend, really in Gigi, Ada entities are transformed into the
--  corresponding GCC entities which are then elaborated. The issue is when
--  this elaboration must be performed.

--  It is semantically correct to delay all such elaboration to the freeze
--  point, but there are three reasons for wanting to elaborate at least
--  some entities earlier than the freeze point:

--    1. Delaying the elaboration of an entity requires the creation of a
--       freeze node for an entity, and we prefer not to waste the space
--       and time involved in generating and processing these freeze nodes
--       for all entities.

--    2. Delaying the elaboration is less efficient in the case where some
--       variable values must be obtained at the point of declaration. If
--       we can read the variables at the point of elaboration, then there
--       is no need to make copies. If on the other hand, elaboration is
--       delayed to the freeze point, then copies have to be made to capture
--       the values of the variables at the point of declaration.

--    3. Delaying the elaboration of types can cause problems both in the
--       size, and possibly correctness of debugging information in some
--       cases, so we prefer to elaborate early if possible.

--  First, let's look at the issue of processing required at the freeze
--  point in the front end. There are two kinds of processing here:

--    1. Semantic processing required when the entity is frozen. For example
--       the size of a fixed-point type depends on the accumulated rep
--       clauses, and is most easily calculated at the freeze point. Since
--       from the very definition of freezing, it is impossible to need this
--       size any earlier, this delay causes no problems. All such semantic
--       processing is carried out directly in the Freeze unit as part of
--       the processing for Freeze_Entity, which is called at the freeze
--       point of all entities.

--    2. Expansion processing required if code is being generated. An example
--       of such processing is the generation of initialization procedures for
--       records and arrays. This is achieved by generating a freeze node,
--       more accurately an N_Freeze_Entity node, and inserting it into the
--       tree at the freeze point. The expansion of this node (most such
--       expansion is type related, and thus found in Exp_Ch3) carries out
--       the necessary generation of actions at the freeze point.

--  Now, let's look at the backend elaboration issue. There are basically
--  three possible places for elaborating an entity:

--    1. The point where it is initially declared

--    2. For an incomplete or private type, the point where the full
--       declaration is encountered.

--    3. The freeze point.

--  For certain entities, it is necessary to delay elaboration to the freeze
--  point. An example is a type which has a component which is a private type
--  whose full declaration has not been seen yet. Obviously such a type cannot
--  be elaborated at declaration time. It is conceivable in some cases that we
--  could elaborate it as soon as this component type was elaborated, but in
--  practice we have no mechanism for doing so, and consequently if an entity
--  cannot be elaborated at declaration time, we set Has_Delayed_Elaboration
--  to true, and the meaning (as sligntly modified below) is that elaboration
--  must be delayed to the freeze point for the entity. In this case we also
--  need an N_Freeze_Entity node, i.e. this node is doing double duty, in
--  some cases it is there for the front end expander to generate actions at
--  the freeze point, and in some cases, so that Gigi knows where to elaborate
--  the entity (and in some cases it serves both purposes at once).

--  In the normal case (i.e. other than for a private or incomplete type),
--  there are thus exactly two possibilities:

--    1.  Has_Delayed_Elaboration is False. In this case, the entity is
--        elaborated at the point of declaration. There may or may not
--        be an N_Freeze_Entity node present, but if present, Gigi will
--        ignore it.

--    2.  Has_Delayed_Elaboration is True. In this case, the entity is not
--        elaborated at the point of declaration. an N_Freeze_Entity node
--        must be present in this case, and Gigi will elaborate the type
--        when this node is encountered.

--  For the case of incomplete or private types, there are three possible
--  situations:

--    1.  Has_Delayed_Elaboration is False on the private and full entities.
--        In this case, the entity is elaborated at the point of declaration
--        of the private type. As in the normal case, a freeze node may or
--        may not be present but will be ignored. A typical example of a type
--        that can be treated this way is a private type whose full type is
--        a static integer subtype.

--    2.  Has_Delayed_Elaboration is True on the private entity, but False
--        on the full entity. This means that the type should be elaborated
--        at the point of the full declaration. Again, a freeze node may or
--        may not be present but will be ignored. A typical example of a
--        type that can be treated this way is a private type whose full
--        declaration is an array with a non-static index subtype.

--    3.  Has_Delayed_Elaboration is True on both the private entity and the
--        full entity. In this case, an N_Freeze_Entity node must be present,
--        and the type is elaborated when this node is encountered.

--  Note that the front end does not promise to do the best possible job in
--  setting the Has_Delayed_Elaboration flag. A perfectly valid implementation
--  as far as the spec is concerned would be to set this flag all the time.
--  The idea is to elaborate things early as much as possible.
...
--       Points to parent subprogram of a derived subprogram. Also used for a
--       subprogram renaming, where it points to the renamed subprogram.

--    Alignment_Clause (synthesized)
--       Appplies to all entities. If an alignment attribute definition
--       clause is present for the entity, then this function returns the
--       N_Attribute_Definition clause that specifies the alignment. If no
--       alignment clause applies to the type, then the call to this function
--       returns Empty. Note that the call can return a non-Empty value even
--       if Has_Alignment_Clause is not set (happens with subtype and derived
--       type declarations). Note also that a record definition clause with an
--       (obsolescent) mod clause is converted into an attribute definition
...
--    Associated_Node_For_Itype (Node22)
--       Present in all type and subtype entities. Set non-Empty only for
--       Itypes. Set to to point to the associated node for the Itype, i.e.
...
--       non-private. See also Implementation_Base_Type.
...
--    Class_Wide_Type (Field17) [base type only]
--       Present in all type entities. For a tagged type, returns the
...
--    Component_Type (Node10) [implementation base type only]
...
--    Corresponding_Remote_Type (Node8)
...
--       Present in entities whose Has_Discriminant field is set (concurrent
...
--       If the Is_Constrained field is set, Discriminant_Constraint points
--       to an element list containing the discriminant constraints  in the
--       same order in which the discriminants are declared.
--       If the Is_Constrained field is not set but the discriminants of the
--       unconstrained type have default initial values then
--       Discriminant_Constraint points to an element list giving these
--       default initial values in the same order in which the discriminants
--       are declared. Note that in this case the entity cannot be a tagged
--       record type, because discriminants in this case cannot have defaults.
--       If the entity is a tagged record implicit type, then
--       Discriminant_Constraint is inherited from the first subtype
--       (so that the itype is subtype conformant with its first subtype,
--       which is needed when the first subtype override primitive operations
--       inherited by the implicit base type).
...
--       parameters require extra implicit information to be passed (e.g.
--       the flag indicating if an unconstrained variant record argument is
--       constrained, and the accessibility level for access parameters. See
--       description of Extra_Formal_Constrained, Extra_Formal_Accessibility
--       fields for further details. Extra formal parameters are constructed
--       to represent these values, and chained to the end of the list of
--       formals using the Extra_Formal field (i.e. the Extra_Formal field
--       of the last "real" formal points to the first extra formal, and the
--       Extra_Formal field of each extra formal points to the next one, with
--       Empty indicating the end of the list of extra formals.

--    Extra_Formal_Accessibility (Node13)
--       Present in formal parameters in the non-generic case if expansion
--       is active. Normally Empty, but if a parameter is one for which a
--       dynamic accessibility check is required, then an extra formal of
--       type Natural is created (see description of field Extra_Formal),
--       and the Extra_Formal_Accessibility field of the formal parameter
--       points to the entity for this extra formal.

--    Extra_Formal_Constrained (Node14)
--       Present in formal parameters in the non-generic case if expansion
--       is active. Normally Empty, but if a parameter is one for which a
--       dynamic indication of its constrained status is required, then an
--       extra formal of type Boolean is created (see description of field
--       Extra_Formal), and the Extra_Formal_Constrained field of the
--       formal parameter points to the entity for this extra formal.
...
--       new entity ingerits the chain at the point of declaration. This
...
--    Has_Delayed_Elaboration (Flag146)
--       Present in all entities. Set to indicate that the elaboration of the
--       entity by Gigi (to produce the corresponding GCC entity) must be
--       delayed to the freezing point for the entity, as signalled by the
--       presence of an explicit freeze node. If this flag is set then it
--       must also be the case that Has_Delayed_Freeze is set. See separate
--       section ("Delayed Freezing and Elaboration") for further details.
...
--    Has_Interrupt_Handler (Flag124) [base type only]
--       Present in all entities. Can be set only in protected type entities.
--       indicating that the protected type contains at least one procedure
--       to which a pragma Interrupt_Handler applies. This is used to ensure
--       that no objects are declared at non-library level.
...
--       All other representation clauses (i.e. Size and Alignment clauses)
--       are not considered significant since they do not affect stored
--       bit patterns.

--    Has_Pending_Instantiations (Flag114)
--       Present in entities that have finalization lists. Cleanup actions
--       for the corresponding scopes are expanded only after the instance
--       bodies has been analyzed, to insure that finalizable objects and
--       tasks within those instances are properly attached to the cleanup
--       structures of the enclosing scope.
...
--       the type. Note that for anonymous array objects, the flag is set in
--       the anonymous type that is created, not the object.
...
--       in this case, so that we still have a concrete type.
...
--       Set to Empty unless an export, import, or interface name pragma has
--       explicitly specified an external name, in which case it references
--       an N_String_Literal node for the specified exteral name. In the case
--       of exceptions, the field is set by Import_Exception/Export_Exception
--       (which can be used in OpenVMS versions only). Note that if this field
--       is Empty, and Is_Imported or Is_Exported is set, then the default
--       interface name is the name of the entity, cased in a manner that is
--       appropriate to the system in use.
--
--    In_Use (Flag8)
...
--       Present in all type entities. This flag is set for a packed array
...
--    Is_Common_Object (Flag126)
--       Present in variables and constants. Set if a valid Common_Object
--       or Psect_Object pragma applies to the variable or constant.
...
--       Present for all entities. Set for all entities that are declared in
--       a package body
...
--       Applies to all entities, true for enumeration types and subtypes
...
--       be exported in OpenVMS implementations of GNAT.
...
--       Present in all entities. Set to indicate tha the entity is an
...
--       only allow the import of exceptions, functions, procedures and
--       variables, but that may well change later on. Exceptions can only
--       be imported in OpenVMS implementations of GNAT.
...
--       flag may be set directly by the expander to request inlining. Also
--       used on packages that contain inlined subprograms. If inlining is
--       enabled, the bodies of these packages must be compiled. Note that
--       this flag can be set explicitly on internal functions that should
--       be inlined where possible.
...
--       Present in composite types that have a limited component. Used to
...
--       immediate scope of the composite type itself (7.3.1 (5)).
...
--       Applies to all type entities. True if the type is a limited type
...
--    Is_Optional_Formal (Flag134)
--       Applies to parameter entities. Set if the parameter is specified as
...
--       used to implement a packed array (either a subtype of Packed_Bytes
--       or Packed_Bytes_Unaligned, or an appropriate modular type). The flag
--       is set if and only if the type appears in the Packed_Array_Type field
--       of some other type entity. It is used by Gigi to activate the special
--       processing for such types (unchecked conversions that would not
--       otherwise be allowed are allowed for such types).
...
--       be passed by reference, as defined in 6.2(4-9).
...
--       Applies to all entities. Set only in record types to which the
--       pragma Unchecked_Union has been validly applied.

--    Is_Unsigned_Type (Flag144)
--       Present in discrete and fixed-point type and subtype entities. This
--       flag is only valid if the entity is frozen. If set it indicates that
--       the representation is known to be unsigned (i.e. that no negative
--       values appear in the range). This is normally just a reflection of
--       the lower bound of the subtype or base type, but there is one case
--       in which the setting is non-obvious, namely the case of an unsigned
--       subtype of a signed type from which a further subtype is obtained
--       using variable bounds. This further subtype is still unsigned, but
--       this cannot be determined by looking at its bounds or the bounds of
...
--       Present in exception entities. Set if the exception was specified
--       in an Import_Exception or Export_Exception pragma with the VMS
--       option for Form. See description of these pragmas for details.
--       This flag can only be set in OpenVMS versions of GNAT.
...
--    Mechanism (stored as Uint22, but returned as Mechanism_Type)
...
--    Not_Assigned (Flag115)
--       Present in all entities, but relevant only for variables, constants
--       and out parameters. This flag is set if the object is never set. For
--       constants, the flag is normally set. The only time that constants
--       will have this flag not set is if they are modified by the generated
--       code (e.g. to set discriminants). The back end must not treat any
--       such constants as constants, since they are from a code generation
...
--       rather than Set_Not_Assigned precisely to deal properly with
...
--       Appplies to discriminants. Set only for a discriminant returned by
...
--       Applies to all entities. This is a synonym for Homonym used in
...
--       Applies to array entities. Returns the number of dimensions of
...
--       Applies to E_Record_Type.  Points to the subtype to use for a
...
--    Referenced_Object (Node20)
--       Present in type entities constructed for unconstrained objects, or
--       objects that depend on discriminants. Points to the expression from
--       which the actual subtype of the object can be evaluated.
...
--    Implementation_Type (synthesized)
--       Applies to all entities. For all entities other than incomplete or
--       private types, this is the identify functoin. For incomplete or
--       private types, it is the implementation type used to implement the
--       private type. This is ofen the same as the full view, but not always,
--       since the full view can itself be a private type. The result returned
--       by Implementation_Type is never a private type. Implementation_Type
--       may return Empty if it is applied to an incomplete or private type
--       whose full type declaration has not yet been processed.
--
--       Note: the reason this attribute applies to all entities, and not
--       just types, is to legitimize code where Implementation_Type is
--       applied to an entity which may or may not be a type, with the
--       intent that if it is a type, its underlying type is taken.
--
--       Note: this is not fully implemented yet, but when it is, it will
--       probably replace the use of Underlying_Type ???

--    Implementation_Type_For_Private (Node16)
--       Present in all private and incomplete types. Points to the entity
--       that is the Implementation_Type for the private type (see above).
--       Set to Empty for an incomplete or private type whose full type
--       declaration has not yet been processed.
...
--       Applies to all entities. If an size clause is present in the rep
...
--       chain is copied for a subtype, it can inherit a size clause that is
...
--       Applies to all entities. This is the identify function except in
...
--
--       Note: This attribute is obsolescent, to be replaced by the use
--       of Implementation_Type when that is completed ???
...
--       is used to suppress warnings for a given entity.
   ...
   --  E_General_Access_Type
   --  E_Access_Subprogram_Type
   --  R_Access_Protected_Subprogram_Type
       E_Anonymous_Access_Type;
   ...
   --    Full_View                     (Node11)
   --    Esize                         (Uint12)
   --    Class_Wide_Type               (Node17)   (base type only)
   --    Referenced_Object             (Node20)
   --    Associated_Node_For_Itype     (Node22)
   ...
   --  E_Access_Subprogram_Type
   --    Equivalent_Type               (Node7)    (remote types only)
   --    Directly_Designated_Type      (Node10)
   --    Needs_No_Actuals (Flag22)
   --    (plus type attributes)
   ...
   --  E_Array_Subtype
   --    First_Index                   (Node9)
   --    Component_Type                (Node10)   (base type only)
   ...
   --    DTC_Entity                    (Node16)
   --    Renaming_Map                  (Uint17)
   --    Corresponding_Equality        (Node19)   (non-generic case only)
   --    Address_Clause                (Node20)
   --    First_Optional_Parameter      (Node23)   (non-generic case only)
   --    Mechanism                     (Uint22)   (returns Mechanism_Type)
   --    Default_Expressions_Processed (Flag108)
   --    Has_Completion                (Flag26)
   --    Has_Controlling_Result        (Flag98)
   --    Has_Master_Entity             (Flag21)
   ...
   --    Is_Inlined                    (Flag11)
   --    Is_Intrinsic_Subprogram       (Flag64)
   --    Is_Machine_Code_Subprogramm   (Flag137)  (non-generic case only)
   --    Is_Private_Descendant         (Flag53)
   --    Is_Pure                       (Flag44)
   --    Needs_No_Actuals              (Flag22)
   --    Return_Present                (Flag54)
   ...
   --    Private_Dependents            (Elist7)
   --    Girder_Constraint             (Elist14)
   --    Implementation_Type_Private   (Node16)
   --    Has_Discriminants             (Flag5)
   ...
   --    First_Entity                  (Node9)
   --    Last_Entity                   (Node10)
   --    Is_Machine_Code_Subprogramm   (Flag137)
   --    Is_Pure                       (Flag44)
   --    Is_Intrinsic_Subprogram       (Flag64)
   --    Default_Expressions_Processed (Flag108)
   ...
   --    Associated_Formal_Package     (Node12)
   --    Generic_Renamings             (Elist14)  (for an instance)
   --    Inner_Instances               (Elist14)  (for a generic package)
   --    First_Private_Entity          (Node16)
   --    Renaming_Map                  (Uint17)
   --    Shadow_Entities               (Node20)
   --    Has_All_Calls_Remote          (Flag79)
   ...
   --    Discriminant_Constraint       (Elist6)
   --    Corresponding_Concurrent_Type (Node7)
   --    Corresponding_Remote_Type     (Node8)    (base type only)
   --    First_Entity                  (Node9)
   --    Last_Entity                   (Node10)
   --    Parent_Subtype                (Node19)
   --    Primitive_Operations          (Elist13)
   ...
   --  E_String_Literal_Subtype
   --    First_Index                   (Node9)    (always Empty)
   --    Component_Type                (Node10)   (base type only)
   --    Packed_Array_Type             (Node14)
   --    String_Literal_Low_Bound      (Node15)
   --    String_Literal_Length         (Uint16)
   ...
   function Block_Node                         (Id : E) return E;
   ...
   function Extra_Formal_Accessibility         (Id : E) return E;
   function Extra_Formal_Constrained           (Id : E) return E;
   ...
   function Has_Delayed_Elaboration            (Id : E) return B;
   ...
   function Has_Pending_Instantiations         (Id : E) return B;
   ...
   function Implementation_Type_Private        (Id : E) return E;
   ...
   function Is_Common_Object                   (Id : E) return B;
   ...
   function Not_Assigned                       (Id : E) return B;
   ...
   function Implementation_Type                (Id : E) return E;
   ...
   procedure Set_Extra_Formal_Accessibility    (Id : E; V : E);
   procedure Set_Extra_Formal_Constrained      (Id : E; V : E);
   ...
   procedure Set_Has_Delayed_Elaboration       (Id : E; V : B := True);
   ...
   procedure Set_Has_Interrupt_Handler         (Id : E; V : B := True);
   ...
   procedure Set_Has_Pending_Instantiations    (Id : E; V : B := True);
   ...
   procedure Set_Implementation_Type_Private   (Id : E; V : E);
   ...
   procedure Set_Is_Common_Object              (Id : E; V : B := True);
   ...
   procedure Set_Not_Assigned                  (Id : E; V : B := True);
   ...
   procedure Set_Esize (Id : E; V : Int);
   --  A handy overloading of Set_Esize that takes the size as an Int

   procedure Set_RM_Size (Id : E; V : Int);
   --  A handy overloading of Set_RM_Size that takes the size as an Int
   ...
   pragma Inline (Extra_Formal_Accessibility);
   pragma Inline (Extra_Formal_Constrained);
   ...
   pragma Inline (Has_Delayed_Elaboration);
   ...
   pragma Inline (Implementation_Type_Private);
   ...
   pragma Inline (Is_Common_Object);
   ...
   pragma Inline (Set_Extra_Formal_Accessibility);
   pragma Inline (Set_Extra_Formal_Constrained);
   ...
   pragma Inline (Set_Has_Delayed_Elaboration);
   ...
   pragma Inline (Set_Implementation_Type_Private);
   ...
   pragma Inline (Set_Is_Common_Object);
   ...
   pragma Inline (Set_Not_Assigned);
