...
--  tree, transforming it in various ways and adding semantic information.
   ...
   --  Replace_Substitute_Tree also preserves the setting of Comes_From_Source.
   --
   --  Note, New_Node may not contain references to Old_Node, for example as
   --  descendents, since the rewrite would make such references invalid. If
   ...
   --  be to a relocated copy of Old_Node (see Relocate_Node procedure).
   --
   --  Replace_Substitute_Tree is used in certain circumstances where it is
   --  desirable to suppress any history of the rewriting operation. Notably,
   --  it is used when the parser has mis-classified a node (e.g. a task
   --  entry call that the parser has parsed as a procedure call), and also
   --  in Sem_Dist to control the stub output (based on the original tree).
   ...
   --  of a subtree which was installed using Rewrite_Substitute_Tree)
   ...
   package Unchecked_Access is
      ...
      function List20 (N : Node_Id) return List_Id;
      pragma Inline (List20);
      ...
      procedure Set_List20 (N : Node_Id; Val : List_Id);
      pragma Inline (Set_List20);
   ...
   package Atree_Private_Part is
      ...
      type Node_Record (Is_Extension : Boolean := False) is record
         ...
         case Is_Extension is
            ...
            --    Field6-10      Used to hold Field19-Field23
            --    Field11        Currently unused
