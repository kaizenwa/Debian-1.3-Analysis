...
package body Ada.Direct_IO is
   ...
   Esiz : constant size_t := (Element_Type'Object_Size + SU - 1) / SU;
   ...
   begin
      --  Note: we read into an intermediate buffer because we may have the
      --  case of discriminated records, where a discriminant check may be
      --  required, or we may need to assign only part of the record.

      DIO.Read (FP (File), Buf'Address, Esiz, DPCount (From));
      Item := Buf;
   end Read;
   ...
   begin
      DIO.Read (FP (File), Buf'Address, Esiz);
      Item := Buf;
   end Read;
