...
package System.File_IO is
   ...
   --  and Form must be in lower case. If no entry matches, then Start is
   --  set to zero. Use_Error can be raised if a malformed string is found,
   --  but there is no guarantee of full syntax checking.
