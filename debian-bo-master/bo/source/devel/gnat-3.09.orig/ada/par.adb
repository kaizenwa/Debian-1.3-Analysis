...
function Par (Configuration_Pragmas : Boolean) return List_Id is
   ...
   Save_Features_On : constant Boolean := Features_On;
   --  Saves state of Features_On flag for restore on exit (so that a recursive
   --  call on the compiler does not disturb the proper setting of Features_On)
   ...
   --  is marked (by setting the Bad_Is_Detected flag true.
   ...
   SS_Eltm_Ortm      : constant SS_Rec := SS_Rec'(F, T, F, T, F, F, F, F);
   ...
   SS_Ortm           : constant SS_Rec := SS_Rec'(F, F, F, T, F, F, F, F);
   ...
   --  Single entry in the table. The fields are used as follows:

   --  The Lreq field is a flag indicating whether the label, if present,
   --  is required to appear on the end line. It is referenced only in the
   --  case of Etyp = E_Name or E_Suspicious_Is where the name may or may not
   --  be required (yes for labeled block, no in other cases). Note that for
   ...
   --  The Ecol field contains the absolute column number (with tabs expanded)
   --  of the expected column of the end assuming normal Ada indentation usage.
   --  If the RM_Column_Check mode is set, this value is used in assessing the
   --  syntactic correctness of various constructs. Otherwise it is used only
   --  to control heuristic error recovery actions.

   --  The Labl field is used only for the LOOP and BEGIN cases, and is the
   ...
   --  The Decl field points to the list of declarations (i.e. the declarative
   --  part) associated with this construct. It is set only in the END [name]
   --  cases, and is set to No_List for all other cases which do not have a
   --  declarative unit associated with them. This is used for determining the
   --  proper location for implicit label declarations.

   --  Sloc is the source location of the opening token of the construct. This
   --  is used to refer back to this line in error messages (such as missing
   --  or incorrect end lines). The Sloc field is not used, and is not set, if
   --  a label is present (the Labl field provides the text name of the label
   --  in this case, which is fine for error messages).

   --  S_Is is relevant only if Etyp is set to E_Suspicious_Is or E_Bad_Is.
   --  It records the location of the IS that is considered to be suspicious.
   ...
   package Ch3 is
      ...
      --  This variable is set by a call to P_Declarative_Part. Normaly it
   ...
   package Sync is
      ...
      --  Used to skip a junk list of items enclosed in parentheses which are
      --  expected to lie on the current line. Tokens are skipped until a
      --  right paren is skipped, or until end of line, whichever is first.
   ...
   package Util is
      ...
      --  past it, otherwise the call has no effect at all.
      ...
      --  is incremented. The caller can then fill in the new top stack entry
      --  at Scope.Table (Scope.Last).
...
begin
      ...
      begin
         loop
            if Token = Tok_EOF then
               ...
               Error_Msg_SC
                  ("only pragmas allowed in gnat.adc");
               return Error_List;
   ...
   --  Normal case of compilation unit
      ...
      then
         --  Always force Ada_95 mode for predefined units
         ...
         if Current_Source_Unit = Main_Unit
      ...
      --  If not predefined unit, set Ada_83 mode from switches
      ...
      --  Set features collection from switches (for main unit only)

      Features_On := Xref_Flag_9 and (Current_Source_Unit = Main_Unit);
