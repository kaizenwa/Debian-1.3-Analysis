...
package body Ada.Strings.Unbounded is
   ...
   begin
      if Position <= Source.Reference'Length - NL + 1 then
         ...
         declare
            Temp : constant String := Source.Reference.all;

         begin
            Free (Source.Reference);
            Source.Reference := new
              String'(Fixed.Overwrite (Temp, Position, New_Item));
         end;
      end if;
   end Overwrite;
   ...
   begin
      Unbounded_String'Read (Stream, Result);
      return Result;
   end Unbounded_String_Input;

   ---------------------------
   -- Unbounded_String_Read --
   ---------------------------
   ...
   is
      ...
      subtype Stream_String is String (1 .. Length);

   begin
      if Item.Reference /= Null_Unbounded_String.Reference then
         Free (Item.Reference);
      end if;

      Item.Reference := new Stream_String;
      Stream_String'Read (Stream, Item.Reference.all);
   end Unbounded_String_Read;

   ----------------------------
   -- Unbounded_String_Write --
   ----------------------------

   --  This defines the stream representation for an Unbounded_String
   ...
   is
   begin
      Natural'Write (Stream, Item.Reference.all'Length);
      String'Write  (Stream, Item.Reference.all);
   end Unbounded_String_Write;
