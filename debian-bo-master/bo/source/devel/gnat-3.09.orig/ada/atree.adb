...
package body Atree is
   ...
   --  and their corresponding original tree nodes. Rewrite_Substitute_Tree
   --  makes an entry in this table for use by Original_Node. By default,
   --  if no call is made to Rewrite_Substitute_Tree, the entry in this
   --  table points to the original unwritten node.
   ...
   package Orig_Nodes is new Table (
   ...
   begin
      if Source <= Empty_Or_Error then
      ...
      elsif Source /= Orig_Nodes.Table (Source) then
         ...
         Orig_Nodes.Table (Orig_Nodes.Last) := Nodes.Last;
   ...
   package body Unchecked_Access is
      ...
      function List20 (N : Node_Id) return List_Id is
      begin
         pragma Assert (Nkind (N) in N_Entity);
         return List_Id (Nodes.Table (N + 3).Field7);
      end List20;
      ...
      procedure Set_List20 (N : Node_Id; Val : List_Id) is
      begin
         pragma Assert (Nkind (N) in N_Entity);
         Nodes.Table (N + 3).Field7 := Union_Id (Val);
      end Set_List20;
