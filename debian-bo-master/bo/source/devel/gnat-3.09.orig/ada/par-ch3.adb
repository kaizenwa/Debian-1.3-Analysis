...
package body Ch3 is
   ...
   --  such a context. Msg_Id saves the Id of the first "declaration expected"
   --  message, since only one such message is generated per declarative part,
   --  and also, this message is changed to "BEGIN expected" if no BEGIN is
   ...
   begin
      ...
      if Token = Tok_Abstract then
         Note_Feature (Abstract_Types, Token_Ptr);
      ...
      --  only runs twice if an error is detected, to try again after possible
      ...
      loop
         case Token is
            ...
            when Tok_Tagged =>
               Note_Feature (Tagged_Types, Token_Ptr);
               Scan; -- past TAGGED
            ...
            when Tok_Limited =>
               ...
               if Token = Tok_Record or else Token = Tok_Null then
                  Note_Feature (Limited_Record_Types, Prev_Token_Ptr);
   ...
   begin
      ...
      --  we assume it was meant to be a colon.
         ...
         if Token = Tok_Constant then
            ...
            Statement_When_Declaration_Expected (Decls, Done, Msg_Id, In_Spec);
            return;
         end if;
      ...
      --  Otherwise we have an error situation
         ...
         --  token after it. Just complain that we need a colon and plough on.
      ...
      Ident_Loop : loop
         ...
         --  Check for const in place of constant

         elsif Token_Name = Name_Const then
            Save_Scan_State (Scan_State);
            Scan; -- past const

            if Token = Tok_Identifier or else Token = Tok_Array then
               Restore_Scan_State (Scan_State);
               Error_Msg_SC ("Incorrect spelling of keyword CONSTANT");
               Token := Tok_Constant;
            else
               Restore_Scan_State (Scan_State);
            end if;
         ...
         --  Aliased case (note that an object definition is required)
            ...
            Note_Feature (Aliased_Objects, Token_Ptr);
            Scan; -- past ALIASED
            Decl_Node := New_Node (N_Object_Declaration, Ident_Sloc);
            Set_Aliased_Present (Decl_Node, True);
   ...
   begin
      ...
      Note_Feature (Modular_Integer_Types, Token_Ptr);
   ...
   begin
      ...
      if Token = Tok_Digits then
         ...
         Note_Feature (Decimal_Fixed_Point, Token_Ptr);
         Scan; -- past DIGITS
         Expr_Node := P_Expression_No_Right_Paren;
         Check_Simple_Expression_In_Ada_83 (Expr_Node);
         Set_Digits_Expression (Def_Node, Expr_Node);
   ...
   begin
      if Token /= Tok_Left_Paren then
         ...
         if Token = Tok_Box then
            ...
            Note_Feature (Unknown_Discriminant_Parts, Token_Ptr);
            Scan; -- past the box
         ...
         Specification_Loop : loop
            ...
            Ident_Loop : loop
               ...
               if Token = Tok_Access then
                  ...
                  Note_Feature (Access_Discriminants, Token_Ptr);
   ...
   begin
      ...
      if Token = Tok_Procedure then
         ...
         Note_Feature (Access_To_Subprogram_Types, Token_Ptr);
         ...
         if Token = Tok_All or else Token = Tok_Constant then
            ...
            if Token = Tok_All then
               Note_Feature (General_Access_Types, Token_Ptr);
               Set_All_Present (Type_Def_Node, True);

            else
               Note_Feature (Access_To_Constant_Types, Token_Ptr);
   ...
   begin
      ...
      loop
         P_Declarative_Items
           (Decls, Done, Missing_Begin_Msg, In_Spec => False);
         exit when Done;
      end loop;
   ...
   begin
      ...
      case Token is
         ...
         when Tok_Identifier =>
            P_Identifier_Declarations (Decls, Done, Msg_Id, In_Spec);
   ...
   begin
      ...
      loop
         P_Declarative_Items (Decls, Done, Msg, In_Spec => True);
         exit when Done;
      end loop;
      ...
      --  Test for body scanned, not acceptable as basic declarative item
