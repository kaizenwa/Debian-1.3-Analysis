...
package Sinfo is
   ...
   --  Cleanup_Call (Node5-Sem)
   --     This is optionally present in a handled statement sequence node.
   --     It represents the call to be made to a cleanup procedure on scope
   --     exit (either normal or abnormal). This is a N_Procedure_Call node
   --     where the name of the procedure is a simple identifier, and so are
   --     all the parameters.
   ...
   --     a subprogram naming declaration when it is a Renaming_As_Body.
   ...
   --     point to the tree for the default expression). If the default
   --     expression has side effects, then a function is constructed that
   --     returns the expression value. In this case, Default_Expression
   --     contains a pointer to the original expression, but Expression is
   --     rewritten to be a call to this function. Default_Expression is
   --     used for conformance checking.

   --  Do_Access_Check (Flag11-Sem)
   --     This flag is set on N_Explicit_Dereference, N_Indexed_Component or
   ...
   --     ??? field to be deleted as soon as new aggregate scheme is in place
   --
   ...
   --     happens bottom up. To delay it we set this flag. This is done
   --     to avoid creating a temporary for each level of a nested aggregates.
   --     This is actually a requirement if we want to generate the proper
   ...
   --     In fact, during bottom up aggregate expansion we use a temporary
   --     to store the aggregate value, and then copy the temporary into the
   --     original array. In top-down aggregate expansion we save the
   --     aggregate in the original array directly.
   ...
   --     initially set true for subprogram specs and bodies and false for
   --     package specs and bodies. Gigi may set the flag in the package
   --     case if it determines that no elaboration code is generated. Note
   --     that this flag is not related to the Is_Preelaborated status, there
   --     can be preelaborated packages that generate elaboration code, and
   --     non-preelaborated packages which do not generate elaboration code.
   ...
   --  Is_Controlling_Argument (Flag16-Sem)
   ...
   --  Next_Interp (Node2-Sem)
   --     Used to chain entries in the N_Interpretation node list used to
   --     store multiple overloaded interpretations of a single node.
   ...
   --  No_Default_Init (Flag7-Sem)
   --     Present in N_Object_Declaration to indicate that the object must
   --     not be initialized (by Initialize or a call to _init_proc). This
   --     is needed for controlled aggregates. When the Object declaration
   --     has an expression, this flag means that this expression should not
   --     be taken into account (needed for in place initialization with
   --     aggregates)

   --  Original_Discriminant (Node2-Sem)
   --     Present in identifiers. Used in references to discriminants that
   --     appear in generic units. Because the names of the discriminants
   --     may be different in  an instance, we use this field to recover
   --     the position of the discriminant in the original type, and replace
   --     it with the discriminant at the same position in the instantiated
   --     type.
   ...
   --     nested instantiations where the substituted node can be copied.
      ...
      --  Cleanup_Call (Node5-Sem) Cleanup call (set to Empty if no cleanup)
      --  First_Real_Statement (Node2-Sem)
      ...
      --  Note the Identifier field is obsolescent, and will be removed
      --  once all instances of cleanup calls use Cleanup_Call ??? It is
      --  the name of a parameterless procedure call.
   ...
   --  show this syntax.
      ...
      --  Expressions (List1) points to list of operands
      --  plus fields for expression
      ...
      --  reference is used to point to a chain of N_Interpretation nodes, one
      ...
      --  Note: N_Interpretation nodes are not yet used ???

      --  N_Interpretation
      --  Sloc is copied from the node to which the interpretation applies
      --  Chars (Name1) is copied from Chars (Etype)
      --  Next_Interp (Node2-Sem)
      --  Entity (Node4-Sem)
      --  Etype (Node5-Sem)
      ...
      --  This node is created during semantic analysis to replace a node
      --  for an expression that is determined to definitely raise constraint
      --  error. The creation of this node will usually be accompanied by a
      --  warning message (unless it appears within the right operand of a
      --  short circuit form whose left argument is static and decisively
      --  eliminates elaboration of the constraint error.

      --  The N_Raise_Constraint_Error node may also stand alone in place
      --  of a declaration or statement, in which case it simply causes
      --  the exception to be raised (i.e. it is equivalent to a raise
      --  statement that raises Constraint_Error). This use is distinguished
      --  by the fact that the Etype in this case is Standard_Void_Type, In
      --  the subexprssion case, the Etype is the same as the type of the
      --  subexpression which it replaces.
      ...
      --  value of the expressions is True.

      --  Sprint syntax: [constraint_error]
      --             or: [constraint_error when condition]
   ...
   --  overloading resolution for expressions), and also the special case of
   --  N_Accept_Statement (which has Itypes and therefore cannot be fit into
   --  the range of statement types ???).
   ...
   procedure Set_Has_No_Elab_Code
     (N : Node_Id; Val : Boolean := True);    -- Flag17
   ...
   pragma Inline (Has_No_Elab_Code);
   ...
   pragma Inline (Set_Cleanup_Call);
   ...
   pragma Inline (Set_Elaborate_Body_Present);
   ...
   pragma Inline (Set_Has_No_Elab_Code);
