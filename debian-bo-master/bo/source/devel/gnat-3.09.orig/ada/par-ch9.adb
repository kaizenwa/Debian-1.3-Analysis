...
package body Ch9 is
   ...
   begin
      Note_Feature (Protected_Units_And_Operations, Prev_Token_Ptr);
      Push_Scope_Stack;
      Scope.Table (Scope.Last).Etyp := E_Name;
      Scope.Table (Scope.Last).Ecol := Start_Column;
      Scope.Table (Scope.Last).Lreq := False;
   ...
   begin
      ...
      if Token = Tok_Do then
         ...
         if Present (Exception_Handlers (Hand_Seq)) then
            ...
            Note_Feature (Exception_Handler_In_Accept, Sloc (Hand_Seq));
         end if;
   ...
   begin
      ...
      Note_Feature (Requeue_Statement, Token_Ptr);
      Scan; -- past REQUEUE
   ...
   begin
      Note_Feature (Delay_Until, Prev_Token_Ptr);
   ...
   begin
      ...
      if Token in Token_Class_Desig then
         ...
         --  Only remaining case is THEN ABORT (asynchronous select)
            ...
            Note_Feature (Asynchronous_Select, Select_Sloc);
            Select_Node :=
