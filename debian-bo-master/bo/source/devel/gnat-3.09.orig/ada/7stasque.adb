...
--  This version of the body implements FIFO queueing, and is stored under
--  under the default name s-tasque in the library. The binder takes care
--  of susbtituing the alternative priority queuing version (s-tasqup) if
--  priority queueing is in effect.
...
package body System.Tasking.Queuing is
   ...
   --  Entry Queues implemented as doubly linked list, FIFO ordered
   ...
   is
   begin
      --  Body of this procedure in FIFO queuing should be null so that
      --  no queue reordering will take place.
      null;
