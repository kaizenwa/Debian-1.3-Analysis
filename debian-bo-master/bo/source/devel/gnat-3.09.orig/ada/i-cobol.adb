...
package body Interfaces.COBOL is
   ...
   function Valid_Binary (Item : in Byte_Array) return Boolean;
   ...
   is
      ...
      R   : Byte_Array (1 .. Len) := Item;

   begin
      if not Valid_Binary (Item) then
   ...
   begin
      ...
      case Packed_Representation is
         when IBM =>
            for J in reverse Item'First .. Item'Last - 1 loop
   ...
   begin
      ...
      begin
         if Len = 1 then
            ...
            B := B4'(B (4), B (3), B (2), B (1));

         else --  B = 8
            B := B8'(B (8), B (7), B (6), B (5),
                     B (4), B (3), B (2), B (1));
         end if;
      end;
   end Swap;
   ...
   --  Note: we assume twos complement format, and there are no invalid
   --  bit patterns for binary values. The only check we make is that
   --  the length is appropriate (1, 2, 4, or 8)

   function Valid_Binary (Item : Byte_Array) return Boolean is
      Len : constant Natural := Item'Length;

   begin
      return Len = 1 or else Len = 2 or else Len = 4 or else Len = 8;
   end Valid_Binary;
   ...
   package body Decimal_Conversions is
      ...
      --  Note that these tests are all done at compile time
