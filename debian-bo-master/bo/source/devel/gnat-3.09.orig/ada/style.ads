...
package Style is
   ...
   procedure Check_Line_Terminator;
   --  Called with Scan_Ptr pointing to the first line terminator terminating
   --  the current line, used to check for appropriate line terminator.
   ...
   procedure Set_Max_Line_Length (N : in out Nat);
   --  This procedure has the opportunity of setting a new value for the
   --  maximum line length which is less than the standard maximum value
   --  permitted by the system (which is the value passed in on entry).
