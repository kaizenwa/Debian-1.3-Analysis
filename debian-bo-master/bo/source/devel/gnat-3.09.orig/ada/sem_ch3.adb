...
package body Sem_Ch3 is
   ...
   --  Derived_Type /= Defining_Identifier (N)).
   ...
   --  No_Elist if T is not a discriminanted type, and to an empty list if
   ...
   --  an access type.  Ie:  its sole purpose is the designated type of an
   ...
   --  Build the list of entities for a constrained discriminanted record
   ...
   --  itypes created.
   --  Typ is the base discriminanted type for the subtype Subt.
   --  Constraints is the list of discriminant constraint for Typ.

   function Constrain_Discrim_Type
   ...
   procedure Delay_Array_Type (T : Entity_Id);
   --  Delay freezing of an array type T and its corresponding indexes.
   --  Note that this routine never sets Has_Delayed_Freeze if Is_Itype
   --  is set, since these are cases where delayed freezing is not required.
   ...
   procedure Process_Incomplete_Dependents
     (N      : Node_Id;
      Full_T : Entity_Id;
      Inc_T  : Entity_Id);
   --  Process all the subtypes of an incomplete type.
   ...
   begin
      ...
      if Present (E) then
         ...
         --  for dispatching functions over an anonyomous access type
      ...
      --  Case of unconstrained type
         ...
         --  All OK, constrain the type with the expression size
            ...
            Set_Is_Constr_Subt_For_U_Nominal (T);

            if Aliased_Present (N) then
               Set_Is_Constr_Subt_For_UN_Aliased (T);
            end if;
      ...
      if Constant_Present (N) then
         ...
         --  A variable is considered to be modified if it is initialized.
         --  We also leave Not_Assigned clear if the object type is fully
         --  initialized since the default initialization is sufficient.

         if No (E) and then not Is_Fully_Initialized_Type (T) then
            Set_Not_Assigned (Id);
         end if;
      end if;
      ...
      --  ??? Restore the limitedness of Exception_Occurrence

      if Chars (T) = Name_Exception_Occurrence
        and then Chars (Scope (Scope (T))) = Name_Ada
      then
         Set_Is_Limited_Record (T, True);
      end if;
      ...
      then
         ...
         then
            ...
            begin
               ...
               if Present (Implicit_Call)
                 and then not Has_Completion (Implicit_Call)
                 and then Scope (Implicit_Call) = Scope (Id)
               then
                  ...
                  Error_Msg_NE ("?implicit call to& will raise Program_Error",
                    N, Implicit_Call);
               end if;
            end;
         end if;
      ...
      if Present (Prev_Entity) and then Is_Frozen (Prev_Entity) then
   ...
   begin
      ...
      --  Otherwise process the full constant declaration
         ...
         if Present (E) and then No (Etype (E)) then
            ...
            --  ??? bypass check for Ada.Execptions.Null_Occurrence because
            --  of glitch in RM that would otherwise make impossible to
            --  represent Exception_Occurrence as a limited record

            if Chars (T) = Name_Exception_Occurrence
              and then Chars (Scope (Scope (T))) = Name_Ada
            then
               Set_Is_Limited_Record (T, False);
            end if;
         end if;
      end if;
   ...
   begin
      ...
      if Is_Integer_Type (T)  then
         ...
         if T = Universal_Fixed then
            ...
            begin
               Rewrite_Substitute_Tree (E, Conv);
               Analyze (E);
            end;
      ...
      if not Is_OK_Static_Expression (E) then
         ...
         Rewrite_Substitute_Tree (N, Make_Integer_Literal (Sloc (N), Uint_0));
   ...
   begin
      ...
      then
         T := Make_Defining_Identifier (Sloc (P), New_Internal_Name ('S'));
   ...
   begin
      ...
      Rewrite_Substitute_Tree (E, Make_Integer_Literal (Sloc (E), Uint_1));
   ...
   begin
      ...
      case Nkind (Def) is
         ...
         when N_Access_To_Object_Definition =>
            ...
            if (Distribution_Stub_Mode = Compile_Caller_Stub_Spec or
   ...
   begin
      ...
      then
         ...
         then
            declare
               ...
               Priv_Constr : constant Boolean
                 := Is_Constrained (Priv_Parent)
                    or else Nkind (Priv_Indic) = N_Subtype_Indication;
               Full_Constr : constant Boolean
                 := Is_Constrained (Full_Parent)
                    or else Nkind (Full_Indic) = N_Subtype_Indication;
            ...
            begin
               ...
               while Present (Priv_Discr) and then Present (Full_Discr) loop
                  ...
                  Priv_Discr := Next_Discriminant (Priv_Discr);
                  Full_Discr := Next_Discriminant (Full_Discr);
               end loop;
         ...
         --  discrimiants, then the parent subtype of the full view shall
      ...
      if Is_Tagged_Type (Full_T) then
         ...
         begin
            if Is_Tagged_Type (Priv_T) then
               ...
               --  In this case the partial view is untagged. Let's find
               --  out all the primitives we missed (those that appear between
               --  the two views)
               ...
               while Present (Prim) and then Prim /= Priv_T loop
                  if (Ekind (Prim) = E_Procedure
                    ...
                    and then Find_Dispatching_Type (Prim) = Full_T
                  then
      ...
      while Present (Inc_Elmt) loop
         ...
         if Ekind (Inc_Subt) = E_Subprogram_Type then
            ...
            if Has_Discriminants (Full_T) then
               Disc_Constraint := Discriminant_Constraint (Inc_Subt);
            else
               Disc_Constraint := No_Elist;
            end if;

            Build_Discriminated_Subtype (Full_T, New_Subt, Disc_Constraint, N);
            Set_Full_View (Inc_Subt, New_Subt);
         end if;

         Inc_Elmt := Next_Elmt (Inc_Elmt);
      end loop;
   ...
   begin
      ...
      if Nkind (S) = N_Subtype_Indication then
         ...
         then
            ...
            Rewrite_Substitute_Tree (S, New_Copy_Tree (Subtype_Mark (S)));
   ...
   begin
      ...
      while Present (Elmt) loop
         ...
         if Is_CPP_Class (Etype (T))
           and then not Is_CPP_Class (T)
   ...
   -----------------------------------------
   -- Check_Constrained_Aliased_Component --
   -----------------------------------------
      ...
      if not Is_Limited_Type (T) then
         if Ekind (T) = E_Record_Type then
            ...
            while Present (C) loop
               ...
               then
                  Error_Msg_N
                    ("aliased component must be constrained", C);
               end if;
            ...
            then
               Error_Msg_N
                 ("aliased component type must be constrained", T);
            end if;
         end if;
      end if;
   ...
   procedure Check_Completion (Body_Id : Node_Id := Empty) is
      ...
      begin
         ...
         --  we had missing subunits, or else something is very wrong.

         if not Comes_From_Source (E) then

            if Errors_Detected > 0 or else Subunits_Missing then
               return;
            else
               pragma Assert (False); null;
            end if;
            ...
            --  reference to the non-completed declaration.
               ...
               then
                  ...
                  declare
                     ...
                     Decl      : Node_Id := Get_Declaration_Node (Candidate);
   ...
   begin
      ...
      Set_Size_Info      (Implicit_Base,                (Base_Type));
      Set_First_Rep_Item (Implicit_Base, First_Rep_Item (Base_Type));
   ...
   begin
      ...
      Set_RM_Size (Implicit_Base, Uint_0);
   ...
   procedure Modular_Type_Declaration (T : Entity_Id; Def : Node_Id) is
      ...
      begin
         ...
         if Bits <= 8 then
            ...
            Set_Esize (T, System_Max_Binary_Modulus_Power);
         end if;
   ...
   begin
      ...
      Analyze_And_Resolve (Low_Bound  (Scalar_Range (T)), T);
      Analyze_And_Resolve (High_Bound (Scalar_Range (T)), T);
   ...
   begin
      ...
      Table_Type := Create_Itype (E_Enum_Table_Type, Parent (Def), T, 'T');
   ...
   procedure Delay_Array_Type (T : Entity_Id) is
      Ind : Node_Id;

   begin
      if not Is_Itype (T) then
         Set_Has_Delayed_Freeze (T);

         --  ??? If an array subtype is delayed, it is wiser to delay the
         --  subscript subtypes along with the array but this seems to cause
         --  problems with the length check code in the case of unconstrained
         --  object decls. For now, limit the fix to the embedded implicit
         --  types case where this is really necessary.

         if Is_Type (Scope (T)) then
            Ind := First_Index (T);
            while Present (Ind) loop
               if not Is_Frozen (Etype (Ind))
                 and then not Is_Itype (Etype (Ind))
               then
                  Set_Has_Delayed_Freeze (Etype (Ind));
               end if;
   ...
   begin
      ...
      while Present (Component) loop
         ...
         if Has_Controlled_Component (Etype (Component))
   ...
   begin
      ...
      then
         ...
         elsif not Is_Limited_Record (Current_Scope) then
   ...
   is
      ...
      --  D within the discriminant list of the discriminanted type T.
      ...
      begin
         ...
         --  known to belong to the discriminanted type, falling through the
   ...
   begin
      ...
      while Present (Constr) loop
         ...
         --  Positional association forbidded after a named association.
      ...
      for J in Discr_Expr'Range loop
         ...
         --  and we are in a derived type declaration we have a discriminant
         --  renaming. Establish link between new and old discriminant.
         ...
         --  If we have found a discriminant in the constraint 3.4(26) and
         --  3.8(18) demand that no range checks are performed are after
         --  evaluation. In all other cases perform a range check.
   ...
   is
      ...
      --  variables local to Create_Constrained_Components
   ...
   begin
      ...
      --  ??? if the discriminants are only used in the variant part of the
      --  base record type to specify which components are valid, this routine
      --  will duplicate components that are not constrained by the
      --  discriminant. Their Etypes will not be duplicated, however.
      --  At some later stage we should avoid the duplication in this case.
      ...
      while Present (Old_C) loop
         ...
         Set_Etype
           (New_C,
            Constrain_Discrim_Type (Old_T, Subt, Decl_Node, Typ, Constraints));

         Old_C := Next_Component (Old_C);
      end loop;
   ...
   is
      ...
      --  T is an array or discriminanted type, C is a list of constraints
      ...
      begin
         ...
         if Has_Unknown_Discriminants (Btyp)
           and then Present (Implementation_Type (Btyp))
         then
            Btyp := Implementation_Type (Btyp);
         end if;
      ...
      begin
         ...
         then
            ...
            pragma Assert (Discrim_Scope = Typ
               ...
               --  Same as above but for the case where the discriminants
   ...
   --  Start of processing for Constrain_Discrim_Type
   ...
   begin
      ...
      then
         if No (Underlying_Type (Parent_Type))
           or else Has_Private_Component (Parent_Type)
         then
            Error_Msg_N
      ...
      case Ekind (Parent_Type) is
         ...
         when Incomplete_Or_Private_Kind =>
            ...
            then
               ...
               --  ??? more work needs to be done to get this to
               ...
               then
                  declare
                     ...
                     Full_N  : Node_Id;
                     Full_P  : Entity_Id;
                     P_Scope : Entity_Id := Scope (Base_Type (Parent_Type));

                     procedure Copy_And_Build;
                     --  local utility to copy parent.
                     ...
                     begin
                        if Ekind (Parent_Type) in Record_Kind
                          or else (Ekind (Parent_Type) in Enumeration_Kind
                            and then
                              Root_Type (Parent_Type) /= Standard_Character
                             and then
                                Root_Type (Parent_Type) /=
                                                      Standard_Wide_Character
                              and then
                                not Is_Generic_Type (Root_Type (Parent_Type)))
                        then
                           Full_N := New_Copy_Tree (N);
                           Insert_After (N, Full_N);
                           Build_Derived_Type
                             (Full_N, Parent_Type, Full, True);
                        else
                           Build_Derived_Type (N, Parent_Type, Full, True);
                        end if;
                     end Copy_And_Build;

                  begin
                     Full := Make_Defining_Identifier (Sloc (Derived_Type),
                                                       Chars (Derived_Type));
                     Set_Is_Itype (Full);
                     Set_Full_View (Derived_Type, Full);

                     if not In_Open_Scopes (P_Scope) then
                        New_Scope (P_Scope);
                        Install_Private_Declarations (P_Scope);
                        Install_Visible_Declarations (P_Scope);
                        Copy_And_Build;
                        End_Package_Scope (P_Scope);
                     ...
                     elsif Get_Sloc_Unit_Number (Sloc (N)) /=
                           Get_Sloc_Unit_Number (Sloc (Parent_Type))
                     then
                     ...
                     Set_Scope                (Full, Current_Scope);
                     Set_Is_First_Subtype     (Full,
                                                Is_First_Subtype
                                                  (Derived_Type));
                     Set_Has_Size_Clause      (Full, False);
                     Set_Has_Alignment_Clause (Full, False);
                     Set_Next_Entity          (Full, Empty);
                     Set_Has_Delayed_Freeze   (Full);
                     Set_Is_Frozen            (Full, False);
                     Set_Freeze_Node          (Full, Empty);
                     Set_Depends_On_Private   (Full,
                     Has_Private_Component    (Full));
                  end;
               end if;
            end if;
   ...
   begin
      ...
      then
         ...
         while Present (Discr_Con_El) loop
            ...
            Discr_Con_El := Next_Elmt (Discr_Con_El);
            Discr := Next_Discriminant (Discr);
         end loop;
      end if;
   ...
   begin
      if not Is_Constrained (Parent_Type) then
         if Nkind (Indic) /= N_Subtype_Indication then
            ...
            Rewrite_Substitute_Tree (N, New_Indic);
            Analyze (N);
         end if;
      ...
      if Number_Dimensions (Parent_Type) = 1
        ...
        and then Ekind (Scope (Base_Type (Parent_Type))) /= E_Package
        and then Ekind (Scope (Base_Type (Parent_Type))) /= E_Generic_Package
      then
   ...
   begin
      ...
      Convert_Scalar_Bounds (Parent_Type, Derived_Type, Loc);
   ...
   --  in the root type was renamed in the derived type, then the derived type
   --  will contain explicit girder discriminants. Explicit girder
   ...
   --  objects of the derived type (see precise definition in einfo.ads). As an
   ...
   --  ??? I am sure there are other unconfortable cases that we will have to
   --      deal with.
   ...
   begin
      ...
      if Constraint_Present and then not Has_Discriminants (Parent_Base) then
         Error_Msg_N
              ("invalid constraint: type has no discriminant",
               Constraint (Indic));

         Constraint_Present := False;
         Rewrite_Substitute_Tree (Indic, New_Copy_Tree (Subtype_Mark (Indic)));
      end if;
      ...
      then
         ...
         if Constraint_Present then
            Dont_Care := Build_Discriminant_Constraints (Parent_Type, Indic);
         end if;
         ...
         Build_Derived_Type (New_Decl, Parent_Base, New_Base, False);
      ...
      if Is_Tagged then
         ...
         then
           ...
           Enclosing_Generic_Body (Parent_Base)
         then
            Error_Msg_N
      ...
      if Discriminant_Specs then
         ...
         then
            ...
            if not Is_Tagged or else Private_Extension then
               Discrim := First_Discriminant (Derived_Type);
               while Present (Discrim) loop
                  if not Is_Tagged
      ...
      Set_Discard_Names
        (Derived_Type, Discard_Names            (Parent_Type));
      ...
      --  Inherit fields from non private record types.
      ...
      if not Is_Tagged then
         ...
         New_Decl := New_Copy_Tree (Parent (Parent_Base), Map => Assoc_List);
         Set_Sloc (New_Decl, Loc);
   ...
   is
      ...
      Implicit_Base : constant Entity_Id  :=
                        Create_Itype (E_Enumeration_Type, N, Parent_Type, 'B');
   ...
   begin
      ...
      if Is_Ordinary_Fixed_Point_Type (Parent_Type)
   ...
   is
      ...
      begin
         ...
         --  Extra underlying places to search, if not found above
         ...
         if Nkind (Result) = N_Defining_Identifier then
            ...
            if False
              or else not Is_Entity_Name (Result)
              or else Ekind (Entity (Result)) /= E_Discriminant
            then
               return Result;
            end if;
         ...
         while Present (Disc) loop
            ...
            if Girder_Discrim_Values then
               Disc := Next_Girder_Discriminant (Disc);
            else
               Disc := Next_Discriminant (Disc);
            end if;
         end loop;
      ...
      begin
         ...
         if Ekind (New_C) = E_Component then
            ...
            then
               ...
               Set_Etype (New_C, Constrain_Discrim_Type (Etype (Old_C),
   ...
   begin
      ...
      if Inherit_Discr then
         ...
         while Present (Parent_Discrim) loop
            ...
            Parent_Discrim := Next_Discriminant (Parent_Discrim);
         end loop;
      end if;
      ...
      then
         ...
         while Present (Girder_Discrim) loop
            ...
            Girder_Discrim := Next_Girder_Discriminant (Girder_Discrim);
         end loop;
      end if;
   ...
   begin
      ...
      elsif In_Instance (Body_Only => True) then
         return True;
   ...
   begin
      ...
      if not Is_Private (Parent_Subp)
        or else Is_Internal (Parent_Subp)
        or else Is_Internal_Name (Chars (Parent_Subp))
        or else
          (In_Open_Scopes (Scope (Base_Type (Parent_Type)))
      ...
      --  There is no need to freeze derived subprograms, because the
      --  back-end never sees a call to them.   ???
      ...
      while Present (Elmt) loop
         ...
         if Ekind (Subp) /= E_Enumeration_Literal then
            if No (Generic_Actual) then
               Derive_Subprogram
                 (New_Subp, Subp, Derived_Type, Parent_Type);
            else
               Derive_Subprogram (New_Subp, Subp,
                 Derived_Type, Parent_Type, Node (Act_Elmt));
               Act_Elmt := Next_Elmt (Act_Elmt);
            end if;
         end if;
   ...
   begin
      if Is_Array_Type (Desig_Type) then
        ...
        or else Is_Task_Type (Desig_Type)
