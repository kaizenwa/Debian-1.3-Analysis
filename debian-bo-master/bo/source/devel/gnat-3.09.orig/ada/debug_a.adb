...
package body Debug_A is
   ...
   --  Fatal_Error_Node correctly. Note that if we have more than 100 recursion
   --  levels, we just don't reset the right value on exit, which is not
   --  crucial, since this is only for debugging!
   ...
   begin
      ...
      if Debug_A_Depth in 1 .. Max_Node_Ids then
         Fatal_Error_Node := Node_Ids (Debug_A_Depth);
      end if;
