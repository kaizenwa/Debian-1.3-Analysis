...
struct dot_set_stack {
    struct dot_set_stack *next;
    struct dot_set_stack *prev;
    int noreorder;
    int noat;
...
static struct dot_set_stack *dot_set_stack_last = NULL;
     ...
     int x;
{
  ...
  *input_line_pointer = '\0';
  ...
  else if (strncmp (name, "push", 4) == 0)
    {
      struct dot_set_stack *s;

      s = (struct dot_set_stack *) xmalloc(sizeof(struct dot_set_stack));
      s->next = NULL;
      s->prev = dot_set_stack_last;
      if (dot_set_stack_last != NULL)
        dot_set_stack_last->next = s;
      dot_set_stack_last = s;

      s->noreorder = mips_noreorder;
      s->noat = mips_noat;
      s->warn_about_macros = mips_warn_about_macros;
      s->nomove = mips_nomove;
      s->nobopt = mips_nobopt;
      s->isa = mips_isa;
    }
  else if (strncmp (name, "pop", 3) == 0)
    {
      struct dot_set_stack *s = dot_set_stack_last;

      /* If we're switching the reordering mode we need to handle the
	 delay slots right.  */
      if (mips_noreorder != s->noreorder)
	{
	  if (s->noreorder == 0)
	    {
	      if (mips_noreorder)
	  ...
	  mips_noreorder = s->noreorder;
	}

      mips_noat = s->noat;
      mips_warn_about_macros = s->warn_about_macros;
      mips_nomove = s->nomove;
      mips_nobopt = s->nobopt;
      mips_isa = s->isa;

      if (s->prev)
        s->prev->next = NULL;
      dot_set_stack_last = s->prev;
      free(s);
    }
