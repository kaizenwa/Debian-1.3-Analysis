...
#if 0
  /*
   * Enabeling this code results in every file linked being set to
   * E_MIPS_ARCH_1 which again might produce bogus warnings when this
   * files is linked again with other files which still have correctly
   * e_flags set to != E_MIPS_ARCH_1.
   * By disabeling this code the output bfd gets it's architecture bits
   * in e_flags set to the bits from the first input bfd.  We probably
   * should better set the architecture level of the output file to the
   * highest level of the input files.
   */
