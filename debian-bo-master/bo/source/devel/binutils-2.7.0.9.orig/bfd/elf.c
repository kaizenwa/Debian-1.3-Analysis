     ...
     bfd *obfd;
{
  ...
  /* Minimum sanity check. It may not mean anything. */
  c = 0;
  for (s = ibfd->sections; s != NULL; s = s->next)
  {
    int found;

    /* We don't check those sections. */
    if ((s->vma == 0 && s->lma == 0) || (s->flags & SEC_ALLOC) == 0)
      continue;

    found = 0;
    for (m = mfirst; m != NULL && found == 0; m = m->next)
    {
      for (i = 0; i < m->count; i++)
      {
	if (m->sections [i] == s->output_section)
	{
	  found = 1;
	  break;
	}
    ...
    if (found == 0)
    {
      /* Not found */
      ((*_bfd_error_handler)
	("%s: error: section `%s' not in segment",
	bfd_get_filename (ibfd),
	(s->output_section == NULL
		? "*unknown*" : s->output_section->name)));
      c++;
    }
  }
  ...
  if (c)
  {
    /* That is very bad. */
    ((*_bfd_error_handler)
	("%s: fatal error: missing sections",
	bfd_get_filename (ibfd)));
    abort ();
  }
