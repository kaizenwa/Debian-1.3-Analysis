...
#  wm positionfrom .fsBox program
  ...
  # bindings
  bindtags .fsBox.files.files {Listbox . all .fsBox.files.files}
...
proc FSBoxFSInsertPath {} {# xf ignore me 6
  ...
  for {set fsBoxCounter 1} {$fsBoxCounter <= $fsBoxLast} {incr fsBoxCounter 1} {
    if {"$fsBoxNewEntry" == \
  ...
  foreach fsBoxCounter $fsBoxElementList {
    ...
    if {$fsBox(extensions)} {
      ...
      if {"$fsBoxExtension" != "" && "$fsBoxRootName" != ""} {
        ...
        for {set fsBoxCounter1 1} {$fsBoxCounter1 <= $fsBoxLast} {incr fsBoxCounter1 1} {
          if {"*$fsBoxExtension" == \
