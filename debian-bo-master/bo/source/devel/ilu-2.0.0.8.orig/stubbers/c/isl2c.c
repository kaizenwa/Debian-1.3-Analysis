...
static void generateCommon (Interface parse)
{
  ...
  if (CommonFile == NULL) 
    sprintf(filename, "%s/%s-common.c", OutputDirectory, c_interface_name(parse));
  else 
    sprintf(filename, "%s/%s.c", OutputDirectory, CommonFile);
...
static void generateSurrogate (Interface parse)
{
  ...
  if (SurrogateFile == NULL) 
    sprintf(filename, "%s/%s-surrogate.c", OutputDirectory, c_interface_name(parse));
  else 
    sprintf(filename, "%s/%s.c", OutputDirectory, SurrogateFile);
...
static void generateTrue (Interface parse)
{
  ...
  if (TrueFile == NULL) 
    sprintf(filename, "%s/%s-true.c", OutputDirectory, c_interface_name(parse));
  else 
    sprintf(filename, "%s/%s.c", OutputDirectory, TrueFile);
...
static void generateHeaders(Interface parse)
{
  ...
  if (HeaderFile == NULL) 
    sprintf(filename, "%s/%s.h", OutputDirectory, c_interface_name(parse));
  else 
    sprintf(filename, "%s/%s.h", OutputDirectory, HeaderFile);
...
[-cname COMMONFILENAME] [-hdrmap INTERFACEHEADERTRANSLATIONFILE ] ISLFILE [ISLFILE ...]\n", ProgramName);
}
