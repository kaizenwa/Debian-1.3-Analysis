...
$Id: typeuid.c,v 1.40 1996/07/16 01:15:29 janssen Exp $
*/
...
static void ast_recurse (Type t, struct buffer_s *data)
{
  ...
  if (type_kind(t) == alias_Type)
    ...
    {
      print0ToBuffer (data, "(record (supertype)");
...
char *FigureTypeUID (Type t)
{
  ...
  if (verbose && ! t->builtIn)
    fprintf (stderr, "figuring uid for <%s> %p %s\n", full_type_name(t), t, (t->importInterfaceName != NULL) ? t->importInterfaceName : "");
