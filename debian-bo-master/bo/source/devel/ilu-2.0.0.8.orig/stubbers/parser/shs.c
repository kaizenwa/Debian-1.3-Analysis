...
$Id: shs.c,v 1.9 1995/03/10 04:13:43 janssen Exp $
*/
...
static char endianTest[4] = {0, 0, 0, 1};
...
void SHSInit (SHS_CTX *ctx)
{
    ...
    intFromChars( (unsigned int *) &testInt, (unsigned char *) endianTest );
    littleEndian = (*((int *)endianTest)==1 ? 0 : 1);
...
static void onem(UINT4 *m, UINT4 *h)
{
    ...
    if (littleEndian)
        for (i=0; i < 16; i++ ) {
            intFromChars((unsigned int *) &(m[i]), (unsigned char *) &(m[i]) );
        };
