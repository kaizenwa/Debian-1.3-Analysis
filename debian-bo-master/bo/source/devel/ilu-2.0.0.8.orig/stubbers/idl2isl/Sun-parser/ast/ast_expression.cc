...
coerce_value(AST_Expression::AST_ExprValue *ev, AST_Expression::ExprType t)
{
  ...
  case AST_Expression::EV_short:
    ...
    case AST_Expression::EV_long:
      if (ev->u.lval > (long) SHRT_MAX || ev->u.lval < (long) SHRT_MIN)
	return NULL;
    ...
    case AST_Expression::EV_float:
      if (ev->u.fval > (float) SHRT_MAX || ev->u.fval < (float) SHRT_MIN)
	return NULL;
      ev->u.sval = (short) ev->u.fval;
      ev->et = AST_Expression::EV_short;
      return ev;
    case AST_Expression::EV_double:
      if (ev->u.dval > (double) SHRT_MAX || ev->u.dval < (double) SHRT_MIN)
	return NULL;
  ...
  case AST_Expression::EV_long:
    ...
    case AST_Expression::EV_bool:
      ...
      return ev;
    case AST_Expression::EV_float:
      if (ev->u.fval > (float) LONG_MAX || ev->u.fval < (float) LONG_MIN)
	return NULL;
      ev->u.lval = (long) ev->u.fval;
      ev->et = AST_Expression::EV_long;
      return ev;
    case AST_Expression::EV_double:
      if (ev->u.dval > (double) LONG_MAX || ev->u.dval < (double) LONG_MIN)
	return NULL;
  ...
  case AST_Expression::EV_char:
    switch (ev->et) {
    case AST_Expression::EV_short:
      if (ev->u.sval > (short) CHAR_MAX || ev->u.sval < (short) CHAR_MIN)
	return NULL;
      ev->u.cval = (char) ev->u.sval;
      ev->et = AST_Expression::EV_char;
      return ev;
    case AST_Expression::EV_ushort:
      if (ev->u.usval > (unsigned short) CHAR_MAX)
	return NULL;
      ev->u.cval = (char) ev->u.usval;
      ev->et = AST_Expression::EV_char;
      return ev;
    case AST_Expression::EV_long:
      if (ev->u.lval > (long) CHAR_MAX || ev->u.lval < (long) CHAR_MIN)
	return NULL;
      ev->u.cval = (char) ev->u.lval;
      ev->et = AST_Expression::EV_char;
      return ev;
    case AST_Expression::EV_ulong:
      if (ev->u.ulval > (unsigned long) CHAR_MAX)
	return NULL;
    ...
    case AST_Expression::EV_bool:
      ...
      return ev;
    case AST_Expression::EV_float:
      if (ev->u.fval > (float) CHAR_MAX || ev->u.fval < (float) CHAR_MIN)
	return NULL;
      ev->u.cval = (char) ev->u.fval;
      ev->et = AST_Expression::EV_char;
      return ev;
    case AST_Expression::EV_double:
      if (ev->u.dval > (double) CHAR_MAX || ev->u.dval < (double) CHAR_MIN)
	return NULL;
    ...
    case AST_Expression::EV_char:
      return ev;
    case AST_Expression::EV_octet:
      if (ev->u.oval > (unsigned char) CHAR_MAX)
	return NULL;
  ...
  case AST_Expression::EV_octet:
    switch (ev->et) {
    case AST_Expression::EV_short:
      if (ev->u.sval < 0 || ev->u.sval > (short) UCHAR_MAX)
	return NULL;
      ev->u.oval = (unsigned char) ev->u.sval;
      ev->et = AST_Expression::EV_octet;
      return ev;
    case AST_Expression::EV_ushort:
      if (ev->u.usval > (unsigned short) UCHAR_MAX)
	return NULL;
      ev->u.oval = (unsigned char) ev->u.usval;
      ev->et = AST_Expression::EV_octet;
      return ev;
    case AST_Expression::EV_long:
      if (ev->u.lval < 0 || ev->u.lval > (long) UCHAR_MAX)
	return NULL;
      ev->u.oval = (unsigned char) ev->u.lval;
      ev->et = AST_Expression::EV_octet;
      return ev;
    case AST_Expression::EV_ulong:
      if (ev->u.ulval > (unsigned long) UCHAR_MAX)
	return NULL;
    ...
    case AST_Expression::EV_bool:
      ...
      return ev;
    case AST_Expression::EV_float:
      if (ev->u.fval < 0.0 || ev->u.fval > (float) UCHAR_MAX)
	return NULL;
      ev->u.oval = (unsigned char) ev->u.fval;
      ev->et = AST_Expression::EV_octet;
      return ev;
    case AST_Expression::EV_double:
      if (ev->u.dval < 0.0 || ev->u.dval > (double) UCHAR_MAX)
	return NULL;
...
AST_Expression::eval_bin_op(AST_Expression::EvalKind ek)
{
  ...
  ExprType promotedTypeLeft	= pd_v1->promotedType();
  ExprType promotedTypeRight	= pd_v2->promotedType();
  ExprType resultType;

  if (promotedTypeLeft == promotedTypeRight)
    resultType = promotedTypeLeft;
  else if (promotedTypeLeft == EV_ulong && promotedTypeRight == EV_long ||
           promotedTypeLeft == EV_long  && promotedTypeRight == EV_ulong)
    resultType = EV_long;
  else {
    //!! Issue error message.
    return NULL;
  ...
  pd_v1->set_ev(pd_v1->coerce(resultType));
  if (pd_v1->ev() == NULL)
    return NULL;
  pd_v2->set_ev(pd_v2->coerce(resultType));
  if (pd_v2->ev() == NULL)
    return NULL;

  retval = new AST_ExprValue;
  retval->et = resultType;
...
AST_Expression::eval_bit_op(AST_Expression::EvalKind ek)
{
  ...
  case EC_left:
    ...
    break;
  case EC_right:
    // CORBA spec says "0 fill for the vacated bits."
    retval->u.lval = pd_v1->ev()->u.ulval >> pd_v2->ev()->u.lval;
    break;
  default:
    return NULL;
  }
...
AST_Expression::eval_un_op(AST_Expression::EvalKind ek)
{
  ...
  ExprType resultType = pd_v1->promotedType();
  ...
  case EV_ulong:
    ...
    case EC_u_minus:
      ...
      // Negated integer literals should be promoted to type long.
      if (pd_v1->ec() == EC_none)
	retval->et = resultType = EV_long;
...
/*
 * Return the "promoted" type of an AST_Expression.
 */
AST_Expression::ExprType
AST_Expression::promotedType(void)
{
	ExprType type = pd_ev->et;
