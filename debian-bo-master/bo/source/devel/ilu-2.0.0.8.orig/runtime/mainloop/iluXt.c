/*
 ...
 $Id: iluXt.c,v 1.8 1996/06/06 00:26:56 spreitze Exp $
...
static void default_ilu_Xt_run (int *stop)
{
  XEvent event;
  StackFrame ours;
...
static void default_ilu_Xt_exit (int *stop)
{
  StackFrame *p;
  ...
  if (p == NULL)	/* that frame isn't on the stack! */
    {
      fprintf (stderr, "ILU Xt ExitMainLoop:  bad \"stop\" parameter, %p, passed.\n", stop);
      return;
    }
  for (p = run_stack;  p != NULL;  p = p->next)
...
static void callback_proc (Rock client_data, int *fd, XtInputId *id)
{
  client_data->handler (client_data->fd, client_data->rock);
}

static ilu_boolean register_input (int fd, ilu_IOHandler handler, ilu_private rock)
{
  Rock r;

  DeleteRock (fd, Input);
  r = AddRock (fd, handler, rock, Input);
  return (((r->id = XtAddInput (fd, (XtPointer)XtInputReadMask, (XtInputCallbackProc)callback_proc, r)) != (XtInputId)NULL) ? ilu_TRUE : ilu_FALSE);
}

static ilu_boolean unregister_input (int fd)
{
...
static ilu_boolean register_output (int fd, ilu_IOHandler handler, ilu_private rock)
{
  Rock r;

  DeleteRock (fd, Output);
  r = AddRock (fd, handler, rock, Output);
  return (((r->id = XtAddInput (fd, (XtPointer)XtInputWriteMask, (XtInputCallbackProc)callback_proc, r)) != (XtInputId)NULL) ? ilu_TRUE : ilu_FALSE);
}

static ilu_boolean unregister_output (int fd)
{
...
static void handle_timer (void * client_data, XtIntervalId *id)
{
...
static void set_alarm (void * alarm, ilu_FineTime t, void (*proc) (ilu_private rock), ilu_private rock)
{
  ilu_FineTime t2;
  ...
  al->id = XtAddTimeOut (al->timeout, (XtTimerCallbackProc)handle_timer, (void *) al);
}

static void clear_alarm (void * alarm)
{
  iluXtAlarm al = (iluXtAlarm) alarm;
...
void ilu_Xt_Initialize (void (*user_run_main_loop)(int *), void (*user_exit_main_loop)(int *))
{
  ...
  ml.ml_run = (user_run_main_loop == NULL || user_exit_main_loop == NULL) ? default_ilu_Xt_run : user_run_main_loop;
  ml.ml_exit = (user_run_main_loop == NULL || user_exit_main_loop == NULL) ? default_ilu_Xt_exit : user_exit_main_loop;
