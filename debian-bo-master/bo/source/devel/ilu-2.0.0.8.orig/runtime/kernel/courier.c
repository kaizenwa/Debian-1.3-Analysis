...
*/
/* $Id: courier.c,v 1.117 1996/07/09 23:47:04 janssen Exp $ */
...
static ilu_cardinal CourierMaxStringSize = 100000;
...
static ilu_refany _courier_CreateDataBlock (ILU_ERRS((no_memory)) *err)
{
...
{
  return ((key1->cri_pnumber == key2->cri_pnumber) AND (key1->cri_version == key2->cri_version));
}
...
by a pre-1.6.4-p8 stubber.  Please re-stub, re-compile, and re-link.\n",
	...
	{
	  if (NOT ((s->cri_pnumber = _ilu_atoi(class->cl_singleton + 9, &next),
		    (s->cri_pnumber > 0 && (next != NIL) && (*next == '_'))) AND
    ...
    {
      ilu_cardinal hash = _ilu_CRC32 ((ilu_bytes) class->cl_unique_id, strlen(class->cl_unique_id));
...
static courierinfo *_courier_CourierInformationForClass (ilu_Class class)
{
  ...
  s = (courierinfo *) _ilu_hash_FindInTable (RegistryHashTable,
					    class->cl_unique_id);
  if (s == NIL)
						   ...
						   ilu_boolean try_adds)
{
  ...
  s = (courierinfo *) _ilu_hash_FindInTable (ProgramNumberHashTable,
					      (ilu_refany) &dummy);
  if (s == NIL)
    {
      if (try_adds)
...
static void 
_courier_OutputString(ilu_Call call, ilu_string s, ilu_cardinal len,
		      ilu_cardinal limit, ILU_ERRS((IoErrs)) * err)
{
...
static void 
_courier_InputString(ilu_Call call, ilu_string * s, ilu_cardinal * len,
		     ilu_cardinal limit, ILU_ERRS((IoErrs)) * err)
{
  ...
  /*ARGSUSED*/
static          ilu_cardinal
_courier_SizeOfString(ilu_Call call, ilu_string i, ilu_cardinal l,
		      ilu_cardinal limit, ILU_ERRS((IoErrs)) * err)
{
...
static void 
_courier_InputArray(ilu_Call c, ILU_ERRS((IoErrs)) * err)
{
  ILU_CLER(*err);
}
...
static void _courier_OutputRecord (ilu_Call c, ILU_ERRS((IoErrs)) * err)
{
  ILU_CLER(*err);
}

static void _courier_InputRecord (ilu_Call c, ILU_ERRS((IoErrs)) * err)
{
  ILU_CLER(*err);
}
...
static          ilu_cardinal
_courier_SizeOfRecord(ilu_Call c, ILU_ERRS((IoErrs)) * err)
{
  ILU_CLER(*err);
  return (0);
}
		    ...
		    ILU_ERRS((IoErrs)) * err)
{
  ...
  case ilu_rhrc_nothing:
    return ans;
  default:
    _ilu_Assert(FALSE, "courier_ReadHeader: transport_begin_message");
  }
		    ...
		    ILU_ERRS((IoErrs)) * err)
{
  ...
  q->trans = _ilu_BufferInputMessage(t, err);
...
static          ilu_boolean
_courier_AbandonDelayedInterp(ilu_refany x, ILU_ERRS((internal)) * err)
{
  Queued          q = (Queued) x;
  ilu_boolean     ans;
		      ...
		      ILU_ERRS((IoErrs)) * err)
{
  ...
  DEBUG(COURIER_DEBUG,
	(stderr, "_courier_StartRequest:  pnum/vers 0x%lx/%ld\n",
	 pnumber, version));

  packetSize = argSize
    + 12;			/* for fields of header */

  if (transport_begin_message(callport(call), FALSE, err)
      == ilu_rhrc_error)
    goto dun;
...
{
  transport_end_message(callport(call), FALSE, NIL, err);
  return;
}
...
{
  ilu_Transport   bs = callport(call);
  if (!transport_end_message(bs, TRUE, msg, err)) 
    return (ilu_FALSE);
...
_courier_FinishRequest(ilu_Call call, ilu_Message * msg,
		       ILU_ERRS((IoErrs)) * err)
{
  return (_courier_SendPacket(call, FALSE, msg, err));
}

static          ilu_boolean
_courier_FinishReply(ilu_Call call,
		     ILU_ERRS((IoErrs)) * err)
{
  ilu_Message     msg = {NIL, 0};
  return (_courier_SendPacket(call, TRUE, &msg, err));
}

static          ilu_boolean
_courier_FinishException(ilu_Call call,
			 ILU_ERRS((IoErrs)) * err)
{
  ilu_Message     msg = {NIL, 0};
  return (_courier_SendPacket(call, TRUE, &msg, err));
		    ...
		    ilu_cardinal argSize, ILU_ERRS((IoErrs)) * err)
{
  ...
  if (transport_begin_message(trans, FALSE, err) == ilu_rhrc_error)
    goto faild;
