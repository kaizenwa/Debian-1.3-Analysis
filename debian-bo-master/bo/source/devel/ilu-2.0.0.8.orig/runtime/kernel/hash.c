...
*/
/* $Id: hash.c,v 1.48 1996/04/26 21:52:53 mdavidso Exp $ */
...
ilu_boolean 
_ilu_hash_AddToTable(HashTable ht, ilu_refany key, ilu_refany obj)
{
  struct hashTableSlot *slot;
  ilu_shortcardinal count;
...
static void _ilu_hash_PrintHashTable (HashTable ht)
{
  HashEnumerator he;
  ilu_refany key, data;
  ...
  if (ht == (HashTable) 0)
    ...
    {
      _ilu_hash_BeginEnumeration (ht, &he);
      while (_ilu_hash_Next(&he, &key, &data))
	{
	  if (ht->ht_compfn == _ilu_hash_StringCompare)
...
ilu_boolean _ilu_hash_StringCompare (ilu_refany key1, ilu_refany key2)
{
...
#else
  ilu_cardinal crc = _ilu_CRC32 ((ilu_bytes) str, (ilu_cardinal) strlen(str));
  return (crc % size);
#endif
}
...
{
  HashTable       new = (HashTable) ilu_malloc(sizeof(struct hashTable));
  new->ht_size = size;
  new->ht_nPairs = 0;
  new->ht_slots = (struct hashTableSlot *) ilu_malloc(
			     sizeof(struct hashTableSlot) * size);
  if (hashfn == NULLFN)
    new->ht_hashfn = _ilu_hash_HashString;
  else
    new->ht_hashfn = hashfn;
  if (compfn == NULLFN)
    new->ht_compfn = _ilu_hash_StringCompare;
  else
