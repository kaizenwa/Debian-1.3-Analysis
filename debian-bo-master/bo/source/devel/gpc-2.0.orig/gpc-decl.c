...
#ifdef GPC
/* Allow c-style character escapes if 1 */
int flag_c_escapes = 0;

/* allow gpc to use c style numbers (0 -> octal 0x -> hex) if nonzero */
int flag_c_numbers = 0;

/* allow nested comments if 1 */
...
/* What kind of reserved words should we recognize */
int flag_what_pascal = PASCAL_ISO;
 ...
 * like TeX...
 */
int flag_short_circuit = 0;

/*
 * Use lazy i/o for files.
 */
int flag_lazy_io = 0;
     ...
     char *p;
{
  ...
  if (!strcmp (p, "-ftraditional") || !strcmp (p, "-traditional"))
    ...
    flag_signed_char = 0;
  else if (!strcmp (p, "-fchar-escapes"))
    flag_c_escapes = 1;
  else if (!strcmp (p, "-fc-numbers"))
    flag_c_numbers = 1;
  else if (!strcmp (p, "-fshort-circuit"))
    flag_short_circuit = 1;
  else if (!strcmp (p, "-flazy-io"))
    flag_lazy_io = 1;
  else if (!strcmp (p, "-fobject-pascal"))
    flag_what_pascal = PASCAL_OBJECT;
  else if (!strcmp (p, "-fstandard-pascal"))
    flag_what_pascal = PASCAL_ISO;
  else if (!strcmp (p, "-fextended-pascal"))
    flag_what_pascal = PASCAL_EXTEND;
  else if (!strcmp (p, "-fborland-pascal"))
    flag_what_pascal = PASCAL_BORLAND;
     ...
     int main_program;
{
  ...
  /* This depends of how gpc-parse.y does things */
  if (main_program)
    return current_binding_level->level_chain
      	&& current_binding_level->level_chain->level_chain == global_binding_level;
     ...
     int functionbody;
{
  ...
  if (functionbody)
      ...
      for (link = named_labels; link; link = TREE_CHAIN (link))
	{
       ...
       * that was not used in it's scope.
       */
      for (link = declared_labels; link; link = TREE_CHAIN (link))
	{
	  ...
	  for (lab = named_labels; lab; lab = TREE_CHAIN (lab))
	    if (DECL_NAME (TREE_VALUE (lab)) == TREE_VALUE (link))
	      break;
	  if (! lab)
	    warning ("Declared label `%s' not defined in scope ending here",
...
#ifdef GPC
      if (t != 0 && ! PASCAL_REDEFINABLE_DECL (t) && duplicate_decls (x, t))
#else
...
#ifdef GPC
/*
 *  For each label in label declaration part of Pascal program.
 *  ID is the name of the label, TOP_LEVEL_LABELS is nonzero only
 *  when we are declaring labels for the main program.
 *
 *  Function is used only to keep track of labels declared in a
 *  label statement.
 */
void
...
int top_level_labels;
{
     ...
     * if we are not in the main program
     */
    if (! top_level_labels)
      {
     ...
     tree id;
{
  ...
  if (pedantic)
	...
	if (TREE_VALUE (link) == id)
      ...
      if (! link)
	warning ("ISO Pascal requires labels to be declared in LABEL declaration part");
    }
#endif /* GPC */
...
init_decl_processing ()
{
  ...
  if (flag_signed_char && pedantic)
    warning ("ISO Pascal wants to use unsigned characters");
...
#define PUSHDECL pushdecl
  ...
  TYPE_SIZE (boolean_type_node) = 0;
  fixup_unsigned_type (boolean_type_node);
  TREE_UNSIGNED (boolean_type_node) = 1;
  ...
  complex_zero_node = build_complex (real_zero_node, real_zero_node);
     ...
     int initialized;
{
  ...
  for (spec = declspecs; spec; spec = TREE_CHAIN (spec))
    {
	 ...
	 grokdeclarator: the only time id is either const_id or
	 type_id is when we are defining a CONST or TYPE */
      if (id == type_id) {
	  specbits |= 1 << RID_TYPEDEF;
	  goto found;
      } else if (id == const_id) {
	  specbits |= 1 << RID_CONST;
	  goto found;
      } else if (id == inline_id) {
	  specbits |= 1 << RID_INLINE;
	  goto found;
      } else if (id == auto_id) {
	  ...
	  /* For now, only function return value assignments are made
	     volatile, so gcc won't optimize the `unnecessary' assignments
	     away with -O */
	  specbits |= 1 << RID_VOLATILE;
	  goto found;
      }
#else
...

  {
	...
	DECL_ARG_TYPE_AS_WRITTEN (decl) = type_as_written;
      ...
      {
#ifdef GPC
	/* Structure field.  A FUNCTION_TYPE */
        /* indicates a method.               */
...
#ifdef GPC
    else if (constp) /* it's a Pascal CONSTant */
	decl = build_decl (CONST_DECL, declarator, type);
#endif /* GPC */
    else
      {
	/* It's a variable.  */
     ...
     tree info;
{
  ...
  if (TYPE_LANG_SPECIFIC (record))
    {
      if (TYPE_LANG_SPECIFIC (record)->elts[0])
	abort ();
    }
  else
    TYPE_LANG_SPECIFIC (record) = allocate_type_lang_specific ();

  TYPE_LANG_SPECIFIC (record)->elts[0] = info;
  TYPE_LANG_SPECIFIC (record)->len     = -1;
}
     ...
     tree record;
{
  ...
  if (TYPE_LANG_SPECIFIC (record))
    return TYPE_LANG_SPECIFIC (record)->elts[0];
     ...
     tree attributes;
{
   ...
   * record tag info.
   *
   * If c-tree.h struct lang_type() has one more tree type node added
 ...
 * in the TREE node (see gpc-lang.h).
 */
int
  ...
  for (dir = directive; dir; dir = TREE_CHAIN (dir))
    {
      if (TREE_VALUE (dir) != d_forward
        ...
        {
          error ("Only directives `Forward', `External', `Extern', `Static',");
          error ("`C', `C_language', `AsmName', and `Attribute' are implemented");
          result = 0;
        }
    }
...
int nested;
{
    ...
    for (dir = directive; dir; dir = TREE_CHAIN (dir))
      {
        if (TREE_VALUE (dir) == d_extern)
          TREE_VALUE (dir) = d_external;
        else if (TREE_VALUE (dir) == d_c_lang1)
          TREE_VALUE (dir) = d_c_lang;
        else if (nested && TREE_VALUE (dir) == d_forward)
          type = tree_cons (NULL_TREE, auto_id, type); /* Use `auto' function with nested forwards */
        else if (!nested && TREE_VALUE (dir) == d_forward)
        ...
        if (TREE_VALUE (dir) == d_attribute)
          attributes = DECL_MACHINE_ATTRIBUTES (TREE_VALUE (dir));
        if (TREE_VALUE (dir) == d_c_lang || TREE_VALUE (dir) == d_asmname)
          name_dir = TREE_VALUE (dir);
      }

    d = start_decl (heading, type, 0, attributes, NULL_TREE);

    /* With the C directive leave the possible first letter in lowercase. */
    /* Otherwise the casing is not preserved. */
    /* @@@ Actually, this does not preserve the casing, but it puts */
    /* everything to lowercase.  Is this intended? */
    if (name_dir == d_c_lang)
      DECL_ASSEMBLER_NAME (d) = de_capitalize (TREE_OPERAND (heading, 0));

    /* AsmName directive: direct specification of assembler name. */
    /* In this case, casing *is* preserved. */
    if (name_dir == d_asmname)
      DECL_ASSEMBLER_NAME (d) =
	get_identifier (TREE_STRING_POINTER (DECL_ASSEMBLER_NAME (name_dir)));
	...
	/* @@@ Is this the correct obstack? */
	temp = (struct lang_decl *) obstack_alloc (&permanent_obstack,
...
/* Check whether a TREE_LIST `dir' contains a FORWARD directive
 * (Subroutine of start_function ()
 * #if 0 and start_pascal_function () #endif)
 */
static int
contains_forward_directive (dir)
     tree dir;
{
  tree d;
  for (d = dir; d; d = TREE_CHAIN (d))
    if (TREE_VALUE (d) == d_forward)
      return 1;
  return 0;
}
 ...
 * for the routine we start declaring now. If so, alter the
...
int  nested;
{
	...
	if (temp && temp->directive)
	  {
	     ...
	     */
	    if (! contains_forward_directive (temp->directive))
		...
		if (TREE_PURPOSE (args))
		  warning ("Parameters in forward declared routine are not allowed");

		if (TREE_VALUE (type) != void_type_node)
		  warning ("Function type in forward declared routine is not allowed");
	      }
    ...
    if (! start_function (type, heading, NULL_TREE, NULL_TREE, nested))
      return NULL_TREE;
     ...
     int nested;
{
  ...
  if (old_decl != 0)
	...
	if (spec
	    && contains_forward_directive (spec->directive)
     ...
     tree to_type;
{
  ...
  /* Force a copy of the TO_TYPE node on top of the
   * lang decl node. This seems very silly indeed.
   *
   * But there may be many references to the object
   * and I don't know where they are -> I have to
   * replace the object itself.
   *
   * Because the ref is a type node, the copy will
   * fit in it.
   */
     ...
     tree list;
{
  ...
  for (; scan; scan = TREE_CHAIN (scan)) {
    ...
    if (type == error_mark_node) {
      warning ("?? Error mark node in type decl list");
      continue;
    }
    
    /* Look up the tags list for a forward pointer type
     * that might be solved by the type decls at this level.
     */
    if (!resolve_forward_pointer (decl, TREE_TYPE (decl)))
      continue;
    
    if (TREE_CODE (type) != POINTER_TYPE)
      continue;

    /* Now output the debugging info for the fixed forward
     * pointers.
     */
    ...
    /* Force the symbol debug info out again */
    dbxout_set_type_status (TYPE_MAIN_VARIANT (type),
			    0);

    /* Otherwise dbxout won't output the type number
     * of the object the pointer points to
     */
    dbxout_set_type_status
      (TYPE_MAIN_VARIANT (TREE_TYPE(type)), 0);
  ...
  /* Report everything we did not match */
  report_unresolved_forward_pointers (TRUE);
}
 ...
 *
 * Proceeds to the inner layers of Pascal variant record
 *
...
tree fields;
{
	...
	for (field = fields;
	    ...
	    if (name == NULL_TREE) /* proceed through variant part */
	...
	break;
    default:
	warning ("INTERNAL ERROR: handle_one_level is broken");
    }
}
...
tree element;
{
    ...
    case RECORD_TYPE:
	{
		...
		rec_addr = start_decl (get_identifier ("<with_element>"),
				       build_tree_list(NULL_TREE,
						       build_pointer_type (TREE_TYPE (element))),
				       0, NULL_TREE, NULL_TREE);
		finish_decl (rec_addr, NULL_TREE, NULL_TREE);
		expand_expr_stmt (build_modify_expr (rec_addr,
						     NOP_EXPR,
    ...
    default:
	warning ("WITH element is of wrong type");
    }
}
#endif /* GPC */

