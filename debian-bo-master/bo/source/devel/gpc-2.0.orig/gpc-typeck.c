...
#ifdef GPC
static int comp_object_pointer_types	PROTO((tree, tree));
#endif /* GPC */
...
#ifdef GPC
  if (PASCAL_TYPE_PACKED (t1) != PASCAL_TYPE_PACKED (t2))
    return 0;
#endif
...

#ifdef GPC
/* Check if two pointers to objects are assignment compatible.
 * This is the case if rhs has the same type as lhs or if it
 * has the type of lhs as a parent type.
 */
static int
comp_object_pointer_types (lhs, rhs)
     tree lhs, rhs;
{
  register tree r;
  if (IS_OBJECT_TYPE (lhs)
      && IS_OBJECT_TYPE (rhs))
    for (r = rhs; r; r = TYPE_LANG_SPECIFIC (r)->elts [1])
      {
        if (comptypes (lhs, r))
          return 1;
      }
...
#ifdef GPC /* No array conversion in GPC */
  if (code == ARRAY_TYPE)
    return exp;
#endif /* GPC */
  if (code == ARRAY_TYPE)
...
#ifdef GPC_245_OR_LESS
	      if (ret)
		  if (DECL_NAME (field) ||
		      TREE_CODE (TREE_TYPE (field)) == RECORD_TYPE)
		      return chainon (ret,
				      build_tree_list (NULL_TREE, field));
		  else
		      return ret; /* The intermediate pseudo union
				     should not cause any offsets
				     and appears as nonexisting to
				     the outside world. */
#else
	      /* In GCC-2.5.0 this has changed: Now we need to include
		 the invisible UNION_TYPE to the list of fields
		 we need to build_component_ref of to reach the field.
 ...
 *
 * @@@@@ Pass the TYPE nodes, not the code nodes to make more thorough
 *       checks.
 *
...
int
compatible_types_p (code0, code1)
...
int
compatible_assignment_p (code0, code1)
     enum tree_code code0;
     enum tree_code code1;
{
  return    (code0 == code1)
...
int
compatible_relop_p (code0, code1)
     enum tree_code code0;
     enum tree_code code1;
{
  return compatible_types_p (code0, code1)
    	 || (code0 == REAL_TYPE    && code1 == INTEGER_TYPE)
...
#ifdef GPC
    error ("request for member `%s' in something not a record, schema or object type",
	    IDENTIFIER_POINTER (component));
#else
	  ...
	  if (TYPE_SIZE (t) == 0 && TREE_CODE (t) != ARRAY_TYPE)
	    {
          ...
          /* already happened.                                        */
	  if (warn_void_dereference && TREE_CODE (t) == VOID_TYPE)
     ...
     tree array, index;
{
  ...
  {
#ifdef GPC
    tree ar = convert_array_to_pointer (array);
#else
     ...
     tree function, params;
{
...

  {
...
#ifdef GPC
    /*
     * Handle functions returning String schemas
     *
	...
	expand_expr_stmt (build_modify_expr (temp_string, NOP_EXPR, result));
	result = temp_string;
      }
#endif /* GPC */
...
#ifdef GPC
/*
 * Pascal conformant array covention follows:
 *
 ...
 *  or if the array parameter with this bit set is not preceeded by
  ...
  for (valtail = values, typetail = typelist, parmnum = 0;
	...
	val = TREE_OPERAND (val, 0);
	 ...
	 of the formal parameter types of the called function.
	  ...
	  (! type || TREE_TYPE (val) != TREE_TYPE (type)))
        { /* but receiving type does not match, or varargs */
	  /* If the routine has parameters, it will
	     never come here if it needs evaluation
	     (it has the parenthesized argument list then) */
		       ...
		       && PASCAL_TYPE_STRING (TREE_TYPE (type)))))
	{
	   ...
	   * but still convert the CHAR or fixed-string to a new schema variable.
	   */
	...
	keep_this_val = NULL_TREE;
      ...
      /* For conformant array types we have first the indices that
       * are missing from the actual parameter list ->
	...
	{
	  int   level = conf_array_indices / 2;
      ...
      if (type != 0)
	{
	  ...
	  if (TYPE_SIZE (type) == 0)
		...
		{
		  /* This is a void pointer parameter, which is a GPC extension
		   * to make it easier to pass objects to other languages.
		   *
		   * Use with care.
		   *
	      ...
	      else if (type == cstring_type_node)
		{
		  /* This is a __cstring__ parameter, which is a GPC extension
     ...
     int convert_p;
{
...
#ifdef GPC
  /* Use the base type of the subrange rather than INTERGER_TYPE
   */
  ...
  switch (code)
	...
	{
	  if (! is_string_type (op0) || ! is_string_type (op1))
	    warning ("ordered comparison between arrays that are not of string type");
	  else
	    result_type = boolean_type_node;
	  converted = 1;
	}
     ...
     tree ifexp, op1, op2;
{
  ...
  if (TYPE_MAIN_VARIANT (type1) == TYPE_MAIN_VARIANT (type2))
	       ...
	       && comptypes(type1, type2))
	{
	  ...
	  converted = 1;
#endif
	  if (! is_string_type (op1) || ! is_string_type (op2))
	    {
	      warning ("compared arrays are not of string type");
     ...
     int parmnum;
{
...
#ifdef GPC
  /* default_conversion() does not convert array_type in gpc.*/
#endif
  ...
  if (ORDINAL_REAL_OR_COMPLEX_TYPE (codel)
      && compatible_assignment_p (codel, coder))
#else /* GPC */
...
#ifdef GPC
          || comp_object_pointer_types (ttl, ttr)
#endif /* GPC */
  ...
  /* Array assignments */
    ...
    {
      if (TREE_TYPE (rhstype) != TREE_TYPE (type))
	{
	  error ("array %s to different type of array", errtype);
	  return error_mark_node;
	}
	...
	{
	  error ("set %s to different type of set", errtype);
	  return error_mark_node;
	}
...
#ifdef GPC
  if (!INTEGRAL_TYPE_P (type) && code != ERROR_MARK)
#else
