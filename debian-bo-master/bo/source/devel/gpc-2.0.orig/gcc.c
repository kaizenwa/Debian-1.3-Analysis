...
static struct compiler default_compilers[] =
{
  ...
  {".p", "@pas"},
  {"@pas",
   "gpc-cpp -lang-pascal %{nostdinc*} %{C} %{v} %{A*} %{I*} %{P} %I\
	%{!fchar-escapes:-nocharescape} %{fnested-comments:-nestedcomments}\
	%{fborland-pascal:-nestedcomments}\
...
/* jtv@hut.fi: @@@@@ FIX THIS! Should have lang specific libs */
...
%{!fsyntax-only: \
 %{!c:%{!M:%{!MM:%{!E:%{!S:ld %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \
			...
			%{!nostdlib:%{!nodefaultlibs:libgpc%s %G libm.a%s %G %L %G}}\
...
%{!fsyntax-only: \
 %{!c:%{!M:%{!MM:%{!E:%{!S:ld %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \
			...
			%{static:} %{L*} %D %{T*} %o\
                        %{fbounds-checking:libcheck.a%s} \
			%{!nostdlib:%{!nodefaultlibs:-lgpc %G -lm %L %G}}\
			%{!A:%{!nostdlib:%{!nostartfiles:%E}}}\n }}}}}}";
#else /* EMX */
static char *link_command_spec = "\
%{!fsyntax-only: \
 %{!c:%{!M:%{!MM:%{!E:%{!S:%{!Zomf:ld}%{Zomf:emxomfld}\
			...
			%{!nostdlib:%{!nodefaultlibs:\
			  -lgpc -lc_alias \
			  %{Zcrtdll=*:-lc_static -l%* -los2 -lemx2}\
			  %{!Zcrtdll:%{!Zcrtdll=*:%G -lm %L %G\
			     %{Zno-rte:-los2}\
...
#ifdef GPC
/* Concatenate command line arguments containing blanks
 * surrounded by quotation marks and remove the quotation marks.
 */
void
concat_args (argcp, argv)
     int *argcp;
     char **argv;
{
  int argc = *argcp, i = 0, quoted = 0;
  while (i < argc)
    {
      char *p = argv [i];
      while (*p)
      ...
      if (quoted && i + 1 < argc)
        {
          int j;
          argv [i] = concat3 (argv [i], " ", argv [i + 1]);
          for (j = i + 2; j < argc; j++)
            argv [j - 1] = argv [j];
          quoted = 0;
          argc--;
...
#if 0		/* Don't think it's a bug. Enable this and mktemp() barfs for djgpp */
		obstack_grow (&obstack, t->suffix, t->length);
		  /* ^^^ This was missing. BUG??? ^^^ - PG */
#endif  /* 0 */
		delete_this_arg = 1;
#else
...
/*
 * Add files determined by --automake to the linker input files.
 */
char *
add_automake_files (explicit_link_files)
     char *explicit_link_files;
{
  char automake_info_filename [256];
  FILE *automake_info;
  strcpy (automake_info_filename, input_basename);
  automake_info_filename [basename_length] = 0;
  strcat (automake_info_filename, ".gpc");
  automake_info = fopen (automake_info_filename, "rt");
  if (automake_info)
    {
      int i = n_infiles;
      char buffer [256];
      while (!feof (automake_info))
        {
          int j;
          if (i >= n_infiles)
            {
              int new_n_infiles = n_infiles + 16;
              char **new_outfiles = (char **) xmalloc (new_n_infiles * 
                                                       sizeof (char*));
              char *new_explicit_link_files = xmalloc (new_n_infiles);
              bcopy ((char *) outfiles, 
                     (char *) new_outfiles,
                     (n_infiles) * sizeof (char*));
              bcopy ((char*) explicit_link_files, 
                     (char *) new_explicit_link_files, 
                     n_infiles);
              n_infiles = new_n_infiles;
              outfiles = new_outfiles;
            }
          fscanf (automake_info, "%s", buffer);
          /* Don't add an object file which is already present */
          for (j = 0; j < i; j++)
            if (outfiles [j] && strcmp (outfiles [j], buffer) == 0)
              break;
          if (j >= i)
            {
              /* Add the object file at the beginning */
              for (j = i; j > 0; j--)
                {
                  outfiles [j] = outfiles [j - 1];
                  explicit_link_files [j] = explicit_link_files [j - 1];
                }
              outfiles [0] = save_string (buffer, strlen (buffer));
              explicit_link_files [0] = 1;
              i++;
            }
        }
      n_infiles = i;
      fclose (automake_info);
      unlink (automake_info_filename);  /* "%d" in the spec should do this! */
    }
  return explicit_link_files;
}
     ...
     char **argv;
{
...
#ifdef GPC
  concat_args (&argc, argv); /* PG: handle "..." args containing blanks */
#endif /* GPC */
...
#if defined(GPC) && defined(__DJGPP__)
  /* With DJGPP, read GPC specs from "specs.gpc" rather than "specs" */
  specs_file = find_a_file (&startfile_prefixes, "specs.gpc", R_OK);
  /* Read the specs file unless it is a default one.  */
  if (specs_file != 0 && strcmp (specs_file, "specs.gpc"))
  ...
  if (verbose_flag)
	...
	fprintf (stderr, "gpc driver version %s executing gpc version %s\n",
