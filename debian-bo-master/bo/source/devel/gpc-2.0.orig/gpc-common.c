     ...
     tree strings;
{
...
#ifdef GPC
      if (pedantic)
	warning ("ISO Pascal does not allow string catenations");
#endif
  ...
  indices = build_index_2_type (build_int_2 (1, 0),
				build_int_2
				  (nchars - (wide_flag ? wchar_bytes : 1), 0));
#endif /* GPC */
     ...
     tree arg1, arg2;
{
  ...
  if (TREE_CODE (arg1) == TREE_LIST)
    arg1 = TYPE_NAME (TREE_VALUE (arg1));
  else
    arg1 = TYPE_NAME (TREE_TYPE (arg1));
  if (arg1 && TREE_CODE (arg1) == TYPE_DECL)
    arg1 = DECL_NAME (arg1);
  if (TREE_CODE (arg2) == TREE_LIST)
    arg2 = TYPE_NAME (TREE_VALUE (arg2));
  else
    arg2 = TYPE_NAME (TREE_TYPE (arg2));
  if (arg2 && TREE_CODE (arg2) == TYPE_DECL)
    arg2 = DECL_NAME (arg2);
  if (! arg1 || ! arg2)
    return NULL_TREE;
  else
    return get_identifier (concat (op_name, "_",
                           concat (IDENTIFIER_POINTER (arg1), "_",
                                   IDENTIFIER_POINTER (arg2))));
}

/*
 * Convert the operations for sets to their corresponding bitwise
 * boolean operators. Other that sets, just call build_binary_op
 * with the supplied args (no conversion)
 *
 ...
 */
tree
build_pascal_op (code, exp1, exp2)
enum tree_code code;
tree exp1;
tree exp2;
{
    ...
    if (op_name)
      {
	tree func = get_operator_identifier (op_name, exp1, exp2);
        if (func)
    ...
    /* Convert a set constructor to a set modeled after exp2 if that
     * is of SET_TYPE
     */
    if (t1 == CONSTRUCTOR)
      {
	...
	exp1 = construct_set
	  	(exp1,
		 (t2 == SET_TYPE
		  && TREE_CODE (TREE_TYPE (TREE_TYPE (exp2))) != VOID_TYPE)
		 	? TREE_TYPE (exp2) : NULL_TREE,
		 1);
      }
	...
	exp2 = construct_set
	  	(exp2,
		 (t1 == SET_TYPE
		  && TREE_CODE (TREE_TYPE (TREE_TYPE (exp1))) != VOID_TYPE)
		 	? TREE_TYPE (exp1) : NULL_TREE,
		 1);
      }
	...
	case NE_EXPR: rts_code = NEQ; break;
	case LT_EXPR: rts_code = '<'; break;
	case LE_EXPR: rts_code = LTE; break;
	case GT_EXPR: rts_code = '>'; break;
	case GE_EXPR: rts_code = GTE; break;
	}
	if (rts_code)
	  {
	    if (pedantic)
	      warning ("Pascal does not allow comparison of different string and char types");
	...
	switch (code)
	  ...
	  case BIT_XOR_EXPR:
	    if (pedantic)
	      warning ("ISO Pascal does not support symmetric diff operation");
	    break;
	  case EQ_EXPR:
	  case NE_EXPR:
	  case LE_EXPR:
    ...
    } else if (t1 == SET_TYPE || t1 == CONSTRUCTOR
	       || t2 == SET_TYPE || t2 == CONSTRUCTOR) {
	/* No other set type operations come through here */
	binary_op_error (code);
	return error_mark_node;
    }
    return build_binary_op (code, exp1, exp2, 0);
}
     ...
     tree arg1, arg2;
{
  ...
  tree op_func, arg;
  if (pedantic)
    warning ("ISO Pascal does not define PXSC operators");
 ...
 *
 *   RESTRICTED,PROTECTED,BINDABLE,PACKED are ISO-10206 standard qualifiers
 *   for types. (PROTECTED is not handled here.)
 *
     ...
     int copy;
{
  ...
  if (copy && need_copy)
    {
      ...
      type = change_main_variant (type, type);
  ...
  if (PASCAL_TYPE_BINDABLE (type) && PASCAL_TYPE_RESTRICTED (type))
    error ("The bindability denoted by a restricted-type shall be nonbindable (6.4.2.5)");
     ...
     int extension;
{
  ...
  if (t_byte + t_short + t_long + t_longlong > 1)
    {
      error ("Illegal to give multiple size qualifiers");
      return type;
    }
