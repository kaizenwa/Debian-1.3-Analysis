/* C Compatible Compiler Preprocessor (CCCP)
   ...
   Modified to accept Pascal, Jukka Virtanen <jtv@hut.fi>, Mar 1995
   Changes for emx by Eberhard Mattes merged, Peter Gerwinski, Aug 1995
   Modified to handle Borland Pascal directives, Peter Gerwinski, Oct 1995
...
#endif

/* Processed language, for warnings. */

static char *language = "ANSI C";
...
/* Nonzero means handle Pascal comments, and don't handle C comments */

static int pascal_comments;

/* Nonzero means that the input language is Pascal */

static int lang_pascal = 0;
...
/* Nozero means do not recognize '\'-character escapes */

static int no_charescape = 0;

/* Nozero means the pascal comments '{' and '*)' and
 * '(*' and '}' match as comment delimiters as the ISO standard
 * requires. If zero, the '{' matches only '}' and '(*' matches '*)'
 */

static int no_nested_comments = 1;
...
/* This is the start character of the two char comment leader
 *
 * The second char of these comments sequences is always '*'
 */

static char comment_start_char = '/';

/* And this is the matching end char of a two char comment end sequence */

static char comment_end_char   = '/';
...
#ifdef GPC
static void handle_pascal_directive PROTO((FILE_BUF *, FILE_BUF *, U_CHAR *));
#endif /* GPC */
...
static U_CHAR *skip_to_end_of_comment PROTO((FILE_BUF *, int *, int, int));
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
	...
	if (! strcmp (argv[i], "-lang-pascal")) {
	  ...
	  /* Recognize pascal comments (* comment *) and { another comment }
	   * The C comments are not recognized.
	   */
	  comment_start_char = '(';
	  comment_end_char   = ')';
	}
	...
	if (!strcmp (argv[i], "-nostdinc"))
	  ...
	  no_precomp = 1;
	else if (!strcmp (argv[i], "-nocharescape"))
	  no_charescape = 1;
	else if (!strcmp (argv[i], "-nestedcomments"))
	  {
	    if (!pascal_comments)
	      warning ("Must give -lang-pascal option to allow nesting of pascal comments");
	    else
	      no_nested_comments = 0;
	  }
	break;
  ...
  if ((fp->length > 0 && fp->buf[fp->length - 1] != '\n')
      ...
      || (fp->length > 1 &&
	  (fp->buf[fp->length - 2] == '\\' && !no_charescape))) {
    fp->buf[fp->length++] = '\n';
    missing_newline = 1;
  }
  fp->buf[fp->length] = '\0';
     ...
     U_CHAR *bp;
{
  ...
  if (*p != comment_start_char && *p != comment_end_char && *p != '*')
    return;
  ...
  while (*p == '*' || *p == comment_start_char || *p == comment_end_char)
    *bp++ = *p++;
  ...
  while (1) {
	...
	*obp++ = *ibp++;
	...
	if (!traditional) {
	  while (1) {
	    if (is_hor_space[*bp])
	      bp++;
	    else if ((*bp == '\\' && !no_charescape) && bp[1] == '\n')
	      bp += 2;
	    else if (*bp == comment_start_char && bp[1] == '*') {
	      bp += 2;
	      while (!(*bp == '*' && bp[1] == comment_end_char))
		bp++;
	      bp += 2;
	    }
	...
	*obp++ = *ibp;
	...
	while (ibp < limit) {
	  ...
	  case '{':
	    ...
	    break;
	  case '(':
	    if (no_nested_comments && warn_comments && ibp < limit && *ibp == '*')
	      warning ("`(*' within comment");
	    break;
	  case '*':
	    if (ibp < limit && *ibp == ')')
	  ...
	  case '\n':
	    ...
	    /* Copy the newline into the output buffer, in order to
	       ...
	       is seen.  */
#ifdef GPC
	    if (!put_out_comments && !begin_pascal_directive)
	      *obp++ = '\n';
	    ++op->lineno;
            break;
          case '$':
	...
	goto randomchar;
      if ((!no_charescape && *ibp == '\\') && ibp[1] == '\n')
	newline_fix (ibp);

      if (*ibp != '*' && !(cplusplus_comments && *ibp == '/'))
	goto randomchar;
      if (ip->macro != 0)
	goto randomchar;
      if (ident_length)
	...
	if (lintcmd != NULL) {
	 ...
	 For -traditional, a comment is equivalent to nothing.  */
      if (! put_out_comments) {
	if (traditional && !pascal_comments)
	  obp--;
	else
	  obp[-1] = ' ';
      }
	...
	*obp++ = '*';
	...
	while (ibp < limit) {
	  switch (*ibp++) {
	  case '{':
	    if (no_nested_comments && pascal_comments && warn_comments)
	      warning ("`{' within comment");
	    break;
	  case '}':
	    if (pascal_comments)
	    ...
	    break;
	  case '(':
	    if (pascal_comments && warn_comments && *ibp == '*')
	      warning ("`(*' within comment");
	    break;
	  case '/':
	    if (!pascal_comments && warn_comments && *ibp == '*')
	  ...
	  case '*':
	    if ((!no_charescape && *ibp == '\\') && ibp[1] == '\n')
	      newline_fix (ibp);
	    if (ibp >= limit || *ibp == comment_end_char)
	      goto comment_end;
	    break;
	  case '\n':
	    ++ip->lineno;
	    ...
	    ++op->lineno;
            break;
          case '$':
            if (pascal_comments && ibp == before_bp + 1)
              {
                if (!put_out_comments)
                  {
                    obp[-1] = '(';
	  ...
	  for (hp = hashtab[MAKE_POS (hash) % HASHSIZE]; hp != NULL;
		  ...
		  && hp->value.defn->nargs >= 0)
		{
		  ...
		  while (1) {
		    ...
		    else if (*ibp == comment_start_char && ibp[1] == '*') {
		      if (put_out_comments) {
			*obp++ = comment_start_char;
			*obp++ = '*';
		      } else if (! traditional) {
     ...
     FILE_BUF *ip, *op;
{
  ...
  while (1) {
	...
	pedwarn ("%s in preprocessing directive", char_name[*bp]);
      bp++;
    } else if (pascal_comments && *bp == '{') {
      ip->bufp = bp + 1;
      skip_to_end_of_comment (ip, &ip->lineno, 0, '}');
      bp = ip->bufp;
    } else if ((!no_charescape && *bp == '\\') && bp[1] == '\n') {
    } else if (*bp == comment_start_char && (bp[1] == '*'
			      || (cplusplus_comments && bp[1] == '/'))) {
      ip->bufp = bp + 2;
      skip_to_end_of_comment (ip, &ip->lineno, 0, 0);
      bp = ip->bufp;
    } else if ((!no_charescape && *bp == '\\') && bp[1] == '\n') {
      bp += 2; ip->lineno++;
    } else break;
	  ...
	  *cp++ = c;
	  ...
	  case '(':
		...
		|| (cplusplus_comments && *xp == '/')) {
		 ...
		 already_output != 0 prevents outputting comment now.  */
	      skip_to_end_of_comment (ip, already_output, 0, 0);
	      if (keep_comments)
		while (xp != ip->bufp)
		  *cp++ = *xp++;
...
#ifdef GPC
/*
 * Handle a Pascal directive within a comment:
 *   (*$I FileName *)   --   #include "filename.pas"
 *   (*$M Hello! *)     --   output message "Hello!" to stderr
...
{
  register U_CHAR *p, *q;
  U_CHAR dir_name [256], filename [256];
  struct directive *kt;
  /* 
   * Make the first word of the directive low-case
   * and create a null-determinated copy
   */
  do
    {
      p = ip->bufp;
      ...
      *q = 0;
      if (! *dir_name)
        error ("empty Pascal preprocessor directive");
      else if (! dir_name [1])
        {
          /*
           * Handle Pascal one-char directives
           */
          switch (dir_name [0])
            {
              case 'd':  /* #define */
		...
		for (kt = directive_table; kt->type != T_INCLUDE; kt++)
		  ;
		do_include (filename, filename + strlen (filename), op, kt);
                p = limit;
                break;
              case 'm':  /* Message to stderr -- handled by compiler. */
                p = limit;
                break;
              case 'n':  /* Nested comments */
                no_nested_comments = (*p++ == '-');
                break;
              case 'p':  /* Be "pedantic" */
                pedantic = (*p++ == '+');
                break;
              ...
              case 'x':  /* Extended syntax */
                p++;
                break;   /* Handled by compiler.  Just don't complain. */
              default:
                p++;
                warning ("unknown GNU Pascal directive `$%c'", dir_name[0]);
            }
          /*
          ...
          if (p < limit)
             {
                ...
                {
                  warning ("garbage after Pascal preprocessor directive");
                  p = limit;
                }
            }
        }
      else
        {
           ...
           * First kill newlines within range of directive.
           * Don't worry about line numbers:  This text has
           ...
           * It overwrites the closing brace.
           * In case it is read it will artificially
     ...
     struct file_name_list *dirptr;
{
  ...
  if ((fp->length > 0 && fp->buf[fp->length - 1] != '\n')
      /* Backslash-newline at end is not good enough.  */
      || (fp->length > 1
	  && (!no_charescape && fp->buf[fp->length - 2] == '\\'))) {
    fp->buf[fp->length++] = '\n';
    missing_newline = 1;
  }
  fp->buf[fp->length] = '\0';
  ...
  while (p < limit) {
    ...
    *exp_p++ = c;
	...
	break;
      ...
      case '\\':
	if (!no_charescape && p < limit && expected_delimiter) {
	...
	/* @@@@@@@ Check this out */
	warning ("gpc-ccp (collect_expansion): found a '{'-comment");
	break;
	...
	if (*p == '*') {
	  ...
	  exp_p--;
	  p += 1;
	  while (p < limit && !(p[-2] == '*' && p[-1] == comment_end_char))
	    p++;
#if 0
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `#assert'", language);
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `#unassert'", language);
     ...
     struct arglist *tokens;
{
  ...
  if (pedantic && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow testing assertions", language);
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `#ident'", language);
...
{
  if (pedantic)
    pedwarn ("%s does not allow `#sccs'", language);
  return 0;
}
     ...
     struct directive *keyword;
{
  ...
  if (ip->fname != 0 && keyword->type == T_IFNDEF) {
	...
	U_CHAR *save_bufp = ip->bufp;
	ip->bufp = p + 1;
	p = skip_to_end_of_comment (ip, &junk, 1, 0);
	ip->bufp = save_bufp;
      } else {
	goto fail;
      }
     ...
     FILE_BUF *op;
{
  ...
  while (bp < endb) {
...
#endif /* GPC */
	bp = skip_to_end_of_comment (ip, &ip->lineno, 0, 0);
      }
      break;
    case '\"':
    case '\'':
...
#ifdef GPC
    directive:
      /* @@@@@@@@@ This is probably wrong. TEST ME */
#endif /* GPC */
      /* # keyword: a # must be first nonblank char on the line */
     if (pascal_directive) {
       ip->bufp = bp - 1;
	...
	if (is_hor_space[*bp])
	  bp++;
#ifdef GPC
	else if ((!pascal_directive)
                 && (!no_charescape && *bp == '\\') && bp[1] == '\n')
#else /* GPC */
	else if ((!no_charescape && *bp == '\\') && bp[1] == '\n')
#endif /* GPC */
	  bp += 2;
	...
	if (ident_length == kt->length
	  ...
	  case T_ELIF:
	    if (if_stack == instack[indepth].if_stack) {
#ifdef GPC
	      error ("`%c%s' not within a conditional",
                     pascal_directive ? '$' : '#', 
                     kt->name);
#else /* GPC */
     ...
     register U_CHAR *p;
{
  ...
  while (1) {
    if ((!no_charescape && *p == '\\') && p[1] == '\n')
      p += 2;
    if (is_hor_space[*p])
      p++;
    else if (*p == comment_start_char) {
      if ((!no_charescape && p[1] == '\\') && p[2] == '\n')
	newline_fix (p + 1);
      if (p[1] == '*') {
	p += 2;
...
static U_CHAR *
skip_to_end_of_comment (ip, line_counter, nowarn, match)
     register FILE_BUF *ip;
     ...
     int match;
{
  ...
  if (output) {
    if (pascal_comments && match == '}')
      *op->bufp++ = '{';
    else {
      *op->bufp++ = comment_start_char;
      *op->bufp++ = '*';
    }
  }
  if (cplusplus_comments && bp[-1] == '/') {
	...
	*op->bufp++ = *bp;
	...
	bp++;
      ...
      op->bufp[-1] = '*';
      *op->bufp++ = comment_end_char;
      *op->bufp++ = '\n';
    } else {
      while (bp < limit) {
	if ((!no_charescape && bp[-1] != '\\') && *bp == '\n') {
	  break;
	} else {
	  if (*bp == '\n' && line_counter)
	    ++*line_counter;
  ...
  while (bp < limit) {
    ...
    switch (*bp++) {
    case '{':
      if (pascal_comments && (match == '}' || no_nested_comments)
	  && warn_comments && !nowarn)
	warning ("`{' within comment");
      break;
    case '(':
      if (pascal_comments && (!match || no_nested_comments)
	  && warn_comments && !nowarn && bp < limit && *bp == '*')
	warning ("`(*' within comment");
      break;
    case '/':
      if (!pascal_comments && warn_comments && !nowarn && bp < limit && *bp == '*')
	warning ("`/*' within comment");
      break;
    case '\n':
     ...
     register FILE_BUF *ip;
{
  ...
  while (p != limit) {
    ...
    case '{':
      if (! pascal_comments)
	break;
      ip->bufp = p-1;	/* @@@ Check if p is correct */
      p = skip_to_end_of_comment (ip, &lines_dummy, 0, '}');
      p = ip->bufp;
      break;
    case '(':
      if (pascal_comments && *p == '*') {
	ip->bufp = p;
	p = skip_to_end_of_comment (ip, &lines_dummy, 0, 0);
	p = ip->bufp;
	break;
      }
      depth++;
  ...
  if (nargs >= 0) {
	...
	if (ap->stringify != 0) {
	  ...
	  for (; i < arglen; i++) {
	    ...
	    /* Escape these chars */
	    if (c == '\"' || (in_string && c == '\\' && !no_charescape))
	      xbuf[totlen++] = '\\';
	    if (isprint (c))
	      xbuf[totlen++] = c;
	    else {
...
#endif
      if (c == '\"' || (c == '\\' && !no_charescape)) /* escape these chars */
	totlen++;
      else if (!isprint (c))
	totlen += 3;
    }
     ...
     int rest_args;
{
  ...
  while (bp < limit) {
	...
	if (no_nested_comments)
	  {
	    if (bp[0] == '*' && (!no_charescape && bp[1] == '\\') && bp[2] == '\n')
	      newline_fix (bp + 1);
	    if (bp[0] == '*' && bp[1] == comment_end_char)
	      break;	/* @@@ Should increment bp? */
	  }
	if (*bp == '}')
	  break;
	if (*bp == '\n') ++*newlines;
	bp++;
      ...
      break;
    case '(':
      if (!no_charescape && bp[1] == '\\' && bp[2] == '\n')
	newline_fix (bp + 1);
      if (pascal_comments && bp[1] == '*') {
	if (bp + 1 >= limit)
	  break;
	*comments = 1;
	bp += 2;
	while (bp + 1 < limit) {
	  if (no_nested_comments && bp[0] == '}')
	    break;
	  if (bp[0] == '*'
	      && (!no_charescape && bp[1] == '\\') && bp[2] == '\n')
	    newline_fix (bp + 1);
	  if (bp[0] == '*' && bp[1] == comment_end_char)
	    break;
	  if (*bp == '\n') ++*newlines;
	...
	*comments = 1;
	...
	break;
      ...
      if (pascal_comments || bp[1] != '*' || bp + 1 >= limit)
	break;
      *comments = 1;
      bp += 2;
      while (bp + 1 < limit) {
	...
	if (no_nested_comments)
	  {
	    if ((!no_charescape && *ibp == '\\') && ibp[1] == '\n')
	      newline_fix (ibp);
	    if (ibp[0] == '*' && ibp[1] == comment_end_char)
	      {
		ibp++;
		break;
	      }
