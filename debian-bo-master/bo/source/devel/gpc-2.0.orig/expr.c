     ...
     int pad;
{
  ...
  emit_cmp_insn (ptr, last, GEU, NULL_RTX, Pmode, 0, 0);
  emit_jump_insn (gen_bgeu (done));
     ...
     tree len;
{
  ...
  rtx dest_rtx = expand_expr (dest, NULL_RTX, Pmode, EXPAND_NORMAL);
  rtx dest_mem = gen_rtx (MEM, BLKmode,
			  memory_address (BLKmode, dest_rtx));

  rtx src_mem = gen_rtx (MEM, BLKmode,
...
#ifdef GPC
      {
	/* ISO Pascal round(x): if x >= 0.0 then trunc (x+0.5) else trunc (x-0.5); */
