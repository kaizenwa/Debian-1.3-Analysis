...
#if MACINTOSH
#include <stdio.h>
char *FileNameMacToUnix(const char *fn);
char *FileNameUnixToMac(const char *fn);
char *FileNameMacConvert(char *(*func)(char *),const char *fn);
#endif					/* MACINTOSH */
 ...
 */
hidden novalue file_src Params((char *fname, FILE *f));
 ...
 */
static novalue file_src(fname, f)
char *fname;
FILE *f;
   {
   union src_ref ref;
 ...
 */
novalue include(trigger, fname, system)
 ...
 *  system header files.
 */
novalue init_files(opt_lst, opt_args)
char *opt_lst;
char **opt_args;
   {
   int n_paths = 0;
...
#if AMIGA || ATARI_ST || VM || MVS || (MACINTOSH && !MPW && !THINK_C)
   /* probably needs something */
Deliberate Syntax Error
#endif					/* AMIGA || ATARI_ST || ... */
...
char *
FileNameUnixToMac(const char *fn) {
  char *q,*e,*r;
  int full;
...
char *
FileNameMacToUnix(const char *fn) {
  char *q,*e,*r;
 ...
 */
char *
FileNameMacConvert(char *(*func)(char *),const char *fn) {
   char *newfp, *newmem;
