...
hidden struct 	fentry *alcfhead
   Params((struct fentry *blink,word name, int fid, struct rentry *rlist));
hidden struct 	rentry *alcfrec	
   Params((struct rentry *link,struct gentry *gp, int fnum));
hidden struct 	gentry *alcglobal
   Params((struct gentry *blink, word name, int flag,int nargs,int procid));
 ...
 */
novalue putfield(fname, gp, fnum)
word fname;
struct gentry *gp;
int fnum;
   {
