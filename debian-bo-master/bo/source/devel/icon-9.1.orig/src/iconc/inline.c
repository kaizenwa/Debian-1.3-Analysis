 ...
 */
hidden novalue          arth_arg  Params(( struct il_code *var,
                                    struct val_loc *v_orig, int chk,
                                    struct code *cnv));
hidden int              body_fnc  Params((struct il_code *il));
hidden novalue          chkforblk Params((noargs));
hidden novalue          cnv_dest  Params((int loc, int is_cstr,
                                    struct il_code *src, int sym_indx,
                                    struct il_c *dest, struct code *cd, int i));
hidden novalue          dwrd_asgn Params((struct val_loc *vloc, char *typ));
hidden struct il_c     *line_ilc  Params((struct il_c *ilc));
hidden int              gen_if    Params((struct code *cond_cd,
                                    struct il_code *il_then,
                                    struct il_code *il_else,
                                    struct val_loc **locs));
hidden int              gen_il    Params((struct il_code *il));
hidden novalue          gen_ilc   Params((struct il_c *il));
hidden novalue          gen_ilret Params((struct il_c *ilc));
hidden int              gen_tcase Params((struct il_code *il, int has_dflt));
hidden novalue          il_var    Params((struct il_code *il, struct code *cd,
                                    int indx));
hidden novalue          mrg_locs  Params((struct val_loc **locs));
hidden struct code     *oper_lbl  Params((char *s));
hidden novalue          part_asgn Params((struct val_loc *vloc, char *asgn,
                                    struct il_c *value));
hidden novalue          rstr_locs Params((struct val_loc **locs));
hidden struct val_loc **sav_locs  Params((noargs));
hidden novalue         sub_ilc    Params((struct il_c *ilc, struct code *cd,
                                    int indx));
 ...
 */
novalue gen_inlin(il, r, strt, fail, c, ip, ns, st, n, dcl_var, n_va)
struct il_code *il;
struct val_loc *r;
struct code **strt;
struct code **fail;
 ...
 */
static novalue arth_arg(var, v_orig, chk, cnv)
struct il_code *var;
...
struct il_code *il;
   ...
   if (!proto_prt) {
      ...
      switch (ret_val) {
         case RetInt:
            fprintf(inclfile, "C_integer %s Params((", oper_nm);
            break;
         case RetDbl:
            fprintf(inclfile, "double %s Params((", oper_nm);
            break;
         case RetNoVal:
            fprintf(inclfile, "novalue %s Params((", oper_nm);
            break;
         case RetSig:
            fprintf(inclfile, "int %s Params((", oper_nm);
            break;
         }
     }
 ...
 */
static novalue il_var(il, cd, indx)
struct il_code *il;
struct code *cd;
int indx;
   {
 ...
 */
static novalue part_asgn(vloc, asgn, value)
struct val_loc *vloc;
char *asgn;
struct il_c *value;
   {
 ...
 */
static novalue dwrd_asgn(vloc, typ)
struct val_loc *vloc;
char *typ;
   {
   struct code *cd;
 ...
 */
static novalue sub_ilc(ilc, cd, indx)
struct il_c *ilc;
struct code *cd;
int indx;
   {
 ...
 */
static novalue cnv_dest(loc, is_cstr, src, sym_indx, dest, cd, i)
int loc;
int is_cstr;
struct il_code *src;
int sym_indx;
