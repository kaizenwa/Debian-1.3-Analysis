 ...
 * Prototypes
 */
hidden novalue postqual		Params((dptr dp));
hidden novalue markblock	Params((dptr dp));
hidden novalue markptr		Params((union block **ptr));
hidden novalue sweep		Params((struct b_coexpr *ce));
hidden novalue sweep_stk	Params((struct b_coexpr *ce));
hidden novalue reclaim		Params((noargs));
hidden novalue cofree		Params((noargs));
hidden novalue scollect		Params((word extra));
hidden int     qlcmp		Params((dptr  *q1,dptr  *q2));
hidden novalue adjust		Params((char *source, char *dest));
hidden novalue compact		Params((char *source));
hidden novalue mvc		Params((uword n, char *src, char *dest));

#ifdef MultiThread
hidden novalue markprogram	Params((struct progstate *pstate));
#endif					/*MultiThread*/
...
static novalue markprogram(pstate)
...
static novalue adjust(source,dest)
char *source, *dest;
   {
