...
hidden	int	colorphrase	Params((char *buf, long *r, long *g, long *b));
...
hidden	novalue	gfcontrol	Params((FILE *f));
...
int p;
   ...
   for (i = 0; i < 256; i++)
      palette[i].valid = palette[i].transpt = 0;
   palette[TCH1].transpt = 1;
   palette[TCH2].transpt = 1;
...
XPoint *p;
int n;
void (*helper)	Params((wbp, XPoint [], int));
   {
   int    i, j, steps;
...
FILE *OpenConsole()
   ...
   if (!ConsoleBinding) {
      ...
      mystrncpy(ConsoleTitle, StrLoc(kywd_prog), StrLen(kywd_prog));
