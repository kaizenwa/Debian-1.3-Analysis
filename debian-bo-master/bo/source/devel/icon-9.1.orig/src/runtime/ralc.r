 ...
 * Prototypes.
 */
hidden struct region *findgap	Params((struct region *curr, word nbytes));
hidden struct region *newregion	Params((word nbytes, word stdsize));
...
word nbytes;
{
   ...
   if (rp = findgap(curr, nbytes)) {	/* check all regions on chain */
   ...
   if (rp = newregion(nbytes, newsize)) {
      rp->prev = curr;
      rp->next = NULL;
      curr->next = rp;
      rp->Gnext = curr;
