 ...
 */
#ifdef EventMon
hidden struct ef_marker *vanq_bound Params((struct ef_marker *efp_v,
                                      struct gf_marker *gfp_v));
hidden novalue           vanq_proc Params((struct ef_marker *efp_v,
                                     struct gf_marker *gfp_v));
#endif					/* EventMon */
 ...
 */
   ...
   if (!is:null(curpstate->eventmask) && (
      ...
      if (InRange(code, ipc.opnd, records)) {
            ...
            switch (Type(value_tmp)) {
               case T_List: {
                 ...
                 if (BlkLoc(k_current) == BlkLoc(k_main) &&
                  ...
                  for (bp = bp->list.listhead; bp != NULL;
 ...
 *   a procedure.
 */
static novalue vanq_proc(efp_v, gfp_v)
