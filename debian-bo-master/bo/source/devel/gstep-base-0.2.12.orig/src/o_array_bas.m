...
_o_array_alloc_with_zone(NSZone *zone)
{
  ...
  xx = (o_array_t *) NSZoneMalloc(zone, sizeof(o_array_t));

  /* The `o_malloc()' was successful. */
  if (xx != 0)
    {
      _o_array_set_serial_number(xx);
      xx->magic_number = _OBJECTS_MAGIC_array;
      xx->name = 0;
      xx->zone = zone;
...
/* Handles the universal, low-level deallocation of array structures. */
inline void
_o_array_dealloc(o_array_t *xx)
{
  ...
  if (xx != 0)
  {
    ...
    if (xx->name != 0)
      NSZoneFree(o_array_zone(xx), (char *)(xx->name));

    /* Release XX's extra. */
    o_array_unset_extra(xx);

    /* Free up XX itself. */
    NSZoneFree(o_array_zone(xx), xx);
...
_o_array_copy_with_zone(o_array_t *xx, NSZone *zone)
{
  ...
  new = _o_array_alloc_with_zone(zone);

  if (new != 0)
  {
    /* Copy over XX's name. */
    o_array_set_name(new, o_array_name(xx));

    /* Copy over XX's extras. */
    o_array_set_extra_callbacks(new, o_array_extra_callbacks(xx));
    o_array_set_extra(new, o_array_extra(xx));
  }
...
_o_array_description(o_array_t *xx)
{
  return [NSString
	   ...
	   @"name = %s;\nextra = %s;\nzone = %s;\n",
	   o_array_magic_number(xx),
	   o_array_serial_number(xx),
	   [o_array_name(xx) cStringNoCopy],
	   [o_describe(o_array_extra_callbacks(xx), o_array_extra(xx), xx)
		      cStringNoCopy],
	   [NSZoneName(o_array_zone(xx)) cStringNoCopy]];
}
