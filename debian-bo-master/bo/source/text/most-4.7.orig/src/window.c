...
static void check_dirty_flag (void)
{
   ...
   Most_Num_Lines = Most_Win->n_lines = most_count_lines (Most_Beg, Most_Eob);
}
...
void most_redraw_window (void)
{
   ...
   Most_C_Pos = Most_Beg + Most_Win->top_offset;
...
{
   Most_Win->beg_line = Most_C_Line;
   Most_Win->top_offset = Most_C_Pos - Most_Beg;
   Most_Win->col = Most_Column;
...
void most_init_display ()
{
   ...
   else if (h > SLtt_Screen_Rows) most_resize_display (0);
...
{
   char str[30], ch, *strp;
   static char neew[256];
   int i,ii,r,x,line_p = 60, len;
   int max_len;
   Most_Buffer_Type *buf = Most_Win->buf;
   ...
   i = ii = 0;
   neew[ii++] = '-';
   if (Most_Win->lock) neew[ii++] = '*'; else neew[ii++] = '-';
   strp = " MOST: ";
   while(*strp != '\0') neew[ii++] = *strp++;
   
   len = strlen (buf->file);
   if (len > max_len)
     {
        neew[ii++] = '.'; neew[ii++] = '.'; neew[ii++] = '.';
	strcpy (&neew[ii], buf->file + (len - max_len));
	ii += max_len;
     }
   else
     {
	strcpy (&neew[ii], buf->file);
	ii += len;
     }
   
   while(ii < line_p) neew[ii++] = ' ';
   ...
   i = 0; while(ch = str[i++], ch != '\0') neew[ii++] = ch;
   
   while(ii < SLtt_Screen_Cols) neew[ii++] = '-';
   neew[SLtt_Screen_Cols] = '\0';
   
   most_tt_reverse_video();
   SLsmg_write_string (neew);
   most_tt_normal_video();
   strcpy(Most_Win->status,neew);
}
...
void most_two_windows()
{
   ...
   if (line + nnew->bot - nnew->top > Most_Num_Lines)
	...
	Most_C_Pos = Most_Beg + old->top_offset;
	...
	Most_Win->curs_offset = Most_C_Pos - Most_Beg;
	most_other_window(-1);
     }
   else
     {
	...
	nnew->top_offset = Most_C_Pos - Most_Beg;
	nnew->curs_offset = Most_C_Pos - Most_Beg;
	most_update_status();
	Most_Win = old;
     }
   most_update_status();
