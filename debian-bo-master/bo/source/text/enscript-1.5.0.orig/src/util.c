...
static void cfg_fatal ___P ((char *file, int line, char *fmt, ...));
...
parse_font_spec (char *spec, char **name_return, FontPoint *size_return)
{
  ...
  *name_return = (char *) xcalloc (1, i + 1);
  ...
  message (2, "parse_font_spec(): name=%.*s, size=%g/%g\n", i, *name_return,
	   size_return->w, size_return->h);

  if (size_return->w < 0.0 && size_return->h < 0.0)
    message (0, _("%s: warning: font size is negative\n"), program);
  else if (size_return->w < 0.0)
    message (0, _("%s: warning: font width is negative\n"), program);
  else if (size_return->h < 0.0)
    message (0, _("%s: warning: font height is negative\n"), program);
...
read_font_info (void)
{
  ...
  message (2, _("reading AFM info for font \"%s\"\n"), Fname);
...
#define COUR "Courier"
	  ...
	  switch (encoding)
	    ...
	    case ENC_LATIN5:
	      (void) afm_font_encoding (font, AFM_ENCODING_LATIN5, enc_flags);
	      break;
      ...
      for (i = 0; i < 256; i++)
	{
	  ...
	  (void) afm_font_charwidth (font, Fpt.w, (unsigned int) i,
				     &w0x, &w0y);
	  ...
	  if (type == '%')
		...
		case 'D':
		  if (str[i + 1] == '{')
			...
			buf2[j] = str[i];
		      if (str[i] != '}')
			fatal (_("%s: too long format for %%D{} escape"),
			       context_name);
		...
		default:
		  fatal (_("%s: unknown `%%' escape `%c' (%d)"), context_name,
			 str[i], str[i]);
		  break;
		}
	    }
		...
		case '(':	/* $(ENVVAR)  */
		  ...
		  if (str[i] == '\0')
		    fatal (_("%s: no closing ')' for $() escape"),
			   context_name);
		  if (str[i] != ')')
		    fatal (_("%s: too long variable name for $() escape"),
			   context_name);
