...
do {									\
  if (arg == NULL)							\
    {									\
      fprintf (stderr, _("%s: primitive `%s': too few arguments\n"),	\
	       program, prim_name);					\
      exit (1);								\
    }									\
} while (0)
...
#define LAST_ARG()							\
  do {									\
    if (arg != NULL)							\
      {									\
	fprintf (stderr, _("%s: primitive `%s': too many arguments\n"),	\
		 program, prim_name);					\
	exit (1);							\
      }									\
  } while (0)

#define MATCH_ARG(type) match_arg (prim_name, type, &arg, env)
...
static Node *prim_print ___P ((char *prim_name, List *args, Environment *env));
...
static Node *
match_arg (prim_name, type, argp, env)
     char *prim_name;
     NodeType type;
     ListItem **argp;
     Environment *env;
{
  ...
  if (type != nVOID && n->type != type)
    {
      fprintf (stderr, _("%s: primitive `%s': illegal argument type\n"),
	       program, prim_name);
      exit (1);
    }
  *argp = arg->next;
  ...
  for (i = namerules->head; i; i = i->next)
    {
      ...
      if (re_search (&n->u.re.compiled, current_fname, strlen (current_fname),
  ...
  for (i = startrules->head; i; i = i->next)
    {
      ...
      if (re_search (&n->u.re.compiled, inbuf, data_in_buffer, 
...
DEFUN (prim_prereq)
{
  ...
  if (sscanf (cp, "%d.%d.%d", &rver[0], &rver[1], &rver[2]) != 3)
    {
      fprintf (stderr,
	       _("%s: primitive `%s': malformed version string `%s'\n"),
	       program, prim_name, cp);
      exit (1);
    }
...
DEFUN (prim_regmatch)
{
  ...
  /* Search for match. */
  i = re_search (&re->u.re.compiled, str->u.str.data, str->u.str.len,
  ...
  for (i = 0; i < fmt->u.str.len; i++)
      ...
      else if (cp[i] == '%')
	{
	  ...
	  switch (cp[i])
		...
		no_match:
		  fprintf (stderr,
			   _("%s: primitive `%s': argument %d doesn't match format\n"),
			   program, prim_name, argument_count);
		  exit (1);
		}
		...
		{
		  fprintf (stderr,
			   _("%s: primitive `%s': no extra options can be specified for %%s\n"),
			   program, prim_name);
		  exit (1);
		}
	    ...
	    default:
	      fprintf (stderr,
		       _("%s: primitive `%s': illegal type specifier `%c'\n"),
		       program, prim_name, cp[i]);
	      exit (1);
	      break;
	    }
	}
...
DEFUN (prim_substring)
{
  ...
  if (start->u.integer > end->u.integer)
    {
      fprintf (stderr,
	       _("%s: primitive `%s': start offset is bigger than end offset\n"),
	       program, prim_name);
      exit (1);
    }
  if (start->u.integer > str->u.str.len || end->u.integer > str->u.str.len)
    {
      fprintf (stderr, _("%s: primitive `%s': too large offsets\n"),
	       program, prim_name);
      exit (1);
    }
