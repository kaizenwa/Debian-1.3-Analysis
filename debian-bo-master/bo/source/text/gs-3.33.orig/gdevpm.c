...
pm_open(gx_device *dev)
{
	...
	if (DosSetMem(pmdev->bitmap, MIN_COMMIT, PAG_COMMIT | PAG_DEFAULT)) {
		...
		fprintf(stderr,"pm_open: failed committing shared BMP memory\n");
		return gs_error_limitcheck;
	}
...
gx_color_value one;
	...
	switch(pmdev->BitsPerPixel) {
	  ...
	  case 8:
		one = (gx_color_value) (gx_max_color_value / 255);
		prgb[0] = pmdev->bmi->argbColor[(int)color].bRed * one;
		prgb[1] = pmdev->bmi->argbColor[(int)color].bGreen * one;
		prgb[2] = pmdev->bmi->argbColor[(int)color].bBlue * one;
		break;
	  case 4:
...
bppe:		param_signal_error(plist, "BitsPerPixel", ecode);
	...
	if ( ecode >= 0 )
	  {
		static const gx_device_color_info pm_fake_16bit_color = dci_color(16,31,31);
		gx_device_color_info old_color_info = dev->color_info;
		/* prevent default routine from objecting to -dBitsPerPixe=16 */
		/* when underlying bitmap is 24bits/pixel */
		if (bpp == 16)
		    dev->color_info = pm_fake_16bit_color;
	...
	if ( ecode < 0 )
	  {
		if ( bpp != old_bpp )
		  pm_set_bits_per_pixel(pmdev, bpp);
		return ecode;
	  }
...
/* device is a pointer to Ghostscript device from GSDLL_DEVICE message */
unsigned long 
gsdll_lock_bitmap(unsigned char *device, int flag)
{
gx_device *dev = (gx_device *)device;
APIRET rc;
    if (flag)
...
pm_run_gspmdrv(gx_device_pm *pmdev)
{
	...
	if ( (rc = DosQueryModuleName(pppib->pib_hmte, 256, progname)) != 0 ) {
...
static const gx_device_color_info pm_16bit_color = dci_color(24,31,31);
...
static const gx_device_color_info pm_2color = dci_black_and_white;
	switch (bpp) {
		...
		break;
	    case 16:
		/* bitmap is stored as 24 bits/pixel */
		pmdev->color_info = pm_16bit_color;
		pmdev->nColors = 32768;
		break;
	    case 8:
