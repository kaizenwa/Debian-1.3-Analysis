...
#define print_save(sav)\
  if_debug3('u', "[u]save 0x%lx: cdata = 0x%lx, id = %lu\n",\
    (ulong)(sav), (ulong)(sav)->client_data, (ulong)(sav)->id);
	lsave->id = sid;
...
{	alloc_save_t *save = mem->saved;
	...
	if_debug4('u', "[u]restore space %u from 0x%lx, cdata = 0x%lx, id = %lu\n",
		  mem->space, (ulong)save, (ulong)save->client_data,
		  (ulong)save->id);
	  ...
	  {	csucc = cp->cnext;	/* save before relinking */
		if ( cp->outer == 0 )
		  ...
		  {	chunk_t *outer = cp->outer;
			...
			{ obj_header_t *hp = (obj_header_t *)cp->chead;
			  hp->o_large = 0;
			  hp->o_size =
			    sizeof(chunk_head_t) - sizeof(obj_header_t);
			  hp->o_type = &st_bytes;
			}
