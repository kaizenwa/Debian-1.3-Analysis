...
#define svga_procs(open) {\
	...
	svga_get_bits, svga_get_params, svga_put_params,\
...
/* Get parameters.  We add TextAlphaBits and GraphicsAlphaBits. */
int
svga_get_params(gx_device *dev, gs_param_list *plist)
{	int code = gx_default_get_params(dev, plist);
	if ( code < 0 ) return code;
	code = param_write_int(plist, "TextAlphaBits", &fb_dev->alpha_text);
	if ( code < 0 ) return code;
	return param_write_int(plist, "GraphicsAlphaBits", &fb_dev->alpha_graphics);
}
...
{	int ecode = 0;
	...
	switch ( code = param_read_int(plist, (param_name = "TextAlphaBits"), &atext) )
	{
	case 0:
	...
	switch ( code = param_read_int(plist, (param_name = "GraphicsAlphaBits"), &agraphics) )
	{
	case 0:
