...
{	alloc_save_t *asave;
	...
	if_debug2('u', "[u]vmrestore 0x%lx, id = %lu\n",
		  (ulong)vmsave, (ulong)op->value.saveid);
	 ...
	 * object was allocated one save level less deep than the current
	 * level, so ifree_object won't actually free it; however, it
	 * points to a gsave object that definitely *has* been freed.
	 * In order not to trip up the garbage collector, we clear
	 * the gsave pointer now.
	 */
	  vmsave->gsave = 0;
