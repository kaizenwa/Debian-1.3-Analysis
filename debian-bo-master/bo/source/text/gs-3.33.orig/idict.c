 ...
 *
 * Note that name keys with indices above packed_max_value require using
 * the unpacked form.
 */
...
#define packed_name_key(nidx)\
  ((nidx) <= packed_max_value ? pt_tag(pt_literal_name) + (nidx) :\
...
{	dict *pdict = pdref->value.pdict;
	...
	code = ialloc_ref_array(&pdict->keys, a_all, asize, "dict_create(keys)");
	if ( code >= 0 )
...
/* Create the contents (keys and values) of a dictionary. */
...
{	dict *pdict = pdref->value.pdict;
	...
	code = ialloc_ref_array(&pdict->values, a_all, asize, "dict_create(values)");
	if ( code < 0 )
	  return code;
	...
	if ( pack )
	   {	uint ksize = (asize + packed_per_ref - 1) / packed_per_ref;
		...
		ref_packed *pkp;
		ref_packed *pzp;
		code = ialloc_ref_array(&arr, a_all, ksize, "dict_create(packed keys)");
...
{	dict *pdict = pdref->value.pdict;
	...
	{	uint count = nslots(pdict);
		...
		ref *nkp;
		old_keys = pdict->keys;
		ialloc_save_change(pdref, (ref_packed *)&pdict->keys,
				   "dict_unpack(keys)");
...
top:	if ( (code = dict_find(pdref, pkey, &pvslot)) <= 0 )	/* not found */
		...
		if ( dict_is_packed(pdict) )
		   {	ref_packed *kp;
			if ( !r_has_type(pkey, t_name) ||
			     name_index(pkey) > packed_max_value
			   )
			...
			if ( ref_must_save(&pdict->keys) )
			   {	/* See initial comment for why it is safe */
				...
				/* array itself is new. */
				ialloc_save_change(&pdict->keys, kp,
						   "dict_put(key)");
			   }
	...
	if_debug8('d', "[d]0x%lx: put key 0x%lx 0x%lx\n  value at 0x%lx: old 0x%lx 0x%lx, new 0x%lx 0x%lx\n",
		  ...
		  ((ulong *)pkey)[0], ((ulong *)pkey)[1], (ulong)pvslot,
...
{	ref *pvslot;
	...
	if ( dict_is_packed(pdict) )
	   {	ref_packed *pkp =
		...
		if ( ref_must_save(&pdict->keys) )
		  ialloc_save_change(&pdict->keys, pkp, "dict_undef(key)");
...
{	dict *pdict = pdref->value.pdict;
	...
	ref_assign_old(pdref, &pdict->keys, &dnew.keys, "dict_resize(keys)");
	ref_assign_old(pdref, &pdict->values, &dnew.values, "dict_resize(values)");
