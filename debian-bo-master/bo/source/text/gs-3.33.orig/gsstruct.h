...
#define struct_proc_reloc_ptrs(proc)\
  void proc(P3(void /*obj_header_t*/ *ptr, uint size, gc_state_t *gcst))
	struct_proc_reloc_ptrs((*reloc_ptrs));
		 ...
		 * Finalization procedures must not allocate or free
		 * any objects in any space managed by the allocator,
		 * and they cannot assume that any objects in such spaces
		 * referenced by this structure still exist.  However,
		 * finalization procedures may allocate, free, and
		 * reference objects allocated in other ways, such as
		 * objects allocated with malloc by libraries.
		 */
...
#define gs__st_ptr(scope_st, stname, stype, sname, penum, preloc)\
  ...
  ENUM_PTRS_END\
  private RELOC_PTRS_BEGIN(preloc) ;\
    *(stype *)vptr = gs_reloc_struct_ptr((void *)*(stype *)vptr, gcst); /* discard const */\
  RELOC_PTRS_END\
  gs__st_composite_only(scope_st, stname, stype, sname, penum, preloc)
...
#define RELOC_PTR(typ, elt)\
  ((typ *)vptr)->elt =\
    gs_reloc_struct_ptr((void *)((typ *)vptr)->elt, gcst) /* discard const */
...
/*
 * Boilerplate for suffix subclasses.
 */
...
#define RELOC_PREFIX(supst)\
  (*supst.reloc_ptrs)(vptr,size,gcst)
	...
	/* Note that unlike other subclass declarations, */
	/* these name the superclass's enum and reloc procedures, */
	/* rather than the superclass's descriptor. */

#define gs__st_suffix_add0(scope_st, stname, stype, sname, supenum, supreloc)\
  gs__st_composite_only(scope_st, stname, stype, sname, supenum, supreloc)
#define gs_public_st_suffix_add0(stname, stype, sname, supenum, supreloc)\
  gs__st_suffix_add0(public_st, stname, stype, sname, supenum, supreloc)
#define gs_private_st_suffix_add0(stname, stype, sname, supenum, supreloc)\
  gs__st_suffix_add0(private_st, stname, stype, sname, supenum, supreloc)
...
#define gs__st_ptrs_add0(scope_st, stname, stype, sname, penum, reloc, supstname, member)\
