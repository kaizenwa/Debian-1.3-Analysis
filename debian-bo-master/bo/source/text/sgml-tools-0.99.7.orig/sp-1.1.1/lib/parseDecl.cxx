				     ...
				     Boolean allowAfdr)
{
  ...
  if (!syntax().lookupReservedName(name, result)) {
    if (allowAfdr && name == sd().execToDoc("AFDR")) {
      *result = Syntax::rANY;
      if (currentMarkup())
...
Boolean Parser::implyDtd(const StringC &gi)
{
  ...
  StringC str;
  StringC name;
  if (!entityCatalog().lookup(*entity, syntax(), sd().docCharset(),
			      messenger(), str)) {
    if (!entityCatalog().defaultDoctype(sd().docCharset(),
					messenger(),
					name,
					str))
      return 0;
...
Boolean Parser::parseAfdrDecl()
{
  ...
  if (!parseParam(allowMinimumLiteral, declInputLevel, parm))
    return 0;
  if (parm.literalText.string() != sd().execToDoc("ISO/IEC 10744:1992"))
