...
struct document *
psscan(fileP,filename,scanfilenameP,cmd_scan_pdf)
    FILE **fileP;
    char *filename;
    char **scanfilenameP;
    char *cmd_scan_pdf;
{
	...
	section_len = line_len;
        ...
        char *scanfilename;
        char cmd[512];
        scanfilename=file_getTmpFilename(NULL,filename);
        sprintf(cmd,cmd_scan_pdf,filename,scanfilename);
        INFSMESSAGE(scan command,cmd)
#ifdef VMS
 	if ((system(cmd)&7)==1 && !file_fileIsNotUseful(scanfilename)) {
#else
 	if (!system(cmd) && !file_fileIsNotUseful(scanfilename)) {
#endif
           FILE *tmpfile = fopen(scanfilename, "r");
 	   if (tmpfile) {
 	      struct document *retval;
              INFMESSAGE(is PDF)
              fclose(*fileP);
 	      *fileP = tmpfile;
 	      retval = psscan(fileP,scanfilename,scanfilenameP,cmd_scan_pdf);
              if (retval) {
                 *scanfilenameP = (char*)PS_XtNewString(scanfilename);
              } else {
                 char s[512];
                 sprintf(s,"Scanning\n%s\nfailed.",scanfilename);
              ...
              ps_io_exit(fd);
              GV_XtFree(scanfilename);
   	      return retval;
 	   } else {
              char s[512];
              sprintf(s,"Execution of\n%s\nfailed.",cmd);
              NotePopupShowMessage(s);
              INFMESSAGE(cannot open scanfile) ENDMESSAGE(psscan)
              fclose(tmpfile);
              unlink(scanfilename);
              GV_XtFree(scanfilename);
              ps_io_exit(fd);
           ...
           unlink(scanfilename);
           GV_XtFree(scanfilename);
           INFMESSAGE(scan subprocess failed) ENDMESSAGE(psscan)
           ps_io_exit(fd);
 	   return NULL;
        }
    } else {
	...
	if (line[0] != '%' ||
	    ...
	    doc->media = (struct documentmedia *)
			 PS_malloc(sizeof (struct documentmedia));
...
#define LINE_CHUNK_SIZE     512
#define MAX_PS_IO_FGETCHARS_BUF_SIZE 131072
	 ...
	 if (FD_BUF_END >= FD_LINE_BEGIN+num) {
      ...
      while (FD_BUF_SIZE < FD_BUF_END+LINE_CHUNK_SIZE+1) {
         if (FD_BUF_SIZE > MAX_PS_IO_FGETCHARS_BUF_SIZE) {
            INFMESSAGE(buffer became too large)
            ENDMESSAGE(ps_io_fgetchars)
            FD_STATUS      = FD_STATUS_BUFTOOLARGE;
            return(NULL);
         }
         if (FD_LINE_BEGIN) {
            INFMESSAGE(moving line to begin of buffer)
            memmove((void*)FD_BUF,(void*)(FD_BUF+FD_LINE_BEGIN),
