...
#define offset(field) XtOffsetOf(GhostviewRec, ghostview.field)
    ...
    {XtNautoSetup,XtCBoolean, XtRBoolean, sizeof(Boolean),
       offset(auto_setup), XtRImmediate, (XtPointer)True},
    ...
    Cardinal *num_params;
{
    ...
    IIMESSAGE1(event->xbutton.x,event->xbutton.y)
    ...
    XSetWindowAttributes *attributes;
{
	...
	*valueMask |= CWCursor;
    ...
    if (gvw->ghostview.auto_setup) Setup(w);
    ...
    Widget w;
{
   ...
   BEGINMESSAGE(Resize)
   if (gvw->ghostview.auto_setup) {
      Layout(w, False, False);
      if (!XtIsRealized(w)) {INFMESSAGE(not realized) ENDMESSAGE(Resize) return;}
      INFIIMESSAGE(resizing to:, w->core.width,w->core.height)
      if (Setup(w)) {
    ...
    Dimension *width, *height;
{
	...
	case XtPageOrientationUpsideDown:
	    new_width = (Dimension)(
                ((float)(gvw->ghostview.urx-gvw->ghostview.llx+1)
                *gvw->ghostview.xdpi)/72.0+0.5
            );
	    new_height = (Dimension)(
                ((float)(gvw->ghostview.ury-gvw->ghostview.lly+1)
                *gvw->ghostview.ydpi)/72.0+0.5
            );
	    break;
	case XtPageOrientationLandscape:
	case XtPageOrientationSeascape:
	    ...
	    break;
	case XtPageOrientationUnspecified:
	  /* should not be reached */
            break;
	}
    } else if (xfree) {
	...
	case XtPageOrientationUpsideDown:
	    newydpi = gvw->core.height * 72.0 /
		      (gvw->ghostview.ury - gvw->ghostview.lly+1);
	...
	case XtPageOrientationSeascape:
	    newydpi = gvw->core.height * 72.0 /
		      (gvw->ghostview.urx - gvw->ghostview.llx+1);
	    newxdpi = newydpi * gvw->ghostview.xdpi / gvw->ghostview.ydpi;
	    gvw->ghostview.xdpi = newxdpi;
	    gvw->ghostview.ydpi = newydpi;
	    new_width = (Dimension)(
	...
	case XtPageOrientationUnspecified:
	  /* should not be reached */
        ...
        INFMESSAGE(x and y are not free)
   ...
   Widget w;
{
  ...
  /* Be sure to update properties */
   ...
   if (gvw->ghostview.auto_setup) StartInterpreter(w);
...
#if defined(SYSV) || defined(USG) || defined(linux) /* for linux ###jp###*/
#define vfork fork
#endif
...
#endif
{
   ...
   switch (gvw->ghostview.orientation) {
      case XtPageOrientationPortrait:
         p->psx    = gvw->ghostview.llx + (locx * 72.0)/gvw->ghostview.xdpi;
	 p->psy    = gvw->ghostview.ury - (locy * 72.0)/gvw->ghostview.ydpi;
         p->width  = ((abs(x2-x1)* 72.0)/gvw->ghostview.xdpi);
         p->height = ((abs(y2-y1)* 72.0)/gvw->ghostview.ydpi);
   	 break;
      case XtPageOrientationLandscape:
	 p->psx    = gvw->ghostview.llx + (locy * 72.0)/gvw->ghostview.ydpi;
	 p->psy    = gvw->ghostview.lly + (locx * 72.0)/gvw->ghostview.xdpi;
         p->width  = ((abs(y2-y1)* 72.0)/gvw->ghostview.ydpi);
         p->height = ((abs(x2-x1)* 72.0)/gvw->ghostview.xdpi);
	 break;
      case XtPageOrientationUpsideDown:
	 p->psx    = gvw->ghostview.urx - (locx * 72.0)/gvw->ghostview.xdpi;
	 p->psy    = gvw->ghostview.lly + (locy * 72.0)/gvw->ghostview.ydpi;
         p->width  = ((abs(x2-x1)* 72.0)/gvw->ghostview.xdpi);
         p->height = ((abs(y2-y1)* 72.0)/gvw->ghostview.ydpi);
	 break;
      case XtPageOrientationSeascape:
	 p->psx    = gvw->ghostview.urx - (locy * 72.0)/gvw->ghostview.ydpi;
	 p->psy    = gvw->ghostview.ury - (locx * 72.0)/gvw->ghostview.xdpi;
         p->width  = ((abs(y2-y1)* 72.0)/gvw->ghostview.ydpi);
         p->height = ((abs(x2-x1)* 72.0)/gvw->ghostview.xdpi);
	 break;
      case XtPageOrientationUnspecified:
   ...
   FMESSAGE(p->xdpi)FMESSAGE(p->ydpi)

   ENDMESSAGE(GhostviewGetBBofArea) 
}

/*###################################################################################*/
/* GhostviewGetAreaofArea */
/*###################################################################################*/
...
#endif
{
   ...
   switch (gvw->ghostview.orientation) {
      case XtPageOrientationPortrait:
         x1 = ((  psx1 - gvw->ghostview.llx)*gvw->ghostview.xdpi/72.0);
         y1 = (( -psy1 + gvw->ghostview.ury)*gvw->ghostview.ydpi/72.0);
         x2 = ((  psx2 - gvw->ghostview.llx)*gvw->ghostview.xdpi/72.0);
         y2 = (( -psy2 + gvw->ghostview.ury)*gvw->ghostview.ydpi/72.0);
   	 break;
      case XtPageOrientationLandscape:
	 y1 = ((  psx1 - gvw->ghostview.llx)*gvw->ghostview.ydpi/72.0);
	 x1 = ((  psy1 - gvw->ghostview.lly)*gvw->ghostview.xdpi/72.0);
	 y2 = ((  psx2 - gvw->ghostview.llx)*gvw->ghostview.ydpi/72.0);
	 x2 = ((  psy2 - gvw->ghostview.lly)*gvw->ghostview.xdpi/72.0);
	 break;
      case XtPageOrientationUpsideDown:
	 x1 = (( -psx1 + gvw->ghostview.urx)*gvw->ghostview.xdpi/72.0);
	 y1 = ((  psy1 - gvw->ghostview.lly)*gvw->ghostview.ydpi/72.0);
	 x2 = (( -psx2 + gvw->ghostview.urx)*gvw->ghostview.xdpi/72.0);
	 y2 = ((  psy2 - gvw->ghostview.lly)*gvw->ghostview.ydpi/72.0);
	 break;
      case XtPageOrientationSeascape:
	 y1 = (( -psx1 + gvw->ghostview.urx)*gvw->ghostview.ydpi/72.0);
	 x1 = (( -psy1 + gvw->ghostview.ury)*gvw->ghostview.xdpi/72.0);
	 y2 = (( -psx2 + gvw->ghostview.urx)*gvw->ghostview.ydpi/72.0);
	 x2 = (( -psy2 + gvw->ghostview.ury)*gvw->ghostview.xdpi/72.0);
	 break;
      case XtPageOrientationUnspecified:
	/* never reached */
         break;
