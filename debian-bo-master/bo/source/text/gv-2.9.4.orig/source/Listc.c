...
WidgetClass listcWidgetClass = (WidgetClass)&listcClassRec;
...
Cardinal *num_params;
{
  ...
  if ( (CvtToItem(w, event->xbutton.x, event->xbutton.y, &item))
       == OUT_OF_RANGE)
    XawListcUnhighlight(w);		        /* Unhighlight current item. */
  else if ( lw->list.is_highlighted != item )   /* If this item is not */
    XawListcHighlight(w, item);	                /* highlighted then do it. */
}
...
XawListcChange(Widget w, char ** list, int nitems, int longest,
...
#endif
#else
XawListcChange(w, list, nitems, longest, resize_it)
Widget w;
char ** list;
int nitems, longest;
Boolean resize_it;
#endif
{
    ...
    ENDMESSAGE(XawListcChange)
}

/*	Function Name: XawListcUnhighlight
...
/*	Function Name: XawListcHighlight
...
void
#if NeedFunctionPrototypes
XawListcHighlight(Widget w, int item)
#else
XawListcHighlight(w, item)
Widget w;
int item;
#endif
{
...
/*	Function Name: XawListcShowCurrent
...
XawListReturnStruct *
#if NeedFunctionPrototypes
XawListcShowCurrent(Widget w)
#else
XawListcShowCurrent(w)
Widget w;
#endif
{
    ListWidget lw = ( ListWidget ) w;
