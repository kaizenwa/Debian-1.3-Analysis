...
set_status_line("(Jed %v) Brief: %b    (%m%a%n%o)  %p,%c   %t", 1);
...
define brief_search_cmd ()
{
   ...
   if (Brief_Search_Forward)
     {
	if (Brief_Regexp_Search) search_fun = &re_search_forward;
	else search_fun = &search_forward;
     }
   else
     {
	if (Brief_Regexp_Search) search_fun = &re_search_backward;
	else search_fun = &search_backward;
     }
   search_fun ();
}
