...
define keycode ()
{
   variable ch, key = Null_String, timeout;
#ifdef XWINDOWS
   variable fmt = "Press any key (RETURN to exit): `%s' (keysym 0x%X)";
#else
   variable fmt = "Press any key (RETURN to exit): `%s'";
#endif
...
#ifdef XWINDOWS
	flush (Sprintf (fmt, key, X_LAST_KEYSYM, 2));
#else
	flush (Sprintf (fmt, key, 1));
#endif
	...
	while (input_pending (timeout))
	  {
		...
		case '\r':
		  ...
		  return;
	       ...
	       { pop ();
		  if (ch > 127) strcat ("\\d", string (ch)); else char (ch);
	       }
	     ch = ();
	     key = strcat (key, ch);
	  }
