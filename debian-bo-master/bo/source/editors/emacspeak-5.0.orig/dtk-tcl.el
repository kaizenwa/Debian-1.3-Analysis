;;; $Id: dtk-tcl.el,v 5.0 1996/11/22 18:04:26 raman Exp $
;;; $Author: raman $ 
...
;;; various speech drivers written in tcl
;;; This module is TCL and Dectalk specific. 
...
See command dtk-toggle-stop-immediately-while-typing bound to \\[dtk-toggle-stop-immediately-while-typing]. ")
...
(defconst dtk-bracket-regexp "[][{}<>|]"
  "Brackets that are special to dtk and tcl.
 {} is special to tcl.
...
(defsubst  dtk-move-across-a-chunk (separator complement )
  ...
  (let ((done nil )
        (orig (point ))
        (moved (skip-syntax-forward complement)))
    (while (and  (not done )
                 (not (eobp )))
      (skip-syntax-forward separator)
      (cond
       ...
       ((zerop moved) (goto-char orig )
        (setq done t))
       ;; Success if we found non-punctuation delimiter
       ((not (= (char-syntax (preceding-char ))
                46;; "."
                ))
        (setq done t ))
                                        ; if  sep is punct ,   skip   ws and succeed 
       ((or (eolp)
...
(defsubst dtk-force ()
  ...
  (process-send-string dtk-speaker-process
                       "dectalk_speak {} \n"))
                                        ...
                                        ;Write out the string to the dectalk via TCLSH.
...
has been completed.
If outline-minor-mode is on and selective display is in effect,
only speak upto the first ctrl-m. "
  ...
  (unless  dtk-quiet
    ...
    (let ((ctrl-m nil ))
      (when (and (boundp  'selective-display )
                 selective-display 
                 (setq ctrl-m (string-match "\015" text )))
                 (emacspeak-auditory-icon 'ellipses)
        (setq text (substring  text 0 ctrl-m ))))
    (let ((syntax-table (syntax-table ))
          (pronunciation-table (and (boundp 'emacspeak-pronounce-pronunciation-table)
    ...
    (when  wait
      (accept-process-output dtk-speaker-process 0 1))
    (dtk-force)
    (when wait 
