...
int ParseSearchOptions(int replace, char *str, unsigned long &Options) {
    char *p = str;
...
int ParseSearchReplace(EBuffer *B, char *str, int replace, SearchReplaceOptions &opt) {
    int where = 0;
    int len = 0;
    char *p = str;
...
int EBuffer::FindStr(char *Data, int Len, int Options) {
    int OL = CP.Row, OC = CP.Col, LLen, Start, End;
    int C, L;
    PELine X;
    char *P;
    ...
    while (1) {
        ...
        while (((!(Options & SEARCH_BACK)) && (C <= End - Len)) || ((Options & SEARCH_BACK) && (C >= Start))) {
            if (((!(Options & SEARCH_WORD) ||
                 (((C == 0) || (WGETBIT(Flags.WordChars, P[C - 1]) == 0)) &&
                  ((C + Len >= End) || (WGETBIT(Flags.WordChars, P[C + Len]) == 0))))
                &&
...
int EBuffer::Find(SearchReplaceOptions &opt) {
    ...
    if (Options & SEARCH_GLOBAL) {
        if (Options & SEARCH_BLOCK) {
            ...
            if (Options & SEARCH_BACK) {
                ...
                if (SetPosR(LineLen(RCount - 1), RCount) == 0) goto error;
            } else {
    ...
    if (Options & SEARCH_ALL)
        Msg(INFO, "%d match(es) found.", opt.resCount);
    else {
        if (opt.resCount == 0) {
            Msg(INFO, "[%s] not found", opt.strSearch);
            return 0;
        }
    }
    return 1;
...
again:
        if (FindStr(L->Chars + P, xlen, SEARCH_BACK | SEARCH_NEXT) == 1) {
            M = RLine(Match.Row);
            ...
            if (SetPos(O.Col, O.Row) == 0) return 0;
            {
...
int EBuffer::SearchAgain(ExState &State, int Options) {
...
int EBuffer::ScanForRoutines() {
	...
	L = RLine(line);
	    ...
	    Msg(BUSY, "Routines: %d", rlst.Count);
	}
    }
    RxFree(regx);
    return 1;
...
int EBuffer::GetMatchBrace(EPoint &M, int MinLine, int MaxLine, int show) {
    ...
    if (show) 
        Msg(INFO, "No match (%d missing).", CountX);
    return 0;
}
...
int EBuffer::SearchWord(int SearchFlags) {
    ...
    while ((P > 0) && ((ChClass(L->Chars[P - 1]) == 1) || (L->Chars[P - 1] == '_'))) 
        P--;
    while (len < sizeof(word) && P < L->Count && (ChClass(L->Chars[P]) == 1 || L->Chars[P] == '_'))
        word[len++] = L->Chars[P++];
    word[len] = 0;
    if (len == 0)
        return 0;
...
int EBuffer::FindTagWord() {
    ...
    word[len] = 0;
    if (len == 0) {
        Msg(INFO, "No word at cursor.");
        return 0;
    }

    if (TagFind(this, View, word) == 0) {
        Msg(INFO, "Tag '%s' not found.", word);
        return 0;
    } else
        return 1;
}
