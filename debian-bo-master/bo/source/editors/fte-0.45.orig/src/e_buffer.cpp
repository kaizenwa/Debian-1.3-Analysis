...
EBuffer::EBuffer(EModel **ARoot, char *AName)
    :EModel(ARoot), TP(0,0), CP(0,0), BB(-1,-1), BE(-1,-1),
...
int EBuffer::SetFileName(char *AFileName, char *AMode) {
    FileOk = 0;
...
/*    if (AllocVis(VCount + 1) == 0) return 0;
    memmove(VV + VLine + 1, VV + VLine, sizeof(int) * (VCount - VLine));
    VCount++;
    Vis(VLine, Row - VLine);*/
    return 1;
}
