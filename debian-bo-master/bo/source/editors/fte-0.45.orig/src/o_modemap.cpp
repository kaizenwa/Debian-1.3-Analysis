...
void EventMapView::AddLine(char *Line) {
    if (BList) {
        BCount++;
...
void EventMapView::DumpKey(char *aPrefix, EKey *Key) {
    char KeyName[128] = "";
    char Entry[2048] = "";
    char *p;
    int id;
    ...
    for (int i = 0; i < Macros[id].Count; i++) {
        ...
        if (Macros[id].cmds[i].type == CT_COMMAND) {
            ...
            sprintf(p, "%d ", Macros[id].cmds[i].u.num);
...
void EventMapView::DumpMap(char *aPrefix, EKeyMap *aKeyMap) {
    EKey *Key;
...
EventMapView::EventMapView(EModel **ARoot, EEventMap *Map): EList(ARoot, "Event Map") {
    BCount = 0;
    BList = 0;
