...
ExInput::ExInput(char *APrompt, char *ALine, unsigned int ABufLen, Completer AComp, int Select, int AHistId): ExView() {
    assert(ABufLen > 0);
    MaxLen = ABufLen - 1;
    Comp = AComp;
    SelStart = SelEnd = 0;
...
//                SelStart = SelEnd = 0;
        ...
        case ('Q') | kfCtrl:
            ...
            if (keyType(Event.Key.Code) == kfCtrl)
                Event.Key.Code &= ~kfCtrl;
        default:
            {
                char Ch;
...
void ExInput::RepaintStatus() {
    ...
    if (Pos > strlen(Line)) 
        Pos = strlen(Line);
    //if (Pos < 0) Pos = 0;
    if (LPos + FLen <= Pos) LPos = Pos - FLen;
