...
static RxNode *NewEscape(char **Regexp) {
    char Ch = **Regexp;
    ++*Regexp;
    switch (Ch) {
      case 0: return 0;
...
#define SETOP(set,n) \
    do { \
      set[n >> 3] |= (unsigned char)(1 << (n & 7)); \
    } while (0)

static RxNode *NewSet(char **Regexp) {
...
static RxNode *RxComp(char **Regexp) {
    RxNode *F = 0;
    RxNode *N = 0;
    int C;
    char Ch;
...
//	RxDump(1, n);
        switch (n->fWhat) {
          ...
          case RE_INSET:
            if (rex == eop) return 0;
            if ((n->fChar[(*rex) >> 3] & (1 << ((*rex) & 7))) == 0) return 0;
            rex++;
            break;
          case RE_NOTINSET:
            if (rex == eop) return 0;
            if (n->fChar[(*rex) >> 3] & (1 << ((*rex) & 7))) return 0;
            rex++;
            break;
          case RE_JUMP:
            n = n->fPtr;
...
int RxExec(RxNode *Regexp, char *Data, int Len, char *Start, RxMatchRes *Match, unsigned int RxOpt) {
    char Ch;
    if (Regexp == 0) return 0;
    ...
    case RE_CHAR:    // search for a character to match
        ...
        if (Start == eop) 
            break;
        while ((Start = (char *) memchr(Start, Ch, (int) (eop - Start))) != 0)
        {
...
static int add(int *len, char **s, char *a, int alen, int &flag) {
    int NewLen = *len + alen;
    int i;
...
int RxReplace(char *rep, char *Src, int len, RxMatchRes match, char **Dest, int *Dlen) {
    int dlen = 0;
    char *dest = 0;
    char Ch;
    int n;
