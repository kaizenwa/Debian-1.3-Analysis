...
int EBuffer::FindFold(int Line) { // optimize
...
int EBuffer::FindNearFold(int Line) {
...
void EBuffer::UpdateVis(EPoint &M, int Row, int Delta) {
    if (Delta < 0) {
        if (M.Row > Row)
            if (M.Row < Row - Delta)
                M.Row = Row;
...
void EBuffer::UpdateVisible(int Row, int Delta) {
    EView *w;
...
int EBuffer::FoldCreate(int Line) {
    int n;
...
int EBuffer::FoldCreateByRegexp(char *Regexp) {
    RxNode *R;
    RxMatchRes RM;
    int L;
    PELine X;
    ...
    for (L = 0; L < RCount; L++) {
        ...
        if (RxExec(R, X->Chars, X->Count, X->Chars, &RM) == 1) 
            if (FoldCreate(L) == 0) return 0;
    }
    RxFree(R);
    return 1;
}

int EBuffer::FoldDestroy(int Line) {
    int f = FindFold(Line);
...
int EBuffer::FoldPromote(int Line) {
    int f = FindFold(Line);
...
int EBuffer::FoldDemote(int Line) {
    int f = FindFold(Line);
...
int EBuffer::FoldClose(int Line) {
    int f = FindNearFold(Line);
    int l, top;
    int level;
...
int EBuffer::FoldToggleOpenClose() {
    int Line = VToR(CP.Row);
    int f;
