...
int AddCRegexp(int file, int line, int msg, char *regexp) {
...
EMessages::EMessages(EModel **ARoot, char *ADir, char *ACommand): EList(ARoot, "Messages") {
    CompilerMsgs = this;
    ErrCount = 0;
    ErrList = 0;
    Running = 0;
...
void EMessages::AddError(char *file, int line, char *msg, char *text) {
    Error *pe;
...
int EMessages::CompilePrevError(EView *V) {
    if (ErrCount > 0) {
        ...
        if (Row > 0) {
            ...
            V->Msg(INFO, "No previous error.");
            return 0;
        }
    } else {
        V->Msg(INFO, "No errors.");
        return 0;
    }
    return 1;
}
...
int EMessages::CompileNextError(EView *V) {
    if (ErrCount > 0) {
        ...
        if (Row < ErrCount - 1) {
            ...
            if (Running)
                V->Msg(INFO, "No more errors (yet).");
            else
                V->Msg(INFO, "No more errors.");
            return 0;
        }
    } else { 
        if (Running) 
            V->Msg(INFO, "No errors (yet).");
        else
            V->Msg(INFO, "No errors.");
        return 0;
    }
...
void EMessages::DrawLine(PCell B, int Line, int Col, ChColor color, int Width) {
    if (Line < ErrCount)
        if (Col < strlen(ErrList[Line]->text)) {
            char str[1024];
            int len;
