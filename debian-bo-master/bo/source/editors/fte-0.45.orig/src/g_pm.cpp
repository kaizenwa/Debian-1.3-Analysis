...
void InsertHistory(HWND hwnd, int id, int maxlen) {
    ...
    for (i = 0; i < count; i++) {
        if (GetInputHistory(id, str, maxlen, i) == 1)
...
MRESULT EXPENTRY FileDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
    ...
    case WM_INITDLG:
        ...
        WinRestoreWindowPos("FTEPM", "FileDlg", hwnd);
        break;
    case WM_COMMAND:
        switch (SHORT1FROMMP(mp1)) {
        case DID_OK:
            WinShowWindow(hwnd, FALSE);
            WinStoreWindowPos("FTEPM", "FileDlg", hwnd);
            break;
...
int DLGGetFile(GView *View, char *Prompt, unsigned int BufLen, char *FileName, int Flags) {
    FILEDLG dlg;
...
int DLGPickChoice(GView *View, char *ATitle, int NSel, va_list ap, int Flags) {
    ChoiceInfo choice;
...
int GViewPeer::PMSetCursorPos() {
    if (wState & sfFocus) {
        ...
        WinCreateCursor(hwndView,
                        pmData->cxChar * cX, pmData->cyChar * (wH - cY - 1), pmData->cxChar, 2,
                        CURSOR_TYPE,
                        NULL);
        WinShowCursor(hwndView, TRUE);
    }
