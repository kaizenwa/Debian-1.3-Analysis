...
int EBuffer::LineNew() {
    ...
    if (!MoveLineStart()) return 0;
    if (!LineIndent()) return 0;
    if (TrimLine(VToR(CP.Row - 1)) == 0) return 0;
    return 1;
}
...
int EBuffer::LineIndent() {
    if (BFI(this, BFI_AutoIndent)) {
        ...
        case INDENT_C: return Indent_C(this, L, 1);
#endif
#ifdef CONFIG_INDENT_REXX
        case INDENT_REXX: return Indent_REXX(this, L, 1);
#endif
#ifdef CONFIG_INDENT_SIMPLE
        case INDENT_SIMPLE: return Indent_SIMPLE(this, L, 1);
#endif
        default: return Indent_Plain(this, L, 1);
        }
    }
    if (BFI(this, BFI_Trim))
        if (TrimLine(VToR(CP.Row)) == 0) return 0;
...
int EBuffer::SetLeftMargin() {
    ...
    Msg(INFO, "LeftMargin set to %d.", BFI(this, BFI_LeftMargin) + 1);
    return 1;
}

int EBuffer::SetRightMargin() {
    BFI(this, BFI_RightMargin) = CP.Col;
    Msg(INFO, "RightMargin set to %d.", BFI(this, BFI_RightMargin) + 1);
    return 1;
}
...
int EBuffer::ChangeMode(char *AMode) {
    ...
    Msg(ERROR, "Mode '%s' not found.", AMode);
    return 0;
}
