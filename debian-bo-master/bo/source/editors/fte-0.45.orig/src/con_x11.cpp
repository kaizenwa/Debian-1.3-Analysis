...
static void SetColor(int i) {
    assert (0 <= i && i <= 15);
    Colors[i].blue  = dcolors[i].b << 8;
    Colors[i].green = dcolors[i].g << 8;
    Colors[i].red   = dcolors[i].r << 8;
...
static int SetupXWindow() {
    ...
    selection_buffer = XInternAtom(display, "fte_clip", False);
    ...
    sizeHints.min_width = 20 * FontCX;
    sizeHints.min_height = 6 * FontCY;
    classHints.res_name = "fte";
    classHints.res_class = "Fte";
    XSetClassHint(display, win, &classHints);
    XSetStandardProperties(display, win, winTitle, winTitle, 0, NULL, 0, &sizeHints);
...
int ConDone(void) {
    ...
    return (XCloseDisplay(display) == 0);
}
...
int ConSetTitle(char *Title, char *STitle) {
    strncpy(winTitle, Title, sizeof(winTitle) - 1);
...
int ConGetTitle(char *Title, int MaxLen, char *STitle, int SMaxLen) {
    ...
    Title[MaxLen - 1] = 0;
    strncpy(Title, winSTitle, SMaxLen);
    Title[SMaxLen - 1] = 0;
    return 0;
}
...
void DrawCursor(int Show) {
	...
	rc = XDrawImageString(display, win, GCs[attr & 0xFF],
			      CursorX * FontCX,
...
int ConPutBox(int X, int Y, int W, int H, PCell Cell) {
	...
	while (len > 0) {
	    ...
	    rc = XDrawImageString(display, win, GCs[attr & 0xFF],
				  x * FontCX,
				  fontStruct->max_bounds.ascent + (Y + i) * FontCY,
				  (char *)temp, l);
	    x += l;
	    len -= l;
	}
...
int ConSetSize(int X, int Y) {
	...
	*p++ = 0x07;
    ...
    MX = ScreenCols; if (X < MX) MX = X;
    MY = ScreenRows; if (Y < MY) MY = Y;
    if (X < MX) MX = X;
    p = NewBuffer;
    for (i = 0; i < MY; i++) {
...
void UpdateWindow(int xx, int yy, int ww, int hh) {
    ...
    xx /= FontCX;
    yy /= FontCY;
    if (xx + ww > ScreenCols) ww = ScreenCols - xx;
    if (yy + hh > ScreenRows) hh = ScreenRows - yy;
    Refresh = 1;
//    frames->Repaint();
//    frames->Update();
...
void ResizeWindow(int ww, int hh) {
    ...
    if (ScreenCols != ww || ScreenRows != hh) {
        ...
        ConSetSize(ww, hh);
        Refresh = 1;
        UpdateWindow(0, 0, ScreenCols * FontCX, ScreenRows * FontCY);
        Refresh = 0;
    }
}
...
void ConvertKeyToEvent(KeySym key, KeySym key1, char *keyname, char *keyname1, int etype, int state, TEvent *Event) {
    ...
    if (key < 256 || (key1 < 256 && (myState == kfAlt || myState == (kfAlt | kfShift)))) {
        if (myState == kfAlt || myState == (kfAlt | kfShift)) //?
            key = key1;
	...
	*Event = Pending;
    ...
    while (Event->What == evNone) {
        ...
        if ((WaitTime == -1 || WaitTime > MouseAutoDelay) && (LastMouseEvent.What == evMouseAuto) && (EventMask & evMouse)) {
            ...
            rc = select(sizeof(fd_set) * 8, FD_SET_CAST() &read_fds, NULL, NULL, &timeout);
            if (rc == 0) {
                   ...
                   (LastMouseEvent.Mouse.Buttons) && (EventMask & evMouse))
        {
            ...
            rc = select(sizeof(fd_set) * sizeof(char), FD_SET_CAST() &read_fds, NULL, NULL, NULL);
        } else {
        /*	    if (WaitTime > 1000) WaitTime = 1000;*/
            timeout.tv_sec = 0;
            timeout.tv_usec = WaitTime * 1000 + 1;
            rc = select(sizeof(fd_set) * sizeof(char), FD_SET_CAST() &read_fds, NULL, NULL, &timeout);
        }
