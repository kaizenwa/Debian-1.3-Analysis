...
variable Mode_List_Exts = "c,h,cc,cpp,hpp,sl,txt,doc,tex,f,for,pro,idl,1,pl,v,verilog,vhd,vhdl,vt,sp,cir,spice,ps,py";
variable Mode_List_Modes = "c,c,c,c,c,slang,text,text,tex,fortran,fortran,idl,idl,nroff,perl,verilog,verilog,vhdl,vhdl,vhdl,spice,spice,spice,ps,python";
...
Mode_List_Exts = strcat (Mode_List_Exts, ",letter,article,followup");
Mode_List_Modes = strcat (Mode_List_Modes, ",text,text,text");
#endif
...
define command_line_hook () %{{{
{
   ...
   !if (strcmp(command_line_arg(i), "-info"))
     {
	info_reader (i + 1);
	return;
     }
...
#endif
	  {
	     tmp = str_replace_all (substr ((), 3, -1), "-", "_");
	     (not (strncmp (file, "--", 2)) 
	      and is_defined (tmp))
	       :
	     eval (tmp);
