...
!if (is_defined("Rmail_Spool_Mailbox_File"))
{
   ...
   !if (strlen (Rmail_Spool_Mailbox_File))
     {
...
%if (2 != file_status (Rmail_Spool_Mailbox_File))
%{
%   error (strcat ("Mail spool directory does not exist: ", Rmail_Spool_Mailbox_File));
%}
%$1 = getenv("MAIL"); if (strlen($1)) Rmail_Spool_Mailbox_File = $1;
...
define rmail_get_header (h, continue_flag)
{
   bob();
   !if (strcmp(h, "Date: "))
     {
	...
	if (re_fsearch("^Date:[ \t]+[a-zA-Z]+ \\([a-zA-Z]+\\) +0?\\([1-3]?\\d\\)[ ,]+"))
	  return Sprintf("%s-%s", regexp_nth_match(2), regexp_nth_match(1), 2);
	return Null_String;
     }
...
define rmail_extract_headers ()
{
   ...
   from = rmail_get_header("From: ", 0);
   !if (strlen(from)) from = rmail_get_header("From ", 0);
   
   date = rmail_get_header("Date: ", 0);
   subject = rmail_get_header("Subject: ", 0);
...
define rmail_parse_mime ()
{
   ...
   h = rmail_get_header ("Content-Transfer-Encoding: ", 1);
...
define rmail_process_newmail (mbox)
{
   ...
   forever
     {
	push_mark_eol ();
	if (re_fsearch(from)) go_up_1 (); else eob();
	narrow();
	++n;
	flush (Sprintf ("Prcoessing %d messages...", n, 1));
...
define rmail_get_newmail_from_file (file, tmp_mbox)
{
   variable n;
   variable nomail, cmd;
   variable getmail = dircat(Jed_Bin_Dir, "getmail");
   
   nomail = Sprintf ("No new mail in %s", file, 1);
   ...
   if (file_status(getmail) != 1)
     {
	error (strcat (getmail, " not found!"));
     }
   
   cmd = Sprintf("%s %s %s", getmail, file, tmp_mbox, 3);
   push_mark(); 
   if (pipe_region(cmd))
     error ("getmail returned error.");
   
   n = rmail_process_newmail (tmp_mbox);
...
{
   rmail_get_newmail_from_file (Rmail_Spool_Mailbox_File, Rmail_Tmp_Mbox);
}
...
define rmail_skip_quotes ()
{
   ...
   if (1 == rmail_get_message (0))
	...
	if (re_fsearch (str))
	  {
	     while (orelse
		     { push_mark (); narrow (); bol ();
			re_fsearch (str); widen (); }
		     { skip_white (); eolp ();} )
	       {
		  !if (down_1 ()) break;
	       }
...
define rmail_forward_message ()
{
   ...
   if (bol_fsearch ("Subject: "))
	...
	push_mark ();
	eol ();
	subj = Sprintf ("Fwd: [%s]", bufsubstr (), 1);
     }
   pop_spot ();
...
define str_remove_spaces (f)
{
   strcompress (str_replace_all (f, " ", ","), ",");
}
...
define rmail_complex_get_from (from)
{
   ...
   %  This does not deal with nested (()) comments.  I have seen one of these.
  
   f = strtrim(str_uncomment_string (regexp_nth_match (1), "\"'(", "\"')"));
   
   f = str_remove_spaces (f);
   
   if (strlen(f))
     {
	% add to list, see if any part of it is in the list.
	n = 0;
	while (sub_f = extract_element (f, n, ','), strlen (sub_f))
	  {
	     n++;
	     !if (is_list_element(Rmail_Dont_Reply_To, strlow(sub_f), ','))
	       {
		  if (strlen (from))
		      from = strncat (from, ",",  sub_f, 3);
		  else from = sub_f;
	       }
	  }
     }
...
define rmail_reply ()
{
   ...
   variable subj = Null_String, to = Null_String, cc = Null_String;
   variable cbuf = whatbuf ();
   ...
   while (down_1 () and not (looking_at_char ('-')));
...
define rmail_edit ()
{
   ...
   if (1 != file_status (file))
     verror ("Unable to read %s", file, 1);
...
define rmail_folder_newmail ()
{
   ...
   n = rmail_get_newmail_from_file (file, Rmail_Tmp_Mbox);
...
define rmail_build_folder_list ()
{
   ...
   while (bol_fsearch("Folder:\t"))
	...
	push_mark ();
	() = ffind ("\t"); 
	slist = strncat (slist, ",", bufsubstr (), 3);
     }
   pop_spot ();
   setbuf (cbuf);
   slist;
...
!if (keymap_p($1))
{
   ...
   definekey("rmail_delete_cmd", "D", $1);
...
define rmail_check_lock_file(lck)
{
   ...
   variable errbuf;
   variable ourhost = getenv ("HOST");
   variable lockhost = Null_String;
   variable lock_buffer;
   variable flags;
   
   lock_buffer = strcat (" ", lfn);
   ERROR_BLOCK
     {
   ...
   vinsert ("%s:%s", ourhost, this_pid, 2);
   
   if (lck)
     {
	if (strlen (lockhost))
	  {
	     if (strcmp (lockhost, ourhost))
	       {
	...
	if (strlen (lpid) and strcmp (lpid, this_pid))
	  {
   ...
   if (strcmp (lockhost, ourhost) or strcmp(lpid, this_pid)) return 0;
