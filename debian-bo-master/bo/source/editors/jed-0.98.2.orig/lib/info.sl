...
define info_menu ()
{
   ...
   if ((LAST_CHAR == '\r') and looking_at ("*Note ") and not(bolp ()))
     {
...
define info_reader (arg_num)
{
   ...
   if (arg_num != MAIN_ARGC)
     {
	node = "top";
	file = command_line_arg (arg_num);
	arg_num++;
	
	% Goto top incase the requested node does not exist.
	info_find_node (Sprintf ("(%s)top", file, 1));

	if (arg_num != MAIN_ARGC)
	  {
	     node = command_line_arg (arg_num);
	     arg_num++;
	  }
	
	info_find_node (Sprintf ("(%s)%s", file, node, 2));
     }
}
