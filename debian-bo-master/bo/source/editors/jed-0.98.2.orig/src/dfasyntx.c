...
static void make_dfa (Highlight *h) 
{
   ...
   if (!(tail->nfa_set = (unsigned char *)SLMALLOC(setsize)))
     goto error;
   ...
   compute_closure (h, tail->nfa_set);
     ...
     */
   if (!(tail->next = (DFA *) SLMALLOC(sizeof(DFA))))
     goto error;
   tail = tail->next;
   memset ((char *) tail, 0, sizeof (DFA));
		  ...
		  if (dest_empty)
		  ...
		  else 
		    {
			 ...
			 if (!memcmp((char *) search->nfa_set, (char *) destination, setsize))
		       ...
		       if (!search) 
			 {
			    if (!(search = tail->next = (DFA *) SLMALLOC(sizeof(DFA))))
			      goto error;
			    tail = tail->next;
			    if (!(tail->nfa_set = (unsigned char *)SLMALLOC(setsize)))
			      goto error;
...
static int dfa_try_keyword (register unsigned short *q, int n,
...
static void dfa_syntax_highlight (register unsigned short *p,
...
void define_highlight_rule (char *rule, char *colour, char *name) 
{
   ...
   if ( (i = map_color_object_to_number (colour)) != -1)
     add_rule (table->hilite, rule, strlen(rule), flags, i << 8);
}

void build_highlight_table (char *name) 
{
   Syntax_Table_Type *table;
   
   table = find_syntax_table (name, 1);
   
   if (table && table->hilite && !table->hilite->dfa)
     {
	flush_message ("Creating DFA syntax table...");
	make_dfa (table->hilite);
	if (table->hilite->filename)
	  save_dfa (table->hilite, name);
     }
}

static char *Dfa_Cache_Dir;

void jed_set_dfa_cache_dir (void)
{
   char *str;
   ...
   SLFREE (Dfa_Cache_Dir);
   Dfa_Cache_Dir = str;
}


void enable_highlight_cache (char *file, char *name) 
{
   Syntax_Table_Type *table;
   char fname[1024];
   ...
   *fname = 0;
   if (Dfa_Cache_Dir != NULL)
     {
	safe_strcpy (fname, Dfa_Cache_Dir, sizeof(fname));
	fixup_dir (fname);
     }
   
   safe_strcat (fname, file, sizeof(fname));
   
   if (table->hilite->filename != NULL)
     SLFREE (table->hilite->filename);
   
   table->hilite->filename = SLmake_string (fname);
   
   if (!table->hilite->dfa)
     load_dfa (table->hilite, name);
}
