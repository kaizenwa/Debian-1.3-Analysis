  ...
  if(new)
    {
      ...
      p_atos(buf, "open %s", portlist[new]);
      if(Check(p_open(&serial, portlist[new],-1), buf))
	serial = 0;
    }
  if(serial)
    {
...
#define rl portlist
	    ...
	    flow = (pm_xonxoff ? 1 : (pm_nortscts ? 3 : 2));
            if (   uOpenDialog (cmds[5]+1)
                || uAddChoiceList ("Port", &port, rl[0],rl[1],rl[2],rl[3],NULL)
	    ...
	    if((logfd && !log) || (!logfd && log)) /* Changed */
		...
		else
		  {
		    ...
		    *buf = 0;
		    ...
		    if(p_open(&logfd,buf+1,P_FSTREAM|P_FREPLACE|P_FUPDATE))
