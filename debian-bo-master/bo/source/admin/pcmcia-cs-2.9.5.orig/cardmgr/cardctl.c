...
"cardinfo.c 1.17 1997/04/14 05:35:48 (David Hinds)";
#endif
...
static int execute(FILE *stab, char *action)
{
    char s[133], class[33], dev[33];
    int ret;
    
    while (fgets(s, 132, stab))
	if (s[0] != 'S') {
	    sscanf(s, "%*d\t%s\t%*s\t%*d\t%s", class, dev);
	    sprintf(s, "./%s %s %s", class, action, dev);
	    ret = system(s);
...
static int do_scheme(char *new)
{
	...
	stab = fopen("/var/run/stab", "r");
	if (stab) {
	    if (execute(stab, "check") != 0) {
