...
static char *version =
"pcnet_cs.c 1.49 1997/04/23 15:49:37 (David Hinds)";
#endif
...
static hw_info_t hw_info[] = {
    ...
    { "Grey Cell GCS2220", 0x0000, 0x00, 0x47, 0x43, 0 },
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_detach(0x%p)\n", link);
#endif
...
static hw_info_t *get_hwinfo(dev_link_t *link)
{
	...
	if (j != 0)
	    cs_error(ReleaseWindow, j);
	return hw_info+i;
    }
    else {
	i = CardServices(ReleaseWindow, link->win);
	if (i != 0)
	    cs_error(ReleaseWindow, i);
	return NULL;
    }
} /* get_hwinfo */
...
static void pcnet_config(dev_link_t *link)
{
    ...
    pcnet_dev_t *info;
    struct device *dev;
    int i, j, start_pg, stop_pg, cm_offset;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_config(0x%p)\n", link);
#endif
	...
	i = setup_shmem_window(link, start_pg, stop_pg, cm_offset);
    else
	i = setup_dma_config(link, start_pg, stop_pg);
    if (i != 0)
	goto config_undo;

    ei_status.name = hw_info->id;
    ei_status.word16 = 1;
    ...
    printk(KERN_INFO "%s: %s, port %#3lx, irq %d,",
	   dev->name, hw_info->id, dev->base_addr, dev->irq);
    if (info->flags & USE_SHMEM)
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_release(0x%p)\n", link);
#endif

    if (link->open) {
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "pcnet_cs: release postponed, '%s' "
		   "still open\n", info->node.dev_name);
#endif
		       ...
		       event_callback_args_t *args)
{
	...
	if (pc_debug)
	    printk(KERN_DEBUG "pcnet_cs: registration complete\n");
	break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_open('%s')\n", dev->name);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_close('%s')\n", dev->name);
#endif
			      ...
			      int stop_pg, int cm_offset)
{
    ...
    link->win = (window_handle_t)link->handle;
    i = CardServices(RequestWindow, &link->win, &req);
    CS_EXIT_TEST(i, RequestWindow, config_failed);
    ...
    CS_EXIT_TEST(i, MapMemPage, config_failed);
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "pcnet_cs: unloading\n");
#endif
