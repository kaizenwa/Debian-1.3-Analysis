...
"cs.c 1.145 1997/03/16 20:03:53 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: register_ss_entry(%d, 0x%p)\n",
	       nsock, ss_entry);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: shutdown_socket(%ld)\n", i);
#endif
...
static void setup_socket(u_long i)
{
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: setup_socket(%ld): applying power\n", i);
#endif
	...
	if (pc_debug)
	    printk(KERN_DEBUG "cs: setup_socket(%ld): no card!\n", i);
#endif
} /* setup_socket */
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: resetting socket %ld\n", i);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: reset done on socket %ld\n", i);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug > 1)
	    printk(KERN_DEBUG "cs: socket %ld not ready yet\n", i);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "cs: send_event(sock %d, event %d, pri %d)\n",
...
static void parse_events(void *info, u_long events)
{
	...
	if ((s->state & SOCKET_PRESENT) &&
	    !(s->state & (SOCKET_SUSPEND|SOCKET_SHUTDOWN_PENDING)))
	    do_shutdown(s);
...
#ifdef PCMCIA_DEBUG
		if (pc_debug)
		    printk(KERN_DEBUG "cs: delaying pending setup\n");
#endif
		del_timer(&s->setup);
	    }
	    s->setup.function = &setup_socket;
	...
	if (!(s->state & SOCKET_RESET_PENDING))
	    ...
	    printk(KERN_DEBUG "cs: ready change during reset\n");
#endif
    }
} /* parse_events */
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: received suspend notification\n");
#endif
	if (down) {
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: received resume notification\n");
#endif
	if (!down) {
...
static int alloc_io_space(socket_info_t *s, u_long attr, ioaddr_t *base,
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: bind_device(): client 0x%p, sock %d, "
	       "dev %s\n", client, client->Socket,
	       (char *)client->dev_info);
#endif
    return CS_SUCCESS;
} /* bind_device */
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: bind_mtd(): attr 0x%lx, offset 0x%lx, "
	       "dev %s\n", req->Attributes, req->CardOffset,
	       (char *)req->dev_info);
#endif
    return CS_SUCCESS;
} /* bind_mtd */
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: deregister_client(%p)\n", handle);
#endif
    if (CHECK_HANDLE(handle))
	return CS_BAD_HANDLE;
    if (handle->state &
				  ...
				  config_info_t *config)
{
    ...
    if (handle->Function == BIND_FN_ALL) {
	if ((s->functions > 0) && (config->Function >= s->functions))
	    return CS_BAD_ARGS;
	c = &s->config[config->Function];
    }
    else
	c = CONFIG(handle);
    
    if ((s->functions == 0) || !(c->state & CONFIG_LOCKED)) {
	config->Attributes = 0;
    ...
    /* !!! This is a hack !!! */
    if (c->state & CONFIG_LOCKED) {
	memcpy(&config->Attributes, &c->Attributes, 33);
...
static int get_status(client_handle_t handle, status_t *status)
{
    ...
    if (handle->Function == BIND_FN_ALL) {
	if ((s->functions > 0) && (status->Function > s->functions))
	    return CS_BAD_ARGS;
	...
	if (c->Present & PRESENT_PIN_REPLACE) {
	    read_cis_mem(s, 1, c->ConfigBase+CISREG_PRR, 1, &reg);
	    status->CardState |=
	...
	if (c->Present & PRESENT_EXT_STATUS) {
	    read_cis_mem(s, 1, c->ConfigBase+CISREG_ESR, 1, &reg);
	    status->CardState |=
...
static int register_client(client_handle_t *handle, client_reg_t *req)
{
    ...
    *handle = client;
    ...
    if (client->Function != BIND_FN_ALL) {
	socket_info_t *s = socket_table[ns];
	if (s->functions == 0) {
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: register_client(): client 0x%p, sock "
	       "%d, dev %s\n", client, client->Socket,
	       (char *)client->dev_info);
#endif
...
static int release_irq(client_handle_t handle, irq_req_t *req)
{
	...
	FREE_IRQ(req->AssignedIRQ, handle->dev_info);
				 ...
				 config_req_t *req)
{
    ...
    c->ConfigBase = req->ConfigBase;
    c->Present = c->CardValues = req->Present;
    if (req->Present & PRESENT_COPY) {
	c->Copy = req->Copy;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_SCR, 1, &c->Copy);
    }
	...
	write_cis_mem(s, 1, req->ConfigBase+CISREG_COR, 1, &c->Option);
	busy_loop(HZ/25);
    }
    if (req->Present & PRESENT_STATUS) {
	c->Status = req->Status;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_CCSR, 1, &c->Status);
    }
    if (req->Present & PRESENT_PIN_REPLACE) {
	c->Pin = req->Pin;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_PRR, 1, &c->Pin);
    }
    if (req->Present & PRESENT_EXT_STATUS) {
	c->ExtStatus = req->ExtStatus;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_ESR, 1, &c->ExtStatus);
    }
    if (req->Present & PRESENT_IOBASE_0) {
	i = c->io.BasePort1 & 0xff;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_IOBASE_0,
		      1, (caddr_t)&i);
	i = (c->io.BasePort1 >> 8) & 0xff;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_IOBASE_1,
		      1, (caddr_t)&i);
    }
    if (req->Present & PRESENT_IOSIZE) {
	i = c->io.NumPorts1 + c->io.NumPorts2 - 1;
	write_cis_mem(s, 1, req->ConfigBase+CISREG_IOSIZE,
		      1, (caddr_t)&i);
    }
...
static int cs_request_irq(client_handle_t handle, irq_req_t *req)
{
    ...
    if ((req->Attributes & IRQ_HANDLE_PRESENT) &&
	REQUEST_IRQ(irq, req->Handler,
		    ((req->Attributes & IRQ_TYPE_DYNAMIC_SHARING) || 
		     (s->functions > 1)) ? SA_SHIRQ : 0,
		    (char *)handle->dev_info, handle->dev_info))
	return CS_IN_USE;
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: resetting socket %d\n", i);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: suspending socket %d\n", i);
#endif
    send_event(s, CS_EVENT_PM_SUSPEND, CS_EVENT_PRI_LOW);
    s->ss_entry(s->sock, SS_SetSocket, &dead_socket);
    s->state |= SOCKET_SUSPEND;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: waking up socket %d\n", i);
#endif
    setup_socket(i);
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: user eject request on socket %d\n", i);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: user insert request on socket %d\n", i);
#endif
