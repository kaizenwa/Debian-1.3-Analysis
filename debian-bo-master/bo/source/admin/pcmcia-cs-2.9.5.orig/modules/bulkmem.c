...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "Adding 0x%p to queue 0x%p\n", entry, head);
#endif
    entry->next = head;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "Unqueueing 0x%p\n", entry);
#endif
    entry->next->prev = entry->prev;
    entry->prev->next = entry->next;
}

static void retry_erase(erase_busy_t *busy, u_long cause)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "Trying erase request 0x%p...\n", busy);
#endif
    if (busy->next)
	remove_queue(busy);
...
#ifdef PCMCIA_DEBUG
	if (pc_debug > 1)
	    printk(KERN_DEBUG "  Status = %ld, requeueing.\n", req.Status);
#endif
	switch (req.Status) {
	case MTD_WAITREQ:
	case MTD_WAITPOWER:
	...
	case MTD_WAITRDY:
	    ...
	    busy->timeout.expires = RUN_AT(req.Timeout*HZ/1000);
...
void retry_erase_list(erase_busy_t *list, u_long cause)
{
    erase_busy_t tmp = *list;

#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "Rescanning erase queue list 0x%p\n", list);
#endif
    if (list->next == list)
	return;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "Erase timeout for entry 0x%lx\n", arg);
#endif
...
/*======================================================================
    ...
    We don't support MTDRequestWindow or MTDReleaseWindow -- clients
    should use the normal Card Services calls.
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: setup_regions(0x%p, %d, 0x%p)\n",
	       handle, attr, list);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: register_mtd(0x%p, '%s', 0x%lx)\n",
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "cs: open_memory(0x%p, 0x%lx) = 0x%p\n",
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "cs: close_memory(0x%p)\n", handle);
#endif
