...
"tcic.c 1.81 1997/04/15 16:00:57 (David Hinds)";
#endif
...
static void tcic_interrupt IRQ(int irq, void *dev, struct pt_regs *regs);
...
int tcic_init(void)
{
    ...
    printk(", %d sockets\n" KERN_INFO "  irq mask (", sockets);
    mask = tcic_cap.irq_mask & irq_mask;
    if (do_scan && (scan = irq_scan(mask))) {
	mask = scan;
	printk("scanned");
    }
    else
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: tcic_interrupt()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: tcic_timer()\n");
#endif
    timer_pending = 0;
    tcic_interrupt IRQ(0, NULL, NULL);
} /* tcic_timer */
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: GetStatus(%d) = %#2.2x\n",
	       lsock, *value);
#endif
    return 0;
} /* tcic_get_status */
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: GetSocket(%d) = flags %#3.3lx, "
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: SetSocket(%d, flags %#3.3lx, "
	       "Vcc %d, Vpp %d, io_irq %d, csc_mask %#2.2lx)\n",
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: GetIOMap(%d, %d) = %#2.2x, %d ns, "
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: SetIOMap(%d, %d, %#2.2x, %d ns, "
	       "%#4.4x-%#4.4x)\n", lsock, io->map, io->flags,
	       io->speed, io->start, io->stop);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: GetMemMap(%d, %d) = %#2.2x, %d ns, "
	       "%#5.5lx-%#5.5lx, %#5.5lx\n", lsock, mem->map,
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "tcic: SetMemMap(%d, %d, %#2.2x, %d ns, "
	       "%#5.5lx-%#5.5lx, %#5.5lx)\n", lsock, mem->map,
	       mem->flags, mem->speed, mem->sys_start, mem->sys_stop,
	       mem->card_start);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug) 
	printk(KERN_DEBUG "tcic_ioctl(%d, %d, 0x%p)\n", lsock, cmd, arg);
#endif
