...
static char *version =
"iflash2_mtd.c 1.24 1997/03/16 20:08:00 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "iflash2_mtd: abort\n");
#endif
...
#ifdef PCMCIA_DEBUG
		if (pc_debug > 1)
		    printk(KERN_DEBUG "iflash2_mtd: raising Vpp...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: lowering Vpp...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_detach(0x%p)\n", link);
#endif
...
static void flash_config(dev_link_t *link)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_config(0x%p)\n", link);
#endif
	...
	while (ret == CS_SUCCESS) {
	    ...
	    printk(KERN_INFO "iflash2_mtd: %s at 0x%lx, ",
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_release(0x%p)\n", link);
#endif
...
static int flash_read(dev_link_t *link, char *buf, mtd_request_t *req)
{
    ...
    region_info_t *region;
    mtd_mod_win_t mod;
    u_long from, length, nb, time, cell;
    int ret;
    
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_read(0x%p, 0x%ld, 0x%p,"
    ...
    time = jiffies - time;
    if (pc_debug && (time > 1))
	printk(KERN_DEBUG "iflash2_mtd: read complete, time = %ld, "
...
static int flash_write(dev_link_t *link, char *buf, mtd_request_t *req)
{
    ...
    flash_region_t *flash;
    region_info_t *region;
    u_long from, length, nb, time, retry, cell;
    status_t status;
    int ret;

#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_write(0x%p, 0x%lx, "
	       "0x%p, 0x%lx, 0x%lx)\n", link, req->MediaID, buf,
    ...
    time = jiffies - time;
    if (pc_debug && (time > 1))
	printk(KERN_DEBUG "iflash2_mtd: write complete, time = %ld, "
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_erase(0x%p, 0x%lx, "
	       "0x%lx, 0x%lx)\n", link, req->MediaID,
	       req->DestCardOffset, req->TransferLength);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "iflash2_mtd: erase complete, time = %ld\n",
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2_mtd: flash_event(%x)\n", event);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "iflash2_mtd: unloading\n");
#endif
