...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
        printk(KERN_DEBUG "ibmtr_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
        printk(KERN_DEBUG "ibmtr_detach(0x%p)\n", link);
#endif
...
static void ibmtr_config(dev_link_t *link)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
        printk(KERN_DEBUG "ibmtr_config(0x%p)\n", link);
#endif
    ...
    i = CardServices(RequestWindow, &info->sram_win_handle, &req);
    CS_EXIT_TEST(i, RequestWindow, config_failed);

    mem.CardOffset = (u_long) req.Base;
    mem.Page = 0;
    i = CardServices(MapMemPage, info->sram_win_handle, &mem);
    CS_EXIT_TEST(i, MapMemPage, config_failed);
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
        printk(KERN_DEBUG "ibmtr_release(0x%p)\n", link);
#endif

    if (link->open) {
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "ibmtr_cs: release postponed, '%s' "
		   "still open\n", info->node.dev_name);
#endif
                       ...
                       event_callback_args_t *args)
{
    ...
    case CS_EVENT_REGISTRATION_COMPLETE:
        if (pc_debug)
            printk(KERN_DEBUG "ibmtr_cs: registration complete\n");
        break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ibmtr_cs: unloading\n");
#endif
