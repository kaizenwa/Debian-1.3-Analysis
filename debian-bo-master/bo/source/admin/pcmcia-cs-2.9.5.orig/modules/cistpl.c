		  ...
		  long addr, long len, u_char *ptr)
{
    ...
    u_char *sys = (u_char *)(mem->sys_start + (addr & 0x0fff));
    int inc = 1;
    
#ifdef PCMCIA_DEBUG
    if (pc_debug > 3)
	printk(KERN_DEBUG "cs: read_cis_mem(%d, %#lx, %ld)\n",
	       attr, addr, len);
#endif
    mem->card_start = addr & ~0x0fff;
    mem->flags &= ~MAP_ATTRIB;
    if (attr) { mem->flags |= MAP_ATTRIB; inc++; }
    
    for (; len > 0; mem->card_start += 0x1000, sys -= 0x1000) {
	s->ss_entry(s->sock, SS_SetMemMap, mem);
#ifdef PCMCIA_DEBUG
	if (pc_debug > 3)
	    printk(KERN_DEBUG "cs:  %#2.2x %#2.2x %#2.2x %#2.2x %#2.2x ...\n",
		   sys[0], sys[inc], sys[2*inc], sys[3*inc], sys[4*inc]);
#endif
...
static void read_cis_cache(socket_info_t *s, int attr,
			   long addr, long len, u_char *ptr)
{
    int i;
    char *caddr;
...
static long follow_link(socket_info_t *s, tuple_t *tuple)
{
    u_char link[5];
    long ofs;
	...
	read_cis_cache(s, tuple->Flags & MFC_IS_ATTR,
	...
	read_cis_cache(s, tuple->Flags & IS_ATTR, ofs, 5, link);
...
int get_next_tuple(client_handle_t handle, tuple_t *tuple)
{
	...
	if ((link[0] == CISTPL_LONGLINK_A) ||
		...
		read_cis_cache(s, attr, ofs + (attr ? 4 : 2), 4,
		...
		if (handle->Function == BIND_FN_ALL) {
		    ...
		    read_cis_cache(s, attr, ofs + (attr ? 4 : 2), 1,
				 (u_char *)&tmp);
		    tuple->Flags = attr | LINK_MFC | tmp;
		    tuple->LinkOffset = ofs + (attr ? 6 : 3);
		}
		else {
		    /* Follow exactly one of the links */
		    tuple->Flags = attr | LINK_MFC | 1;
		    tuple->LinkOffset = ofs + (attr ? 2 : 1) *
			(3 + handle->Function * 5);
		}
		if (attr)
    ...
    if (i == 1000) {
	printk(KERN_NOTICE "cs: overrun in get_next_tuple for "
	       "socket %d\n", handle->Socket);
	return CS_NO_MORE_ITEMS;
    }
...
int get_tuple_data(client_handle_t handle, tuple_t *tuple)
{
	...
	return CS_SUCCESS;
    if (tuple->Flags & IS_ATTR)
	read_cis_cache(s, 1, tuple->CISOffset+((tuple->TupleOffset+2)<<1),
		       MIN(len, tuple->TupleDataMax), tuple->TupleData);
    else
	read_cis_cache(s, 0, tuple->CISOffset+(tuple->TupleOffset+2),
		       MIN(len, tuple->TupleDataMax), tuple->TupleData);
    return CS_SUCCESS;
} /* get_tuple_data */
...
int validate_cis(client_handle_t handle, cisinfo_t *info)
{
    ...
    /* First tuple should be CISTPL_DEVICE */
    if (strict_cis && (tuple.TupleCode != CISTPL_DEVICE))
	return CS_SUCCESS;

    for (info->Chains = 1; info->Chains < 500; info->Chains++) {
	ret = get_next_tuple(handle, &tuple);
	if (ret != CS_SUCCESS) break;
    }
    if (info->Chains == 500) {
	printk(KERN_NOTICE "cs: too many tuples for socket %d\n",
	       handle->Socket);
	info->Chains = 0;
    }
...
static int parse_longlink(tuple_t *tuple, cistpl_longlink_t *link)
{
	...
	return CS_BAD_TUPLE;
    link->addr = *(u_long *)tuple->TupleData;
    return CS_SUCCESS;
} /* parse_longlink */
