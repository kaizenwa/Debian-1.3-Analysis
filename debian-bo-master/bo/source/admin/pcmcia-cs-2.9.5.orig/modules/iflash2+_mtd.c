...
static char *version =
"iflash2+_mtd.c 1.30 1997/03/16 20:08:00 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: suspending erase...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: resuming erase...\n");
#endif
    writew(IF_READ_ESR, esr);
...
#ifdef PCMCIA_DEBUG
		if (pc_debug > 1)
		    printk(KERN_DEBUG "iflash2+_mtd: raising Vpp...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: lowering Vpp...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_detach(0x%p)\n", link);
#endif
...
static void flash_config(dev_link_t *link)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_config(0x%p)\n", link);
#endif
    ...
    ret = CardServices(RequestWindow, &dev->ESRwin, &req);
    if (ret != 0) {
	cs_error(RequestWindow, ret);
	link->state &= ~DEV_CONFIG_PENDING;
	flash_release((u_long)link);
	...
	while (ret == CS_SUCCESS) {
	    ...
	    printk(KERN_INFO "iflash2+_mtd: %s at 0x%lx, ",
...
static void flash_release(u_long arg)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_release(0x%p)\n", link);
#endif
    ...
    dev = link->priv;
    if (dev->ESRwin) {
	int ret = CardServices(ReleaseWindow, dev->ESRwin);
	if (ret != CS_SUCCESS)
	    cs_error(ReleaseWindow, ret);
    }
    if (dev->vpp_usage == 0)
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_read(0x%p, 0x%ld, "
	       "0x%p, 0x%lx, 0x%lx)\n", link, req->MediaID, buf,
	       req->SrcCardOffset, req->TransferLength);
#endif
...
#ifdef PCMCIA_DEBUG
	    if (pc_debug > 1)
		printk(KERN_DEBUG "iflash2+_mtd: delaying read...\n");
#endif
...
#ifdef PCMCIA_DEBUG
    time = uticks() - time;
    if (pc_debug && (time < 10000000))
	printk(KERN_DEBUG "iflash2+_mtd: read complete, time = %ld,"
...
static int basic_write(struct wait_queue **queue, char *esr, char *dest,
		       char *buf, u_long nb, u_long is_krnl)
{
    u_short npb;
    int ret;
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "iflash2+_mtd: odd address fixup "
		   "at 0x%p...\n", dest);
#endif
...
static int flash_write(dev_link_t *link, char *buf, mtd_request_t *req)
{
    ...
    flash_region_t *flash;
    region_info_t *region;
    u_long from, length, nb, retry, cell;
    status_t status;
    int ret;
#ifdef PCMCIA_DEBUG
    u_long time;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_write(0x%p, 0x%lx, "
	       "0x%p, 0x%lx, 0x%lx)\n", link, req->MediaID, buf,
...
#ifdef PCMCIA_DEBUG
	if (pc_debug > 1)
	    printk(KERN_DEBUG "iflash2+_mtd: delaying write...\n");
#endif
	req->Status = MTD_WAITREQ;
	return CS_BUSY;
    }
    ...
    time = uticks() - time;
    if (pc_debug && (time < 10000000))
	printk(KERN_DEBUG "iflash2+_mtd: write complete, time = %ld,"
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_erase(0x%p, 0x%lx, "
	       "0x%lx, 0x%lx)\n", link, req->MediaID,
	       req->DestCardOffset, req->TransferLength);
#endif
...
#ifdef PCMCIA_DEBUG
	    if (pc_debug > 1)
		printk(KERN_DEBUG "iflash2+_mtd: delaying erase...\n");
#endif
	    req->Status = MTD_WAITREQ;
	    return CS_BUSY;
	}
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "iflash2+_mtd: erase complete, time = %ld\n",
...
#ifdef PCMCIA_DEBUG
    if (pc_debug > 1)
	printk(KERN_DEBUG "iflash2+_mtd: flash_event(%x)\n", event);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "iflash2+_mtd: unloading\n");
#endif
