...
wv_82593_reconfig(device *	dev)
{
  ...
  if(!(link->open) || (set_bit(0, (void *)&dev->tbusy) != 0))
    {
...
wv_init_info(device *	dev)
{
  ...
  printk(KERN_NOTICE "%s: WaveLAN at %#x,", dev->name, base);
		      ...
		      struct pt_regs *	regs)
{
  ...
  while(1)
      ...
      if(wv_wait_completed)
	{
      ...
      /* If a transmission has neen done */
...
wavelan_attach(void)
{
  ...
  link->priv = dev = kmalloc(sizeof(struct device), GFP_KERNEL);
