...
static char *version =
"aha152x_cs.c 1.28 1997/04/14 15:41:17 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_detach(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_config(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_release(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "aha152x_cs: release postponed, "
		   "device still open\n");
#endif
			 ...
			 event_callback_args_t *args)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_event()\n");
#endif
	...
	if (pc_debug)
	    printk(KERN_DEBUG "aha152x_cs: registration complete\n");
	break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "aha152x_cs: unloading\n");
#endif
