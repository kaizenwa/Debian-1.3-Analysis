...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_detach(0x%p)\n", link);
#endif
...
static void fmvj18x_config(dev_link_t *link)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_config(0x%p)\n", link);
#endif
    ...
    */
	...
	switch (buf[0]) {
	case MANFID_TDK:
	    cardtype = TDK; link->conf.ConfigIndex = 0x04;
	    break;
	case MANFID_CONTEC:
	    cardtype = CONTEC; link->conf.ConfigIndex = 1;
	    break;
	case MANFID_FUJITSU:
	    cardtype = LA501; link->conf.ConfigIndex = 1;
	    break;
	default:
	    cardtype = MBH10304;
	    link->conf.ConfigIndex = 0x60;
	}
    } else {
	/* old type card */
	cardtype = MBH10302;
	...
	ret = CardServices(RequestIRQ, link->handle, &link->irq);
	if (ret != CS_SUCCESS) {
	    cs_error(RequestIRQ, ret);
	    goto end;
	}
	...
	do {
		...
		while (tuple.TupleData[0] != CISTPL_FUNCE_LAN_NODE_ID ) {
		    ret = CardServices(GetNextTuple, handle, &tuple) ;
		    if (ret != CS_SUCCESS) goto end;
		    ret = CardServices(GetTupleData, handle, &tuple) ;
		    if (ret != CS_SUCCESS) goto end;
		}
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_release(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "fmvj18x_cs: release postponed, '%s' "
		   "still open\n", link->dev->dev_name);
#endif 
			  ...
			  event_callback_args_t *args)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_event()\n");
#endif
	...
	if (pc_debug)
	    printk(KERN_DEBUG "fmvj18x_cs: registration complete\n");
	break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fmvj18x_cs: unloading\n");
#endif
