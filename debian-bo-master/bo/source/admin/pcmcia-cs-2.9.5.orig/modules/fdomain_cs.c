...
static char *version =
"fdomain_cs.c 1.17 1997/03/16 20:03:23 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_detach(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_config(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_release(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "fdomain_cs: release postponed, "
		   "device still open\n");
#endif
			...
			event_callback_args_t *args)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_event()\n");
#endif
	...
	if (pc_debug)
	    printk(KERN_DEBUG "fdomain_cs: registration complete\n");
	break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "fdomain_cs: unloading\n");
#endif
