...
static char *version =
"skeleton.c 1.36 1997/03/16 20:06:32 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_detach(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_config(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_release(0x%p)\n", link);
#endif
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "skeleton_cs: release postponed, '%s' "
		   "still open\n", link->dev->dev_name);
#endif
			  ...
			  event_callback_args_t *args)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_event()\n");
#endif
	...
	if (pc_debug)
	    printk(KERN_DEBUG "skeleton_cs: registration complete\n");
	break;
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "skeleton_cs: unloading\n");
#endif
