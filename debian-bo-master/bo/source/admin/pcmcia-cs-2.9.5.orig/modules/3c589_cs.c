...
static char *version =
"3c589_cs.c 1.93 1997/04/21 03:09:45 (David Hinds)";
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "3c589_attach()\n");
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "3c589_detach(0x%p)\n", link);
#endif
...
static void tc589_config(dev_link_t *link)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "3c589_config(0x%p)\n", link);
#endif
    ...
    if (i != CS_SUCCESS) {
	printk(KERN_NOTICE "3c589_cs: unable to parse CIS\n");
	...
	for (j = 0x300; j < (multi ? 0x380 : 0x400); j += 0x10) {
	    link->io.BasePort1 = j;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "3c589_release(0x%p)\n", link);
#endif
    
    if (link->open) {
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "3c589_cs: release postponed, '%s' "
		   "still open\n", link->dev->dev_name);
#endif
		       ...
		       event_callback_args_t *args)
{
	...
	if (pc_debug)
	    printk(KERN_DEBUG "3c589_cs: registration complete\n");
	break;
#endif
...
static int el3_open(struct device *dev)
{
    ...
    if (tc589_debug > 3)
	printk(KERN_DEBUG "%s: opened, status %4.4x.\n",
...
static int el3_start_xmit(struct sk_buff *skb, struct device *dev)
{
    ...
    if (tc589_debug > 4) {
	printk(KERN_DEBUG "%s: el3_start_xmit(length = %ld) called, "
...
static int el3_rx(struct device *dev)
{
    ...
    if (tc589_debug > 5)
	printk(KERN_DEBUG "%s: in rx_packet(), status %4.4x, "
	       "rx_status %4.4x.\n", dev->name,
...
#define BLOCK_INPUT(buf, len) insl(ioaddr+RX_FIFO, buf, (len+3)>>2)
		...
		printk(KERN_DEBUG "%s: couldn't allocate a sk_buff of"
		       " size %d.\n", dev->name, pkt_len);
	}
	lp->stats.rx_dropped++;
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "3c589_cs: unloading\n");
#endif
