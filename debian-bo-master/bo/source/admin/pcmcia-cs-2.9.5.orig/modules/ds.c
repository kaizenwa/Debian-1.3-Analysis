...
"ds.c 1.68 1997/04/15 16:00:27 (David Hinds)";
#endif
...
static FS_SIZE_T ds_read(struct inode *inode, struct file *file,
...
{
    CardServices(ReportError, "ds", (void *)func, (void *)ret);
}
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds: register_pcmcia_driver('%s')\n",
	       (char *)dev_info);
#endif
...
#ifdef PCMIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds: unregister_pcmcia_driver('%s')\n",
	       (char *)dev_info);
#endif
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds: ds_event(%d, %d, 0x%p)\n",
...
/*======================================================================
    ...
    I use a really simple scheme for reporting card status changes.
    When a status change happens, the SOCKET_EVENT_PENDING flag is
    set.  Reading from the socket device returns the current status,
    and resets the PENDING flag.
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds_open(socket %d)\n", i);
#endif
			 ...
			 char *buf, U_FS_SIZE_T count)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds_read(socket %d)\n", i);
#endif
    ...
    put_user(get_queued_event(user), (long *)buf);
    return 4;
} /* ds_read */
			  ...
			  CONST char *buf, U_FS_SIZE_T count)
{
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds_write(socket %d)\n", i);
#endif
    ...
    if (s->req_pending) {
	s->req_pending--;
	get_user(s->req_result, (long *)buf);
...
#ifdef PCMCIA_DEBUG
    if (pc_debug)
	printk(KERN_DEBUG "ds_ioctl(socket %d, %#x, %#lx)\n",
	       i, cmd, arg);
#endif
...
#ifdef PCMCIA_DEBUG
	    if (pc_debug)
		printk(KERN_DEBUG "ds_ioctl(): verify_read = %d\n", err);
#endif
	    return err;
	}
    }
...
#ifdef PCMCIA_DEBUG
	    if (pc_debug)
		printk(KERN_DEBUG "ds_ioctl(): verify_write = %d\n", err);
#endif
	    return err;
	}
    }
...
#ifdef PCMCIA_DEBUG
	if (pc_debug)
	    printk(KERN_DEBUG "ds_ioctl: ret = %d\n", ret);
#endif
	switch (ret) {
