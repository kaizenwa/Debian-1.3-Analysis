/*
   Time-stamp: <96/07/19 20:14:51 yusuf>

   $Id: util.c,v 1.16 1996/07/27 20:42:33 yusuf Exp $	

*/

#ifndef lint
static char vcid[] = "$Id: util.c,v 1.16 1996/07/27 20:42:33 yusuf Exp $";
#endif /* lint */
...
#ifdef RCSID
static char rcsid[] = "$Id: util.c,v 1.16 1996/07/27 20:42:33 yusuf Exp $";
#endif
    ...
    int eof_ok;          /* set if EOF acceptable as a result */
{
    ...
    errno = 0;
    do {
	len = read(gz_ifd, (char*)inbuf+insize, INBUFSIZ-insize);
...
void flush_outbuf()
{
    ...
    write_buf(gz_ofd, (char *)outbuf, outcnt);
...
void flush_window()
{
    ...
    if (!test) {
	write_buf(gz_ofd, (char *)window, outcnt);
    }
    bytes_out += (ulg)outcnt;
    outcnt = 0;
}
...
#endif
    if (casemap('A') == 'a') strlwr((char *) fname);
    return (char *) fname;
}
