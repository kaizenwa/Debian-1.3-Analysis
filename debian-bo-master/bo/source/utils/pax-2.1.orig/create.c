/* $Source: /usr/local/src/pax/RCS/create.c,v $
 *
 * $Revision: 2.2 $
 *
...
#ifndef lint
static char        *ident = "$Id: create.c,v 2.2 1996/10/20 12:10:20 istewart Exp $";
...
static void	    writetar __P ((char *, Stat *));
static void	    writecpio __P ((char *, Stat *));
static char	    tartype __P ((int));
...
#endif
{
	...
	*name++ = '\0';
    ...
    sprintf(&hdr[124], "%011lo ", (off_t) asb->sb_size);
	...
	if (strlen(from->l_name) > 99) {
	     ...
	     */
	    sprintf(&hdr[124], "%011lo ", (off_t) from->l_size);
	} else {
    ...
    strcpy(&hdr[265], finduname((uid_t) asb->sb_uid));
    strcpy(&hdr[297], findgname((gid_t) asb->sb_gid));
