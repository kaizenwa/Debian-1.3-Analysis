/* $Source: /usr/local/src/pax/RCS/wildmat.c,v $
 *
 * $Revision: 2.2 $
 *
...
#ifndef lint
static char        *ident = "$Id: wildmat.c,v 2.2 1996/10/20 12:10:20 istewart Exp $";
...
#endif
{
	...
	case '[':
		...
		if (*pattern == '-'
		    ? (*source <= *++pattern) && (*source >= (char)last)
		    : (*source == *pattern)) {
		    matched = 1;
		}
	    }
