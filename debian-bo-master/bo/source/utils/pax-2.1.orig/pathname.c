/* $Source: /usr/local/src/pax/RCS/pathname.c,v $
 *
 * $Revision: 2.2 $
 *
...
#ifndef lint
static char        *ident = "$Id: pathname.c,v 2.2 1996/10/20 12:10:20 istewart Exp $";
...
#endif
{
    if (mkdir(name, (mode_t) (asb->sb_mode & S_IPOPN)) < 0) {
	return (-1);
    }
    if (asb->sb_mode & S_IPEXE) {
	chmod(name, (mode_t) (asb->sb_mode & S_IPERM));
    }
    if (f_owner) {
