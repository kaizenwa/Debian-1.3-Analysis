/* UnDel File System: Midnight Commander file system.
   ...
   written by Ted Ts'o (tytso@lurch.mit.edu) for the ext2fs package.
...
undelfs_shutdown ()
{
    /* should shutdown the ext2 toolkit */
}
...
void com_err (const char *str, long err_code, const char *s2, ...)
{
    ...
    sprintf (cptr, " %s (%s: %d) ", s2, str, err_code);
...
undelfs_readdir (void *vfs_info)
{
	...
	sprintf (dirent_dest, "%ld:%ld",
		 delarray [readdir_ptr].ino,
