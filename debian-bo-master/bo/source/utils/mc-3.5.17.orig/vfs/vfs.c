...
{
    struct vfs_stamping *stamp, *st1, *st2, *st3;
    ...
    for (stamp = stamps, st1 = NULL; stamp != NULL; st1 = stamp, stamp = stamp->next)
        if (stamp->v == v && stamp->id == id) {
            if (stamp->parent != NULL) {
                ...
                for (st2 = stamp->parent, st3 = st2->next; st3 != NULL; st2 = st3, st3 = st3->next)
                    free (st2);
                free (st2);
            }
            if (st1 == NULL) {
...
static vfsid vfs_ncs_getid (vfs *nvfs, char *dir, struct vfs_stamping **par)
{
    vfsid nvfsid;
    int freeit = 0;
...
void vfs_add_noncurrent_stamps (vfs *oldvfs, vfsid oldvfsid, struct vfs_stamping *parent)
{
    ...
    if (nvfs != oldvfs || nvfsid != oldvfsid) {
        struct vfs_stamping *stamp;

	if (get_other_type () == view_listing){
	    n2vfs = vfs_type (opanel->cwd);
    	    n2vfsid = vfs_ncs_getid (n2vfs, opanel->cwd, &par);
	} else {
	    n2vfs = (vfs *) -1;
	    n2vfsid = (vfs *) -1;
	}
...
static void vfs_stamp_path (char *path)
{
    ...
    vfs_addstamp (vfs, id, par ? par->parent : par);
}
...
void vfs_add_current_stamps (void)
{
    ...
    if (get_other_type () == view_listing){
	WPanel *p = (WPanel *) get_panel_widget (get_other_index ());
	vfs_stamp_path (p->cwd);
    }
}
...
int mc_chdir (char *path)
{
    ...
    oldvfs = vfs_type (current_dir);
    oldvfsid = vfs_ncs_getid (oldvfs, current_dir, &parent);
...
static void vfs_setup_wd (void)
{
    ...
    get_current_wd (current_dir, MC_MAXPATHLEN);
    if (errno != 0)
	current_dir [0] = 0;
...
int parse_ls_lga (char *p, struct stat *s, char **filename, char **linkname)
{
    ...
    if (((S_ISLNK (s->st_mode) || 
        (numcols == idx + 3 && s->st_nlink > 1))) /* Maybe a hardlink? (in extfs) */
        && (!strcmp (columns [idx+1], "->"))){
	if (filename)
	    *filename = strdup (columns [idx]);
	if (linkname)
	    *linkname = strdup (columns [idx+2]);
	idx += 2;
    } else {
