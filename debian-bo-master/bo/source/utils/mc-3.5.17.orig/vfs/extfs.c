...
static struct extfs_entry *extfs_find_entry (struct extfs_entry *dir, char *name, int make_dirs);
...
static FILE *open_extfs_archive (int fstype, char *name, struct extfs_archive **pparc)
{
    ...
    cmd = copy_strings (LIBDIR "extfs/", extfs_prefixes [fstype], 
                        " list ", local_name ? local_name : name, 0);
    result = popen (cmd, "r");
    free (cmd);
    if (result == NULL) {
    ...
    while (fgets (buffer, 4096, extfsd) != NULL) {
        ...
        if (parse_ls_lga (buffer, &hstat, &current_file_name, &current_link_name)) {
            ...
            if (*cfn) {
                ...
                pent = extfs_find_entry (current_archive->root_entry, q, 1);
                if (pent == NULL) {
		...
		if (!S_ISLNK (hstat.st_mode) && current_link_name != NULL) {
	            pent = extfs_find_entry (current_archive->root_entry, current_link_name, 0);
	            if (pent == NULL) {
	                message_1s (1, " Error ", "Inconsistent extfs archive");
                    /* FIXME: Should clean everything one day */
			free (buffer);
...
__extfs_find_entry (struct extfs_entry *dir, char *name,
		    struct extfs_loop_protect *list, int make_dirs);
			  ...
			  struct extfs_loop_protect *list)
{
    ...
    looping->entry = entry;
    looping->next = list;
    pent = __extfs_find_entry (entry->dir, entry->inode->linkname, looping, 0);
    free (looping);
    if (pent == NULL)
    	extfserrno = ENOENT;
    return pent;
...
static struct extfs_entry *extfs_find_entry (struct extfs_entry *dir, char *name, int make_dirs)
{
    ...
    errloop = 0;
    notadir = 0;
    res = __extfs_find_entry (dir, name, NULL, make_dirs);
    if (res == NULL) {
    	if (errloop)
    	    extfserrno = ELOOP;
    	else if (notadir)
...
void extfs_run (char *file)
{
    ...
    cmd = copy_strings (LIBDIR "extfs/", extfs_prefixes [archive->fstype], 
                        " run ", get_archive_name(archive), " ", q, 0);
    shell_execute(cmd, 0);
    free(cmd);
    free(p);
}
...
static void *extfs_open (char *file, int flags, int mode)
{
	...
	return NULL;
    q = (*p == '/') ? p + 1 : p;
    entry = extfs_find_entry (archive->root_entry, q, 0);
    if (entry == NULL) {
        free (p);
    	return NULL;
    }
...
void extfs_init (void)
{
	...
	if (key [strlen (key)-1] == ':'){
	    key [strlen (key)-1] = 0;
	    extfs_need_archive [extfs_no] = 0;
	} else
	    extfs_need_archive [extfs_no] = 1;
