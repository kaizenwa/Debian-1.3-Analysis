...
/* "$Id: tree.c,v 1.3 1995/02/21 19:07:24 miguel Exp $" */
...
void start_tree_check (WTree *tree)
{
    ...
    /* Here is the bug reported by Michael */
    tree->check_start = current->next;
    len = strlen (tree->check_name);
    if (tree->check_name [0] == PATH_SEP && tree->check_name [1] == 0)
	tree->check_sublevel = current->sublevel;
    else
	tree->check_sublevel = current->sublevel + 1;

    current = tree->check_start;
    while (current
	   && strncmp (current->name, tree->check_name, len) == 0
	   && current->sublevel >= tree->check_sublevel){
	current->mark = 1;
	current = current->next;
    }
}
...
void do_tree_check (WTree *tree, const char *subname)
{
	...
	name = copy_strings (tree->check_name, PATH_SEP_STR, subname, 0);
...
void load_tree (WTree *tree)
{
    ...
    filename = copy_strings (home_dir, PATH_SEP_STR ".mc.tree", 0);
	...
	while (fgets (name, MC_MAXPATHLEN, file)){
		...
		if (s){
			...
			tree_append_entry (tree, oldname);
		    }
		}
	    } else {
		tree_append_entry (tree, name);
		strcpy (oldname, name);
	    }
	}
	fclose (file);
...
void show_tree (WTree *tree)
{
	...
	if (current->sublevel == topsublevel){
		...
		if (!use_colors && !tree->is_panel) set_attr (0, 1);
		else set_attr (1, 0);
	    /* Show full name */
...
int tree_rescan_cmd (WTree *tree)
{
    DIR *dirp;
    struct dirent *dp;
    struct stat buf;
...
void tree_copy (WTree *tree, char *default_dest)
{
    ...
    sprintf (cmd_buf, "Copy \"%s\" directory to:",
	     name_trunc (tree->selected_ptr->name, 60));
...
{
    define_label_data (h, (Widget *) NULL,
...
static int my_mkdir_rec (char *s, mode_t mode)
{
    ...
    if (s [strlen (s) - 1] != PATH_SEP)
        p = copy_strings (s, PATH_SEP_STR "..", NULL);
    else
    	p = copy_strings (s, "..", NULL);
    q = vfs_canon (p);
    free (p);
