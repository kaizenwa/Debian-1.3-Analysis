...
struct {
    ...
    { B_CANCEL, 0, 14, "[ Cancel ]",  'c', 2, },
    { B_ENTER,  0,  0, "[[ Save ]]", 's', 3, },
};
...
static void learn_refresh (void)
{
    ...
    draw_box (learn_dlg, 1, 2, 21, 70);
    
    attrset (COLOR_HOT_NORMAL);
    dlg_move (learn_dlg, 1, (72 - 19) / 2);
    addstr (" Learn keys ");
}
#endif
...
static void init_learn (void)
{
    ...
    learn_dlg = create_dlg (0, 0, 23, 75, dialog_colors,
			      learn_callback, "[Learn keys]", "Learn keys",
			      DLG_CENTER);
    x_set_dialog_title (learn_dlg, "Learn keys");

#define XTRACT(i) BY+learn_but[i].y, BX+learn_but[i].x, learn_but[i].ret_cmd, learn_but[i].text, learn_but[i].hkey, learn_but[i].hpos, 0, 0, NULL
    ...
    for (i = j - 1, key = key_name_conv_tab + j - 1; i >= 0; i--, key--) {
        ...
        sprintf (buffer, " %-18s", key->longname);
	add_widget (learn_dlg, learnkeys [i].button = (Widget *)
	    button_new (y, x, B_USER + i, buffer, 0, -1, learn_button, 0, NULL));
	add_widget (learn_dlg, learnkeys [i].label = (Widget *)
	    label_new (y, x + 20, "", NULL));
	if (i % 13)
	    y--;
	else {
	    x -= COLSHIFT;
