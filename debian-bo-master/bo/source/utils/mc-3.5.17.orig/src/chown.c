...
static struct {
    ...
    int hkey, hpos;
     ...
     { B_CANCEL, 0, 54, "[ Cancel ]", 'c', 2, },
     { B_ENTER, 0, 45, "[ Set ]", 's', 2, },
     { B_SETUSR, 0, 29, "[ Set users ]", 'u', 6, },
     { B_SETGRP, 0, 13, "[ Set groups ]", 'g', 6, },
     { B_SETALL, 0, 0, "[ Set all ]", 'a', 6, },
};
...
#define XTRACT(i) BY+chown_but[i].y, BX+chown_but[i].x, chown_but[i].ret_cmd, chown_but[i].text, chown_but[i].hkey, chown_but[i].hpos, 0, 0, NULL
