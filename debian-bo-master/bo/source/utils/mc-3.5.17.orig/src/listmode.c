...
struct {
    ...
    { B_ADD,    0, 22, "[ Add item ]", 'a', 2, },
    { B_REMOVE, 0, 10, "[ Remove ]",  'r', 2, },
    { B_ENTER,  0,  0, "[[ Ok ]]", 'o', 3, },
};
...
#define XTRACT(i) BY+listmode_but[i].y, BX+listmode_but[i].x, listmode_but[i].ret_cmd, listmode_but[i].text, listmode_but[i].hkey, listmode_but[i].hpos, 0, 0, NULL
