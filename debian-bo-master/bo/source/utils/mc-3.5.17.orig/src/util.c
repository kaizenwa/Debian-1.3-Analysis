...
/* "$Id: util.c,v 1.17 1995/02/21 19:07:33 miguel Exp $" */
...
char *strip_home(char *dir)
{
	...
	return newdir;
        ...
        if ((p = strchr (dir + 6, PATH_SEP)) != NULL) {
            *p = 0;
            if ((q = ftpfs_gethome (dir)) == NULL)
		return dir;
            *p = PATH_SEP;
            if (q != NULL && strcmp (q, PATH_SEP_STR) && !strncmp (p, q, strlen (q) - 1)) {
                strncpy (newdir, dir, p - dir);
                strcpy (newdir + (p - dir), "/~");
        ...
        char  *pth = (dir[3] == '/' && dir[4] == '/') ? dir + 5 : dir + 3;

        if ((p = strchr (pth, PATH_SEP)) != NULL) {
            *p = 0;
            if ((q = mcfs_gethome (dir)) == NULL)
		return dir;
            *p = PATH_SEP;
            if (q != NULL && strcmp (q, PATH_SEP_STR) && !strncmp (p, q, strlen (q) - 1)) {
                strncpy (newdir, dir, p - dir);
