...
/*  [] = "$Id: wtools.c,v 1.3 1995/02/21 19:07:51 miguel Exp $" */
...
{
    dialog_repaint (h, REVERSE_COLOR, COLOR_HOT_NORMAL);
}
...
Listbox *create_listbox_window (int cols, int lines, char *title, char *help)
{
	...
	button_new (lines+3, (cols/2)-5, B_CANCEL, "[ Cancel ]",'c',2,0, 0, "c"));
...
int query_dialog (char *header, char *text, int flags, int count, ...)
{
	...
	va_end (ap);
		...
		add_widgetl (query_dlg, button_new
			    (0, 0, B_USER+i, buttonnames [i] + 1,
			     *(buttonnames [i]), -1, 0, 0, NULL),
			     i ? XV_WLAY_RIGHTOF : XV_WLAY_CENTERROW);
	    else
		add_widgetl (query_dlg, button_new
			    (0, 0, B_USER+i, buttonnames [i],
			     *(buttonnames [i]), -1, 0, 0, NULL), 
			     i ? XV_WLAY_RIGHTOF : XV_WLAY_CENTERROW);
	}  
#endif	
...
#ifdef HAVE_X
	add_widgetl (query_dlg, button_new(0, 0, B_EXIT, "OK", 0, -1, 0, 0, NULL),
	    XV_WLAY_CENTERROW);
...
#else
	add_widget (query_dlg, label_new (2, 3, text, NULL));
	add_widget (query_dlg, button_new(0, 0, 0, "", 0, -1, 0, 0, NULL));
#endif /* HAVE_X */
...
Chooser *new_chooser (int lines, int cols, char *help, int flags)
{
	...
	add_widget (c->dialog, button_new (lines-button_lines+1,
				    4, B_CANCEL, "[ Cancel ]", 'c', 2, 0, 0, "button-cancel"));
    }		    
...
int quick_dialog_skip (QuickDialog *qd, int nskip)
{
	...
	case quick_checkbox:
	    widget = check_new (ypos, xpos, *qw->result, qw->text,
				qw->hotkey, qw->hotkey_pos, qw->tkname);
	    break;

	case quick_radio:
	    r = radio_new (ypos, xpos, qw->hotkey, qw->str_result, 1, qw->tkname);
	    r->pos = r->sel = qw->value;
	    widget = r;
	    break;
	    
	case quick_button:
	    widget = button_new (ypos, xpos, qw->value, qw->text,
				qw->hotkey, qw->hotkey_pos, 0, 0, qw->tkname);
	    break;
...
char *real_input_dialog_help (char *header, char *text, char *help, char *def_text)
{
    ...
    { quick_input,  4, 80, 0, 0, 0, 0, 58, 0, 0, 0, XV_WLAY_BELOWCLOSE, "input" },
...
int input_dialog_help_2 (char *header, char *text1, char *text2, char *help, char **r1, char **r2)
{
    QuickDialog Quick_input;
    QuickWidget quick_widgets [] = {
    { quick_button, 6, 10, 4, 0, "[ Cancel ]", 'c', 2, B_CANCEL, 0, 0,
	  XV_WLAY_DONTCARE, "button-cancel" },
    { quick_button, 3, 10, 4, 0, "[ Ok ]", 'o', 2, B_ENTER, 0, 0,
	  XV_WLAY_DONTCARE, "button-ok" },
    { quick_input,  4, 80, 4, 0, 0, 0, 58, 0, 0, 0, XV_WLAY_BELOWCLOSE, "input-pth" },
    { quick_label,  3, 80, 3, 0, 0, 0, 0, 0, 0, 0, XV_WLAY_DONTCARE, "label-pth" },
    { quick_input,  4, 80, 3, 0, 0, 0, 58, 0, 0, 0, XV_WLAY_BELOWCLOSE, "input-lbl" },
    { quick_label,  3, 80, 2, 0, 0, 0, 0, 0, 0, 0, XV_WLAY_DONTCARE, "label-lbl" },
    { 0 } };
