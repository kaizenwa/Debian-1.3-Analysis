...
static pid_t subshell_pid;	/* The subshell's process ID */
...
/* {{{ quote_directory */
void quote_directory (int fd, char *string)
{
    int q;
...
void do_subshell_chdir (char *directory, int do_update, int reset_prompt)
{
    ...
    quote_directory (subshell_pty, temp);
    write (subshell_pty, "\n", 1);
    free (temp);
	...
	fprintf (stderr, "\rWarning: couldn't change to %s.\n\r", cpanel->cwd);
