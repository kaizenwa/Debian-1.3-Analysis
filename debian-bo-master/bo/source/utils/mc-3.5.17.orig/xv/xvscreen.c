...
static copymove_in_panel (struct copy_move_in_panel *cmip)
{
...
static user_drop (struct user_drop *usdr)
{
    WPanel *panel = current_panel;
    
    xv_set_current_panel (usdr->panel);
    regex_command (usdr->filename, "Drop", usdr->drops);
    free (usdr->drops);
    free (usdr);
    xv_set_current_panel (panel);
}
...
static void start_drag (Xv_window paint, Event *event, Canvas canvas)
{
    ...
    WPanel *panel;
    char *p, *q;
    int i, copy = (event_shift_is_down (event) || event_ctrl_is_down (event));
    int trailslash;
...
static void xv_insert_panel_item (Canvas canvas, WPanel *panel, int idx)
{
    ...
    for (i = 0; i < top; i++) {
        ...
        if (panel->format [i].string_fn) {
            txt = (*panel->format [i].string_fn) (fe, 0);
        } else
            txt = " ";
        if (panel->format [i].string_fn == string_file_type) {
	    panel->format [i].field_len = 18;
	    p [0] = 3 + ((*txt == '/') ? 1 : 0);
	    p [1] = 0;
	    p += 2;
        } else {
	    len = xv_get_text_len (panel, txt) + 2;
	    if (panel->format [i].field_len < len)
	        panel->format [i].field_len = len;
	    p [0] = just_mode + 1;
	    strcpy (p + 1, txt);
	    p = index (p + 1, 0) + 1;
	}                
...
void x_fill_panel (WPanel *panel)
{
    ...
    for (i = 0; i < panel->fmt_count; i++)
        panel->format [i].field_len = 2;
        
    for (i = 0; i < panel->count; i++)
        xv_insert_panel_item (canvas, panel, i);
        
    for (i = 0, panel->total_width = 2 * PANELITEMBORDER; 
         i < panel->fmt_count; i++)
        panel->total_width += panel->format [i].field_len;
...
static void xv_panel_repaint_item (WPanel *panel, int idx, Xv_Window paint_window)
{
    ...
    for (i = 0, p = fe->cache; *p; p = strchr (p, 0) + 1, i++) {
        if (*p <= 2) { /* i.e. either left or right justified text */
            ...
            if (*p == J_LEFT + 1)
            	...
            	x = rect.r_left + panel->format [i].field_len - w - 1;
        ...
        rect.r_left += panel->format [i].field_len;
    }
}
...
static void xv_filedepmenu_cmd (char *todo)
{
    ...
    p = strchr (todo, ' ');
    *p = 0;
    regex_command (p + 1, todo, NULL, &movedir);
    free (todo);
}
