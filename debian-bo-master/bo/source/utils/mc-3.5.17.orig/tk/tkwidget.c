...
/* Removes the [ and ] from the text mode button version */

static char *
clean_button (char *text)
{
    static char buffer [20];
    char *p, *q;

    if ((strlen (text) + 1) >= sizeof (buffer)){
	return text;
    }
    strcpy (buffer, text);
    for (p = buffer; *p == ' ' || *p == '['; p++)
	;
    for (q = buffer + strlen (buffer); q > buffer && 
        (!*q || *q == ' ' || *q == ']'); q--);
    *(++q) = 0;
...
x_button_set (WButton *b, char *text)
{
    ...
    tk_evalf ("%s%s configure -text {%s}", cmd+1, postfix, clean_button (text));
}
...
x_create_button (Dlg_head *h, widget_data parent, WButton *b)
{
    ...
    tk_evalf ("newbutton %s %s {%s} %d", cmd+1, cmd, clean_button (b->text), is_default_button (b));
    return 1;
}
