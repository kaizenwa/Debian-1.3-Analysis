...
void
set_attr (int h, int m)
{
}

void
repaint_file (WPanel *panel, int file_index, int move)
{
}
...
x_fill_panel (WPanel *panel)
{
	...
	format_file (buffer, panel, i, panel_width);
...
tk_panel_set_size (int index, int boot)
{
    ...
    /* The +2 is here since a lot of code assumes text mode */
    w->cols = cols + 2;
    w->lines = lines + 2;

    panel_update_format (p);
    if (!boot)
	    paint_frame (p);
    x_fill_panel (p);
}
...
x_create_panel (Dlg_head *h, widget_data parent, WPanel *panel)
{
    ...
    cmd = tk_new_command  ((void *) container, panel, tk_panel_callback, 'p');
