...
/*
   ...
   Returns '\n' if out of bounds.
 */
int edit_get_byte (WEdit * edit, int byte_index)
{
    unsigned long p;
...
void push_action (WEdit * edit, long c)
{
	...
	return;
    if (spm1 != edit->stack_bottom && ((sp - 2) & M_STACK_SIZE) != edit->stack_bottom) {
	int d;
...
/*if the sp wraps round and catches the stack_bottom then erase the first set of actions on the stack to make space - by moving stack_bottom forward one "key press" */
    if (((edit->stack_pointer + 2) & M_STACK_SIZE) == edit->stack_bottom)
	do {
...
static void edit_move_to_top (WEdit * edit)
{
	...
	edit_cursor_move (edit, (int) edit_move_forward3 (edit, 0, edit->prev_col, 0));
	edit->force |= REDRAW_PAGE;
	edit->search_start = 0;
	update_curs_row(edit);
    }
...
/* left right arrow key response. |amount| > 1 moves word */
static void edit_lr_move_cmd (WEdit * edit, int amount)
{
    if (amount < 0) {
	if (amount < -1) {
	    int c;
	    while (strchr ("\t\n ,;", edit_cursor_move (edit, -1)));
	    while (!strchr ("\t\n ,;\xFF", (unsigned char) (c = edit_cursor_move (edit, -1))));
	    if(c >= 0)
		edit_cursor_move (edit, 1);
	    edit->force |= REDRAW_PAGE;
	} else
	...
	if (amount > 1) {
	    int c;
	    while (!strchr ("\t\n ,;\xFE", edit_cursor_move (edit, 1)));
	    while (strchr ("\t\n ,;", (unsigned char) (c = edit_cursor_move (edit, 1))));
	    if(c >= 0)
		edit_cursor_move (edit, -1);
	    edit->force |= REDRAW_PAGE;
	} else
...
static void edit_tab_cmd (WEdit * edit)
{
	...
	return;
				...
				/*insert a half tab (usually four spaces) unless there is a
				   ...
				   full tab. This needs some additions (FIXME) */
...
/* this does an auto indent, and also shows how you can use macros to do interesting things */
static void edit_enter_with_indent (WEdit *edit)
{
    struct macro et[21] = {{CK_Return, -1}, {-1, 'O'}, {CK_Return, -1}, {CK_Word_Left, -1},
	{CK_Word_Left, -1}, {CK_Home, -1}, {-1, ' '},
	{CK_Home, -1}, {CK_Mark, -1}, {CK_Word_Right, -1},
	{CK_Mark, -1}, {CK_Home, -1}, {CK_Delete, -1},
	{CK_End, -1}, {CK_Word_Right, -1}, {CK_Copy, -1},
	{CK_Mark, -1}, {CK_Mark, -1}, {CK_End, -1},
	{CK_BackSpace, -1}, {CK_Delete, -1}};

    edit_execute_macro (edit, et, 21);
}
...
int edit_translate_key (WEdit * edit, long x_key, int x_state, int *cmd, int *ch)
{
    int command = -1;
...
/* This executes a command without checking for macro recording or macro commands */
/* it also does not register the command as a key press. This means if many of these
are called, a single undo command will undo all of them */
/* it also does not check for the Undo command */
int edit_execute_command (WEdit * edit, int command, int char_for_insertion)
{
    if (edit->found_len)
...
/* CK_Pipe_Block */
	...
	s[0] = command - 2000;
	s[1] = 0;
	if (edit_load_macro_cmd (edit, m, &nm, s))
...
/* either command or char_for_insertion must be passed as -1 */
void Cedit_execute_command (WEdit * edit, int command, int char_for_insertion)
{
    edit_execute_command (edit, command, char_for_insertion);
