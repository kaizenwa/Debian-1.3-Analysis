...
void edit_status (WEdit * edit)
{
    ...
    if(w < 0)
	w = 0;
    s = malloc (3 * w / 2 + 80); /* 80? well this was giving a segfault so I'm not taking chances */
    memset (s, ' ', w);
    if (w > 4) {
	i = w > 24 ? 18 : w - 6;
	i = i < 13 ? 13 : i;
	...
	i = strlen (s);
	s[i] = ' ';
	status_string (edit, s + i + 2, w - i - 2);
    } else {
	s[w] = 0;
    }
...
static void xprint_to_widget (Window win, long row, int start_col, float start_col_real, long end_col, unsigned short line[])
{
...
/* discard part of the line that is the same on the screen as the cache remembers it: */
	for (i = 0, x = start_col; x <= end_col; x++, i++)
	    if (cache[x][row] != line[i])
		break;

	for (x = end_col; x >= start_col; line[(x--) - start_col] = 0)
	    if (cache[x][row] != line[x - start_col])
		break;
	start_col += i;
	line += i;
    }
	...
	if (style & (MOD_BOLD * 256))
	    fg = Ccolor (8);
/* print all characters with the same style (i.e. same foreground and background) at once: */
...
/* if we printed a cursor: */
	if (style & (MOD_CURSOR * 256)) {
	    Csetcolor (Ccolor (18));
	    Cline (win, x, y + 2, x, y + M_height - 1);    /* non focussed cursor form */
	    Cline (win, x + 1, y + 2, x + M_width - 1, y + 2);
	    cursor_found = 1;
...
void edit_render_tidbits (CWidget * widget)
{
    ...
    XGetInputFocus (CDisplay, &win, &isfocussed);
