...
/*
   ...
   Almost anything can go into this file. The code below is an example that
   may by appended or modified by the user.
 */
...
/* ordinary translations. (Some of this may be redundant.) Note that keys listed
    ...
    {OUR_BACKSPACE_KEY, CK_BackSpace, OUR_DELETE_KEY, CK_Delete, '\n', CK_Return,
     ...
     KEY_RIGHT, CK_Right, KEY_UP, CK_Up, KEY_DOWN, CK_Down, ALT('\t'), CK_Enter, ALT('\n'), CK_Enter,
     ...
     XCTRL ('P'), /*spell check */ CK_Pipe_Block (1), XCTRL ('p'),
     CK_Pipe_Block (1), KEY_F (15), CK_Insert_File, 
     ...
     KEY_F (17), CK_Find_Again, XCTRL ('d'), CK_Date, 0, 0};
...
#define DEFAULT_NUM_LOCK        0
    ...
    if(x_key == XCTRL ('q')) {
	char_for_insertion = edit_raw_key_query (" Insert Literal ", " Press any key: ", 0);
	goto fin;
    }
