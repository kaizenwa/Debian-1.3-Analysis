...
/* what constitutes a whole word? */
char *whole_chars = "0123456789abcdefghijklmnopqrstuvwxyz_";
...
{
    char *exp = Cgetfile (edit->widget->winid, 40, 40, edit->dir, edit->filename, " Save As ");
...
int edit_load_cmd (WEdit * edit)
{
    ...
    exp = Cgetfile (edit->widget->winid, 40, 40, edit->dir, edit->filename, " Load ");
...
void edit_search_replace_dialog (WEdit * edit, char **search_text, char **replace_text, char **arg_order, char *heading)
{
    Window parent, win;
    XEvent xev;
    CEvent cev;
    ...
    win = Cdrawheadedwindow ("replace", parent, 20, 20, R_W, height, heading, &h);
    h += 5;
    Cwidget ("replace")->position = CALWAYS_ON_TOP;
    Cdrawtext ("replace.t1", win, 10, h, " Enter search text : ");
    Cdrawtextinput ("replace.sinp", win, 10, h + 25, 335, 20, 256, *search_text);

    if (replace_text) {
	Cdrawtext ("replace.t2", win, 10, h + 55, " Enter replace text : ");
	Cdrawtextinput ("replace.rinp", win, 10, h + 80, 335, 20, 256, *replace_text);
	Cdrawtext ("replace.t3", win, 10, h + 110, " Enter argument order : ");
	Cdrawtextinput ("replace.ainp", win, 10, h + 135, 335, 20, 256, *arg_order);
    }
    if (!replace_text)
	h -= 110;
    Cdrawswitch ("replace.ww", win, 10, h + 165, C_BLACK, C_FLAT, replace_whole);
    Cdrawtext ("replace.t4", win, 48, h + 165 + 6, " Whole words only ");
    Cdrawswitch ("replace.case", win, 10, h + 205, C_BLACK, C_FLAT, replace_case);
    Cdrawtext ("replace.t5", win, 48, h + 205 + 6, " Case sensitive ");
    Cdrawswitch ("replace.reg", win, 10, h + 245, C_BLACK, C_FLAT, replace_regexp);
    Cdrawtext ("replace.t6", win, 48, h + 245 + 6, " Regular expression ");

    if (replace_text) {
	Cdrawswitch ("replace.pr", win, 10 + R_W / 2, h + 165, C_BLACK, C_FLAT, replace_prompt);
	Cdrawtext ("replace.t7", win, 48 + R_W / 2, h + 165 + 6, " Prompt on replace ");
	Cdrawswitch ("replace.all", win, 10 + R_W / 2, h + 205, C_BLACK, C_FLAT, replace_all);
	Cdrawtext ("replace.t8", win, 48 + R_W / 2, h + 205 + 6, " Replace all ");
    }
    Cdrawswitch ("replace.scanf", win, 10 + R_W / 2, h + 245, C_BLACK, C_FLAT, replace_scanf);
    Cdrawtext ("replace.t9", win, 48 + R_W / 2, h + 245 + 6, " Scanf expression ");

    if (!replace_text)
	h += 110;

    Cdrawbitmapbutton ("replace.ok", win, R_W - 60, h, 40, 40, Ccolor (6), C_FLAT, tick_bits);
    Cdrawbitmapbutton ("replace.cancel", win, R_W - 60, h + 60, 40, 40, Ccolor (18), C_FLAT, cross_bits);
    ...
    CRestoreState (&s);
    XSetInputFocus (CDisplay, wedit ? wedit->winid : CMain, RevertToNone, CurrentTime);
}
...
void edit_search_dialog (WEdit * edit, char **search_text)
{
    edit_search_replace_dialog (edit, search_text, 0, 0, " Search ");
}

void edit_replace_dialog (WEdit * edit, char **search_text, char **replace_text, char **arg_order)
{
    edit_search_replace_dialog (edit, search_text, replace_text, arg_order, " Replace ");
}
...
int string_regexp_search (WEdit * edit, char *pattern, char *string, int len, int match_type, int *found_len)
{
    static regex_t r;
...
long edit_find_string (WEdit * edit, long start, char *exp, int *len)
{
    long p, q;
    ...
    *len = l;
	...
	if (replace_case) {
	    for (p = start; p < edit->last_byte && p < start + MAX_REPL_LEN; p++)
		buf[p - start] = edit_get_byte (edit, p);
	} else {
	    for (p = start; p < edit->last_byte && p < start + MAX_REPL_LEN; p++) {
	...
	while (*buf) {
		...
		if (buf == mbuf || buf == mbuf + MAX_REPL_LEN / 2) {	/* search the window after it moves half its length */
		    int found_start =
		    string_regexp_search (edit, exp, buf, q, match_normal, len);
		    if (*len) {
			free (mbuf);
	    ...
	    if (q + start < edit->last_byte) {
		if (replace_case) {
		    buf[q] = edit_get_byte (edit, q + start);
		} else {
		    c = edit_get_byte (edit, q + start);
		    buf[q] = lcase (c);
		}
		q++;
	    }
	...
	if (replace_case) {
	    for (p = start; p <= edit->last_byte - l; p++) {
		if (edit_get_byte (edit, p) == exp[0]) {	/* check if first char matches */
		    for (f = 0, q = 0; q < l && f < 1; q++)
			if (edit_get_byte (edit, q + p) != exp[q])
			    f = 1;
		    if (f == 0)
			return p;
		}
	    ...
	    for (p = start; p <= edit->last_byte - l; p++) {
		if (lcase (edit_get_byte (edit, p)) == exp[0]) {
		    for (f = 0, q = 0; q < l && f < 1; q++)
			if (lcase (edit_get_byte (edit, q + p)) != exp[q])
			    f = 1;
		    if (f == 0)
			return p;
		}
...
long edit_find (WEdit * edit, char *exp, int *len)
{				/*front end to find_string to check for
				   whole words */
    long p;
    p = edit->search_start;

    while ((p = edit_find_string (edit, p, exp, len)) >= 0) {
	if (replace_whole) {
/*If the bordering chars are not in whole_chars then word is whole */
	    if (!strchr (whole_chars, lcase (edit_get_byte (edit, p - 1)))
		&& !strchr (whole_chars, lcase (edit_get_byte (edit, p + *len))))
		return p;
	} else
	    return p;
...
/*
   TODO: I wanted the user to be able to enter a scanf 'char *fmt, ...)' expression,
   and be able to replace that with a printf 'char *fmt, ...)' expression. This would
   give real power in search and replace.
 */
...
void edit_replace_cmd (WEdit * edit, int again)
{
    ...
    if (again) {		/*ctrl-hotkey for replace again. */
	if (!old1 || !old2)
	    return;
	exp1 = strdup (old1);
	exp2 = strdup (old2);
	...
	do {
	    int len = 0;
	    edit->search_start = edit_find (edit, exp1, &len);
	    /*returns negative on not found */
...
void edit_search_cmd (WEdit * edit, int again)
{
	...
	if (*exp) {
	    ...
	    edit->search_start = edit_find (edit, exp, &len);
...
void edit_quit_cmd (WEdit * edit)
{
    ...
    if (edit->modified) {
#ifdef MIDNIGHT
	switch (query_dialog (" Quit ", " File was modified, Save with exit? ", 0, 3, " Yes ", " No ", " Cancel quit ")) {
#else
	switch (Cquerydialogue (edit->widget->winid, 20, 20, " Quit ", " Current text was modified without a file save. \n Save with exit? ", " Yes ", " No ", " Cancel quit ", NULL)) {
#endif
	case 0:
	    edit_push_markers (edit);
	    edit_set_markers (edit, 0, 0);
...
void edit_goto_cmd (WEdit *edit)
{
    ...
    int l;
#ifdef MIDNIGHT
    f = input_dialog (" Goto line ", " Enter line: ", "");
#else
    f = Cinputdialog (edit->widget->winid, 20, 20, 150, "", " Goto line ", " Enter line: ");
#endif
    if (f) {
	if (*f) {
	    l = atoi (f) - 1;
	    edit_move_display (edit, l - edit->num_widget_lines / 2);
	    edit_move_to_line (edit, l);
	    free (f);
	}
    }
}
...
int edit_save_block_cmd (WEdit * edit) {
    ...
    exp = Cgetfile (edit->widget->winid, 40, 40, edit->dir, catstrs (home_dir, CLIP_FILE, 0), " Save Block ");
...
int edit_insert_file_cmd (WEdit * edit) {
    char *exp = Cgetfile (edit->widget->winid, 40, 40, edit->dir, catstrs (home_dir, CLIP_FILE, 0), " Insert File ");
    edit->force |= REDRAW_COMPLETELY;
...
#else
	CDisableAlarm ();
	system (catstrs ("/bin/sh ", home_dir, shell_cmd, 0));
	CEnableAlarm ();
#endif
    } else {
#ifdef MIDNIGHT
