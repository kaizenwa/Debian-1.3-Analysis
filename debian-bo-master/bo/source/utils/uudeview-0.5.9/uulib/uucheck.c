...
char * uucheck_id = "$Id: uucheck.c,v 1.8 1996/09/20 10:06:57 fp Exp $";
...
UUPreProcessPart (fileread *data, int *ret)
{
  ...
  if (data->flags & FL_SINGLE) {
    ...
    else if (data->uudet == B64ENCODED) {
      /*
       * exception for MIME-encoded files. Assume it's the first
       * part. I wonder why it's got no part number?
       */
      if (result->filename != NULL)
        result->subfname = _FP_strdup (result->filename);
      else { /* if not, escape to UNKNOWN. We need to fill subfname */
