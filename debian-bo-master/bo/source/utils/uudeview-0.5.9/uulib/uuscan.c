...
char * uuscan_id = "$Id: uuscan.c,v 1.18 1996/09/28 11:32:21 fp Exp $";
	  ...
	  fileread *result)
{
  ...
  int bhflag=0, vflag, nflag, haddh=0, hadct=0;
  ...
  *errcode = UURET_OK;
  ...
  while (!feof (datei)) {
	...
	*p2 = c;
    ...
    /*
     * Some reduced MIME handling. Only use if boundary == NULL.
     */
	...
	if (vflag == 0) {
	  if (UUNetscapeCollapse (line))
	    vflag = UUValidData (line, 0, &bhflag);
	}
      }
      /*
       * Base64 and XXencoded files are very hard to distinguish (their
       * alphabets differ in only one character). If we had a 'begin'
       * line, this line should be XX
       */
