 ...
 * cvs version
 */
char * uutcl_id = "$Id: uutcl.c,v 1.7 1996/07/11 20:31:46 fp Exp $";
	   ...
	   char *subject,  int isemail)
{
  ...
  if (towhom==NULL ||
    ...
    interp->result = "oops: Parameter check failed in SendAFile()";
    return UURET_ILLVAL;
  }

#ifndef HAVE_POPEN
  interp->result = "error: Your system does not support sending of files";
  return UURET_ILLVAL;
#else
  if (isemail && (uue_mailprog == NULL || *uue_mailprog == '\0')) {
    interp->result = "error: Cannot Email file: option not configured";
    return UURET_ILLVAL;
  }
  else if (!isemail && (uue_inewsprog == NULL || *uue_inewsprog == '\0')) {
    interp->result = "error: Cannot Post file: option not configured";
    return UURET_ILLVAL;
  }
  ...
  if ((command = (char *) malloc (len)) == NULL) {
    interp->result = "error: Out of memory allocating some bytes";
    return UURET_NOMEM;
  }
...
#endif
  if (argc==1 || uutcl_HaveArg (argc, argv, "tk_photo"))
#ifdef TK_PHOTO
    Tcl_AppendResult (interp, " { tk_photo 1 } ", NULL);
#else
    Tcl_AppendResult (interp, " { tk_photo 0 } ", NULL);
#endif
		  ...
		  int argc, char *argv[])
{
  ...
  if ((iter = UUGetFileListItem (atoi (argv[1]))) == NULL) {
    interp->result = "invalid file number";
    return TCL_ERROR;
  }
		   ...
		   int argc, char *argv[])
{
  ...
  if (iter->binfile == NULL) {
    interp->result = "unknown error while decoding";
    return TCL_ERROR;
  }
	      ...
	      int argc, char *argv[])
{
  ...
  if (*argv[2] == '\0') {
    interp->result = "illegal file name";
    return TCL_ERROR;
  }
  ...
  if (argv2 && argv2 < argc) {
    *linperfile = atoi (argv[argv2]);
    ...
    if (*linperfile != 0 && *linperfile < 200) {
      interp->result = "illegal number of lines per file";
      return TCL_ERROR;
    }
  }
  if (argv1 && argv1 < argc) {
    switch (atoi (argv[argv1])) {
    case 0:
      *encoding = UU_ENCODED;
      break;
    case 1:
	...
	*encoding = B64ENCODED;
	break;
      default:
	interp->result = "invalid encoding method";
	return TCL_ERROR;
      }
      break;
    }
 ...
 * Syntax:
 *    uu_EncodeToFile source path [ dest subject intro lines encoding ]
 */
		    ...
		    int argc, char *argv[])
{
  ...
  char errstring[256], olddir[256], newdir[256];
...
#if 0
  if ((res = UUEncodeFile (encoding, linperfile,
			   argv[1], argv[2],          /* source, path */
			   (argc>3) ? argv[3] : NULL, /* destname     */
			   (argc>5) ? argv[5] : NULL, /* introfile    */
			   (argc>4) ? argv[4] : NULL, /* subject      */
			   UUE_TOFILE, NULL)) != UURET_OK);
#endif
 ...
 * Syntax:
 *    uu_EncodeToMail source addr [ dest subject intro lines encoding ]
 *
...
#if 0
  if ((res = UUEncodeFile (encoding, linperfile,
			   argv[1], argv[2],          /* source, addr */
			   (argc>3) ? argv[3] : NULL, /* destname     */
			   (argc>5) ? argv[5] : NULL, /* introfile    */
			   (argc>4) ? argv[4] : NULL, /* subject      */
			   UUE_MAILTO, NULL)) != UURET_OK);
#endif
  if ((res = SendAFile (interp, NULL, argv[1], encoding, linperfile,
			(argc>3) ? argv[3] : NULL, argv[2],
			(argc>4) ? argv[4] : NULL, 1)) != UURET_OK) {
    /*
 ...
 * Syntax:
 *    uu_EncodeToNews source addr [ dest subject intro lines encoding ]
 *
...
#if 0
  if ((res = UUEncodeFile (encoding, linperfile,
			   argv[1], argv[2],          /* source, group */
			   (argc>3) ? argv[3] : NULL, /* destname      */
			   (argc>5) ? argv[5] : NULL, /* introfile     */
			   (argc>4) ? argv[4] : NULL, /* subject       */
			   UUE_POSTTO, NULL)) != UURET_OK)
#endif
  if ((res = SendAFile (interp, NULL, argv[1], encoding, linperfile,
			(argc>3) ? argv[3] : NULL, argv[2],
			(argc>4) ? argv[4] : NULL, 0)) != UURET_OK) {
    /*
     * If res==UURET_ILLVAL, SendAMail has already filled in the result
     */
    if (res != UURET_ILLVAL) {
...
Uu_Init (Tcl_Interp *interp)
{
  char tmp[32];
#ifdef TK_PHOTO
  extern int Gfp_Init (Tcl_Interp *);
#endif
  ...
  if (UUInitialize () != UURET_OK) {
    interp->result = "Error initializing decoding engine";
    return TCL_ERROR;
  }
...
#ifdef TK_PHOTO
  /*
   * Initialize the GFP Image format
   */
  (void) Gfp_Init (interp);
#endif
