 ...
 */
static
int compare_names(queueent *ent1, queueent *ent2)
{
 ...
 */
static
int compare_mtimes(queueent *ent1, queueent *ent2)
{
 ...
 */
static
int compare_ctimes(queueent *ent1, queueent *ent2)
{
 ...
 */
static
int compare_sizes(queueent *ent1, queueent *ent2)
{
  return reverse *
    ((ent2->dirent.dsize + ent2->dirent.rsize) -
     (ent1->dirent.dsize + ent1->dirent.rsize));
}
...
void sortfiles(darray *files, int flags, int options)
{
  ...
  darr_sort(files, (int (*)(void *, void *)) compare);
}
...
int outpath(dstring *str, queueent *ent, int flags)
{
  ...
  if (flags & (HLS_ESCAPE | HLS_QUOTE | HLS_QMARK_CTRL))
      ...
      for (ptr = path; *ptr; ++ptr)
	{
	  ...
	  if (dstr_append(str, add, -1) < 0)
	    return -1;
	}
    }
  else
    {
      if (dstr_append(str, path, -1) < 0)
	return -1;
    }
  ...
  if (flags & HLS_INDICATOR)
	...
	c = ':';
      else if (strcmp(ent->dirent.type, "APPL") == 0)
	c = '*';

      if (c && dstr_append(str, &c, 1) < 0)
	return -1;
    }
	       ...
	       int flags, int options, int width)
{
  ...
  for (i = 0; i < sz; ++i)
	...
	printf("%c%c %4s/%4s %9lu %9lu %s %s\n",
	       ...
	       ent->fdflags & HFS_FNDR_ISINVISIBLE ? 'i' : ' ',
	       ent->type, ent->creator, ent->rsize, ent->dsize,
	       timebuf + 4, strs[i]);
    }
}
...
int showfiles(darray *files, int flags, int options, int width)
{
  ...
  for (i = 0; i < sz; ++i)
    {
      if (outpath(&str, &ents[i], flags) < 0 ||
	  dl_append(&list, dstr_string(&str)) < 0)
	{
	  result = -1;
	  break;
	}
	    ...
	    int flags, int options, int width)
{
  ...
  if (fsz)
    {
      ...
      if (showfiles(files, flags, options, width) < 0)
	result = -1;
  ...
  for (i = 0; i < dsz; ++i)
      ...
      if (dir == 0)
	{
      ...
      while (hfs_readdir(dir, &ent.dirent) >= 0)
	{
	  ...
	  if ((ent.dirent.flags & HFS_ISDIR) && (flags & HLS_RECURSIVE))
		...
		result = -1;
	      ...
	      if (dstr_append(&str, ent.dirent.name, -1) < 0)
		result = -1;
...
int queuepath(hfsvol *vol, char *path, darray *dirs, darray *files, int flags)
{
  ...
  if (hfs_stat(vol, path, &ent.dirent) < 0)
    {
      hfs_perrorp(path);
...
# endif
  ...
  fargv = hfs_glob(vol, argc - optind, &argv[optind], &fargc);
  if (fargv == 0)
    {
  ...
  if (result == 0)
    {
      if (fargc == 0)
	{
	  if (queuepath(vol, ":", dirs, files, flags) < 0)
	    result = 1;
	}
      else
	{
	  for (i = 0; i < fargc; ++i)
	    {
	      if (queuepath(vol, fargv[i], dirs, files, flags) < 0)
		{
		  result = 1;
		  break;
		}
  ...
  if (result == 0 && process(vol, dirs, files, flags, options, width) < 0)
    result = 1;
