...
set gThinFont		{-*-helvetica-medium-r-normal-*-*-120-*-*-p-*-*-*}
set gMessageFont	{-*-helvetica-medium-r-normal-*-*-100-*-*-p-*-*-*}
set gTypeFont		{-*-lucidatypewriter-medium-r-*-*-*-120-*-*-m-*-*-*}
set gTitleFont		{-*-lucida-bold-i-normal-*-*-250-*-*-p-*-*-*}
...
proc do_rename {side} {
    ...
    set box $side.mid.lb
    set sel [$box curselection]
    assoc [lindex $dir($side) [lindex $sel 0]] item
...
proc help_about {} {
    ...
    label $f.copy -text $copyright -font $gThinFont
    label $f.email -text "E-Mail: [hfs author]" -font $gThinFont

    pack $f.banner $f.space $f.vers $f.copy $f.email -side top
    pack $f -expand 1
...
proc help_license {} {
    alert note "License" [fixtext [hfs license]] 190
}
...
proc ufs_automode {vol array} {
	...
	return binh
    } elseif {[regexp -nocase {\.(txt|c|h)$} $name]} {
	return text
...
proc show_file {vol fname} {
    ...
    after idle [list $f.text config -state disabled]
...
proc show_info {vol list} {
	...
	if {$ishfs && [string compare $info(type) "APPL"] == 0} {
	...
	while {$cwd != 1} {
	    set dirinfo [$vol dirinfo $cwd]
	    set where "[lindex $dirinfo 0]:$where"
	    set cwd [lindex $dirinfo 1]
	}
    } else {
	set where [$vol cwd]
    ...
    frame $f.name.space -width 5
    label $f.name.icon -bitmap $icon
    label $f.name.text -text $info(name) -font $gThinFont
    pack $f.name.space $f.name.icon $f.name.text -side left -padx 2m
...
proc makelist {w click other} {
    ...
    label $w.info.x.icon
    label $w.info.x.msg -font $gMessageFont -text ""
