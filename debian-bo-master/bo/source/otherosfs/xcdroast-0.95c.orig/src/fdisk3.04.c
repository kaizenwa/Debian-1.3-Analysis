 ...
 * version numbering at 3.0. Note that this is alpha software -
 * it works on my machine (with > 9 GB disks), but I have no MS/DOS,
 * DRDOS, OS/2, FreeBSD or Disk Manager or anything else that might
 * complicate matters.
 * [Now, two months later, no serious problems have been reported.]
 * I hope that with the help of some feedback this can evolve into a
 * program that can replace the original fdisk.
...
#define PNO(p) pnumber(p, z)
    ...
    if (partno < 4) {
	fatal("fdisk bug: only %d partitions defined.\n", partno);
	return 0;
    }
...
extended_partition(char *dev, int fd, struct part_desc *ep, struct disk_desc *z) {
	...
	p = (struct partition *) (s->data + 0x1be);
...
msdos_partition(char *dev, int fd, unsigned long start, struct disk_desc *z) {
    ...
    pt = (struct partition *) (s->data + 0x1be);
	...
	partitions[pno].offset = ((char *) pt) - s->data;
...
static void version(void) {
    printf("fdisk version " VERSION " (aeb@cwi.nl, " DATE ")\n");
}
...
static char short_opts[] = "cdfilnqsu:vx?1A::C:DH:I:LN:O:RS:TU::V";
