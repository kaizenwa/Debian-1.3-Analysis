...
static int f_read(Stream_t *Stream, char *buf, off_t where, size_t len)
{	
	return f_io(Stream, buf, where, len, (iofn) read);
}

static int f_write(Stream_t *Stream, char *buf, off_t where, size_t len)
{
...
#ifdef JPD
	printf("f_geom:media=%0X=>cyl=%d,heads=%d,sects=%d,ssize=%d,use2m=%X\n",
...
static int f_data(Stream_t *Stream, long *date, size_t *size,
...
/* ZIP or other scsi device on Solaris or SunOS system.
   ...
   or write the ZIP drive.  We thus replace the f_read and f_write routines
   with our own scsi_read and scsi_write routines, that use the uscsi ioctl
   interface.  By James Dugal, jpd@usl.edu, 11-96.  Tested under Solaris
   2.5 and SunOS 4.3.1_u1 using GCC.
...
{
   unsigned char cdb[6],buf[255];

   memset(&cdb, 0, sizeof cdb);
   memset(buf,0,255);
   cdb[0]=SCSI_MODE_SENSE;
   cdb[4]=255;
   if (scsi_cmd(fd, (unsigned char *)cdb, 6, SCSI_IO_READ, buf,255)==0)
   {
       scsi_sector_size=
	   ((unsigned)buf[9]<<16)|((unsigned)buf[10]<<8)|(unsigned)buf[11];
			 ...
			 const char *name, int mode, char *errmsg, int mode2)
{
	...
	fcntl(This->fd, F_SETFL, FD_CLOEXEC);

/* this didn't work on BeOS, so I just removed it */
#ifdef __BEOS__
	if(strcmp("/dev/floppy_disk",name))
#endif
