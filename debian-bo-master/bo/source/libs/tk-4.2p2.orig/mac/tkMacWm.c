 ...
 *
 * SCCS: @(#) tkMacWm.c 1.51 96/12/08 19:24:44
 */
				 ...
				 * be mapped. */
{
	 ...
	 */
	LocalToGlobal(&where);
	TkMacWindowOffset((WindowRef) macWin->portPtr, &xOffset, &yOffset);
	where.h -= xOffset;
	where.v -= yOffset;
    ...
    short zoomPart)		/* Either inZoomIn or inZoomOut */
{
	...
	if (zoomPart == inZoomIn) {
	    Rect zoomRect = qd.screenBits.bounds;
 ...
 * TkMacMakeMenuWindow --
 *
 *	Set the style bit for the passed in toplevel.  If the argument
 *	popup is true we make a plain window (same as overrideredirect).
 *	If popup is false we make a floating window.
 *
 * Results:
 *	None.
 *
...
TkMacMakeMenuWindow(
    ...
    int popup)			/* True if pop-up, false if tear-off. */
{
    if (popup) {
