    ...
    char **argv;		/* Argument strings. */
{
    ...
    /* (1.1) Find out where */
    if (Tix_TranslateIndex(wPtr, interp, argv[0], &at) != TCL_OK) {
    ...
    char **argv;		/* Argument strings. */
{
    ...
    if (Tix_TranslateIndex(wPtr, interp, argv[0], &index) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    ListEntry ** toPtr_ret;
{
    ...
    if (Tix_TranslateIndex(wPtr, interp, argv[0], &from) != TCL_OK) {
	return TCL_ERROR;
    }
    if (argc == 2) {
	if (Tix_TranslateIndex(wPtr, interp, argv[1], &to) != TCL_OK) {
	    return TCL_ERROR;
	}
    } else {
	to = from;
