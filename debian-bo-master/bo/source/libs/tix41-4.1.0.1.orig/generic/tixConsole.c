 ...
 * redistribution of the original file "tkConsole.c". These license
...
static Tcl_Interp *initInterp = NULL;
...
static int		ConsoleClose _ANSI_ARGS_((ClientData instanceData,
			    ...
			    Tcl_File outFile));
static void		TixConsolePrint _ANSI_ARGS_((
			    Tcl_Interp *interp, Tcl_File outFile,
 ...
 * 	channels.  All I/O will be discarded until TixConsoleInit is
    ...
    Tcl_Interp * interp;
{
    ...
    stdinChannel = Tcl_CreateChannel(&consoleChannelType, "console0",
	    inFile, NULL, (ClientData) NULL);
    if (stdinChannel != NULL) {
	Tcl_SetChannelOption(NULL, stdinChannel, "-translation", "lf");
	Tcl_SetChannelOption(NULL, stdinChannel, "-buffering", "none");
    }
    Tcl_SetStdChannel(stdinChannel, TCL_STDIN);

    stdoutChannel = Tcl_CreateChannel(&consoleChannelType, "console1",
    ...
    stderrChannel = Tcl_CreateChannel(&consoleChannelType, "console2",
    ...
    Tcl_Interp *interp;			/* Interpreter to use for prompting. */
{
    ...
    static int inited = 0;
    static char initCmd[] = "source $tix_library/Console.tcl";

    if (interp != initInterp) {
	Tcl_AppendResult(interp, "TixConsoleInit() and TixConsoleCreate() ",
	    "called from different interpreters", NULL);
	return TCL_ERROR;
    }
    ...
    if (stdinChannel) {
	Tcl_RegisterChannel(consoleInterp, stdinChannel);
    }
    if (stdoutChannel) {
	Tcl_RegisterChannel(consoleInterp, stdoutChannel);
    }
    if (stderrChannel) {
	Tcl_RegisterChannel(consoleInterp, stderrChannel);
    }
    ...
    if (Tix_Init(consoleInterp) != TCL_OK) {
	goto error;
    }
...
#ifdef _WINDOWS
	MessageBox(NULL, Tcl_GetVar(consoleInterp, "errorInfo",
	    TCL_GLOBAL_ONLY),"Error creating console", MB_OK|MB_ICONSTOP);
#else
	fprintf(stderr, "Error creating console: %s\n", consoleInterp->result);
#endif
    }
    return TCL_OK;
    ...
    int *errorCode;			/* Where to store error code. */
{
    *errorCode = 0;
    ...
    if (gStdoutInterp == NULL && initInterp != NULL) {
	TixConsoleInit(initInterp);
    }
    if (gStdoutInterp != NULL) {
	TixConsolePrint(gStdoutInterp, outFile, buf, toWrite);
    }
 ...
 *	main interpreter.  If the user or a running script causes the
 *	main window to be destroyed, then we need to inform the console
 *	interpreter by invoking "tixConsoleExit".
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	Invokes the "tixConsoleExit" procedure in the console interp.
 *
 *----------------------------------------------------------------------
 */
    ...
    XEvent *eventPtr;
{
	...
	Tcl_Eval(info->consoleInterp, "tixConsoleExit");
    }
}

/*
...
static void
TixConsolePrint(interp, outFile, buffer, size)
    ...
    long size;			/* Size of text buffer. */
{
    ...
    if (fd == 2) {
	cmd = "tixConsoleOutput stderr ";
    } else {
	cmd = "tixConsoleOutput stdout ";
    }
 ...
 * Tix_ConsoleInitCmd --
 *
 *	Tcl interface for creating a stand-alone console window.
 *----------------------------------------------------------------------
 */

int Tix_ConsoleInitCmd(clientData, interp, argc, argv)
