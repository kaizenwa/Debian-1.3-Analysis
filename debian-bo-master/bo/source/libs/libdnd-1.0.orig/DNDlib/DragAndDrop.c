...
void DndSenderWarning(void);
void DndStartAction(Widget widget,
		    XtPointer data,
		    XEvent *event,
		    Boolean *p);
void DndDispatchEvent(Widget widget,
		      XtPointer data,
		      XEvent *event,
		      Boolean* p);
...
DndDispatchEvent(Widget widget,XtPointer data,XEvent *event,Boolean* p)
{
    ...
    if(!DndIsDropMessage(event)) return;
    /* Updates the "LastDropVersion" for our control */
    LastDropVersion=DndProtocolVersion(event);
...
{
    fprintf(stderr,"DND warning: DndSourceWidget called.\n"
