...
#endif
#ifndef _POSIX_THREADS_PRIORITY_SCHEDULING
#define _POSIX_THREADS_PRIORITY_SCHEDULING
#endif
...
/* This is the value this implementation supports.
   Just a hint, actually, since LinuxThreads will support arbitrarily many 
   threads, the only limit is the number of processes supported
   by the kernel. */
...
struct _pthread_queue {
  pthread_t head;               /* First element, or NULL if queue empty.  */
  pthread_t tail;               /* Last element, or NULL if queue empty.  */
};
...
#define PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP {0, 0, 0, 1, {0, 0}}
...
/* Guarantees that the initialization function INIT_ROUTINE will be called
