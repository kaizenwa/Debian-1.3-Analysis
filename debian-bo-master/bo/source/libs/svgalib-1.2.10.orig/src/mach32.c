...
static char vfifo8 = VFIFO8, vfifo16 = VFIFO16, vfifo24 = VFIFO24, vfifo32 = VFIFO32,
 mach32_apsiz = 0;
static char dac_override = 127, svga_clock = SVGA_CLOCK;
...
*mach32_modemixup;
static unsigned short mach32_apadd = 1, mach32_memcfg, mach32_ge_pitch,
 mach32_ge_off_h;
...
/*Populating database: */
	...
	printf("Using Mach32 driver 2.0b (%s apert%s (%s), %dK mem, DAC %d%s%s).\n",
	       (mach32_chiptype & MACH32_APERTURE) ?
...
/*Rate down clocks exceeding mem bandwidth */
	...
	if (__svgalib_vertrefresh.max < OFF_ALLOWANCE(vert)) {
	    if (verbose)
		puts(" (rejected, vsync too high)");
	    continue;
	}
	if (__svgalib_vertrefresh.min > ADD_ALLOWANCE(vert)) {
	    if (verbose)
...
static int mach32_setmode(int mode, int previous)
{
	...
	return vga_driverspecs.setmode(mode, previous);
...
static char *mach32_conf_commands[] =
{
    ...
    "setuplinear", "blit", "ramdac", "svgaclock", "vendor",
    NULL};
