...
static void et4000_setregs(const unsigned char regs[], int mode)
{
    ...
    port_out(0x36, CRT_I);
    port_out((port_in(CRT_D) & 0x40) | (regs[EXT + 6] & 0xbf), CRT_D);
    port_out(0x37, CRT_I);
    port_out((port_in(CRT_D) & 0xbb) | (regs[EXT + 7] & 0x44), CRT_D);
    port_out(0x3f, CRT_I);
    port_out(regs[EXT + 8], CRT_D);
...
static int et4000_setmode(int mode, int prv_mode)
{
	...
	__vga_hicolor(et4000_dac, STD_DAC);
	...
	case 1 << 15:
	    __vga_hicolor(et4000_dac, HI15_DAC);
	    break;
	case 1 << 16:
	    __vga_hicolor(et4000_dac, HI16_DAC);
	    break;
	case 1 << 24:
	    __vga_hicolor(et4000_dac, TC24_DAC);
	    break;
	}
    __vga_setregs(regs);
...
DriverSpecs et4000_driverspecs =
{
    et4000_saveregs,
    et4000_setregs,
    et4000_unlock,
...
static int et4000_init(int force, int par1, int par2)
{
	...
	if (regs != 0) {
	    et4000_modes = NULL;
	    __vga_readmodes(regs, &et4000_modes, &et4000_dac, &clocks[0]);
	    fclose(regs);
	} else
