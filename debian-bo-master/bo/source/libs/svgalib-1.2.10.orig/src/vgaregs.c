...
void setup_VGA_registers(unsigned char *moderegs, ModeTiming * modetiming,
			 ModeInfo * modeinfo)
{
    int i;
/* Sync Polarities */
...
void outGR(int index, unsigned char val)
{
    int v;
    v = ((int) val << 8) + index;
    outw(GRA_I, v);
...
void outbGR(int index, unsigned char val)
{
    outb(GRA_I, index);
    outb(GRA_D, val);
}

void outCR(int index, unsigned char val)
{
    int v;
    v = ((int) val << 8) + index;
    outw(CRT_IC, v);
...
void outbCR(int index, unsigned char val)
{
    outb(CRT_IC, index);
    outb(CRT_D, val);
}

void outSR(int index, unsigned char val)
{
    int v;
    v = ((int) val << 8) + index;
    outw(SEQ_I, v);
...
void outbSR(int index, unsigned char val)
{
    outb(SEQ_I, index);
    outb(SEQ_D, val);
}

unsigned char inGR(int index)
{
    outb(GRA_I, index);
    return inb(GRA_D);
}

unsigned char inCR(int index)
{
    outb(CRT_IC, index);
    return inb(CRT_DC);
}

unsigned char inSR(int index)
{
    outb(SEQ_I, index);
    return inb(SEQ_D);
}
