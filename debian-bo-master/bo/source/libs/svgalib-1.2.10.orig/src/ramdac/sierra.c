...
void Sierra_32K_savestate(unsigned char *regs)
{
    _ramdac_dactocomm();
    regs[0] = inb(PEL_MSK);
}

void Sierra_32K_restorestate(const unsigned char *regs)
{
    _ramdac_dactocomm();
    outb(PEL_MSK, regs[0]);
}
...
{
    dacspeed = _setDacSpeed(dacspeed, 80000);
...
DacMethods Sierra_32K_methods =
{
    ...
    Sierra_32K_savestate,
    Sierra_32K_restorestate,
...
static int SC1148X_probe(void)
{
    ...
    if (tmp != 0x8E) {
		...
		SC1148X_methods.id = SIERRA_32K;
		SC1148X_methods.name = Sierra_32K_methods.name;
		SC1148X_methods.initialize = Sierra_32K_methods.initialize;
		SC1148X_methods.qualifyCardSpecs = Sierra_32K_methods.qualifyCardSpecs ;
		SC1148X_methods.initializeState = Sierra_32K_methods.initializeState ;
#endif
	    }
	    _ramdac_dactocomm();
	    outb(PEL_MSK, oc);
