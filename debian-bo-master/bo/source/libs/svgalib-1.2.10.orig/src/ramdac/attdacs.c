...
int att20c490_map_clock(int bpp, int pixelclock)
{
    if (bpp == 16)
	return pixelclock * 2;
    if (bpp == 24)
...
int att20c490_map_horizontal_crtc(int bpp, int pixelclock, int htiming)
{
    if (bpp == 16)
	return htiming * 2;
    if (bpp == 24)
...
static void att20c490_qualify_cardspecs(CardSpecs * cardspecs, int dacspeed)
{
    ...
    cardspecs->mapClock = att20c490_map_clock;
    cardspecs->mapHorizontalCrtc = att20c490_map_horizontal_crtc;
}

DacMethods ATT20C490_methods =
{
    ATT20C490,
