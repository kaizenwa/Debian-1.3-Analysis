...
#define S3_LINEAR_MODE_BANKING_864
...
enum {
    ...
    S3_TRIO64, S3_866, S3_868, S3_968
};

static const char *s3_chipname[] =
{"911", "924", "801", "805", "928",
 "864", "964", "Trio32", "Trio64", "866", "868", "968"};
...
#define S3_8514_OFFSET	(S3_CR6D + 1)
...
static void s3_setlinear(int addr);
#endif
...
{
    outCR(0x39, 0x00);		/* Lock system control regs. */
    outCR(0x38, 0x00);		/* Lock special regs. */
}

static void s3_lock_enh(void)
{
    if (s3_chiptype > S3_911)
	outCR(0x40, inCR(0x40) & ~0x01);    /* Lock enhanced command regs. */
    s3_lock();
}
...
static void s3_unlock_enh(void)
{
    s3_unlock();
    if (s3_chiptype > S3_911)
	outCR(0x40, inCR(0x40) | 0x01);     /* Unlock enhanced command regs. */
}
...
static int s3_saveregs(unsigned char regs[])
{
    ...
    cr38 = inCR(0x38);
    outCR(0x38, 0x48);		/* unlock S3 VGA regs (CR30-CR3B) */

    cr39 = inCR(0x39);
    outCR(0x39, 0xA5);		/* unlock S3 system control (CR40-CR4F) */
    /* and extended regs (CR50-CR6D) */

    cr40 = inCR(0x40);		/* unlock enhanced regs */
    outCR(0x40, cr40 | 0x01);
	...
	bmax = 0x6D;
    for (b = 0x30; b <= bmax; b++)
	regs[EXT + b - 0x30] = inCR(b);

#if 0
    /* get S3 ext. SR registers */
    if (s3_chiptype >= S3_864) {
	regs[EXT + 0x78 - 0x30] = inSR(0x08);
	outSR(0x08, 0x06);	/* unlock extended seq regs */
	regs[EXT + 0x79 - 0x30] = inSR(0x09);
	regs[EXT + 0x7A - 0x30] = inSR(0x0A);
	regs[EXT + 0x7D - 0x30] = inSR(0x0D);
	outSR(0x08, regs[EXT + 0x78 - 0x30]);
    }
#endif
    ...
    outCR(0x40, regs[EXT + 0x40 - 0x30] = cr40);
    outCR(0x39, regs[EXT + 0x39 - 0x30] = cr39);
    outCR(0x38, regs[EXT + 0x38 - 0x30] = cr38);
#if 0
...
static void s3_setregs(const unsigned char regs[], int mode)
{
	...
	s3_setlinear(0);	/* make sure linear is off */
#endif
	...
	outCR(0x39, regs[S3_CR39]);
	outCR(0x38, regs[S3_CR38]);
    } else
	s3_lock_enh();
}
			    ...
			    ModeTiming * modetiming, ModeInfo * modeinfo)
{
	...
	if (s3_chiptype == S3_928)
	    /* ARI: Turn on CHAIN4 for 928, since setup_VGA_registers
							 initializes ModeX */
	...
	if (s3_chiptype >= S3_864 ||
	    ...
	    if (s3_memory < 2048 || s3_chiptype == S3_TRIO64)
		clock *= 2;
	...
	if (s3_chiptype >= S3_864) {
		...
		/* use alternate setpage() function */
		s3_driverspecs.setpage = s3_setpage864;
	    } else {
		/* doesn't work for 4bpp. */
		s3_driverspecs.setpage = s3_setpage;
	    }
	}
#endif
	moderegs[S3_CR59] = 0;
    ...
    if (clk_used == &I2061A_clockchip_methods &&
...
static int s3_setmode(int mode, int prv_mode)
{
	...
	res = vga_driverspecs.setmode(mode, prv_mode);
...
{
    outCR(0x13, (width >> 3));	/* lw3-lw11 */
    outCR(0x51, (width & 0x300) >> 4);	/* lw12-lw13 */
}
...
static void s3_setlinear(int addr)
{
	...
	m = s3_memory >> 10;	/* XXXX up to 4M */
	if (!m)
	    m = 1;
	outb(0x3d5, inb(0x3d5) | 0x10 | m);	/* enable linear mode */
	outCR(0x59, (addr >> 8) & 0x03);
	outCR(0x5A, (addr) & 0xFF);
    } else {
...
{
    /* printf("s3_linear(%d,0x%x) called\n", op, param); */
 ...
 */
	...
	if (strcasecmp(ptr, "ICD2061A") == 0 ||
	    ...
	    clk_used = &I2061A_clockchip_methods;
	clk_used->initialize(cardspecs, dac_used);
	ptr = strtok(NULL, " \t");
	if (ptr != NULL) {
	    freq = atof(ptr) * 1000L;
...
static int s3_init(int force, int par1, int par2)
{
	...
	s3_chiptype = -1;
	config = inCR(0x36);	/* get configuration info */
	switch (id & 0xf0) {
	case 0x80:
	    if (rev == 1) {
		s3_chiptype = S3_911;
	...
	if (s3_chiptype > S3_864) {
	    printf("svgalib: s3: chipsets newer than S3-864 is not supported well yet.\n");
	}
    }
