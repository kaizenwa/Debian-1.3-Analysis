...
/*To avoid overflow with 32 bit longs, we count the bits:*/

/*(FIXME:) There may be problems if yt overflows, this just checks if the point
 is behind the cam:*/
    if((xt = (x * s->a11 + y * s->a12 + z * s->a13 + s->s_cam) >> 16) < 1) {
	scr->x = scr->y = 32767; /*line and triangle routines must 
				    reject these values.*/
	return;
    }
    yt = (x * s->a21 + y * s->a22 + z * s->a23) >> 16;
    zt = (x * s->a31 + y * s->a32 + z * s->a33) >> 16;

    scr->x = (long) s->posx + yt * s->xscale / xt;
    scr->y = (long) s->posy - zt * s->yscale / xt;

#else
/*(FIXME:) There may be problems if yt overflows, this just checks if the point
 is behind the cam:*/
    if((xt = x * s->a11 + y * s->a12 + z * s->a13 + s->s_cam) < 1) {
	scr->x = scr->y = 32767; /*line and triangle routines must 
				    reject these values.*/
	return;
    }
    yt = x * s->a21 + y * s->a22 + z * s->a23;
    zt = x * s->a31 + y * s->a32 + z * s->a33;

    scr->x = s->posx + muldiv64 (yt, s->xscale, xt);
    scr->y = s->posy - muldiv64 (zt, s->yscale, xt);

#endif

}

/*--> I wonder if those were worth the effort.*/
...
long TD_finddistance (TD_Solid * s, TD_Point * p)
{
    ...
    return (x * s->a11 + y * s->a12 + z * s->a13 + s->s_cam) >> 16;
}
...
/*
drawtoolpath(struct identitiestype *ident)
{
int tx,ty,tz;
int j;
int px2,py2,px3,py3;
struct ptemptype scr;
if(nocnc==1) return(0);

        tx=(*toolpoints)[0].x / maxcoord * MULSHIFT;
        ty=(*toolpoints)[0].y / maxcoord * MULSHIFT;
        tz=(*toolpoints)[0].z / maxcoord * MULSHIFT;
        
		translate(tx,ty,tz,ident,&scr);

        px2=scr.x;
        py2=scr.y;
        
        if(nmoves!=0) for(j=1;j<nmoves;j++)
        {
        px3=px2;
        py3=py2;

        tx=(*toolpoints)[j].x / maxcoord * MULSHIFT;
        ty=(*toolpoints)[j].y / maxcoord * MULSHIFT;
        tz=(*toolpoints)[j].z / maxcoord * MULSHIFT;
...
{
    int n = s->num_surfaces, w, l, i, render;
    long max = 0;

    struct disttype *sortarray;
