...
static char sccsid[] = "@(#) tkGrab.c 1.46 95/06/23 08:48:10";
...
void
TkInOutEvents(eventPtr, sourcePtr, destPtr, leaveType, enterType)
    ...
    int enterType;		/* Type of events to generate for windows
				 * being left (EnterNotify or FocusIn).  0
    ...
    XEvent *eventPtr;		/* Pointer to the trigger event. */
{
	...
	dispPtr->grabWinPtr = (TkWindow *) Tk_IdToWindow(
		grabEventPtr->event.xany.display,
				 ...
				 * needed. */
{
    ...
    for (winPtr2 = winPtr; winPtr2 != grabWinPtr;
	    winPtr2 = winPtr2->parentPtr) {
	if (winPtr2 == NULL) {
	    for (winPtr2 = grabWinPtr; winPtr2 != NULL;
		    winPtr2 = winPtr2->parentPtr) {
		if (winPtr2 == winPtr) {
