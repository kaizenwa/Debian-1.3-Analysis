...
*     .. Local Scalars ..
      ...
      REAL               ABSAKK, ALPHA, C, COLMAX, R1, R2, ROWMAX
      COMPLEX            S, T, ZDUM
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     .. External Subroutines ..
      EXTERNAL           CHER, CLAEV2, CROT, CSSCAL, CSWAP, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL CLAEV2( A( K-1, K-1 ), A( K-1, K ), A( K, K ), R1,
     $                      R2, C, S )
               R1 = ONE / R1
               R2 = ONE / R2
               CALL CROT( K-2, A( 1, K-1 ), 1, A( 1, K ), 1, C, S )
               CALL CHER( UPLO, K-2, -R1, A( 1, K-1 ), 1, A, LDA )
               CALL CHER( UPLO, K-2, -R2, A( 1, K ), 1, A, LDA )
*
*              Store U(k) and U(k-1) in columns k and k-1
*
               CALL CSSCAL( K-2, R1, A( 1, K-1 ), 1 )
               CALL CSSCAL( K-2, R2, A( 1, K ), 1 )
               CALL CROT( K-2, A( 1, K-1 ), 1, A( 1, K ), 1, C, -S )
            END IF
         END IF
*
...
*                 decomposition of D(k)
*
                  CALL CLAEV2( A( K, K ), CONJG( A( K+1, K ) ),
     $                         A( K+1, K+1 ), R1, R2, C, S )
                  R1 = ONE / R1
                  R2 = ONE / R2
                  CALL CROT( N-K-1, A( K+2, K ), 1, A( K+2, K+1 ), 1, C,
     $                       S )
                  CALL CHER( UPLO, N-K-1, -R1, A( K+2, K ), 1,
     $                       A( K+2, K+2 ), LDA )
                  CALL CHER( UPLO, N-K-1, -R2, A( K+2, K+1 ), 1,
     $                       A( K+2, K+2 ), LDA )
*
*                 Store L(k) and L(k+1) in columns k and k+1
*
                  CALL CSSCAL( N-K-1, R1, A( K+2, K ), 1 )
                  CALL CSSCAL( N-K-1, R2, A( K+2, K+1 ), 1 )
                  CALL CROT( N-K-1, A( K+2, K ), 1, A( K+2, K+1 ), 1, C,
     $                       -S )
               END IF
            END IF
