...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CLACRT, CLAESY, CSCAL, CSPR, CSWAP,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL CLAESY( AP( KC-1 ), AP( KC+K-2 ), AP( KC+K-1 ), R1,
     $                      R2, Z, C, S )
*
...
*
                  R1 = CONE / R1
                  R2 = CONE / R2
                  CALL CLACRT( K-2, AP( KNC ), 1, AP( KC ), 1, C, S )
                  CALL CSPR( UPLO, K-2, -R1, AP( KNC ), 1, AP )
                  CALL CSPR( UPLO, K-2, -R2, AP( KC ), 1, AP )
*
*                 Store the multipliers in columns K and K-1
*
                  CALL CSCAL( K-2, R1, AP( KNC ), 1 )
                  CALL CSCAL( K-2, R2, AP( KC ), 1 )
                  CALL CLACRT( K-2, AP( KNC ), 1, AP( KC ), 1, C, -S )
               ELSE
*
...
*
                     CALL CAXPY( J, -T1, AP( KNC ), 1, AP( JC ), 1 )
                     CALL CAXPY( J, -T2, AP( KC ), 1, AP( JC ), 1 )
*
...
*                 decomposition of D(k)
*
                  CALL CLAESY( AP( KC ), AP( KC+1 ), AP( KNC ), R1, R2,
     $                         Z, C, S )
*
                  IF( CABS1( Z ).NE.ZERO ) THEN
*
...
*
                     R1 = CONE / R1
                     R2 = CONE / R2
                     CALL CLACRT( N-K-1, AP( KC+2 ), 1, AP( KNC+1 ), 1,
     $                            C, S )
                     CALL CSPR( UPLO, N-K-1, -R1, AP( KC+2 ), 1,
     $                          AP( KNC+N-K ) )
                     CALL CSPR( UPLO, N-K-1, -R2, AP( KNC+1 ), 1,
     $                          AP( KNC+N-K ) )
*
*                    Store the multipliers in columns K and K+1
*
                     CALL CSCAL( N-K-1, R1, AP( KC+2 ), 1 )
                     CALL CSCAL( N-K-1, R2, AP( KNC+1 ), 1 )
                     CALL CLACRT( N-K-1, AP( KC+2 ), 1, AP( KNC+1 ), 1,
     $                            C, -S )
                  ELSE
*
...
*                       Update column j of A
*
                        CALL CAXPY( N-J+1, -T1, AP( KC+J-K ), 1,
     $                              AP( JC ), 1 )
                        CALL CAXPY( N-J+1, -T2, AP( KNC+J-( K+1 ) ), 1,
     $                              AP( JC ), 1 )
*
*                       Store the multipliers in columns K and K+1
*
