...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, ZLACRT, ZLAESY, ZSCAL, ZSWAP,
     $                   ZSYR
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL ZLAESY( A( K-1, K-1 ), A( K-1, K ), A( K, K ), R1,
     $                      R2, Z, C, S )
*
...
*
                  R1 = CONE / R1
                  R2 = CONE / R2
                  CALL ZLACRT( K-2, A( 1, K-1 ), 1, A( 1, K ), 1, C, S )
                  CALL ZSYR( UPLO, K-2, -R1, A( 1, K-1 ), 1, A, LDA )
                  CALL ZSYR( UPLO, K-2, -R2, A( 1, K ), 1, A, LDA )
*
*                 Store U(k) and U(k-1) in columns k and k-1
*
                  CALL ZSCAL( K-2, R1, A( 1, K-1 ), 1 )
                  CALL ZSCAL( K-2, R2, A( 1, K ), 1 )
                  CALL ZLACRT( K-2, A( 1, K-1 ), 1, A( 1, K ), 1, C,
     $                         -S )
               ELSE
*
...
*                 decomposition of D(k)
*
                  CALL ZLAESY( A( K, K ), A( K+1, K ), A( K+1, K+1 ),
     $                         R1, R2, Z, C, S )
*
                  IF( CABS1( Z ).NE.ZERO ) THEN
*
...
*
                     R1 = CONE / R1
                     R2 = CONE / R2
                     CALL ZLACRT( N-K-1, A( K+2, K ), 1, A( K+2, K+1 ),
     $                            1, C, S )
                     CALL ZSYR( UPLO, N-K-1, -R1, A( K+2, K ), 1,
     $                          A( K+2, K+2 ), LDA )
                     CALL ZSYR( UPLO, N-K-1, -R2, A( K+2, K+1 ), 1,
     $                          A( K+2, K+2 ), LDA )
*
...
*                       Update column j of A
*
                        CALL ZAXPY( N-J+1, -T1, A( J, K ), 1, A( J, J ),
     $                              1 )
                        CALL ZAXPY( N-J+1, -T2, A( J, K+1 ), 1,
     $                              A( J, J ), 1 )
*
