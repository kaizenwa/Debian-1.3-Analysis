...
*     .. External Subroutines ..
      EXTERNAL           CHPR, CLAEV2, CROT, CSSCAL, CSWAP, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL CLAEV2( AP( KC-1 ), AP( KC+K-2 ), AP( KC+K-1 ), R1,
     $                      R2, C, S )
               R1 = ONE / R1
               R2 = ONE / R2
               CALL CROT( K-2, AP( KNC ), 1, AP( KC ), 1, C, S )
               CALL CHPR( UPLO, K-2, -R1, AP( KNC ), 1, AP )
               CALL CHPR( UPLO, K-2, -R2, AP( KC ), 1, AP )
*
*              Store U(k) and U(k-1) in columns k and k-1
*
               CALL CSSCAL( K-2, R1, AP( KNC ), 1 )
               CALL CSSCAL( K-2, R2, AP( KC ), 1 )
               CALL CROT( K-2, AP( KNC ), 1, AP( KC ), 1, C, -S )
            END IF
         END IF
*
...
*                 decomposition of D(k)
*
                  CALL CLAEV2( AP( KC ), CONJG( AP( KC+1 ) ), AP( KNC ),
     $                         R1, R2, C, S )
                  R1 = ONE / R1
                  R2 = ONE / R2
                  CALL CROT( N-K-1, AP( KC+2 ), 1, AP( KNC+1 ), 1, C,
     $                       S )
                  CALL CHPR( UPLO, N-K-1, -R1, AP( KC+2 ), 1,
     $                       AP( KNC+N-K ) )
                  CALL CHPR( UPLO, N-K-1, -R2, AP( KNC+1 ), 1,
     $                       AP( KNC+N-K ) )
*
*                 Store L(k) and L(k+1) in columns k and k+1
*
                  CALL CSSCAL( N-K-1, R1, AP( KC+2 ), 1 )
                  CALL CSSCAL( N-K-1, R2, AP( KNC+1 ), 1 )
                  CALL CROT( N-K-1, AP( KC+2 ), 1, AP( KNC+1 ), 1, C,
     $                       -S )
               END IF
            END IF
