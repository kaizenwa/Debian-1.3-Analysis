...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, ZLACRT, ZLAESY, ZSCAL, ZSPR,
     $                   ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL ZLAESY( AP( KC-1 ), AP( KC+K-2 ), AP( KC+K-1 ), R1,
     $                      R2, Z, C, S )
*
...
*
                  R1 = CONE / R1
                  R2 = CONE / R2
                  CALL ZLACRT( K-2, AP( KNC ), 1, AP( KC ), 1, C, S )
                  CALL ZSPR( UPLO, K-2, -R1, AP( KNC ), 1, AP )
                  CALL ZSPR( UPLO, K-2, -R2, AP( KC ), 1, AP )
*
...
*                 decomposition of D(k)
*
                  CALL ZLAESY( AP( KC ), AP( KC+1 ), AP( KNC ), R1, R2,
     $                         Z, C, S )
*
                  IF( CABS1( Z ).NE.ZERO ) THEN
*
...
*
                     R1 = CONE / R1
                     R2 = CONE / R2
                     CALL ZLACRT( N-K-1, AP( KC+2 ), 1, AP( KNC+1 ), 1,
     $                            C, S )
                     CALL ZSPR( UPLO, N-K-1, -R1, AP( KC+2 ), 1,
     $                          AP( KNC+N-K ) )
                     CALL ZSPR( UPLO, N-K-1, -R2, AP( KNC+1 ), 1,
     $                          AP( KNC+N-K ) )
*
*                    Store the multipliers in columns K and K+1
*
...
*                       Update column j of A
*
                        CALL ZAXPY( N-J+1, -T1, AP( KC+J-K ), 1,
     $                              AP( JC ), 1 )
                        CALL ZAXPY( N-J+1, -T2, AP( KNC+J-( K+1 ) ), 1,
     $                              AP( JC ), 1 )
*
*                       Store the multipliers in columns K and K+1
*
