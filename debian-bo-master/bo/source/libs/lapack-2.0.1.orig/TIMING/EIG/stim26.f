     ...
     $                   E, TAUP, TAUQ, WORK, LWORK, LLWORK, TIMES,
     $                   LDT1, LDT2, LDT3, OPCNTS, LDO1, LDO2, LDO3,
     $                   INFO )
*
...
*          LINPACK:
*          12: SSVDC (singular values only) (comparable to 7 above)
*              (labeled LINSVD in the output)
*          13: SSVDC (singular values and min(M,N) left singular
*                     vectors) (comparable to 8 above)
*              (labeled LINSVD(l) in the output)
*          14: SSVDC (singular values and M left singular vectors)
*                     (comparable to 9 above)
*              (labeled LINSVD(L) in the output)
*          15: SSVDC (singular values and N right singular vectors)
*                     (comparable to 10 above)
*              (labeled LINSVD(R) in the output)
*          16: SSVDC (singular values and min(M,N) left singular
...
*     .. External Functions ..
      ...
      EXTERNAL           SLAMCH, SECOND, SLARND, SOPLA, SOPLA2
*     ..
*     .. External Subroutines ..
      EXTERNAL           ATIMIN, SBDSQR, SCOPY, SGEBRD, SLACPY, SLATMR,
     $                   SLATMS, SLASET, SORGBR, SPRTBV, SSVDC, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
  ...
  350             CONTINUE
                  ...
                  CALL SORGBR( 'Q', M, MINMN, KU, U, LDA, TAUQ,
     $                         WORK, LWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
...
*
  ...
  380             CONTINUE
                  CALL SLACPY( 'L', M, MINMN, H, LDH, U, LDA )
                  CALL SORGBR( 'Q', M, M, KU, U, LDA, TAUQ, WORK,
     $                         LWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
...
*
  ...
  410             CONTINUE
                  ...
                  CALL SORGBR( 'P', N, N, KVT, VT, LDA, TAUP,
     $                         WORK, LWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
...
*
     ...
     $               SOPLA2( 'SORGBR', 'Q', M, MINMN, KU, 0, NB ) +
     $               SOPLA2( 'SORGBR', 'P', N, N, KVT, 0, NB ) +
     $               OPCNTS( IPAR, ITYPE, IN, 1 ) +
