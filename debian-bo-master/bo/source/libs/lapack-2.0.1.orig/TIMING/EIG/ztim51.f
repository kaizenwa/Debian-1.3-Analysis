      SUBROUTINE ZTIM51( LINE, NSIZES, NN, NTYPES, DOTYPE, NPARMS, NNB,
...
*     .. Array Arguments ..
     ...
     $                   NSHFTS( * )
      DOUBLE PRECISION   AI( * ), AR( * ), BI( * ), BR( * ), HI( * ),
     $                   HR( * ), OPCNTS( LDO1, LDO2, LDO3, * ),
     $                   QI( * ), QR( * ), RWORK( * ), TI( * ),
     $                   TIMES( LDT1, LDT2, LDT3, * ), TR( * ), WR( * ),
     $                   ZI( * ), ZR( * )
      COMPLEX*16         A( * ), B( * ), H( * ), Q( * ), T( * ), W( * ),
     $                   WORK( * ), Z( * )
*     ..
*
*  Purpose
...
*     .. External Functions ..
      ...
      COMPLEX*16         ZLARND
      EXTERNAL           DLAMCH, DOPLA, DSECND, ZLARND
*     ..
*     .. External Subroutines ..
      EXTERNAL           ATIMIN, CQZHES, CQZVAL, CQZVEC, DLACPY, DLASET,
     $                   DPRTBG, XLAENV, ZHGEQZ, ZLACPY, ZLAQZH, ZLARFG,
     $                   ZLATM4, ZTGEVC, ZUNM2R
*     ..
*     .. Intrinsic Functions ..
...
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'ZGGHRD(N)', 'ZGGHRD(Q)', 'ZGGHRD(Z)',
     $                   'ZGGHRD(Q,Z)', 'ZHGEQZ(E)', 'ZHGEQZ(S)',
     $                   'ZHGEQZ(Q)', 'ZHGEQZ(Z)', 'ZHGEQZ(Q,Z)',
     $                   'ZTGEVC(L,A)', 'ZTGEVC(L,B)', 'ZTGEVC(R,A)',
     $                   'ZTGEVC(R,B)', 'CQZHES(F)', 'CQZHES(T)',
...
*
      CALL ATIMIN( 'ZHG', LINE, NSUBS, SUBNAM, TIMSUB, NOUT, INFO )
      IF( INFO.NE.0 )
     $   RETURN
*
...
*
               CALL ZLATM4( KATYPE( ITYPE ), N, 3, 1, .TRUE., ONE, ULP,
     $                      ONE, 2, ISEED, A, N1 )
               IF( 3.LE.N )
     $            A( 3+2*N1 ) = CONE
*
...
*
               CALL ZLATM4( 8, N, 3, 1, .FALSE., ONE, ONE, ONE, 2,
     $                      ISEED, B, N1 )
               IF( 2.LE.N )
     $            B( 2+N1 ) = CONE
*
...
*
                  DO 90 JC = 1, N - 1
                     IC = ( JC-1 )*N1
                     DO 80 JR = JC, N
                        Q( JR+IC ) = ZLARND( 3, ISEED )
                        Z( JR+IC ) = ZLARND( 3, ISEED )
   80                CONTINUE
                     CALL ZLARFG( N+1-JC, Q( JC+IC ), Q( JC+1+IC ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, DBLE( Q( JC+IC ) ) )
                     Q( JC+IC ) = CONE
                     CALL ZLARFG( N+1-JC, Z( JC+IC ), Z( JC+1+IC ), 1,
     $                            WORK( N+JC ) )
                     WORK( 3*N+JC ) = SIGN( ONE, DBLE( Z( JC+IC ) ) )
                     Z( JC+IC ) = CONE
   90             CONTINUE
                  IC = ( N-1 )*N1
...
*
                  DO 110 JC = 1, N
                     DO 100 JR = 1, N
                        A( JR+IC ) = WORK( 2*N+JR )*
     $                               DCONJG( WORK( 3*N+JC ) )*A( JR+IC )
                        B( JR+IC ) = WORK( 2*N+JR )*
     $                               DCONJG( WORK( 3*N+JC ) )*B( JR+IC )
  100                CONTINUE
  110             CONTINUE
                  CALL ZUNM2R( 'L', 'N', N, N, N-1, Q, N1, WORK, A, N1,
     $                         WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 120
                  CALL ZUNM2R( 'R', 'C', N, N, N-1, Z, N1, WORK( N+1 ),
     $                         A, N1, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 120
                  CALL ZUNM2R( 'L', 'N', N, N, N-1, Q, N1, WORK, B, N1,
     $                         WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 120
                  CALL ZUNM2R( 'R', 'C', N, N, N-1, Z, N1, WORK( N+1 ),
...
*
                     ...
                     S1 = DSECND( )
  140                CONTINUE
                     CALL ZLACPY( 'Full', N, N, A, N1, H, LDA )
                     CALL ZLACPY( 'Full', N, N, B, N1, T, LDA )
                     CALL ZLAQZH( .FALSE., .FALSE., N, 1, N, H, LDA, T,
...
*
                     S1 = DSECND( )
                     DO 150 J = 1, IC
                        CALL ZLACPY( 'Full', N, N, A, N1, Z, LDA )
                        CALL ZLACPY( 'Full', N, N, B, N1, Z, LDA )
  150                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
     ...
     $                  DOPLA( 'ZGEQRF', N, N, 0, 0, NB ) +
     $                  DOPLA( 'ZUNMQR', N, N, 0, 0, NB )
                     LDH = LDA
                  ELSE
  ...
  160          CONTINUE
            ELSE IF( RUNHRD ) THEN
               CALL ZLACPY( 'Full', N, N, A, N1, H, N1 )
               CALL ZLACPY( 'Full', N, N, B, N1, T, N1 )
               CALL ZLAQZH( .FALSE., .FALSE., N, 1, N, H, N1, T, N1, Q,
     $                      N1, Z, N1, WORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9997 )SUBNAM( 1 ), IINFO, N,
     $               ITYPE, 0, IOLDSD
...
*
  ...
  180                CONTINUE
                     CALL ZLACPY( 'Full', N, N, A, N1, H, LDA )
                     CALL ZLACPY( 'Full', N, N, B, N1, T, LDA )
                     CALL ZLAQZH( .TRUE., .FALSE., N, 1, N, H, LDA, T,
     $                            LDA, Q, LDA, Z, LDA, WORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
  ...
  220                CONTINUE
                     CALL ZLACPY( 'Full', N, N, A, N1, H, LDA )
                     CALL ZLACPY( 'Full', N, N, B, N1, T, LDA )
                     CALL ZLAQZH( .FALSE., .TRUE., N, 1, N, H, LDA, T,
     $                            LDA, Q, LDA, Z, LDA, WORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
  ...
  260                CONTINUE
                     CALL ZLACPY( 'Full', N, N, A, N1, H, LDA )
                     CALL ZLACPY( 'Full', N, N, B, N1, T, LDA )
                     CALL ZLAQZH( .TRUE., .TRUE., N, 1, N, H, LDA, T,
     $                            LDA, Q, LDA, Z, LDA, WORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
  ...
  300                CONTINUE
                     ...
                     CALL ZLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL ZHGEQZ( 'E', 'N', 'N', N, 1, N, A, LDA, B,
...
*
                     ...
                     DO 310 J = 1, IC
                        ...
                        CALL ZLACPY( 'Full', N, N, T, LDH, Z, LDA )
  310                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
...
*
  ...
  340                CONTINUE
                     CALL ZLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL ZLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL ZHGEQZ( 'S', 'N', 'N', N, 1, N, A, LDA, B,
     $                            LDA, W, W( LDA+1 ), Q, LDA, Z, LDA,
     $                            WORK, LWORK, RWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
  ...
  360          CONTINUE
            ELSE IF( RUNES ) THEN
               CALL ZLACPY( 'Full', N, N, H, LDH, A, N1 )
               CALL ZLACPY( 'Full', N, N, T, LDH, B, N1 )
               CALL ZHGEQZ( 'S', 'N', 'N', N, 1, N, A, N1, B, N1, W,
     $                      W( N1+1 ), Q, N1, Z, N1, WORK, LWORK, RWORK,
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9997 )SUBNAM( 6 ), IINFO, N,
     $               ITYPE, 0, IOLDSD
...
*
  ...
  380                CONTINUE
                     CALL ZLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL ZLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL ZHGEQZ( 'S', 'I', 'N', N, 1, N, A, LDA, B,
     $                            LDA, W, W( LDA+1 ), Q, LDA, Z, LDA,
     $                            WORK, LWORK, RWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
  ...
  400          CONTINUE
            ELSE IF( RUNEQ ) THEN
               CALL ZLACPY( 'Full', N, N, H, LDH, A, N1 )
               CALL ZLACPY( 'Full', N, N, T, LDH, B, N1 )
               CALL ZHGEQZ( 'S', 'I', 'N', N, 1, N, A, N1, B, N1, W,
     $                      W( N1+1 ), Q, N1, Z, N1, WORK, LWORK, RWORK,
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9997 )SUBNAM( 7 ), IINFO, N,
...
*
  ...
  420                CONTINUE
                     CALL ZLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL ZLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL ZHGEQZ( 'S', 'N', 'I', N, 1, N, A, LDA, B,
...
*
  ...
  460                CONTINUE
                     CALL ZLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL ZLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL ZHGEQZ( 'S', 'I', 'I', N, 1, N, A, LDA, B,
     $                            LDA, W, W( LDA+1 ), Q, LDA, Z, LDA,
     $                            WORK, LWORK, RWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 10 ) ) THEN
                        ...
                        S1 = DSECND( )
  550                   CONTINUE
                        CALL ZTGEVC( 'L', 'A', LLWORK, N, A, LDA, B,
...
*
                     IF( TIMSUB( 11 ) ) THEN
                        ...
                        S1 = DSECND( )
  560                   CONTINUE
                        CALL ZLACPY( 'Full', N, N, Q, LDQ, H, LDA )
                        CALL ZTGEVC( 'L', 'B', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 12 ) ) THEN
                        ...
                        S1 = DSECND( )
  580                   CONTINUE
                        CALL ZTGEVC( 'R', 'A', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 13 ) ) THEN
                        ...
                        S1 = DSECND( )
  590                   CONTINUE
                        CALL ZLACPY( 'Full', N, N, Q, LDQ, T, LDA )
                        CALL ZTGEVC( 'R', 'B', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                  DO 660 JC = 1, N
                     DO 650 JR = 1, N
                        H( JR+IC ) = WORK( 2*N+JR )*
     $                               DCONJG( WORK( 3*N+JC ) )*H( JR+IC )
                        T( JR+IC ) = WORK( 2*N+JR )*WORK( 3*N+JC )*
     $                               DCONJG( WORK( 3*N+JC ) )*T( JR+IC )
  650                CONTINUE
  660             CONTINUE
                  CALL ZUNM2R( 'L', 'N', N, N, N-1, Q, N1, WORK, H, N1,
     $                         WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 670
                  CALL ZUNM2R( 'R', 'C', N, N, N-1, Z, N1, WORK( N+1 ),
     $                         H, N1, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 670
                  CALL ZUNM2R( 'L', 'N', N, N, N-1, Q, N1, WORK, T, N1,
     $                         WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 670
                  CALL ZUNM2R( 'R', 'C', N, N, N-1, Z, N1, WORK( N+1 ),
...
*
                     ...
                     S1 = DSECND( )
  700                CONTINUE
                     CALL DLACPY( 'Full', N, N, AR, N1, HR, LDA )
                     CALL DLACPY( 'Full', N, N, AI, N1, HI, LDA )
                     CALL DLACPY( 'Full', N, N, BR, N1, TR, LDA )
                     CALL DLACPY( 'Full', N, N, BI, N1, TI, LDA )
...
*
                     S1 = DSECND( )
                     DO 710 J = 1, IC
                        CALL DLACPY( 'Full', N, N, AR, N1, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, AI, N1, ZI, LDA )
                        CALL DLACPY( 'Full', N, N, BR, N1, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, BI, N1, ZI, LDA )
  710                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
  ...
  720          CONTINUE
            ELSE IF( RUNHES ) THEN
               CALL DLACPY( 'Full', N, N, AR, N1, HR, N1 )
               CALL DLACPY( 'Full', N, N, AI, N1, HI, N1 )
               CALL DLACPY( 'Full', N, N, BR, N1, TR, N1 )
               CALL DLACPY( 'Full', N, N, BI, N1, TI, N1 )
...
*
                     ...
                     S1 = DSECND( )
  780                CONTINUE
                     CALL DLACPY( 'Full', N, N, HR, LDH, AR, LDA )
                     CALL DLACPY( 'Full', N, N, HI, LDH, AI, LDA )
                     CALL DLACPY( 'Full', N, N, TR, LDH, BR, LDA )
                     CALL DLACPY( 'Full', N, N, TI, LDH, BI, LDA )
...
*
                     S1 = DSECND( )
                     DO 790 J = 1, IC
                        CALL DLACPY( 'Full', N, N, HR, LDH, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, HI, LDH, ZI, LDA )
                        CALL DLACPY( 'Full', N, N, TR, LDH, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, TI, LDH, ZI, LDA )
  790                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
...
*
  ...
  820                CONTINUE
                     ...
                     CALL DLACPY( 'Full', N, N, TR, LDH, BR, LDA )
                     CALL DLACPY( 'Full', N, N, TI, LDH, BI, LDA )
                     CALL DLASET( 'Full', N, N, ZERO, ONE, QR, LDA )
                     CALL DLASET( 'Full', N, N, ZERO, ONE, QI, LDA )
                     CALL CQZVAL( LDA, N, AR, AI, BR, BI, ZERO, WR,
...
*
                     ...
                     DO 830 J = 1, IC
                        ...
                        CALL DLACPY( 'Full', N, N, TR, LDH, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, TI, LDH, ZI, LDA )
                        CALL DLASET( 'Full', N, N, ZERO, ONE, ZR, LDA )
                        CALL DLASET( 'Full', N, N, ZERO, ONE, ZI, LDA )
  830                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
  ...
  840          CONTINUE
            ELSE IF( RUNQZ ) THEN
               CALL DLACPY( 'Full', N, N, HR, LDH, AR, N1 )
               CALL DLACPY( 'Full', N, N, HI, LDH, AI, N1 )
               CALL DLACPY( 'Full', N, N, TR, LDH, BR, N1 )
               CALL DLACPY( 'Full', N, N, TI, LDH, BI, N1 )
               CALL DLASET( 'Full', N, N, ZERO, ONE, QR, N1 )
               CALL DLASET( 'Full', N, N, ZERO, ONE, QI, N1 )
...
*
                     ...
                     S1 = DSECND( )
  900                CONTINUE
                     CALL DLACPY( 'Full', N, N, AR, LDS, HR, LDA )
                     CALL DLACPY( 'Full', N, N, AI, LDS, HI, LDA )
                     CALL DLACPY( 'Full', N, N, BR, LDS, TR, LDA )
                     CALL DLACPY( 'Full', N, N, BI, LDS, TI, LDA )
                     CALL DLACPY( 'Full', N, N, QR, LDS, ZR, LDA )
                     CALL DLACPY( 'Full', N, N, QI, LDS, ZI, LDA )
...
*
                     S1 = DSECND( )
                     DO 910 J = 1, IC
                        CALL DLACPY( 'Full', N, N, AR, LDS, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, AI, LDS, ZI, LDA )
                        CALL DLACPY( 'Full', N, N, BR, LDS, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, BI, LDS, ZI, LDA )
                        CALL DLACPY( 'Full', N, N, QR, LDS, ZR, LDA )
                        CALL DLACPY( 'Full', N, N, QI, LDS, ZI, LDA )
  910                CONTINUE
                     S2 = DSECND( )
...
*     End of ZTIM51
*
 9997 FORMAT( ' ZTIM51: ', A, ' returned INFO=', I6, '.', / 9X, 'N=',
