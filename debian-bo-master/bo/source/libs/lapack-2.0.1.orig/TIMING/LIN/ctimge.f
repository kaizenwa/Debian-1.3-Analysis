      SUBROUTINE CTIMGE( LINE, NM, MVAL, NNS, NSVAL, NNB, NBVAL, NLDA,
...
*  =======
*
*  CTIMGE times CGETRF, -TRS, and -TRI.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, CGETRF, CGETRI, CGETRS, CLACPY,
...
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'CGETRF', 'CGETRS', 'CGETRI' /
*     ..
...
*
               IF( TIMSUB( 1 ) ) THEN
                  ...
                  S1 = SECOND( )
   10             CONTINUE
                  CALL CGETRF( M, N, A, LDA, IWORK, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
               ...
               END IF
*
*              Generate another matrix and factor it using CGETRF so
...
*
               IF( TIMSUB( 3 ) ) THEN
                  ...
                  S1 = SECOND( )
   30             CONTINUE
                  CALL CGETRI( M, B, LDA, IWORK, WORK, LDA*NB, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
...
*
            IF( TIMSUB( 2 ) ) THEN
               DO 80 I = 1, NNS
                  ...
                  S1 = SECOND( )
   60             CONTINUE
                  CALL CGETRS( 'No transpose', M, NRHS, A, LDA, IWORK,
     $                         B, LDB, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
