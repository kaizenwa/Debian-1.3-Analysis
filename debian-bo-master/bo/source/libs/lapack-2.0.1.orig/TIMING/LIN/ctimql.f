      SUBROUTINE CTIMQL( LINE, NM, MVAL, NVAL, NK, KVAL, NNB, NBVAL,
...
*  =======
*
*  CTIMQL times the LAPACK routines to perform the QL factorization of
...
*          The values of the matrix dimension K, used in CUNMQL.
*
*  NNB     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, CGEQLF, CLACPY, CLATMS, CTIMMG,
     $                   CUNGQL, CUNMQL, ICOPY, SPRTB4, SPRTB5, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN, REAL
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'CGEQLF', 'CUNGQL', 'CUNMQL' /
...
*
               IF( TIMSUB( 1 ) ) THEN
*
*                 CGEQLF:  QL factorization
*
                  CALL CLACPY( 'Full', M, N, B, LDA, A, LDA )
                  IC = 0
                  S1 = SECOND( )
   10             CONTINUE
                  CALL CGEQLF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  ...
                  OPS = SOPLA( 'CGEQLF', M, N, 0, 0, NB )
                  RESLTS( INB, IM, ILDA, 1 ) = SMFLOP( OPS, TIME, INFO )
               ELSE
*
*                 If CGEQLF was not timed, generate a matrix and factor
*                 it using CGEQLF anyway so that the factored form of
...
*
               IF( TIMSUB( 2 ) ) THEN
*
*                 CUNGQL:  Generate orthogonal matrix Q from the QL
*                 factorization
*
                  ...
                  S1 = SECOND( )
   30             CONTINUE
                  CALL CUNGQL( M, MINMN, MINMN, B, LDA, TAU, WORK, LW,
     $                         INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / REAL( IC )
                  OPS = SOPLA( 'CUNGQL', M, MINMN, MINMN, 0, NB )
...
*
   ...
   90 CONTINUE
*
*     Time CUNMQL separately.  Here the starting matrix is M by N, and
...
*
  ...
  100    CONTINUE
*
*        CUNMQL:  Multiply by Q stored as a product of elementary
*        transformations
*
*        Do for each pair of values (M,N):
*
...
*
                        ...
                        DO 130 ITRAN = 1, 2
                           ...
                           S1 = SECOND( )
  110                      CONTINUE
                           CALL CUNMQL( SIDE, TRANS, M1, N1, K1, A, LDA,
...
*
                           TIME = ( TIME-UNTIME ) / REAL( IC )
                           OPS = SOPLA( 'CUNMQL', M1, N1, K1, ISIDE-1,
     $                           NB )
                           RESLTS( INB, IM, ILDA,
