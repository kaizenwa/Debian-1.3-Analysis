      SUBROUTINE CTIMHE( LINE, NN, NVAL, NNS, NSVAL, NNB, NBVAL, NLDA,
...
*  =======
*
*  CTIMHE times CHETRF, -TRS, and -TRI.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, CHETRF, CHETRI, CHETRS, CLACPY,
...
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               SUBNAM / 'CHETRF', 'CHETRS', 'CHETRI' /
*     ..
...
*
                     ...
                     S1 = SECOND( )
   10                CONTINUE
                     CALL CHETRF( UPLO, N, A, LDA, IWORK, B, LWORK,
     $                            INFO )
                     S2 = SECOND( )
                     TIME = S2 - S1
                     IC = IC + 1
...
*
   30             CONTINUE
               ELSE
*
*                 If CHETRF was not timed, generate a matrix and
*                 factor it using CHETRF anyway so that the factored
...
*
                  ...
                  NB = 1
                  CALL XLAENV( 1, NB )
                  CALL CHETRF( UPLO, N, A, LDA, IWORK, B, LWORK, INFO )
               END IF
*
*              Time CHETRI
*
               IF( TIMSUB( 3 ) ) THEN
                  ...
                  S1 = SECOND( )
   40             CONTINUE
                  CALL CHETRI( UPLO, N, B, LDA, IWORK, WORK, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
...
*
               IF( TIMSUB( 2 ) ) THEN
                  DO 80 I = 1, NNS
                     ...
                     S1 = SECOND( )
   60                CONTINUE
                     CALL CHETRS( UPLO, N, NRHS, A, LDA, IWORK, B, LDB,
     $                            INFO )
                     S2 = SECOND( )
                     TIME = S2 - S1
                     IC = IC + 1
