      SUBROUTINE ZTIMRQ( LINE, NM, MVAL, NVAL, NK, KVAL, NNB, NBVAL,
...
*  =======
*
*  ZTIMRQ times the LAPACK routines to perform the RQ factorization of
...
*          The values of the matrix dimension K, used in ZUNMRQ.
*
*  NNB     (input) INTEGER
...
*     .. External Subroutines ..
     ...
     $                   ZGERQF, ZLACPY, ZLATMS, ZTIMMG, ZUNGRQ, ZUNMRQ
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MAX, MIN
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'ZGERQF', 'ZUNGRQ', 'ZUNMRQ' /
...
*
               IF( TIMSUB( 1 ) ) THEN
*
*                 ZGERQF:  RQ factorization
*
                  CALL ZLACPY( 'Full', M, N, B, LDA, A, LDA )
                  IC = 0
                  S1 = DSECND( )
   10             CONTINUE
                  CALL ZGERQF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  ...
                  OPS = DOPLA( 'ZGERQF', M, N, 0, 0, NB )
                  RESLTS( INB, IM, ILDA, 1 ) = DMFLOP( OPS, TIME, INFO )
               ELSE
*
*                 If ZGERQF was not timed, generate a matrix and factor
*                 it using ZGERQF anyway so that the factored form of
...
*
               IF( TIMSUB( 2 ) ) THEN
*
*                 ZUNGRQ:  Generate orthogonal matrix Q from the RQ
*                 factorization
*
                  CALL ZLACPY( 'Full', MINMN, N, A, LDA, B, LDA )
                  IC = 0
                  S1 = DSECND( )
   30             CONTINUE
                  CALL ZUNGRQ( MINMN, N, MINMN, B, LDA, TAU, WORK, LW,
     $                         INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / DBLE( IC )
                  OPS = DOPLA( 'ZUNGRQ', MINMN, N, MINMN, 0, NB )
...
*
   ...
   90 CONTINUE
*
*     Time ZUNMRQ separately.  Here the starting matrix is M by N, and
...
*
  ...
  100    CONTINUE
*
*        ZUNMRQ:  Multiply by Q stored as a product of elementary
*        transformations
*
*        Do for each pair of values (M,N):
*
...
*
                        ...
                        DO 130 ITRAN = 1, 2
                           ...
                           S1 = DSECND( )
  110                      CONTINUE
                           CALL ZUNMRQ( SIDE, TRANS, M1, N1, K1, A, LDA,
...
*
                           TIME = ( TIME-UNTIME ) / DBLE( IC )
                           OPS = DOPLA( 'ZUNMRQ', M1, N1, K1, ISIDE-1,
     $                           NB )
                           RESLTS( INB, IM, ILDA,
