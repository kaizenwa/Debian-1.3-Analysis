      SUBROUTINE ZTIMSP( LINE, NN, NVAL, NNS, NSVAL, LA, TIMMIN, A, B,
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, DPRTBL, ZCOPY, ZSPTRF, ZSPTRI,
     $                   ZSPTRS, ZTIMMG
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MOD
...
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               SUBNAM / 'ZSPTRF', 'ZSPTRS', 'ZSPTRI' /
*     ..
...
*
            IF( TIMSUB( 1 ) ) THEN
               ...
               S1 = DSECND( )
   10          CONTINUE
               CALL ZSPTRF( UPLO, N, A, IWORK, INFO )
               S2 = DSECND( )
               TIME = S2 - S1
               IC = IC + 1
...
*
            IF( TIMSUB( 3 ) ) THEN
               ...
               S1 = DSECND( )
   30          CONTINUE
               CALL ZSPTRI( UPLO, N, B, IWORK, WORK, INFO )
               S2 = DSECND( )
               TIME = S2 - S1
               IC = IC + 1
               IF( TIME.LT.TIMMIN ) THEN
...
*
     ...
     $               LDB = LDB + 1
                  ...
                  S1 = DSECND( )
   50             CONTINUE
                  CALL ZSPTRS( UPLO, N, NRHS, A, IWORK, B, LDB, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
