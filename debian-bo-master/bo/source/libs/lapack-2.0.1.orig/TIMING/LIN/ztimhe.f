      SUBROUTINE ZTIMHE( LINE, NN, NVAL, NNS, NSVAL, NNB, NBVAL, NLDA,
...
*  =======
*
*  ZTIMHE times ZHETRF, -TRS, and -TRI.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, DPRTBL, XLAENV, ZHETRF, ZHETRI,
     $                   ZHETRS, ZLACPY, ZTIMMG
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MAX
*     ..
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               SUBNAM / 'ZHETRF', 'ZHETRS', 'ZHETRI' /
*     ..
...
*
                     ...
                     S1 = DSECND( )
   10                CONTINUE
                     CALL ZHETRF( UPLO, N, A, LDA, IWORK, B, LWORK,
     $                            INFO )
                     S2 = DSECND( )
                     TIME = S2 - S1
                     IC = IC + 1
...
*
   30             CONTINUE
               ELSE
*
*                 If ZHETRF was not timed, generate a matrix and
*                 factor it using ZHETRF anyway so that the factored
...
*
                  ...
                  NB = 1
                  CALL XLAENV( 1, NB )
                  CALL ZHETRF( UPLO, N, A, LDA, IWORK, B, LWORK, INFO )
               END IF
*
*              Time ZHETRI
*
               IF( TIMSUB( 3 ) ) THEN
                  ...
                  S1 = DSECND( )
   40             CONTINUE
                  CALL ZHETRI( UPLO, N, B, LDA, IWORK, WORK, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
...
*
               IF( TIMSUB( 2 ) ) THEN
                  DO 80 I = 1, NNS
                     ...
                     S1 = DSECND( )
   60                CONTINUE
                     CALL ZHETRS( UPLO, N, NRHS, A, LDA, IWORK, B, LDB,
     $                            INFO )
                     S2 = DSECND( )
                     TIME = S2 - S1
                     IC = IC + 1
