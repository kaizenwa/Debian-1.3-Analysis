      SUBROUTINE ZTIMGT( LINE, NM, MVAL, NNS, NSVAL, NLDA, LDAVAL,
...
*  =======
*
*  ZTIMGT times ZGTTRF, -TRS, -SV, and -SL.
*
*  Arguments
*  =========
*
...
*     ..
*     .. External Functions ..
      DOUBLE PRECISION   DMFLOP, DSECND, SOPGB
      EXTERNAL           DMFLOP, DSECND, SOPGB
*     ..
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, DPRTBL, ZGTSL, ZGTSV, ZGTTRF,
     $                   ZGTTRS, ZTIMMG
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MAX
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'ZGTTRF', 'ZGTTRS', 'ZGTSV ',
     $                   'ZGTSL ' /
      DATA               TRANSS / 'N', 'T', 'C' /
*     ..
...
*
         IF( TIMSUB( 1 ) ) THEN
            CALL ZTIMMG( 14, M, M, A, 3*N, 0, 0 )
            IC = 0
            S1 = DSECND( )
   20       CONTINUE
            CALL ZGTTRF( M, A, A( N ), A( 2*N ), A( 3*N-2 ), IWORK,
     $                   INFO )
            S2 = DSECND( )
            TIME = S2 - S1
            IC = IC + 1
...
*
            ...
            OPS = SOPGB( 'ZGTTRF', M, M, 1, 1, IWORK )
...
*
         ...
         END IF
*
*        Generate another matrix and factor it using ZGTTRF so
            ...
            DO 80 ITRANS = 1, 3
               ...
               DO 70 ILDA = 1, NLDA
                  ...
                  DO 60 I = 1, NNS
                     ...
                     S1 = DSECND( )
   40                CONTINUE
                     CALL ZGTTRS( TRANS, M, NRHS, A, A( N ), A( 2*N ),
...
*
                     TIME = ( TIME-UNTIME ) / DBLE( IC )
                     OPS = SOPGB( 'ZGTTRS', M, NRHS, 0, 0, IWORK )
                     RESLTS( I, IM, ILDA, ISUB ) = DMFLOP( OPS, TIME,
     $                  INFO )
   60             CONTINUE
   70          CONTINUE
   80       CONTINUE
         ...
         IF( TIMSUB( 3 ) ) THEN
            DO 120 ILDA = 1, NLDA
               ...
               DO 110 I = 1, NNS
                  ...
                  S1 = DSECND( )
   90             CONTINUE
                  CALL ZGTSV( M, NRHS, A, A( N ), A( 2*N ), B, LDB,
     $                        INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / DBLE( IC )
                  OPS = SOPGB( 'ZGTSV ', M, NRHS, 0, 0, IWORK )
...
*
         IF( TIMSUB( 4 ) ) THEN
            ...
            S1 = DSECND( )
  130       CONTINUE
            CALL ZGTSL( M, A, A( N ), A( 2*N ), B, INFO )
            S2 = DSECND( )
            TIME = S2 - S1
            IC = IC + 1
            IF( TIME.LT.TIMMIN ) THEN
               CALL ZTIMMG( 14, M, M, A, 3*N, 0, 0 )
               CALL ZTIMMG( 0, M, 1, B, LDB, 0, 0 )
               GO TO 130
            END IF
*
*           Subtract the time used in ZTIMMG.
...
*
 ...
 9998 FORMAT( ' ZGTTRS with TRANS = ''', A1, '''', / )
