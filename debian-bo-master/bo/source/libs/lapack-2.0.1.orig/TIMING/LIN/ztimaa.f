...
*     .. Parameters ..
      ...
      PARAMETER          ( NMAX = 600, LDAMAX = NMAX+4, NMAXB = 5000 )
      INTEGER            LA
...
*     .. Local Arrays ..
     ...
     $                   NXVAL( MAXVAL )
      DOUBLE PRECISION   D( 2*NMAX ),
     $                   RESLTS( MAXVAL, MAXVAL, 2*MXNLDA, 4*MAXVAL )
      COMPLEX*16         A( LA, 3 ), E( 2*NMAX )
*     ..
*     .. External Functions ..
...
*     .. External Subroutines ..
      EXTERNAL           ZTIMB2, ZTIMB3, ZTIMBR, ZTIMGB, ZTIMGE, ZTIMGT,
     $                   ZTIMHE, ZTIMHP, ZTIMHR, ZTIMLQ, ZTIMMM, ZTIMMV,
     $                   ZTIMPB, ZTIMPO, ZTIMPP, ZTIMPT, ZTIMQL, ZTIMQP,
     $                   ZTIMQR, ZTIMRQ, ZTIMSP, ZTIMSY, ZTIMTB, ZTIMTD,
     $                   ZTIMTP, ZTIMTR
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
         ...
         IF( LSAMEN( 4, LINE, 'BAND' ) ) THEN
            IF( LDAMOK ) THEN
               ...
               ELSE
                  ...
                  J2 = 3
               END IF
               CALL ZTIMMV( 'ZGBMV ', NM, MVAL, NN, NVAL, NLDA, LDAVAL,
     $                      TIMMIN, A( 1, 1 ), MKMAX / 2, A( I2, J2 ),
     $                      A( LA-MKMAX / 2+1, 3 ), RESLTS, LDR1, LDR2,
     $                      NOUT )
            ELSE
               WRITE( NOUT, FMT = 9989 )'ZGBMV '
            END IF
            CALL ZTIMMM( 'ZGEMM ', 'K', NN, NVAL, NLDA, LDAVAL, TIMMIN,
...
*
            CALL ZTIMMV( 'ZGEMV ', NN, NVAL, NNB, NBVAL, NLDA, LDAVAL,
     $                   TIMMIN, A( 1, 1 ), LA, A( 1, 2 ), A( 1, 3 ),
     $                   RESLTS, LDR1, LDR2, NOUT )
            CALL ZTIMMM( 'ZGEMM ', 'N', NN, NVAL, NLDA, LDAVAL, TIMMIN,
     $                   A( 1, 1 ), A( 1, 2 ), A( 1, 3 ), RESLTS, LDR1,
     $                   LDR2, NOUT )
         END IF
      END IF
...
*        Level 2 BLAS
*
         CALL ZTIMB2( LINE, NM, MVAL, NN, NVAL, NK, KVAL, NNB, NBVAL,
...
*        Level 3 BLAS
*
         CALL ZTIMB3( LINE, NM, MVAL, NN, NVAL, NK, KVAL, NLDA, LDAVAL,
...
*        QR routines
*
         CALL ZTIMQR( LINE, NN, MVAL, NVAL, NK, KVAL, NNB, NBVAL, NXVAL,
...
*        LQ routines
*
         CALL ZTIMLQ( LINE, NN, MVAL, NVAL, NK, KVAL, NNB, NBVAL, NXVAL,
     $                NLDA, LDAVAL, TIMMIN, A( 1, 1 ), E, A( 1, 2 ),
     $                A( 1, 3 ), D, RESLTS, LDR1, LDR2, LDR3, NOUT )
*
...
*        QL routines
*
         CALL ZTIMQL( LINE, NN, MVAL, NVAL, NK, KVAL, NNB, NBVAL, NXVAL,
     $                NLDA, LDAVAL, TIMMIN, A( 1, 1 ), E, A( 1, 2 ),
     $                A( 1, 3 ), D, RESLTS, LDR1, LDR2, LDR3, NOUT )
*
...
*        RQ routines
*
         CALL ZTIMRQ( LINE, NN, MVAL, NVAL, NK, KVAL, NNB, NBVAL, NXVAL,
     $                NLDA, LDAVAL, TIMMIN, A( 1, 1 ), E, A( 1, 2 ),
     $                A( 1, 3 ), D, RESLTS, LDR1, LDR2, LDR3, NOUT )
*
...
*
         CALL ZTIMQP( LINE, NM, MVAL, NVAL, NLDA, LDAVAL, TIMMIN,
...
*
         CALL ZTIMHR( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NXVAL, NLDA,
...
*
         CALL ZTIMTD( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NXVAL, NLDA,
...
*
         CALL ZTIMBR( LINE, NN, MVAL, NVAL, NK, KVAL, NNB, NBVAL, NXVAL,
...
*
         CALL ZTIMGE( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NLDA,
...
*
         IF( LDAMOK ) THEN
            CALL ZTIMGB( LINE, NM, MVAL, NN, NVAL, NK, KVAL, NNB, NBVAL,
     $                   NLDA, LDAVAL, TIMMIN, A( 1, 1 ),
     $                   A( LA-MKMAX+1, 3 ), IWORK, RESLTS, LDR1, LDR2,
     $                   LDR3, NOUT )
         ELSE
            WRITE( NOUT, FMT = 9988 )LINE( 1: 6 )
         END IF
*
...
*
         CALL ZTIMGT( LINE, NN, NVAL, NK, KVAL, NLDA, LDAVAL, TIMMIN,
...
*
         CALL ZTIMPO( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NLDA,
...
*
         CALL ZTIMPP( LINE, NN, NVAL, NK, KVAL, LA, TIMMIN, A( 1, 1 ),
...
*
         IF( LDAMOK ) THEN
            ...
            ELSE
               ...
               I2 = LA - MKMAX + 1
            END IF
            CALL ZTIMPB( LINE, NM, MVAL, NN, NVAL, NK, KVAL, NNB, NBVAL,
...
*
         CALL ZTIMPT( LINE, NN, NVAL, NK, KVAL, NLDA, LDAVAL, TIMMIN, D,
...
*
         CALL ZTIMHE( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NLDA,
     $                LDAVAL, TIMMIN, A( 1, 1 ), A( 1, 2 ), A( 1, 3 ),
     $                IWORK, RESLTS, LDR1, LDR2, LDR3, NOUT )
*
...
*
         CALL ZTIMHP( LINE, NN, NVAL, NK, KVAL, LA, TIMMIN, A( 1, 1 ),
...
*
         CALL ZTIMSY( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NLDA,
     $                LDAVAL, TIMMIN, A( 1, 1 ), A( 1, 2 ), A( 1, 3 ),
     $                IWORK, RESLTS, LDR1, LDR2, LDR3, NOUT )
*
...
*
         CALL ZTIMSP( LINE, NN, NVAL, NK, KVAL, LA, TIMMIN, A( 1, 1 ),
     $                A( 1, 2 ), A( 1, 3 ), IWORK, RESLTS, LDR1, LDR2,
     $                LDR3, NOUT )
*
...
*        Triangular matrices
*
         CALL ZTIMTR( LINE, NN, NVAL, NK, KVAL, NNB, NBVAL, NLDA,
...
*
         CALL ZTIMTP( LINE, NN, NVAL, NK, KVAL, LA, TIMMIN, A( 1, 1 ),
...
*
         IF( LDAMOK ) THEN
            ...
            ELSE
               ...
               I2 = LA - MKMAX + 1
            END IF
            CALL ZTIMTB( LINE, NM, MVAL, NN, NVAL, NK, KVAL, NLDA,
...
*
 ...
 9983 FORMAT( ' LAPACK VERSION 2.0, released September 30, 1994 ', / )
*
*     End of ZTIMAA
*
      END
