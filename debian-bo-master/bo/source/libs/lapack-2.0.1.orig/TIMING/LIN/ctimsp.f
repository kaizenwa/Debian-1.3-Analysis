      SUBROUTINE CTIMSP( LINE, NN, NVAL, NNS, NSVAL, LA, TIMMIN, A, B,
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, CCOPY, CSPTRF, CSPTRI, CSPTRS,
     $                   CTIMMG, SPRTBL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD, REAL
...
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               SUBNAM / 'CSPTRF', 'CSPTRS', 'CSPTRI' /
*     ..
...
*
            IF( TIMSUB( 1 ) ) THEN
               ...
               S1 = SECOND( )
   10          CONTINUE
               CALL CSPTRF( UPLO, N, A, IWORK, INFO )
               S2 = SECOND( )
               TIME = S2 - S1
               IC = IC + 1
...
*
            IF( TIMSUB( 3 ) ) THEN
               ...
               S1 = SECOND( )
   30          CONTINUE
               CALL CSPTRI( UPLO, N, B, IWORK, WORK, INFO )
               S2 = SECOND( )
               TIME = S2 - S1
               IC = IC + 1
               IF( TIME.LT.TIMMIN ) THEN
...
*
     ...
     $               LDB = LDB + 1
                  ...
                  S1 = SECOND( )
   50             CONTINUE
                  CALL CSPTRS( UPLO, N, NRHS, A, IWORK, B, LDB, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
