      SUBROUTINE CTIMLQ( LINE, NM, MVAL, NVAL, NK, KVAL, NNB, NBVAL,
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, CGELQF, CLACPY, CLATMS, CTIMMG,
     $                   CUNGLQ, CUNMLQ, ICOPY, SPRTB4, SPRTB5, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN, REAL
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'CGELQF', 'CUNGLQ', 'CUNMLQ' /
...
*
                  ...
                  S1 = SECOND( )
   10             CONTINUE
                  CALL CGELQF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*                 factorization
*
                  CALL CLACPY( 'Full', MINMN, N, A, LDA, B, LDA )
                  IC = 0
                  S1 = SECOND( )
   30             CONTINUE
                  CALL CUNGLQ( MINMN, N, MINMN, B, LDA, TAU, WORK, LW,
     $                         INFO )
                  S2 = SECOND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / REAL( IC )
                  OPS = SOPLA( 'CUNGLQ', MINMN, N, MINMN, 0, NB )
...
*
                        ...
                        DO 130 ITRAN = 1, 2
                           ...
                           S1 = SECOND( )
  110                      CONTINUE
                           CALL CUNMLQ( SIDE, TRANS, M1, N1, K1, A, LDA,
...
*
                           TIME = ( TIME-UNTIME ) / REAL( IC )
                           OPS = SOPLA( 'CUNMLQ', M1, N1, K1, ISIDE-1,
     $                           NB )
                           RESLTS( INB, IM, ILDA,
