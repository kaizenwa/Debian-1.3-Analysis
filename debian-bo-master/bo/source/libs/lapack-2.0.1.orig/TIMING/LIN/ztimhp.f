      SUBROUTINE ZTIMHP( LINE, NN, NVAL, NNS, NSVAL, LA, TIMMIN, A, B,
...
*  =======
*
*  ZTIMHP times ZHPTRF, -TRS, and -TRI.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, DPRTBL, ZCOPY, ZHPTRF, ZHPTRI,
     $                   ZHPTRS, ZTIMMG
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MOD
...
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               SUBNAM / 'ZHPTRF', 'ZHPTRS', 'ZHPTRI' /
*     ..
...
*
            IF( TIMSUB( 1 ) ) THEN
               ...
               S1 = DSECND( )
   10          CONTINUE
               CALL ZHPTRF( UPLO, N, A, IWORK, INFO )
               S2 = DSECND( )
               TIME = S2 - S1
               IC = IC + 1
...
*
            ...
            END IF
*
*           Generate another matrix and factor it using ZHPTRF so
...
*
            IF( TIMSUB( 3 ) ) THEN
               CALL ZCOPY( LDA, A, 1, B, 1 )
               IC = 0
               S1 = DSECND( )
   30          CONTINUE
               CALL ZHPTRI( UPLO, N, B, IWORK, WORK, INFO )
               S2 = DSECND( )
               TIME = S2 - S1
               IC = IC + 1
               IF( TIME.LT.TIMMIN ) THEN
...
*
     ...
     $               LDB = LDB + 1
                  ...
                  S1 = DSECND( )
   50             CONTINUE
                  CALL ZHPTRS( UPLO, N, NRHS, A, IWORK, B, LDB, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
                  IF( TIME.LT.TIMMIN ) THEN
