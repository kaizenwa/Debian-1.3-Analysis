      SUBROUTINE ZTIMQL( LINE, NM, MVAL, NVAL, NK, KVAL, NNB, NBVAL,
...
*     .. External Subroutines ..
     ...
     $                   ZGEQLF, ZLACPY, ZLATMS, ZTIMMG, ZUNGQL, ZUNMQL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MAX, MIN
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'ZGEQLF', 'ZUNGQL', 'ZUNMQL' /
...
*
                  ...
                  S1 = DSECND( )
   10             CONTINUE
                  CALL ZGEQLF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*                 factorization
*
                  CALL ZLACPY( 'Full', M, MINMN, A, LDA, B, LDA )
                  IC = 0
                  S1 = DSECND( )
   30             CONTINUE
                  CALL ZUNGQL( M, MINMN, MINMN, B, LDA, TAU, WORK, LW,
     $                         INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / DBLE( IC )
                  OPS = DOPLA( 'ZUNGQL', M, MINMN, MINMN, 0, NB )
...
*
                        ...
                        DO 130 ITRAN = 1, 2
                           ...
                           S1 = DSECND( )
  110                      CONTINUE
                           CALL ZUNMQL( SIDE, TRANS, M1, N1, K1, A, LDA,
...
*
                           TIME = ( TIME-UNTIME ) / DBLE( IC )
                           OPS = DOPLA( 'ZUNMQL', M1, N1, K1, ISIDE-1,
     $                           NB )
                           RESLTS( INB, IM, ILDA,
