      SUBROUTINE ZTIMGB( LINE, NM, MVAL, NK, KVAL, NNS, NSVAL, NNB,
...
*     ..
*     .. External Functions ..
      DOUBLE PRECISION   DMFLOP, DOPLA, DSECND, SOPGB
      EXTERNAL           DMFLOP, DOPLA, DSECND, SOPGB
*     ..
*     .. External Subroutines ..
      EXTERNAL           ATIMCK, ATIMIN, DPRTBL, XLAENV, ZGBTRF, ZGBTRS,
     $                   ZTIMMG
*     ..
*     .. Intrinsic Functions ..
...
*
               IF( TIMSUB( 1 ) ) THEN
*
*                 Do for each value of NB in NBVAL.  Only ZGBTRF is
...
*
                  DO 30 INB = 1, NNB
                     ...
                     CALL XLAENV( 1, NB )
                     IC = 0
                     CALL ZTIMMG( 2, M, N, A, LDA, KL, KU )
                     S1 = DSECND( )
   10                CONTINUE
                     CALL ZGBTRF( M, N, KL, KU, A, LDA, IWORK, INFO )
                     S2 = DSECND( )
                     TIME = S2 - S1
                     IC = IC + 1
...
*
                     ...
                     OPS = SOPGB( 'ZGBTRF', M, N, KL, KU, IWORK )
   ...
   30             CONTINUE
               ...
               END IF
*
*              Generate another matrix and factor it using ZGBTRF so
...
*
               IF( TIMSUB( 2 ) ) THEN
                  DO 60 I = 1, NNS
                     ...
                     S1 = DSECND( )
   40                CONTINUE
                     CALL ZGBTRS( 'No transpose', N, KL, KU, NRHS, A,
...
*
                     TIME = ( TIME-UNTIME ) / DBLE( IC )
                     OPS = DOPLA( 'ZGBTRS', N, NRHS, KL, KU, 0 )
...
*
            IF( IM.EQ.1 .OR. CNAME.EQ.'ZGB   ' ) THEN
