...
*
      CALL ZLACPY( 'All', NROWS, NRHS, B, LDB, WORK, NROWS )
      CALL ZGEMM( TRANS, 'No transpose', NROWS, NRHS, NCOLS,
     $            DCMPLX( -ONE ), A, LDA, X, LDX, DCMPLX( ONE ), WORK,
     $            NROWS )
      NORMRS = ZLANGE( 'Max', NROWS, NRHS, WORK, NROWS, RWORK )
      IF( NORMRS.GT.SMLNUM ) THEN
         ISCL = 1
         CALL ZLASCL( 'General', 0, 0, NORMRS, ONE, NROWS, NRHS, WORK,
     $                NROWS, INFO )
      END IF
*
*     compute R'*A
*
     ...
     $            DCMPLX( ONE ), WORK, NROWS, A, LDA, DCMPLX( ZERO ),
     $            WORK( NROWS*NRHS+1 ), NRHS )
*
*     compute and properly scale error
*
      ERR = ZLANGE( 'One-norm', NRHS, NCOLS, WORK( NROWS*NRHS+1 ), NRHS,
     $      RWORK )
      IF( NORMA.NE.ZERO )
     $   ERR = ERR / NORMA
*
