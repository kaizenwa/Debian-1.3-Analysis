      SUBROUTINE CCHKGT( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, A, AF,
...
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'CGTTRS', INFO, 0, TRANS, N, N, -1,
...
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'CGTRFS', INFO, 0, TRANS, N, N, -1,
     $                         -1, NRHS, IMAT, NFAIL, NERRS, NOUT )
*
               CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                      RESULT( 4 ) )
...
*
     ...
     $                         AF( N+2*M ), IWORK, ANORM, RCOND, WORK,
     $                         INFO )
*
*                 Check error code from CGTCON.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'CGTCON', INFO, 0, NORM, N, N,
