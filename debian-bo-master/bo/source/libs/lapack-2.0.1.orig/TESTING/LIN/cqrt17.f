...
*
      CALL CLACPY( 'All', NROWS, NRHS, B, LDB, WORK, NROWS )
      CALL CGEMM( TRANS, 'No transpose', NROWS, NRHS, NCOLS,
     $            CMPLX( -ONE ), A, LDA, X, LDX, CMPLX( ONE ), WORK,
     $            NROWS )
      NORMRS = CLANGE( 'Max', NROWS, NRHS, WORK, NROWS, RWORK )
      IF( NORMRS.GT.SMLNUM ) THEN
         ISCL = 1
         CALL CLASCL( 'General', 0, 0, NORMRS, ONE, NROWS, NRHS, WORK,
     $                NROWS, INFO )
      END IF
*
*     compute R'*A
*
     ...
     $            CMPLX( ONE ), WORK, NROWS, A, LDA, CMPLX( ZERO ),
     $            WORK( NROWS*NRHS+1 ), NRHS )
*
*     compute and properly scale error
*
      ERR = CLANGE( 'One-norm', NRHS, NCOLS, WORK( NROWS*NRHS+1 ), NRHS,
     $      RWORK )
      IF( NORMA.NE.ZERO )
     $   ERR = ERR / NORMA
*
