...
*
      CALL SLACPY( 'All', NROWS, NRHS, B, LDB, WORK, NROWS )
      CALL SGEMM( TRANS, 'No transpose', NROWS, NRHS, NCOLS, -ONE, A,
     $            LDA, X, LDX, ONE, WORK, NROWS )
      NORMRS = SLANGE( 'Max', NROWS, NRHS, WORK, NROWS, RWORK )
      IF( NORMRS.GT.SMLNUM ) THEN
         ISCL = 1
         CALL SLASCL( 'General', 0, 0, NORMRS, ONE, NROWS, NRHS, WORK,
     $                NROWS, INFO )
      END IF
*
*     compute R'*A
*
      CALL SGEMM( 'Transpose', TRANS, NRHS, NCOLS, NROWS, ONE, WORK,
     $            NROWS, A, LDA, ZERO, WORK( NROWS*NRHS+1 ), NRHS )
*
*     compute and properly scale error
*
      ERR = SLANGE( 'One-norm', NRHS, NCOLS, WORK( NROWS*NRHS+1 ), NRHS,
     $      RWORK )
      IF( NORMA.NE.ZERO )
     $   ERR = ERR / NORMA
*
