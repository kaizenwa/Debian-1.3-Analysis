      SUBROUTINE DCHKPT( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, A, D,
...
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'DPTTRS', INFO, 0, ' ', N, N, -1, -1,
...
*
            SRNAMT = 'DPTRFS'
            CALL DPTRFS( N, NRHS, D, E, D( N+1 ), E( N+1 ), B, LDA, X,
     $                   LDA, RWORK, RWORK( NRHS+1 ), WORK, INFO )
*
*           Check error code from DPTRFS.
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'DPTRFS', INFO, 0, ' ', N, N, -1, -1,
     $                      NRHS, IMAT, NFAIL, NERRS, NOUT )
*
            CALL DGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                   RESULT( 4 ) )
