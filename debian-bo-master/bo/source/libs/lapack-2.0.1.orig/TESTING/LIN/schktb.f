      SUBROUTINE SCHKTB( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, NMAX,
     $                   AB, AINV, B, X, XACT, WORK, RWORK, IWORK,
     $                   NOUT )
*
...
*  =======
*
*  SCHKTB tests STBTRI, -TRS, -RFS, and -CON, and SLATBS.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3        PATH
      INTEGER            I, IDIAG, IK, IMAT, IN, INFO, ITRAN, IUPLO, J,
     $                   K, KD, LDA, LDAB, N, NERRS, NFAIL, NIMAT,
     $                   NIMAT2, NK, NRUN, NT
...
*     .. External Subroutines ..
     ...
     $                   SLACPY, SLARHS, SLATBS, SLATTB, SLASET, STBCON,
...
*
     ...
     $                        CALL ALAHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )UPLO, TRANS, N, KD,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
   40                CONTINUE
...
*
                     IF( RESULT( 7 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'SLATBS', UPLO, TRANS,
     $                     DIAG, 'N', N, KD, IMAT, RESULT( 7 )
                        NFAIL = NFAIL + 1
                     END IF
...
*
 9999 FORMAT( ' UPLO=''', A1, ''', TRANS=''', A1, ''', N=', I5, ', KD=',
     $      I5, ', type ', I2, ', test(', I2, ')=', G12.5 )
 9998 FORMAT( 1X, A6, '( ''', A1, ''', ''', A1, ''', ''', A1, ''', ''',
     $      A1, ''',', I5, ',', I5, ',  ... ), type ', I2, ', ratio =',
     $      G12.5 )
      RETURN
*
*     End of SCHKTB
