      SUBROUTINE SCHKPT( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, A, D,
...
*     .. Local Scalars ..
      ...
      CHARACTER          DIST, TYPE
      CHARACTER*3        PATH
      INTEGER            I, IA, IMAT, IN, INFO, IX, IZERO, J, K, KL, KU,
     $                   LDA, MODE, N, NERRS, NFAIL, NIMAT, NRUN
...
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'SPTTRS', INFO, 0, ' ', N, N, -1, -1,
...
*
            SRNAMT = 'SPTRFS'
            CALL SPTRFS( N, NRHS, D, E, D( N+1 ), E( N+1 ), B, LDA, X,
     $                   LDA, RWORK, RWORK( NRHS+1 ), WORK, INFO )
*
*           Check error code from SPTRFS.
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'SPTRFS', INFO, 0, ' ', N, N, -1, -1,
     $                      NRHS, IMAT, NFAIL, NERRS, NOUT )
*
            CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                   RESULT( 4 ) )
...
*
            DO 70 K = 2, 6
               IF( RESULT( K ).GE.THRESH ) THEN
                  IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $               CALL ALAHD( NOUT, PATH )
                  WRITE( NOUT, FMT = 9999 )N, IMAT, K, RESULT( K )
                  NFAIL = NFAIL + 1
               END IF
   70       CONTINUE
            NRUN = NRUN + 5
