...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION   A( LDA, * ), B( LDB, * ), BB( LDB, * ), D( * ),
     $                   RESULT( 14 ), U( * ), UZ( * ), V( LDU, * ),
     $                   WORK( * ), Z( LDU, * )
*     ..
*
*  Purpose
...
*     .. External Subroutines ..
     ...
     $                   DLATMS, DSBGV, DSGT01, DSPGV, DSYGV, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
                  NTEST = NTEST + 1
*
                  CALL DLACPY( ' ', N, N, A, LDA, Z, LDU )
                  CALL DLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL DSYGV( IBTYPE, 'V', UPLO, N, Z, LDU, BB, LDB, D,
     $                        WORK, NWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
...
*                 Do Test
*
                  CALL DSGT01( IBTYPE, UPLO, N, A, LDA, B, LDB, Z, LDU,
     $                         D, UZ, RESULT( NTEST ) )
*
   90             CONTINUE
                  NTEST = NTEST + 1
...
*
  ...
  130                CONTINUE
                  END IF
*
                  CALL DSPGV( IBTYPE, 'V', UPLO, N, UZ, U, D, Z, LDU,
     $                        WORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
...
*
  ...
  200                   CONTINUE
                     END IF
*
                     CALL DSBGV( 'V', UPLO, N, KA, KB, V, LDU, BB, LDB,
     $                           D, Z, LDU, WORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
...
*                    Do Test
*
                     CALL DSGT01( IBTYPE, UPLO, N, A, LDA, B, LDB, Z,
     $                            LDU, D, UZ, RESULT( NTEST ) )
                  END IF
*
  210          CONTINUE
  220       CONTINUE
