...
*     ..
*     .. Local Scalars ..
      LOGICAL            DBB, DBK, DBL, DES, DEV, DGG, DGK, DGL, DSB,
     $                   DSX, DVX, FATAL, GLM, GQR, GSV, LSE, NEP, SEP,
...
*     .. External Subroutines ..
     ...
     $                   DCKGLM, DCKGQR, DCKGSV, DCKLSE, DDRVBD, DDRVES,
     $                   DDRVEV, DDRVGG, DDRVSG, DDRVST, DDRVSX, DDRVVX,
     $                   DERRBD, DERRED, DERRGG, DERRHS, DERRST, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          LEN, MIN
*     ..
...
*
      IF( .NOT.( DEV .OR. DES .OR. DVX .OR. DSX ) .AND. NTYPES.LE.0 )
     $     THEN
...
*
  ...
  270          CONTINUE
     ...
     $            WRITE( NOUT, FMT = 9980 )'DCHKST', INFO
            END IF
            IF( TSTDRV ) THEN
               CALL DDRVST( NN, NVAL, MAXTYP, DOTYPE, ISEED, THRESH,
...
*
     ...
     $      CALL DERRBD( 'DBDSQR', NOUT )
         IF( TSTERR .AND. TSTDRV )
     $      CALL DERRED( 'DBDSQR', NOUT )
*
         DO 320 I = 1, NPARMS
            NRHS = NSVAL( I )
            CALL XLAENV( 1, NBVAL( I ) )
