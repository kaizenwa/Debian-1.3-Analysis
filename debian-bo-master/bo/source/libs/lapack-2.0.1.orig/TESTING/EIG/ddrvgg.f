...
*
                  IF( NERRS.EQ.0 ) THEN
                     WRITE( NOUNIT, FMT = 9994 )'DGG'
*
*                    Matrix types
*
                     WRITE( NOUNIT, FMT = 9993 )
                     WRITE( NOUNIT, FMT = 9992 )
                     WRITE( NOUNIT, FMT = 9991 )'Orthogonal'
*
*                    Tests performed
*
                     WRITE( NOUNIT, FMT = 9990 )'orthogonal', '''',
     $                  'transpose', ( '''', J = 1, 5 )
*
                  END IF
                  NERRS = NERRS + 1
...
*
 9996 FORMAT( ' DDRVGG: S not in Schur form at eigenvalue ', I6, '.',
     ...
     $      I5, ')' )
*
 9995 FORMAT( ' DDRVGG: Bad complex pair returned by DGEGV at ',
     $      'eigenvalue ', I6, '.', / 9X, 'N=', I6, ', JTYPE=', I6,
     $      ', ISEED=(', 3( I5, ',' ), I5, ')' )
*
 9994 FORMAT( / 1X, A3, ' -- Real Generalized eigenvalue problem driver'
     $       )
*
 9993 FORMAT( ' Matrix types (see DDRVGG for details): ' )
*
 9992 FORMAT( ' Special Matrices:', 23X,
 ...
 9991 FORMAT( ' Matrices Rotated by Random ', A, ' Matrices U, V:',
...
*
 9990 FORMAT( / ' Tests performed:  (S is Schur, T is triangular, ',
     $      'Q and Z are ', A, ',', / 20X,
