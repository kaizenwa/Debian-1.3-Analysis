...
*      Test SGEBRD, SORGBR, and SBDSQR
*      and the driver routine SGESVD
*
...
*      Test SSYGST, SSYGV, SSPGST, SSPGV, SSBGST, and SSBGV
*
...
*     .. Local Scalars ..
     ...
     $                   SEP, SES, SEV, SGG, SGK, SGL, SSB, SSX, SVD,
     $                   SVX, TSTCHK, TSTDIF, TSTDRV, TSTERR
      CHARACTER          C1
      CHARACTER*3        C3, PATH
      CHARACTER*6        VNAME
      CHARACTER*10       INTSTR
...
*     .. Local Arrays ..
     ...
     $                   D( NMAX, 12 ), RESULT( 105 ), TAUA( NMAX ),
     $                   TAUB( NMAX ), WORK( LWORK ), X( 5*NMAX )
*     ..
*     .. External Functions ..
      LOGICAL            LSAMEN
      REAL               SECOND, SLAMCH
...
*     .. External Subroutines ..
     ...
     $                   SCKGLM, SCKGQR, SCKGSV, SCKLSE, SDRVBD, SDRVES,
     $                   SDRVEV, SDRVGG, SDRVSG, SDRVST, SDRVSX, SDRVVX,
     $                   SERRBD, SERRED, SERRGG, SERRHS, SERRST, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          LEN, MIN
*     ..
...
*
      IF( .NOT.( SEV .OR. SES .OR. SVX .OR. SSX ) .AND. NTYPES.LE.0 )
     $     THEN
...
*
  ...
  270          CONTINUE
     ...
     $            WRITE( NOUT, FMT = 9980 )'SCHKST', INFO
            END IF
            IF( TSTDRV ) THEN
               CALL SDRVST( NN, NVAL, MAXTYP, DOTYPE, ISEED, THRESH,
     $                      NOUT, A( 1, 1 ), NMAX, D( 1, 3 ), D( 1, 4 ),
     $                      D( 1, 5 ), D( 1, 6 ), D( 1, 8 ), D( 1, 9 ),
     $                      D( 1, 10 ), A( 1, 2 ), NMAX, A( 1, 3 ),
     $                      D( 1, 12 ), A( 1, 4 ), WORK, LWORK, IWORK,
     $                      RESULT, INFO )
               IF( INFO.NE.0 )
...
*
     ...
     $      CALL SERRBD( 'SBDSQR', NOUT )
         IF( TSTERR .AND. TSTDRV )
     $      CALL SERRED( 'SBDSQR', NOUT )
*
         DO 320 I = 1, NPARMS
            NRHS = NSVAL( I )
            CALL XLAENV( 1, NBVAL( I ) )
