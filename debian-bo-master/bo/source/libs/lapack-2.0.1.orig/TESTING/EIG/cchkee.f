...
*      Test CHEGST, CHEGV, CHPGST, CHPGV, CHBGST, and CHBGV
*
...
*     ..
*     .. Local Scalars ..
      LOGICAL            CBB, CBK, CBL, CES, CEV, CGG, CGK, CGL, CHB,
     $                   CSX, CVX, FATAL, GLM, GQR, GSV, LSE, NEP, SEP,
     $                   SVD, TSTCHK, TSTDIF, TSTDRV, TSTERR
      CHARACTER          C1
      CHARACTER*3        C3, PATH
      CHARACTER*6        VNAME
      CHARACTER*10       INTSTR
...
*     .. Local Arrays ..
     ...
     $                   PVAL( MAXIN )
      REAL               ALPHA( NMAX ), BETA( NMAX ), DR( NMAX, 11 ),
     $                   RESULT( 105 ), RWORK( LWORK )
...
*     .. External Subroutines ..
     ...
     $                   CCKGLM, CCKGQR, CCKGSV, CCKLSE, CDRVBD, CDRVES,
     $                   CDRVEV, CDRVGG, CDRVSG, CDRVST, CDRVSX, CDRVVX,
     $                   CERRBD, CERRED, CERRGG, CERRHS, CERRST, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          LEN, MIN
*     ..
...
*        or CBB).
*
         IF( NEP .OR. SVD .OR. CBB .OR. CGG ) THEN
...
*
      IF( .NOT.( CEV .OR. CES .OR. CVX .OR. CSX ) .AND. NTYPES.LE.0 )
     $     THEN
...
*
  ...
  270          CONTINUE
     ...
     $            WRITE( NOUT, FMT = 9980 )'CCHKST', INFO
            END IF
            IF( TSTDRV ) THEN
               CALL CDRVST( NN, NVAL, MAXTYP, DOTYPE, ISEED, THRESH,
     $                      NOUT, A( 1, 1 ), NMAX, DR( 1, 3 ),
     $                      DR( 1, 4 ), DR( 1, 5 ), DR( 1, 8 ), 
     $                      DR( 1, 9 ), DR( 1, 10 ), A( 1, 2 ), NMAX,
     $                      A( 1, 3 ), DC( 1, 1 ), A( 1, 4 ), WORK,
...
*
     ...
     $      CALL CERRBD( 'CBDSQR', NOUT )
         IF( TSTERR .AND. TSTDRV )
     $      CALL CERRED( 'CBDSQR', NOUT )
*
         DO 320 I = 1, NPARMS
            NRHS = NSVAL( I )
            CALL XLAENV( 1, NBVAL( I ) )
...
*
 ...
 9973 FORMAT( / 1X, 71( '-' ) )
 9972 FORMAT( / ' LAPACK VERSION 2.0, released September 30, 1994 ' )
