     ...
     $                   SSAV, E, WORK, LWORK, RWORK, NOUNIT, INFO )
*
...
*  CGESVD factors A = U diag(S) VT, where U and VT are unitary
...
*     .. Local Scalars ..
      ...
      INTEGER            I, IINFO, IJU, IJVT, IWSPC, IWTMP, J, JSIZE,
     $                   JTYPE, LSWORK, M, MINWRK, MMAX, MNMAX, MNMIN,
     $                   MTYPES, N, NERRS, NFAIL, NMAX, NTEST, NTESTF,
     $                   NTESTT
...
*     .. External Subroutines ..
      EXTERNAL           ALASVM, CBDT01, CGESVD, CUNT03, CLACPY, CLATMS,
     $                   CLASET, CUNT01, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                      ANORM, M-1, N-1, 'N', A, LDA, WORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9995 )'Generator', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
            IWTMP = MAX( 3*MIN( M, N )+MAX( M, N ), 5*MIN( M, N )-4 )
            DO 110 IWSPC = 1, 4
...
*
     ...
     $                      VTSAV, LDVT, WORK, LSWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9994 )'GESVD', IINFO, M, N,
     $               JTYPE, LSWORK, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
 ...
 9995 FORMAT( ' CDRVBD: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
     ...
     $      I5, ')' )
 9994 FORMAT( ' CDRVBD: ', A, ' returned INFO=', I6, '.', / 9X, 'M=',
