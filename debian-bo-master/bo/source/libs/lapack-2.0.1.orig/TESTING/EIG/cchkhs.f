...
*     .. Local Scalars ..
     ...
     $                   JSIZE, JTYPE, MTYPES, N, N1, NERRS, NMATS,
     $                   NMAX, NTEST, NTESTT
...
*     .. External Subroutines ..
     ...
     $                   CHSEQR, CHST01, CLACPY, CLATME, CLATMR, CLATMS,
     $                   CLASET, CTREVC, CUNGHR, CUNMHR, SLABAD, SLAFTS,
     $                   SLASUM, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
  ...
  140       CONTINUE
     ...
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'CTREVC(R)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 180
            END IF
...
*
            ...
            DO 150 J = 1, N
               SELECT( J ) = .TRUE.
  150       CONTINUE
            CALL CTREVC( 'Left', 'Some', SELECT, N, T1, LDA, EVECTL,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'CTREVC(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 180
            END IF
