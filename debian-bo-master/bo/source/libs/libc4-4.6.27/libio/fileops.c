     ...
     register _IO_FILE *fp;
{
  ...
  if (count <= 0)
    {
      /* 
       * I think EAGAIN and EWOULDBLOCK should be treated as
       * EOF. H.J.
       */
#ifdef EAGAIN
...
#if 0
  /* Don't do it if it is opened for write. Some other processes
   * may be writing into it. */
  if ((fp->_flags & _IO_NO_WRITES) && fp->_offset != _IO_pos_BAD \
	&& fp->_IO_read_base != NULL && !_IO_in_backup (fp))
#else
