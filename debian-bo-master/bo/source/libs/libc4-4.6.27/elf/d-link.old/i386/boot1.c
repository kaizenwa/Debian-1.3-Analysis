...
void _dl_boot(int args){
...

  {
    ...
    malloc_buffer = (unsigned char *) _dl_mmap((void*) 0, 4096, PROT_READ | PROT_WRITE, 
...
/* Now we have done the mandatory linking of some things.  We are now
...

  {
      ...
      for(dpnt = (struct dynamic *) tcurr->dynamic_addr; dpnt->d_tag; dpnt++)
	{
	  ...
	  if(dpnt->d_tag == DT_NEEDED)
		...
		SEND_STDERR("Unable to load shared library ");
		SEND_STDERR(tcurr->loadaddr+tcurr->dynamic_info[DT_STRTAB] + 
			    dpnt->d_un.d_val);
		SEND_STDERR("\n");
		_dl_exit(12);
	      };
  ...
  /*
   * Now we need to create the dyn_elf entries that the dynamic linker
   * will use.  We build this from the _dl_loaded_modules linked list.
   */
  ...
  if(_dl_trace_loaded_objects)
	...
	{
	  shared++;
	  SEND_STDERR("Linux ELF dynamic linker: ");
	  SEND_STDERR(argv[0]);
	  SEND_STDERR(": file loaded: ");
	  SEND_STDERR(ttmp->libname);
	  SEND_STDERR("\n");
	};
      if (!shared)
      {
	  SEND_STDERR(argv[0]);
	  SEND_STDERR(": statically linked\n");
      }
  ...
  _dl_brkp = (unsigned int *) _dl_find_hash("___brk_addr", NULL, 1, NULL);
  if (_dl_brkp) *_dl_brkp = brk_addr;

#ifdef IBCS_COMPATIBLE
  _dl_envp = (unsigned int *) _dl_find_hash("_environ", NULL, 1, NULL);
#else
  _dl_envp = (unsigned int *) _dl_find_hash("__environ", NULL, 1, NULL);
#endif
  ...
  _dl_atexit = (int (*)(void *)) _dl_find_hash("atexit", NULL, 1, NULL);
  ...
  for(tpnt = _dl_loaded_modules; tpnt; tpnt = tpnt->next)
	...
	(*_dl_elf_init)();
      ...
      if(_dl_atexit && tpnt->dynamic_info[DT_FINI]) (*_dl_atexit)(tpnt->loadaddr + 
...
int _dl_fixup(struct elf_resolve * tpnt)
{
  ...
  if(tpnt->dynamic_info[DT_RELA]) {
    SEND_STDERR("Unable to handle RELA relocation records\n");
    _dl_exit(1);
  };
  return goof;
}
...
/*  SEND_STDERR("Hey, look where I am!\n"); */
  return 0;
}
