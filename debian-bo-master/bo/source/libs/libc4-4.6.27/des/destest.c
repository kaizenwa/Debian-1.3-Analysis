...
static unsigned char cbc_iv1[8]={0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
static unsigned char cbc_iv2[8]={0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
...
main()
	...
	for (i=0; i<NUM_TESTS; i++)
		...
		bzero(out,8);
		bzero(outin,8);
		ecb_crypt(key_data[i], in, 8,DES_ENCRYPT);

		if (bcmp(in,cipher_data[i],8) != 0)
			{
			printf("Encryption error %2d\nk=%s p=%s o=%s act=%s\n",
				i+1,pt(key_data[i]),pt(in),pt(cipher_data[i]),
				pt(out));
			}
		ecb_crypt(key_data[i], in, 8,DES_DECRYPT);

		if (bcmp(in,plain_data[i],8) != 0)
			{
	...
	printf("Doing cbc\n");
	bcopy(cbc_data, cbc_in,40);
	cbc_crypt(cbc_key, cbc_in, 40, DES_ENCRYPT, cbc_iv1);
	if (bcmp(cbc_in,cbc_ok,32) != 0)
		printf("cbc_encrypt encrypt error\n");
	cbc_crypt(cbc_key,cbc_in,40, DES_DECRYPT, cbc_iv2);
