/**
 *
 * $Id: Region.c,v 1.3 1996/11/28 09:21:42 u27113 Exp $
 *
 ...
 *
 **/
static char rcsid[] = "$Id: Region.c,v 1.3 1996/11/28 09:21:42 u27113 Exp $";
		  ...
		  XmRegion regD)
{
  ...
  for (i = 0; i < regM->numRects; i++)
	...
	__XmAddRectangle(&boxes, &size, &numRects,
      ...
      /* (3) -- part 2 */
      else if (/* x1,y1 is outside (so, x1,y2 as well as x2,y1 is outside)*/
	       VALUE_OUT_RANGE(regS->rects[j].x1,
			       regM->rects[i].x1, regM->rects[i].x2)
	...
	__XmAddRectangle(&boxes, &size, &numRects,
      ...
      /* (4) part 2 */
	       ...
	       /* x2 is outside, but y2 is inside */
	       && VALUE_OUT_RANGE(regS->rects[j].x2,
				  regM->rects[i].x1, regM->rects[i].x2)
	       && VALUE_IN_RANGE(regS->rects[j].y2,
				 regM->rects[i].y1, regM->rects[i].y2))
	...
	__XmAddRectangle(&boxes, &size, &numRects,
      ...
      /* (4) part 3 */
      else if (/* x1 is inside, but y1 is outside */
	       VALUE_IN_RANGE(regS->rects[j].x1,
			      regM->rects[i].x1, regM->rects[i].x2)
	       && VALUE_OUT_RANGE(regS->rects[j].y1,
				  regM->rects[i].y1, regM->rects[i].y2)
	...
	__XmAddRectangle(&boxes, &size, &numRects,
      ...
      /* (4) part 3 */
      else if (/* x1 is outside, y1 is inside */
	       VALUE_OUT_RANGE(regS->rects[j].x1,
			       regM->rects[i].x1, regM->rects[i].x2)
 ...
 * if we start coallescing the rectangles in
