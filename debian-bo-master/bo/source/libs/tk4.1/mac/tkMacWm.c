 ...
 *
 * SCCS: @(#) tkMacWm.c 1.32 96/06/20 18:17:10
 */
...
#define TIMEOUT_MS 2000
    ...
    oldRestrictProc = Tk_RestrictEvents(WaitRestrictProc, (ClientData) &info,
	    &oldRestrictData);
    timeout = Tcl_CreateTimerHandler(TIMEOUT_MS, WaitTimeoutProc,
	    (ClientData) &info);
    while (1) {
	Tcl_DoOneEvent(TCL_WINDOW_EVENTS|TCL_TIMER_EVENTS);
	if (info.foundEvent) {
 ...
 *	events while WaitForEvent is active.  It looks for an event
 *	of the sort wanted by WaitForEvent;  if it is found, it is
 *	returned to WaitForEvent and deleted from the event queue.
 *	Any other event is deferred for processing later.
 *
 * Results:
 *	Returns TK_DISCARD_EVENT if the right event is found (we
 *	return it to WaitForEvent, which will handle it, so the normal
 *	event loop should discard it).
 *
 * Side effects:
    ...
    XEvent *eventPtr;		/* Event that is about to be handled. */
{
    ...
    if ((eventPtr->type != infoPtr->type)
	    || (eventPtr->xany.window != infoPtr->window)
				 ...
				 * unmapped. */
{
	...
	if (WaitForEvent(winPtr->display, winPtr->window,
		mapped ? MapNotify : UnmapNotify, &event) != TCL_OK) {
	    /*
