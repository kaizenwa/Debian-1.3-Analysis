/*
 * $Id: blank.c,v 1.7 1996/12/01 03:16:53 morgan Exp $
 *
 * $Log: blank.c,v $
...
static void bail_out(int really, int code, const char *fn)
{
     fprintf(stderr,"==> called %s()\n  got: `%s'\n", fn, pam_strerror(code));
     if (really && code)
	  exit (1);
}
...
#define MAXENV 15
	 ...
	 for (i=0; i<MAXENV; ++i) {
	     ...
	     bail_out(0,retcode,"pam_putenv");
	 }
	 env = pam_misc_copy_env(pamh);
	 if (env)
     ...
     for (;;) {
	  ...
	  bail_out(0,retcode,"pam_authenticate");
	  ...
	  bail_out(0,retcode,"pam_setcred1");
	  ...
	  bail_out(0,retcode,"pam_open_session");
	  if (retcode != PAM_SUCCESS) {
	  ...
	  bail_out(0,retcode,"pam_close_session");
	  if (retcode != PAM_SUCCESS) {
	       fprintf(stderr,"%s: problem closing a session\n",argv[0]);
	       break;
	  }
	  
	  retcode = pam_setcred(pamh, PAM_CRED_DELETE);
	  bail_out(0,retcode,"pam_setcred2");
