...
/*
 * $Id: pam_mail.c,v 1.2 1997/02/15 16:06:14 morgan Exp $
 *
		   ...
		   , const char **argv)
{
    ...
    flags &= ~PAM_SILENT;
    if (flags != PAM_CRED_ESTABLISH && flags != PAM_CRED_DELETE) {
	return PAM_SUCCESS;
    }
    ...
    if (!(ctrl & PAM_NO_ENV) && (flags & PAM_CRED_ESTABLISH)) {
	char *tmp;
    ...
    if ((flags == PAM_CRED_ESTABLISH && !(ctrl & PAM_NO_LOGIN))
	|| (flags == PAM_CRED_DELETE && ctrl & PAM_LOGOUT_TOO)) {
