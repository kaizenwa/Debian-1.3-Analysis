...
/* $Id: pam_stress.c,v 1.12 1997/02/15 19:06:30 morgan Exp $
 *
		    ...
		    , struct pam_response **response)
{
	 ...
	 == PAM_SUCCESS) {
	  ...
	  if (retval != PAM_SUCCESS) {
	       ...
	       _pam_log(LOG_ERR,"that is: %s",pam_strerror(retval));
	  }
     } else {
...
#undef _LOCAL_STRESS_COMMENT
	  ...
	  if (resp[0].resp && resp[1].resp) {
	       if (strcmp(resp[0].resp,resp[1].resp)) {
		    /* passwords are not the same; forget and return error */

		    _pam_overwrite(resp[0].resp);
		    free(resp[0].resp);
		    _pam_overwrite(resp[1].resp);
		    free(resp[1].resp);
	  ...
	  _pam_overwrite(resp[0].resp);   /* clean up the passwords */
	  free(resp[0].resp);
	  _pam_overwrite(resp[1].resp);
	  free(resp[1].resp);
	  free(resp);
