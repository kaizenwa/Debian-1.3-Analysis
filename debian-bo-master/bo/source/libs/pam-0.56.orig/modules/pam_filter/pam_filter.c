/*
 * $Id: pam_filter.c,v 1.9 1997/02/15 19:07:49 morgan Exp $
 *
 * $Log: pam_filter.c,v $
 ...
 *
 *
 * written by Andrew Morgan <morgan@parc.power.net> with much help from
		      ...
		      , const char **evp, const char *filtername)
{
     ...
     exit(status);
     return -1;             /* should never happen, but shut gcc up */
}
			       ...
			       , const char *name, int which_run)
{
     ...
     if (ctrl & FILTER_DEBUG) {
	  ...
	  _pam_log(LOG_DEBUG, "[%s]", pam_strerror(retval));
     }
