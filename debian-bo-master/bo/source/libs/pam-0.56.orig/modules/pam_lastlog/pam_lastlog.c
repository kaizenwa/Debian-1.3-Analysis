...
/*
 * $Id: pam_lastlog.c,v 1.2 1997/02/15 17:18:21 morgan Exp $
 *
...
{
    int ctrl=(LASTLOG_DATE|LASTLOG_HOST|LASTLOG_LINE|LASTLOG_NEVER);
...
static int last_login_date(pam_handle_t *pamh, int announce, uid_t uid)
{
	...
	if (!win) {
	    D(("bad news reading the %s file for uid=%d", _PATH_LASTLOG, uid));
	    if (announce & LASTLOG_DEBUG) {
		_log_err(LOG_DEBUG, "unable to read %s for uid %d"
			 , _PATH_LASTLOG, uid);
	    }
