 ...
 * It is subject to change before reaching Linux-PAM-1.0 .
 *
 * $Id: pam_env.c,v 1.2 1997/02/15 15:56:48 morgan Exp $
 *
 * $Log: pam_env.c,v $
...
int _pam_make_env(pam_handle_t *pamh)
{
    ...
    if (pamh->env == NULL) {
	_pam_log_error("_pam_make_env: out of memory");
	return PAM_BUF_ERR;
    }
	...
	_pam_log_error("_pam_make_env: no memory for list");
	_pam_drop(pamh->env);
	return PAM_BUF_ERR;
    }
...
int pam_putenv(pam_handle_t *pamh, const char *name_value)
{
    ...
    if (name_value == NULL) {
	_pam_log_error("pam_putenv: no variable indicated");
    ...
    if (l2eq <= 0) {
	_pam_log_error("pam_putenv: bad variable");
	return PAM_BAD_ITEM;
    }
	...
	_pam_log_error("pam_putenv: no env%s found"
		     , pamh->env == NULL ? "":"-list" );
	return PAM_ABORT;
    }
	...
	if (item == -1) {                      /* new variable */
		...
		if (tmp == NULL) {
		    ...
		    _pam_log_error("pam_putenv: cannot grow environment");
		    return PAM_BUF_ERR;
		}
    ...
    if (item < 0) {
	_pam_log_error("pam_putenv: delete non-existent entry; %s"
		       , name_value);
	return PAM_BAD_ITEM;
    }
...
const char *pam_getenv(pam_handle_t *pamh, const char *name)
{
    ...
    if (name == NULL) {
	_pam_log_error("pam_getenv: no variable indicated");
	return NULL;
    }

    if (pamh->env == NULL || pamh->env->list == NULL) {
	_pam_log_error("pam_getenv: no env%s found"
		     , pamh->env == NULL ? "":"-list" );
	return NULL;
    }
...
const char * const *pam_getenvlist(pam_handle_t *pamh)
{
    ...
    if (pamh->env == NULL || pamh->env->list == NULL) {
	_pam_log_error("pam_getenvlist: no env%s found"
		       , pamh->env == NULL ? "":"-list" );
	return NULL;
    }
	...
	_pam_log_error("pam_getenvlist: environment corruption");
	_pam_dump_env(pamh);                 /* only active when debugging */
	return NULL;
    }
	...
	if (pamh->env->list[i] == NULL) {
	    _pam_log_error("pam_getenvlist: environment broken");
	    _pam_dump_env(pamh);              /* only active when debugging */
	    return NULL;          /* somehow we've broken the environment!? */
	}
    }

    /* Seems fine; return environment */

    _pam_dump_env(pamh);                    /* only active when debugging */

    return ((const char * const *) pamh->env->list);
}
