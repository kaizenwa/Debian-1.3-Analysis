 ...
 * Currently maintained by Andrew G. Morgan <morgan@parc.power.net>
 *
 * $Id: pam_handlers.c,v 1.16 1997/02/15 15:57:32 morgan Exp $
 *
 */
...
static int _pam_add_handler(pam_handle_t *pamh
		     ...
		     , int control_flag, const char *mod_path
    ...
    )
{
	...
	if (!_pam_strCMP(this_service, pamh->service_name) || other) {
		...
		_pam_log_error("(%s) illegal module type: %s"
			       , this_service, tok);
		...
		control_flag = PAM_F_REQUIRED;
		D(("*PAM_F_REQUIRED*"));
	    } else if (!_pam_strCMP("requisite", tok)) {
		control_flag = PAM_F_CRITICAL;
		D(("*PAM_F_REQUISITE*"));
	    } else if (!_pam_strCMP("optional", tok)) {
		control_flag = PAM_F_OPTIONAL;
		D(("*PAM_F_OPTIONAL*"));
	    } else if (!_pam_strCMP("sufficient", tok)) {
		control_flag = PAM_F_SUFFICIENT;
		D(("*PAM_F_SUFFICIENT*"));
	    } else {
		/* Illegal flag */
		D(("_pam_init_handlers: bad control flag: %s", tok));
		_pam_log_error("(%s) illegal control flag: %s"
			       , this_service, tok);
		control_flag = PAM_F_REQUIRED;
		must_fail = 1;             /* fail when we dispatch function */
	    }
		...
		_pam_log_error("(%s) no module name supplied", this_service);
		mod_path = NULL;
		must_fail = 1;
	    }
		...
		D(("CONF%s: %s%s %d %d %s %d"
		   , must_fail?"<*will fail*>":""
		   , this_service, other ? "(backup)":""
		   , module_type, control_flag
		   , mod_path, argc));
		for (y = 0; y < argc; y++) {
		    D(("CONF: %s", argv[y]));
		}
	    ...
	    res = _pam_add_handler(pamh, must_fail, other
				   , module_type, control_flag, mod_path
				   , argc, argv, argvlen);
	    if (res != PAM_SUCCESS) {
...
int _pam_init_handlers(pam_handle_t *pamh)
{
    ...
    /* Is the PAM subsystem locked? */
    {
	 ...
	 if ((fd_tmp = open( PAM_LOCK_FILE, O_RDONLY )) != -1) {
	      _pam_log_error("_pam_init_handlers: PAM lockfile ("
...
#else
	      sleep(5);     /* to avoid swamping the system with requests */
	...
	if ( stat(PAM_CONFIG_D, &test_d) == 0 && S_ISDIR(test_d.st_mode) ) {
	    ...
	    if (filename == NULL) {
		_pam_log_error("_pam_init_handlers: no memory; service %s"
			       , pamh->service_name);
		return PAM_BUF_ERR;
	    }
		...
		if (retval != PAM_SUCCESS) {
		    _pam_log_error("_pam_init_handlers: error reading %s"
				   , filename);
		    _pam_log_error("_pam_init_handlers: [%s]"
				   , pam_strerror(retval));
		} else {
		    read_something = 1;
		}
		...
		if (f != NULL) {
		    ...
		    _pam_log_error("_pam_init_handlers: no default config %s"
				   , PAM_DEFAULT_SERVICE_FILE);
		}
    ...
    if (retval != PAM_SUCCESS) {
	/* Read error */
	_pam_log_error("error reading PAM configuration file");
	return PAM_ABORT;
    }
		     ...
		     , int argc, char **argv, int argvlen)
{
	...
	if (mod->dl_handle == NULL) {
	    ...
	    _pam_log_error("[dlerror: %s]", dlerror());
	...
	if (success != PAM_SUCCESS) {	         /* add a malformed module */
	    ...
	    pamh->handlers.modules_used++;
	    _pam_log_error("adding faulty module: %s", mod_path);
