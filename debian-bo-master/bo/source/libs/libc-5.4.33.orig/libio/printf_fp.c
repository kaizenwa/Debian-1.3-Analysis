...
int __printf_fp( FILE *, const struct printf_info *, const ** );
static unsigned int guess_grouping( unsigned int, const char *, wchar_t );
static char *group_number( char *, char *, unsigned int,
								  const char *, wchar_t );
char hack_digit __P (( void ));

int
__printf_fp( FILE *fp, const struct printf_info *info, const **const args )
{
  ...
  /* The significant of the floating-point value in question  */
  MPN_VAR(frac);
  /* and the exponent.	*/
  int exponent;
  ...
  /* This is supposed to be the way to prototype a nested function. vch */
