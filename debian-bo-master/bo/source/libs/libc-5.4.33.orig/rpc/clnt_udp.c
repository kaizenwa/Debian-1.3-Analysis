 ...
 */
#ifdef __STDC__
CLIENT                *clntudp_bufcreate  ( struct sockaddr_in *, u_long,
                                            u_long, struct timeval,
                                            register int *, u_int, u_int );
CLIENT                *clntudp_create     ( struct sockaddr_in *, u_long,
                                            u_long, struct timeval,
                                            register int * );
static enum clnt_stat  clntudp_call       ( register CLIENT *, u_long,
...
clntudp_bufcreate( struct sockaddr_in *raddr, u_long program, u_long version,
						struct timeval wait, register int *sockp, u_int sendsz,
						u_int recvsz )
#else
CLIENT *
...
clntudp_call( register CLIENT	*cl, u_long	proc, xdrproc_t xargs,
...
send_again:
	...
	for (;;) {
#ifdef linux
		struct timeval oneshot_timeout;

		readfds = mask;
		oneshot_timeout = cu->cu_wait;
		switch (select(_rpc_dtablesize(), &readfds, (void *)NULL, 
			       (void *)NULL, &oneshot_timeout)) {
#else
		readfds = mask;
		switch (select(_rpc_dtablesize(), &readfds, (void *)NULL, 
