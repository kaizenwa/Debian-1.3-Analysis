...
bool_t xdr_rmtcall_args( register XDR *, register struct rmtcallargs * );
bool_t xdr_rmtcallres( register XDR *, register struct rmtcallres * );
#ifdef linux
static struct in_addr bc_addr(struct in_addr );
#endif	/* __linux__ */
static int getbroadcastnets( struct in_addr *, int, char * );
enum clnt_stat clnt_broadcast( u_long, u_long, u_long, xdrproc_t,
...
#endif	/* __STDC__ */
{
	/* could use /etc/networks but I'm too lazy */
	return inet_makeaddr(inet_netof(addr), INADDR_BROADCAST);
}
#endif	/* __linux__ */
...
#else
		herror("get_myaddress: gethostbyname");
#endif
		exit(1);
	}
	for (i = 0; hp->h_addr_list[i]; i++) {
		addrs[i] = bc_addr(*((struct in_addr *) hp->h_addr_list[i]));
#if 0
		printf("broadcast_addr[%d] = %s\n", i, inet_ntoa(addrs[i]));
#endif
	}
	return i;
#else
...
#if NLS
                perror(catgets(_libc_cat, RpcMiscSet, RpcMiscBConfig,
			    "broadcast: ioctl (get interface configuration)"));
#else
...
#if NLS
			perror(catgets(_libc_cat, RpcMiscSet, RpcMiscBFlags,
			       "broadcast: ioctl (get interface flags)"));
#else
...
#if NLS
		perror(catgets(_libc_cat, RpcMiscSet,RpcMiscCantCreateBSocket,
...
/*	baddr.sin_addr.S_un.S_addr = htonl(INADDR_ANY); */
...
#if NLS
			perror(catgets(_libc_cat, RpcMiscSet,
                                      RpcMiscBSelectProblem,
				       "Broadcast select problem"));
#else
	...
	try_again:
		...
		if (xdr_replymsg(xdrs, &msg)) {
			...
			if (msg.rm_xid == xid)
#if NLS
				fprintf(stderr,catgets(_libc_cat, RpcMiscSet,
                                      RpcMiscBDesProblem,
