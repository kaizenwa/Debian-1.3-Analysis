...
static char sccsid[] = 	"@(#)svcauth_des.c	2.2 88/08/15 4.0 RPCSRC; from 1.15 88/02/08 SMI";
#endif
...
enum auth_stat _svcauth_des( register struct svc_req *,
									 register struct rpc_msg * );
static void cache_init( void );
static short cache_victim( void );
static void cache_ref( register short );
static short cache_spot( register des_block *, char *, struct timeval * );
int authdes_getucred( struct authdes_cred *, short *, short *,
	short *, register int * );
...
static void cache_ref();		/* note that sid was ref'd */
...
#if (defined(sun) || defined(vax) || defined(linux))
/*
...
#ifdef __STDC__
int
authdes_getucred( struct authdes_cred *adc, short *uid, short *gid,
