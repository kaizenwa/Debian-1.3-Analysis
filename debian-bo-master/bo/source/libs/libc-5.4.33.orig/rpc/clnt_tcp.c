...
CLIENT                  *clnttcp_create   ( struct sockaddr_in *,
                                            u_long, u_long, register int *,
                                            u_int, u_int );
static enum clnt_stat    clnttcp_call     ( register CLIENT *, u_long,
...
static int               readtcp          ( register struct ct_data *,
                                            void *, register int );
static int               writetcp         ( struct ct_data *, void *, int );
#else
 ...
 * NB: The rpch->cl_auth is set null authentication.  Caller may wish
 *     to set this something more useful.
 */

#ifdef __STDC__
CLIENT *
clnttcp_create( struct sockaddr_in *raddr, u_long prog, u_long vers,
					register int *sockp, u_int sendsz, u_int recvsz )
#else
CLIENT *
clnttcp_create( radde, prog, vers, sockp, sendsz, recsz )
...
#endif
{
	...
	xdrrec_create(&(ct->ct_xdrs), sendsz, recvsz,
	    (void *)ct, (void *)readtcp, (void *)writetcp);
	h->cl_ops = &tcp_ops;
...
clnttcp_call( register CLIENT *h, u_long proc, xdrproc_t xdr_args,
...
#ifdef __STDC__
static int
readtcp( register struct ct_data *ct, void *buf, register int len )
#else
static int
readtcp( ct, buf, len )
   register struct ct_data *ct;
...
#endif
{
	...
	while (TRUE) {
		...
		readfds = mask;
		timeout = ct->ct_wait;
		switch (select(_rpc_dtablesize(), &readfds, (void*)NULL, (void*)NULL,
			       &timeout)) {
#else
		readfds = mask;
		switch (select(_rpc_dtablesize(), &readfds, (void*)NULL, (void*)NULL,
...
#ifdef __STDC__
static int
writetcp( struct ct_data *ct, void *buf, int len )
#else
static int
writetcp( ct, buf, len )
   struct ct_data *ct;
