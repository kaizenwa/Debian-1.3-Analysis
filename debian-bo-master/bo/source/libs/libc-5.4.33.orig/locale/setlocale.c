...
extern void _nl_free_locale_file_list( int );
  ...
  for (i = 0; i < LC_ALL; ++i)
    {
      ...
      char *name = (category == LC_ALL ? newnames[i] :
		    category == i ? newnames[0] :
		    (char *) _nl_current_names[i]);
      p = stpcpy (p, _nl_category_names[i]);
      *p++ = '=';
      p = stpcpy (p, name);
      *p++ = ';';
    }
...
setlocale (int category, const char *locale)
{
  ...
  /* We perhaps really have to load some data.  So we determine the
     ...
     bit set.  Use __libc_secure_getenv for this to avoid being 
     hosed by the saved uid. */
  locale_path = NULL;
  locale_path_len = 0;
  ...
  if (category == LC_ALL)
      ...
      if (strchr (locale, ';') != NULL)
	{
	  ...
	  char *np = alloca (strlen(locale) + 1);
	  char *cp;
	  int cnt;
