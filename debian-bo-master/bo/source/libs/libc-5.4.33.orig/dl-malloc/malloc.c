...
/* Linux libc 5.4.x lacks prototypes for the `__' versions. */
extern __ptr_t __mmap   __P((__ptr_t, size_t, int, int, int, off_t));
extern int     __munmap __P((__ptr_t, size_t));
extern __ptr_t __mremap __P((__ptr_t, size_t, size_t, int));
...
#pragma weak malloc_trim = __malloc_trim
#pragma weak malloc_usable_size = __malloc_usable_size
#pragma weak malloc_stats = __malloc_stats
#pragma weak malloc_get_state = __malloc_get_state
#pragma weak malloc_set_state = __malloc_set_state
...
extern void    (*__free_hook) (Void_t* __ptr);
extern Void_t* (*__malloc_hook) (size_t __size);
extern Void_t* (*__realloc_hook) (Void_t* __ptr, size_t __size);
extern Void_t* (*__memalign_hook) (size_t __size, size_t __alignment);
...
extern Void_t* (*__malloc_hook) ();
extern Void_t* (*__realloc_hook) ();
extern Void_t* (*__memalign_hook) ();
/* Activate a standard set of debugging hooks. */
extern void    __malloc_check_init ();
#endif
...
{
  mchunkptr p, oldtop;
  INTERNAL_SIZE_T csz, sz = n * elem_size, oldtopsize;
  Void_t* mem;
...
/*
  ...
  malloc_stats:
    ...
    Prints on stderr the amount of space obtained from the system
    (both via sbrk and mmap), the maximum amount (which may be more
    than current if malloc_trim and/or munmap got called), the maximum
...
#endif
{
  ...
  for(i=0; i<NAV; i++) {
    ...
    if(ms->av[2*i+2] == 0)
      ...
      last(b) = ms->av[2*i+3];
      if(i > 0) {
        /* Make sure the links to the av_-bins in the heap are correct. */
        first(b)->bk = b;
        last(b)->fd = b;
      }
    }
...
/* Malloc hooks: When compiling with MALLOC_HOOKS defined, all calls
   to malloc(), realloc(), free() and memalign() are routed through
   the global function pointers __malloc_hook, __realloc_hook,
   __free_hook and __memalign_hook if they are not NULL.  Therefore
   the malloc implementation can be changed at runtime, if care is
   taken not to call free() or realloc() on pointers obtained with a
   different implementation than the one currently in effect.  (The
   easiest way to guarantee this is to set up the hooks before any
   malloc call, e.g.  with a function pointed to by the global
   variable __malloc_initialize_hook).  You can set the environment
   variable `MALLOC_CHECK_' (note the trailing underscore) to activate
   a standard set of debugging hooks automatically.  Depending on the
   value of check_action (which can be set with mallopt() or with a
   single digit in the environment variable), an invalid free() or
   realloc() will be silently ignored (check_action=0), diagnostics
   will be printed on stderr (check_action=1), or abort() will be
   called (check_action=2). */
...
void (*__malloc_initialize_hook) () = NULL;
void (*__free_hook) () = NULL;
Void_t* (*__malloc_hook) () = malloc_hook_ini;
Void_t* (*__realloc_hook) () = realloc_hook_ini;
Void_t* (*__memalign_hook) () = memalign_hook_ini;
...
#endif
{
  ...
  __malloc_hook = NULL;
  __free_hook = NULL;
  newmem = mEMALIGn(alignment, bytes+1);
...
History:
    ...
    V2.6.4
      * Added `hooks' (mostly for debugging at runtime)
