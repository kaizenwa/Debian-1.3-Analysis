...
/* We do this since mmap/munmap can only deal with pages. */
#define ROUND_TO_PAGE(_S) \
		(((_S) + EXEC_PAGESIZE - 1) & ~(EXEC_PAGESIZE - 1))

#define FREE_BUF(_B, _S) munmap ((_B), ROUND_TO_PAGE (_S))
#define ALLOC_BUF(_B,_S,_R) _B = (char*) mmap(0, ROUND_TO_PAGE (_S), \
			    PROT_READ|PROT_WRITE, \
			    MAP_PRIVATE|MAP_ANONYMOUS, -1, 0); \
			    if (_B == (char *) -1) return (_R)
#else
#define FREE_BUF(_B) free(_B)
#define ALLOC_BUF(_B,_S,_R) _B = (char*)malloc(_S); \
			    if (_B == NULL) return (_R)
#endif
...
#ifdef __linux__
/* this is defined as 'static __inline__' in <sys/stat.h> */
...
extern char* _IO_gen_tempname __P ((const char * dir, const char *pfx,
				    int dir_search, _IO_size_t *lenptr,
				    _IO_FILE **streamptr));
...
#define FILEBUF_LITERAL(CHAIN, FLAGS, FD) \
       { 0, 0, 0, _IO_MAGIC+_IO_LINKED+_IO_IS_FILEBUF+FLAGS, \
         0, 0, 0, 0, 0, 0, 0, 0, 0, CHAIN, FD}
#else
#define FILEBUF_LITERAL(CHAIN, FLAGS, FD) \
...
#include <libc-lock.h>
__libc_lock_define (extern, __libc_libio_lock);

extern void flockfile __P((_IO_FILE *));
extern void funlockfile __P((_IO_FILE *));
extern int ftrylockfile __P((_IO_FILE *));
