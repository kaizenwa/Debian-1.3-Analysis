...
#ifndef _WIN32
#define DLGTEMPLATE void
#endif
 
    const DLGTEMPLATE *ptr = (const DLGTEMPLATE*)GlobalLock(_hTemplate);

    (void) ::CreateDialogIndirectParam(theApp->vHandle(), 
                                       ptr,
                                       theApp->_Frame,
...
//====================>>> vCommandPane::OnPaint <<<==================
  int vCommandPane::OnPaint(HWND hwnd)
  {
    ...
    if (paneType() == P_Status)         // Paint a status bar
      {
        HPEN oldpen = ::SelectObject(hdc, theApp->_ShadowPen);
        ::MoveTo(hdc, 0, 0);
      ...
      {
        HPEN oldpen = ::SelectObject(hdc, theApp->_LightPen);
        ::MoveTo(hdc, 0, 0);
        ::LineTo(hdc, rc.right,0);
        ::SelectObject(hdc, theApp->_ShadowPen);
