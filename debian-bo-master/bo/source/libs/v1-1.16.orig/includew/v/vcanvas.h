...
const int maxCursors = 13;
	...
	void GetTranslate(int& x, int& y) {_cpDC->GetTranslate(x,y);}
	int GetTransX() { return _cpDC->GetTransX(); }
	int GetTransY() { return _cpDC->GetTransY(); }

	vPen GetPen()
	  { return _cpDC->GetPen(); }


	vDC* GetDC() { return _cpDC; }

	// Platform dependent

	HWND DrawingWindow() { return _drawWindow; }
