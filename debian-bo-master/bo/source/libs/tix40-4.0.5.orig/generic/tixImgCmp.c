...
typedef struct CmpMaster {
    ...
    XFontStruct *fontPtr;	/* Information about text font.*/
...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_FONT, "-font", "font", "Font",
       DEF_CMPIMAGE_FONT, Tk_Offset(CmpMaster, fontPtr), 0},
...
static Tk_ConfigSpec textConfigSpecs[] = {
    ...
    {TK_CONFIG_FONT, "-font", (char *) NULL, (char *) NULL,
       "", Tk_Offset(CmpTextItem, fontPtr), TK_CONFIG_NULL_OK},
				 ...
				 * such as TK_CONFIG_ARGV_ONLY. */
{
    ...
    gcValues.font = masterPtr->fontPtr->fid;
    ...
    char **argv;		/* Argument strings. */
{
    ...
    if (p.text->fontPtr) {
	gcValues.font = p.text->fontPtr->fid;
    } else {
	gcValues.font = masterPtr->fontPtr->fid;
    }
    gcValues.graphics_exposures = False;
    ...
    ClientData clientData;
{
	...
	for (p.item = lPtr->itemHead; p.item; p.item=p.item->next) {
	      ...
	      case TYPE_TEXT:
		{
		    ...
		    if (p.text->fontPtr) {
			fontPtr = p.text->fontPtr;
		    } else {
			fontPtr = masterPtr->fontPtr;
		    }
		    
		    p.text->numChars = strlen(p.text->text);
		    TkComputeTextGeometry(fontPtr, p.text->text,
			p.text->numChars,
			p.text->wrapLength,
				 ...
				 * correspond to imageX and imageY. */
{
	...
	for (p.item = lPtr->itemHead; p.item; p.item=p.item->next) {
	      ...
	      case TYPE_TEXT:
		{
		    ...
		    TkDisplayText(Tk_Display(masterPtr->tkwin), drawable,
			fontPtr, p.text->text, p.text->numChars,
			dx, dy+extraY,	    
