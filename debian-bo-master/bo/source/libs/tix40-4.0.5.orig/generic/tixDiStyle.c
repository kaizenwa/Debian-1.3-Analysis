...
static void 		InitHashTables _ANSI_ARGS_((
			    ));
    ...
    char **argv;		/* Argument strings. */
{
    ...
    /* Parse the -refwindow option: this tells the style to
     * use this window to query the default values for background, foreground
     * etc. Usually, you should set the -refwindow to the window that holds
     * the display items which are controlled by this style.
     */
    if (argc > 2) {
	size_t len;
	if (argc %2 != 0) {
...
Tix_DItemStyle* FindDefaultStyle(diTypePtr, tkwin)
    Tix_DItemInfo * diTypePtr;
    Tk_Window tkwin;
{
    Tcl_HashEntry *hashPtr;
