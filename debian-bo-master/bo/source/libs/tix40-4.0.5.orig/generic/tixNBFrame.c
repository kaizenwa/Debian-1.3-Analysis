...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_FONT, "-font", "font", "Font",
       DEF_NOTEBOOKFRAME_FONT, Tk_Offset(WidgetRecord, fontPtr), 0},
    ...
    char **argv;			/* Argument strings. */
{
	...
	if (argc != 3) {
	    result = Tix_ArgcError(interp, argc, argv, 2, "page");
	    goto error;
	}
	...
	if (argc != 4) {
	    ...
	    result = TCL_OK;	/* an empty string is returned to indicate
				 * "nothing selected" */
	    goto done;
	}
    }
	...
	if (argc < 3) {
	    result =  Tix_ArgcError(interp, argc, argv, 2, 
		"page ?option value ...?");
	    goto error;
	}
	if ((tPtr=FindTab(interp, wPtr, argv[2])) == NULL) {
    ...
    int argc;
{
	...
	TkComputeTextGeometry(wPtr->fontPtr, tPtr->text, tPtr->numChars,
    ...
    Drawable drawable;
{
	...
	if (tPtr->state == tixNormalUid) {
	    TkDisplayText(wPtr->display, drawable, wPtr->fontPtr,
    ...
    WidgetPtr wPtr;
{
	...
	for (tPtr=wPtr->tabHead; tPtr; tPtr=tPtr->next) {
	    ...
	    if (tPtr->text != NULL) {
		tPtr->numChars = strlen(tPtr->text);
		TkComputeTextGeometry(wPtr->fontPtr, tPtr->text,
