...
/* ToDo:
 *
 *	This file has the most ugly code in Tix. I need to rewrite it
 *	somehow ...
 */
...
void			Tix_HLComputeGeometry _ANSI_ARGS_((
			    ClientData clientData));
...
static void		CancelRedrawWhenIdle _ANSI_ARGS_((
			    WidgetPtr wPtr));
void			Tix_HLCancelResizeWhenIdle _ANSI_ARGS_((
			    WidgetPtr wPtr));
...
static void		DItemSizeChangedProc _ANSI_ARGS_((
			    Tix_DItem *iPtr));
static int		Tix_HLFixBindings _ANSI_ARGS_((
    ...
    char **argv;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "unknown option \"", argv[0], 
	    ...
	    "hidden, item, next, parent, prev or selection",
	    NULL);
	return TCL_ERROR;
    }
}
    ...
    char **argv;		/* Argument strings. */
{
	...
	if (chPtr->indicator) {
	    ...
	    indOffX = (wPtr->indent-Tix_DItemWidth (chPtr->indicator)) / 2;
	    indOffY = (wPtr->indent-Tix_DItemHeight(chPtr->indicator)) / 2;
    ...
    char **argv;		/* Argument strings. */
{
	...
	if (argc == 1) {
	    ...
	    if ((from = Tix_HLFindElement(interp, wPtr, argv[1])) == NULL) {
		code = TCL_ERROR;
		goto done;
	    }
	    
	    if (argc == 3) {
		if ((to = Tix_HLFindElement(interp, wPtr, argv[2])) == NULL) {
		    code = TCL_ERROR;
		    goto done;
		}
	    }
    ...
    ClientData clientData;	/* Info about my widget. */
{
    ...
    if (wPtr->headerWin) {
	Tk_Window tkwin = wPtr->headerWin;
	wPtr->headerWin = NULL;
#if 0
	/* no need to do so. Also, causes crash on Windows */
 ...
 *--------------------------------------------------------------
 */
static int ConfigElement(wPtr, chPtr, argc, argv, flags, forced)
    WidgetPtr wPtr;
    HListElement *chPtr;
    int argc;
    char ** argv;
    ...
    int y;
{
	...
	while (1) {
		...
		/* This is the bottom-most (?visible) element */
		break;
	    }
    ...
    int xOffset;
{
    ...
    int childIconX = 0, childIconY = 0;		/* center of child's icon */
    int childY, childX;
    int oldY;
    int topBorder;
	...
	childIconX= childX + wPtr->selBorderWidth
	  + Tix_DItemPadX(ptr->col[0].iPtr);
	childIconY= childY + wPtr->selBorderWidth
	  + ptr->height / 2;
    ...
    int xOffset;
{
    ...
    if (chPtr->selected) {
	/*
	 * When the ditem is selected, we have already drawn
	 * the selection background ourself, so we don't want DitemDisplay()
	 * to draw any background for us. So in this case both
	 * TIX_DITEM_NORMAL_BG and TIX_DITEM_SELECTED_BG are *not* set
	 */
	...
	/* set the TIX_DITEM_NORMAL_BG. This will be used unless ACTIVE_BG
	 * and/or DISABLE_BG are set
	 */
 ...
 *----------------------------------------------------------------------
 */
static int Tix_HLFixBindings(interp, wPtr)
    Tcl_Interp * interp;
    WidgetPtr wPtr;
{
    return Tix_GlobalVarEval(interp, "tixFixHListBindings ",
	Tk_PathName(wPtr->dispData.tkwin), NULL);
}
#endif
