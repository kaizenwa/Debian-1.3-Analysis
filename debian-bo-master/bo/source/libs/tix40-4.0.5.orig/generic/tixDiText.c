...
static Tk_ConfigSpec textStyleConfigSpecs[] = {
    ...
    {TK_CONFIG_FONT, "-font", "font", "Font",
       DEF_TEXTSTYLE_FONT, Tk_Offset(TixTextStyle, fontPtr), 0},
...
#if 0
    /* %bordercolor not used */
static void DrawBorder(pixmap, gc, iPtr, x, y, width, height, flags)
    Pixmap pixmap;
    GC gc;
    Tix_DItem * iPtr;
    int x;
    ...
    int flags;
{
    ...
    TixTextItem *itPtr = (TixTextItem *)iPtr;
    if (itPtr->stylePtr->borderWidth <= 0) {
	return;
    }
    if (itPtr->stylePtr->relief == TIX_RELIEF_SOLID) {
	if (!(flags & TIX_DITEM_BOTTOM)) {
	    x1 = x;
	    x2 = x + width-1;
	    y1 = y2 = y + height - 1;
	    
	    for (i=0; i<itPtr->stylePtr->borderWidth; i++) {
		XDrawLine(itPtr->ddPtr->display, pixmap,
		    itPtr->stylePtr->borderGC, x1, y1, x2, y2);
		--y1;
		--y2;
	    }
	}
	if (!(flags & TIX_DITEM_RIGHT)) {
	    y1 = y;
	    y2 = y + height -1;
	    x1 = x2 = x + width - 1;
...
#if 0
    /* %bordercolor not used */
    DrawBorder(pixmap, gc, iPtr, x, y, width, height, flags);
#endif
