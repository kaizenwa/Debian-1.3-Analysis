    ...
    char * flag;
{
    ...
    if (spec = Tix_FindConfigSpecByName(interp, cPtr, flag)) {
	if (spec->isAlias) {
...
TixConfigSpec * Tix_FindConfigSpecByName(interp, cPtr, flag)
    Tcl_Interp * interp;
    TixClassRecord * cPtr;
    char * flag;
{
     ...
     * it should be faster
     * (%% todo, hash table lookup only if there are more than a certain
     * number of options (e.g. 10) in the class)
     */

    key = Tix_GetConfigSpecFullName(classRec, flag);
    hashPtr = Tcl_FindHashEntry(&tixSpecTable, key);
    ckfree(key);
    ...
    /* The user may specified a partial name. Try to match, but will return
     * error if we don't get exactly one match */
    len = strlen(flag);
    ...
    char * value;
{
    ...
    if (c = Tix_FindMethod(interp, context, method)) {
	argv[0] = value;
	return Tix_CallMethod(interp, c, widRec, method, 1, argv);
    }
    if (c = Tix_FindMethod(interp, context, "config")) {
