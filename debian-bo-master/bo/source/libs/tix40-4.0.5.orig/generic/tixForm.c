 ...
 *
 * 	This file implements the algorithm of tixForm.
 *
 * LEGAL WARNINGS:
 *
 *	The Tix Form geometry manager CANNOT be used in any commercial
 *	software that supports interactive user interface specification.
 *	This means if you are writing a GUI builder that includes 
 *	tixForm, it must be FREELY DISTRIBUTED. Otherwise, you are not
 *	even permitted to study, imitate, copy or read this source file.
 *
 *	Please read the file README.LEGAL for more details on the copyright
 *	status of Tix 4.0
 *
...
static void		TixFm_LostMasterProc _ANSI_ARGS_((
			    ClientData clientData, Tk_Window tkwin));
static void 		TixFm_MasterReqProc _ANSI_ARGS_((ClientData clientData,
			    Tk_Window tkwin));
...
static Tk_GeomMgr masterType = {
    "tixForm",			/* name */
    TixFm_MasterReqProc,	/* requestProc */
    TixFm_LostMasterProc,	/* lostSlaveProc */
};
...
static int TestAndArrange(masterPtr)
    register MasterInfo *masterPtr;
{
    register FormInfo *clientPtr;
    int i,j;
...
static void CancelArrangeWhenIdle(masterPtr)
    register MasterInfo * masterPtr;
{
...
static int PinnSide_AttNone(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to pinn down */
...
static int PinnSide_AttPercent(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to pinn down */
    int axis;				/* 0 = x axis, 1 = yaxis */
    int which;				/* 0 = min side, 1= max side */
{
...
static int PinnSide_AttOpposite(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to pinn down */
    int axis;				/* 0 = x axis, 1 = yaxis */
    int which;				/* 0 = min side, 1= max side */
{
    register FormInfo * attachPtr;
...
static int PinnSide_AttParallel(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to pinn down */
    int axis;				/* 0 = x axis, 1 = yaxis */
    int which;				/* 0 = min side, 1= max side */
{
...
{
    register FormInfo *clientPtr = (FormInfo *) clientData;
...
MasterStructureProc(clientData, eventPtr)
    ...
    XEvent *eventPtr;			/* Describes what just happened. */
{
    register MasterInfo *masterPtr = (MasterInfo *) clientData;
...
static void
TixFm_MasterReqProc(clientData, tkwin)
    ClientData clientData;	/* TixForm's information about
				 * window that got new preferred
				 * geometry.  */
    Tk_Window tkwin;		/* Other Tk-related information
...

static void
TixFm_LostMasterProc(clientData, tkwin)
    ...
    FormInfo *clientPtr;
{
    ...
    if (hPtr = Tcl_FindHashEntry(&formInfoHashTable,(char*)clientPtr->tkwin)) {
	Tcl_DeleteHashEntry(hPtr);
    }
    clientPtr->tkwin = NULL;
    masterPtr = clientPtr->master;
...
static int PlaceSide_AttNone(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to Place down */
    int axis;				/* 0 = x axis, 1 = yaxis */
    int which;				/* 0 = min side, 1= max side */
{
    int reqSize;
...
static SpringLink * AllocSpringLink()
{
    return (SpringLink *) malloc(sizeof(SpringLink));
}

static void FreeSpringLink(link)
    SpringLink * link;
{
    ckfree((char*)link);
}
...
static void AddRightSprings(listPtr, clientPtr)
    SpringList * listPtr;
    FormInfo *clientPtr;
{
...
static void AddLeftSprings(listPtr,clientPtr)
    SpringList * listPtr;
    FormInfo *clientPtr;
{
    SpringLink * link = AllocSpringLink();
...
static int PlaceWithSpring(clientPtr, axis, which)
    register FormInfo *clientPtr;	/* The client to Place down */
    int axis;				/* 0 = x axis, 1 = yaxis */
    int which;				/* 0 = min side, 1= max side */
{
  ...
  done2:
	...
	if (link->clientPtr->fill[axis]) {
	    link->clientPtr->posn[axis][0] = disp;
	    disp += gap0;
	    disp += gap1;
	    ...
	    link->clientPtr->posn[axis][1] = disp;
	} else {
	    disp += gap0;
	    link->clientPtr->posn[axis][0] = disp;
	    disp += ReqSize(link->clientPtr->tkwin, axis);
		...
		link->clientPtr->posn[axis][1] =  boundary[1];
	    }
	}
