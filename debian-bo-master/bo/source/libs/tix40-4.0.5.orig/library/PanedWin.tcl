...
proc tixPanedWindow::paneconfigure {w name args} {
	...
	lappend value "-expand {} {} {} $data($name,expand)"
	...
	case [lindex $args 0] {
	    ...
	    -expand {
		return "-expand {} {} {} $data($name,expand)"
	    }
	    -min {
...
proc tixPanedWindow::DoRepack {w} {
    ...
    set totalExp 0
    foreach name $data(items) {
	incr totalExp $data($name,expand)
    }
...
proc tixPanedWindow::xGrow {w name toGrow totalExp p} {
	...
	if {$totalExp == 0} {
	    ...
	    set canGrow [expr $toGrow * $data($name,expand) / $totalExp]
	}
    }
...
proc tixPanedWindow::xShrink {w name toShrink totalExp newSize usedSize} {
	...
	set canShrink [expr $toShrink * $data($name,expand) / $totalExp]
    }
