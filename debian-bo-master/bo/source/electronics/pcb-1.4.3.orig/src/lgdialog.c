...
static	char	*rcsid = "$Id: lgdialog.c,v 143.1 1996/09/16 09:08:41 nau Exp $";
...
Boolean LayerGroupDialog(void)
{
	...
	masterform = XtVaCreateManagedWidget("layerGroupMasterForm",formWidgetClass,
		popup,
		XtNresizable, True,
		NULL);
	...
	for (layer = 0; layer < MAX_LAYER+2; layer++)
	{
		...
		for (i = 0; i < MAX_LAYER; i++)
		{
			lastInRow = XtVaCreateManagedWidget("checkBox", toggleWidgetClass,
				...
				XtNforeground, PCB->Data->Layer[layer].Color,
				XtNlabel, " ",
				LAYOUT_TOP,
				NULL);
