...
static	char	*rcsid = "$Id: printpanner.c,v 143.2 1996/11/18 11:23:01 nau Exp $";
 ...
 */
static void CB_Report(Widget W, XtPointer ClientData, XtPointer CallData)
{
...
Widget PrintPannerCreate(Widget Parent, Widget Top, Widget Left)
{
	...
	masterform = XtVaCreateManagedWidget("printpannerMasterForm", formWidgetClass,
		Parent,
		LAYOUT_TOP,
		XtNfromVert, Top,
		XtNfromHoriz, Left,
...
void PrintPannerSetSliderPosition(Position OffsetX, Position OffsetY)
{
	...
	XtVaGetValues(Panner,
		XtNsliderWidth, &width,
		XtNsliderHeight, &height,
		NULL);
	OffsetX += ((MediaSelection->Width -2*MediaSelection->MarginX- width) /2);
	OffsetY += ((MediaSelection->Height -2*MediaSelection->MarginY -height)/2);
	XtVaSetValues(Panner,
		XtNsliderX, OffsetX,
		XtNsliderY, OffsetY,
		NULL);
 ...
 */
void PrintPannerUpdate(float Scale, Boolean RotateFlag, Boolean OutlineFlag,
 ...
 */
void PrintPannerGetSize(Dimension *Width, Dimension *Height)
{
	*Width = SliderWidth;
	*Height = SliderHeight;
}
 ...
 */
void PrintPannerGetSliderPosition(Position *OffsetX, Position *OffsetY)
{
	*OffsetX = SliderX;
	*OffsetY = SliderY;
}
