...
%
*/
Export unsigned int IsTrue(const char *boolean)
{
...
Export unsigned int XAnnotateImage(Display *display,
  const XPixelInfo *pixel_info,const XAnnotateInfo *annotate_info,Image *image)
{
  GC
    annotate_context;
...
#include <sys/poll.h>
...
#else
  {
    ...
    timer.tv_sec=(int) milliseconds/1000;
    timer.tv_usec=(int) (milliseconds % 1000)*1000;
...
Export unsigned int XDrawImage(Display *display,const XPixelInfo *pixel_info,
  const XDrawInfo *draw_info,Image *image)
{
  GC
    draw_context;
...
%
*/
Export void XGetMontageInfo(XMontageInfo *montage_info)
{
  assert(montage_info != (XMontageInfo *) NULL);
  *montage_info->filename='\0';
  ...
  montage_info->tile=(char *) malloc(MaxTextExtent*sizeof(char));
  if (montage_info->tile != (char *) NULL)
    (void) strcpy(montage_info->tile,DefaultTilePageGeometry);
...
%
*/
Export void XGetResourceInfo(XrmDatabase resource_database,char *client_name,
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  resource_info->browse_command=XGetResourceClass(resource_database,client_name,
  ...
  resource_value=
    XGetResourceClass(resource_database,client_name,"displayWarnings","True");
  ...
  resource_info->launch_command=XGetResourceClass(resource_database,client_name,
  ...
  if (window->matte_image != (XImage *) NULL)
    {
      if (window->matte_image->data != (char *) NULL)
        (void) free(window->matte_image->data);
...
static void FormatLabel(XFontStruct *font_info,char *label,
  const unsigned int width)
{
  ...
  */
  p=label;
  for (q=p; *q != '\0'; q++)
    if (XTextWidth(font_info,p,(int) (q-p)) > width)
      {
        while ((*q != ' ') && (q > p))
          q--;
        if (q == p)
...
Export Image *XMontageImages(const XResourceInfo *resource_info,
  XMontageInfo *montage_info,Image *image)
{
#define MontageImageText  "  Creating visual image directory...  "
  ...
  if (resource_info->image_geometry != (char *) NULL)
    {
      ...
      */
      (void) strcpy(geometry,resource_info->image_geometry);
  ...
  */
  ...
  font_height=montage_info->pointsize;
  ...
  if (display != (Display *) NULL)
    {
      ...
      */
      ...
      FormatLabel(font_info,resource_info->title,((tile_info.width+
        (border_width << 1))*Min(number_images,tiles_per_column)) >> 1);
      for (tile=0; tile < number_images; tile++)
        FormatLabel(font_info,images[tile]->label,tile_info.width+
          (border_width << 1));
  ...
  */
  ...
  if (resource_info->title != (char *) NULL)
    title_offset=((font_height*MultilineCensus(resource_info->title)) << 1)+
      (tile_info.y << 1);
  number_lines=0;
  for (tile=0; tile < number_images; tile++)
  ...
  */
  ...
  images_per_page=
    (number_images-1)/(tiles_per_row*tiles_per_column)+1;
  tiles=0;
      ...
      if (((tile+1) == tiles_per_page) || (((tile+1) % tiles_per_row) == 0))
        {
          ...
          y_offset+=
            height+(tile_info.y+border_width)*2+(font_height+2)*number_lines;
    ...
    if (!concatenate)
      {
        ...
        */
        montage_image->montage=(char *) malloc(MaxTextExtent*sizeof(char));
        count=1;
        for (tile=0; tile < tiles_per_page; tile++)
        ...
        (void) sprintf(montage_image->montage,"%dx%d%+d%+d",
          ...
          (int) (tile_info.height+(tile_info.y+border_width)*2+(font_height+2)*
      ...
      if (montage_info->frame && (bevel_width != 0))
        {
          ...
          */
          ...
          frame_info.height+=(font_height+2)*MultilineCensus(image->label);
          ...
          if (montage_info->shadow)
            {
              ...
              */
              (void) XQueryColorDatabase(resource_info->background_color,
                &color);
          ...
          if (image->label != (char *) NULL)
            {
              ...
              */
              (void) sprintf(annotate_info.geometry,"%ux%u%+d%+d",
                ...
                (border_width << 1)-bevel_width-2 : y_offset+y+tile_info.y+
                image->rows+2));
    ...
    if ((i+1) < images_per_page)
      {
        ...
        */
        ...
        (void) strcpy(montage_image->next->filename,montage_info->filename);
...
Export void XUserPreferences(XResourceInfo *resource_info)
{
  ...
  */
  ...
  value=resource_info->dither ? "True" : "False";
  XrmPutStringResource(&preferences_database,specifier,value);
