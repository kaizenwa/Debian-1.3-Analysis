...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Function XDrawTriangleEast draws a triangle with a highlighted left
...
%
*/
static void XGetWidgetInfo(char *text,XWidgetInfo *widget_info)
{
  /*
...
#define ResetButtonText  "Reset"
  ...
  */
  ...
  windows->widget.x=x-((3*windows->widget.width) >> 2);
  windows->widget.y=y-(windows->widget.height >> 1);
    ...
    if (state & JumpListState)
      {
        ...
        */
        list_info.id=(~0);
        for (i=0; i < colors; i++)
          if (strcmp(colorlist[i],reply) >= 0)
            {
              list_info.id=strcmp(colorlist[i],reply) == 0 ? i : ~0;
              break;
            }
...
%
*/
Export int XCommandWidget(Display *display,XWindows *windows,char **selections,
  XEvent *event)
{
  ...
  if (event == (XEvent *) NULL)
    {
      ...
      */
      if (selection_info != (XWidgetInfo *) NULL)
        free((char *) selection_info);
      selection_info=(XWidgetInfo *)
        malloc(number_selections*sizeof(XWidgetInfo));
      if (selection_info == (XWidgetInfo *) NULL)
        Error("Unable to create Command Widget","Memory allocation failed");
      state|=UpdateConfigurationState;
    }
  /*
    Wait for next event.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Function XConfirmWidget displays a Confirm widget with a notice to the user.
...
%  A description of each parameter follows:
%
%    o status:  Function XConfirmWidget returns True if the user presses Yes
...
#define YesButtonText  "Yes"
  ...
  */
  ...
  if (windows->widget.height < windows->widget.min_height)
    windows->widget.height=windows->widget.min_height;
  XQueryPosition(display,windows->widget.root,&x,&y);
  windows->widget.x=x-(windows->widget.width >> 1);
  windows->widget.y=y-((5*windows->widget.height) >> 3);
  XConstrainWindowPosition(display,&windows->widget);
  /*
    Map Confirm widget.
  */
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        if (strcmp(qualifier,"Do you want to save it") == 0)
...
#define CancelButtonText  "Cancel"
  ...
  */
  ...
  windows->widget.min_width=
    width+25*XTextWidth(font_info,"#",1)+4*QuantumMargin;
  if (windows->widget.width < windows->widget.min_width)
    windows->widget.width=windows->widget.min_width;
  ...
  */
  ...
  /*
    Respond to X events.
  */
  anomaly=(strcmp(action,"Background") == 0) || (strcmp(action,"New") == 0) ||
    (strcmp(action,"Quantize") == 0) || (strcmp(action,"Resize") == 0) ||
    (strcmp(action,"Save") == 0) || (strcmp(action,"Shade") == 0);
  state=UpdateConfigurationState;
  XSetCursorState(display,windows,True);
  do
  {
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Function XFileBrowserWidget displays a File Browser widget with a file query
...
#define UpButtonText  "Up"
  ...
  */
  ...
  XGetWidgetInfo((char *) NULL,&south_info);
  visible_files=0;
  anomaly=(strcmp(action,"Composite") == 0) || (strcmp(action,"Open") == 0) ||
    (strcmp(action,"Map") == 0);
  delay=SuspendTime << 2;
  state=UpdateConfigurationState;
  do
    ...
    if (state & JumpListState)
      {
        ...
        */
        list_info.id=(~0);
        for (i=0; i < files; i++)
          if (strcmp(filelist[i],reply) >= 0)
            {
              list_info.id=strcmp(filelist[i],reply) == 0 ? i : ~0;
              break;
            }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Function XFontBrowserWidget displays a Font Browser widget with a font query
%  to the user.  The user keys a reply and presses the Action or Cancel button
%  to exit.  The typed text is returned as the reply function parameter.
%
    ...
    if (state & JumpListState)
      {
        ...
        */
        list_info.id=(~0);
        for (i=0; i < fonts; i++)
          if (strcmp(fontlist[i],reply) >= 0)
            {
              list_info.id=strcmp(fontlist[i],reply) == 0 ? i : ~0;
              break;
            }
...
Export void XListBrowserWidget(Display *display,XWindows *windows,
  XWindowInfo *window_info,char **list,char *action,char *query,char *reply)
{
#define CancelButtonText  "Cancel"
  ...
  */
  ...
  XQueryPosition(display,window_info->root,&x,&y);
  window_info->x=x-((3*window_info->width) >> 2);
  window_info->y=y-(window_info->height >> 1);
...
%
*/
Export int XMenuWidget(Display *display,XWindows *windows,char *title,
  char **selections,char *item)
{
  Cursor
    cursor;
...
#define NumberPreferences  7
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        preferences_info[4].raised=!resource_info->dither;
  ...
  */
  ...
  resource_info->dither=!preferences_info[4].raised;
...
Export void XTextViewWidget(Display *display,const XResourceInfo *resource_info,
  XWindows *windows,const unsigned int mono,char *title,char **textlist)
{
#define DismissButtonText  "Dismiss"
