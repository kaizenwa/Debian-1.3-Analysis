...
Status XGetWMColormapWindows(Display *display,Window window,Window **colormap_windows,
  int *number_windows)
{
  Atom
    actual_type,
...
int XWMGeometry(Display *display,int screen,char *user_geometry,char *default_geometry,
  unsigned int border_width,XSizeHints *size_hints,int *x,int *y,int *width,int *height,int *gravity)
{
  int
    status;
