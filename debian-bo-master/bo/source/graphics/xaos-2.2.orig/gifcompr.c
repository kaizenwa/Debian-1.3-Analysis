...
extern int GIFNextPixel(ifunptr);

#define maxmaxcode ((code_int)1 << BITS)	/* should NEVER generate this code */
#define maxbits BITS		/* user settable max # bits/code */
...
#ifndef _MAC
/* Can't compile 68K with this much global data.
   Get memory using gifcompr_init routine (see below). */
...
const static code_int hsize = HSIZE;
...
int gifcompr_init(void)
{
    htab = (count_int *) malloc(HSIZE * sizeof(count_int));
    if (htab == nil)
	return 0;

    codetab = (unsigned int *) malloc(HSIZE * sizeof(unsigned int));
    if (codetab == nil) {
	free(htab);
	return 0;
    }
...
#ifdef _MAC
    /* First time thru, allocate memory that used to be in
       global data segment (too much for 68K compiles) */
...
#else
	fputcMac(a_count, g_outfile);
	fwriteMac(accum, 1, a_count, g_outfile);
#endif
	a_count = 0;
    }
}
