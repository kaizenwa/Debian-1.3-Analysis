    ...
    case 'a':
      ...
      switch (Option_String[1])
      {
        ...
        default:
          ...
          if (sscanf (&Option_String[1], DBL_FORMAT_STRING, &floatval) != EOF)
          {
    ...
    case 'm':
      ...
      switch (Option_String[1])
      {
        ...
        case 'B':
          ...
          if (sscanf (&Option_String[2], "%ld", &longval) != EOF)
          {
...
char *value;
{
  ...
  {
    case BUF_SIZE_OP:
      if (sscanf (value, "%d", &opts.File_Buffer_Size) != EOF)
      {
    ...
    case ANTIALIAS_THRESH_OP:
      if (sscanf (value, DBL_FORMAT_STRING, &floatval) != EOF)
        opts.Antialias_Threshold = floatval;
      return;

    case ANTIALIAS_DEPTH_OP:
      if (sscanf (value, "%ld", &longval) != EOF)
    ...
    case HIST_SIZE_OP:
      if (sscanf (value, "%d.%d", &opts.histogram_x, &opts.histogram_y) == EOF)
      {
