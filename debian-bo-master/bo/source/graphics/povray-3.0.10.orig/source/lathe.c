...
#define MAX_INTERSECTIONS_PER_SEGMENT 6
...
typedef struct Lathe_Intersection_Structure LATHE_INT;

struct Lathe_Intersection_Structure
{
  DBL d;  /* Distance of intersection point                  */
  DBL w;  /* Paramter of intersection point with n-th spline */
...
static int intersect_lathe PARAMS((RAY *Ray, LATHE *Lathe, LATHE_INT *Int));
...
ISTACK *Depth_Stack;
{
  ...
  /* Allocate memory for intersections (two per segment). */

  I = (LATHE_INT *)POV_MALLOC(MAX_INTERSECTIONS_PER_SEGMENT*((LATHE *)Object)->Number*sizeof(LATHE_INT), "lathe intersection array");

  Found = FALSE;

  imax = intersect_lathe(Ray, (LATHE *)Object, I);

  if (imax)
  {
    for (i = 0; i < imax; i++)
    {
      if ((I[i].d > DEPTH_TOLERANCE) && (I[i].d < Max_Distance))
      {
        VEvaluateRay(IPoint, Ray->Initial, I[i].d, Ray->Direction);

        if (Point_In_Clip(IPoint, Object->Clip))
        {
          push_entry_i1_d1(I[i].d,IPoint,Object,I[i].n,I[i].w,Depth_Stack);
...
static int intersect_lathe(Ray, Lathe, Intersection)
RAY *Ray;
LATHE *Lathe;
LATHE_INT *Intersection;
{
  int i, ii, j, n1, n2, skip;
  ...
  /* Get distance r0 of the ray from rotation axis (= y axis). */
  ...
  /* Init number of intersections found. */
  ...
  for (j = 0; j < Lathe->Number; j++)
  {
    Entry = Lathe->Spline->Entry[j];

    /* Test if ray misses segment's cylindrical bound. */
    ...
    switch (Lathe->Spline_Type)
    {
      ...
      ************************************************************************/
      ...
      case QUADRATIC_SPLINE:
        ...
        /* First test the bounding cylinder. */

        if (Test_Thick_Cylinder(P, D, Entry.h1, Entry.h2, Sqr(Entry.r1), Sqr(Entry.r2)))
        {
#ifdef LATHE_EXTRA_STATS
          Increase_Counter(stats[Lathe_Bound_Tests_Succeeded]);
#endif
    ...
    while (n1--)
    {
      ...
      if ((w >= 0.0) && (w <= 1.0))
      {
        if (fabs(D[Y]) > EPSILON)
        {
          ...
          if (i > MAX_INTERSECTIONS_PER_SEGMENT * Lathe->Number)
          {
            Error("Too many intersections in intersect_lathe() found.\n");
          }
        }
        else
        {
          ...
          while (n2--)
          {
            ...
            /* Test if we already have this intersection. */

            for (ii = skip = 0; ii < i; ii++)
            {
              if (fabs(Intersection[ii].d - k) < EPSILON)
              {
                skip = TRUE;
  ...
  if ((P[Y] >= Lathe->Height1) && (P[Y] <= Lathe->Height2))
  {
    ...
    if ((r >= Lathe->Radius1) && (r <= Lathe->Radius2))
    {
      for (i = 0; i < Lathe->Number; i++)
      {
        ...
        if ((P[Y] - Entry.h1 >= -EPSILON) &&
            (Entry.h2 - P[Y] >= -EPSILON) && (r <= Entry.r2))
        {
          x[0] = Entry.A[Y];
          x[1] = Entry.B[Y];
          x[2] = Entry.C[Y];
...
UV_VECT *P;
{
  ...
  ****************************************************************************/
  ...
  for (i = 0; i < Lathe->Number; i++)
  {
    ...
    Lathe->Spline->Entry[i].r1 = min(min(x[0], x[1]), min(x[2], x[3]));
    Lathe->Spline->Entry[i].r2 = max(max(x[0], x[1]), max(x[2], x[3]));

    Lathe->Spline->Entry[i].h1 = min(min(y[0], y[1]), min(y[2], y[3]));
    Lathe->Spline->Entry[i].h2 = max(max(y[0], y[1]), max(y[2], y[3]));

    /* Keep track of overall bounding cylinder. */

    xmin = min(xmin, Lathe->Spline->Entry[i].r1);
    xmax = max(xmax, Lathe->Spline->Entry[i].r2);

    ymin = min(ymin, Lathe->Spline->Entry[i].h1);
    ymax = max(ymax, Lathe->Spline->Entry[i].h2);
  }
