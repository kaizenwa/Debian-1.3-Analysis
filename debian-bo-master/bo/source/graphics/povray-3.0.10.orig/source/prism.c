...
ISTACK *Depth_Stack;
{
  ...
  I = (PRISM_INT *)POV_MALLOC((((PRISM *)Object)->Number+2)*sizeof(PRISM_INT), "prism intersection array");

  max_i = intersect_prism(Ray, (PRISM *)Object, &I[0]);
  ...
  if (max_i)
  {
    for (i = 0; i < max_i; i++)
    {
      if ((I[i].d > DEPTH_TOLERANCE) && (I[i].d < Max_Distance))
      {
        ...
        {
          push_entry_i1_i2_d1(I[i].d,IPoint,Object,I[i].t,I[i].n,I[i].w,Depth_Stack);
...
/*
  Prism->Spline->Entry = (PRISM_SPLINE_ENTRY *)POV_REALLOC(Prism->Spline->Entry,
    Prism->Number*sizeof(PRISM_SPLINE_ENTRY), "spline segments of prism");
*/
