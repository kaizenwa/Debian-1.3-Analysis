...
ISTACK *Depth_Stack;
{
  ...
  /* Allocate temporary intersection list. */

  intervals = (Blob_Interval *)POV_MALLOC(2*Blob->Data->Number_Of_Components*sizeof(Blob_Interval), "temporary blob intervals");
  ...
  for (i = in_flag = 0; i < cnt; i++)
  {
    ...
    newcoeffs[3] = (2.0 * l * (l * (2.0 * coeffs[0] * l + 1.5 * coeffs[1]) + coeffs[2]) + coeffs[3]) * w;
    ...
    dk[2] = newcoeffs[4] + 0.50 * newcoeffs[3] + newcoeffs[2] / 6.0;
    dk[3] = newcoeffs[4] + 0.75 * newcoeffs[3] + 0.5 * newcoeffs[2] + 0.25 * newcoeffs[1];
    ...
    root_count = Solve_Polynomial(4, coeffs, roots, Test_Flag(Blob, STURM_FLAG), ROOT_TOLERANCE);
    ...
    for (j = 0; j < root_count; j++)
    {
      ...
      if ((dist >= intervals[i].bound) && (dist <= intervals[i+1].bound) &&
          (dist > DEPTH_TOLERANCE) && (dist < Max_Distance))
      {
        /* Correct distance. */
  ...
  /* Get rid off temporary intersection list. */
