...
Boolean winVis[MAXWIN] =
{FALSE};                /* is window visible  */

Boolean text[MAXWIN] =
{FALSE};                /* is text on  */
Boolean winFreeze[MAXWIN] =
{FALSE};                /* user requested menuFreeze  */
Boolean menuFreeze = FALSE;  /* menuFreeze while menus posted  */
Boolean timerOn = FALSE;  /* timer active  */
Boolean animation = TRUE;  /* idle func animation on  */
...
float angle = 0.0;      /* global rotation angle  */
char *textPtr[1000] =
...
enum {
  RGBA, INDEX, SINGLE, DOUBLE, DEPTH, ACCUM, ALPHA, STENCIL, MULTISAMPLE,
  STEREO, MODES
};
char *modeNames[] =
...
{
  int winId;
  static Boolean someVisible = TRUE;
  ...
  if (state == GLUT_VISIBLE) {
    ...
    winVis[idToIndex(winId)] = TRUE;
  } else {
    PR("Window %d not visible \n", winId);
    winVis[idToIndex(winId)] = FALSE;
  }

  if ((winVis[0] == FALSE) && (winVis[1] == FALSE) && (winVis[2] == FALSE)
    && (winVis[3] == FALSE) && (winVis[6] == FALSE) && (winVis[7] ==
      FALSE)) {
    glutIdleFunc(NULL);
