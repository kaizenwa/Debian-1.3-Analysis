...
float left, right, bottom, top, near, far;  /* ortho, view volume */
...
readVoxelData(void)
{
  ...
  if (tex3dSupported) {
    vptr = (unsigned char *) malloc((vol_width * vol_height * vol_depth));

    fread(vptr, sizeof(char), vol_width * vol_height * vol_depth, file);
    if (using_pipe) {
      pclose(file);
    } else {
      fclose(file);
    ...
    /* size of voxels is twice as the size of vptr, to duplicate alpha value
       = intensity */
    voxels = (unsigned char *) malloc((2 * vol_width * vol_height * vol_depth));
