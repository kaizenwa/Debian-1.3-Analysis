 ...
 */
#if defined(__STDC__) || defined(SYSV) || defined(THINK_C) || defined(applec) || defined(VMS)
...
/*
 * These expand the BSD typedefs.  This is done rather than typedef'ing
 * them so that we don't care whether or not <sys/types.h> defines them
 * (something that's hard to tell without trying to compile).
 */
