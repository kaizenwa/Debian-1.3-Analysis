...
bprint( resp_buf->atpbuf_info.atpbuf_data, resp_buf->atpbuf_dlen );
#endif
	if ( sendto( ah->atph_socket, resp_buf->atpbuf_info.atpbuf_data,
	  resp_buf->atpbuf_dlen, 0, (struct sockaddr *) atpb->atp_saddr,
	  sizeof( struct sockaddr_at )) != resp_buf->atpbuf_dlen ) {
	    if ( ah->atph_rxo ) {
		for ( ; i >= 0; --i ) {
		    free_buf( save_buf->atpbuf_info.atpbuf_xo.atpxo_packet[ i ] );
		}
		free_buf( save_buf );
	    }
	    return -1;
	...
	*/
	if ( ah->atph_rxo && ( resp_buf = alloc_buf()) == NULL ) {
	    return -1;
	}
    }
    free_buf( resp_buf );
