...
tty_lock(char *path, int mode)
{
  ...
  if (mode == 1) {	/* lock */
	...
	if ((pw = getpwnam(_UID_UUCP)) == NULL) {
		if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_user_unkn,
...
tty_hangup(void)
{
  ...
  if (tty_set_state(&tty) < 0) {
	if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_drop,
  ...
  if (tty_set_state(&tty_current) < 0) {
	if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_raise,
...
tty_open(char *name, char *speed)
{
  ...
  if (tty_get_state(&tty_saved) < 0) {
	if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_cant_state,
  ...
  if (tty_get_disc(&tty_sdisc) < 0) {
	if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_cant_disc,
  ...
  if (opt_m == 0) {
	if (tty_set_raw(&tty_current) < 0) {
		if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_cant_raw,
	...
	if (speed != NULL) {
		if (tty_set_speed(&tty_current, speed) != 0) {
			if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_cant_bps,
	...
	if (tty_set_databits(&tty_current, "8") ||
		...
		if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_cant_8n1,
...
usage(void)
{
  fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_usage1,
...
main(int argc, char *argv[])
{
  ...
  opterr = 0;
  while ((s = getopt(argc, argv, "c:ehlLmnp:qs:vd")) != EOF) switch(s) {
	case 'c':
		extcmd = optarg;
		break;
  ...
  if ((ht = get_hwtype(proto)) == NULL && strcmp(proto, "tty")) {
	if (opt_q == 0) fprintf(stderr, NLS_CATGETS(catfd, slattachSet, slattach_unsupp,
  ...
  if (!strcmp(path, "-")) {
	opt_e = 1;
	sp = NULL;
	if (tty_open(NULL, speed) < 0) {   NLS_CATCLOSE(catfd)  return(3); }
  } else {
	if ((sp = strrchr(path, '/')) != NULL) *sp++ = '\0';
	  else sp = path;
	if (tty_open(sp, speed) < 0) {   NLS_CATCLOSE(catfd)  return(3); }
  }
  ...
  if (opt_v == 1) {
	tty_get_name(buff);
	printf( NLS_CATGETS(catfd, slattachSet, slattach_started, "%s started"), proto);
	if (sp != NULL) printf( NLS_CATGETS(catfd, slattachSet, slattach_on, " on %s"), sp);
	printf( NLS_CATGETS(catfd, slattachSet, slattach_iface, " interface %s\n"), buff);
  }
