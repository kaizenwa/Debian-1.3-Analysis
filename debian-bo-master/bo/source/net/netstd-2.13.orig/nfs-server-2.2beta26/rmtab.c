...
static _PRO(char * rmtab_gethost, (struct svc_req *));
static _PRO(int    rmtab_insert, (char *, char *));
static _PRO(void   rmtab_file, (char));
...
void rmtab_add_client(path, rqstp)
dirpath path;
...
void rmtab_del_client(path, rqstp)
dirpath path;
struct svc_req *rqstp;
{
	int		p0, p1, changed;
...
static char *rmtab_gethost(rqstp)
struct svc_req *rqstp;
{
	struct hostent *hp;
        struct in_addr addr;
...
static int rmtab_insert(hostname, path)
char *hostname, *path;
{
	int		hostlen, p0, p1;
	mountlist	cur, prv;
