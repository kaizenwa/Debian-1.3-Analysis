  ...
  while ((opt = getopt(argc, argv, OPTIONS)) > 0)
	...
	*user_name=strcpy((char *)malloc((strlen(optarg)+1)*sizeof(char)),optarg);
    ...
    case 'p':
      if ( optarg!= NULL )
      {
	*passwd=strcpy((char *)malloc((strlen(optarg)+1)*sizeof(char)),optarg);
	(void)strncpy(optarg,"",strlen(optarg));
      }
      break;
    case 'd':
      *debug = 1;
...
void set_signal(int sig)
{
  ...
  default:
    ...
    {
      action.sa_handler = echo_sig;
      action.sa_flags = SA_RESTART | SA_ONESHOT | SA_NOMASK;
...
void echo_sig(int sig)
{
  ...
  if (debug) 
    fprintf(stderr,"rexec: echo_sig: received signal %d\n",sig);

  if ( (aux_sock > 0) && (write(aux_sock, &sigch, 1) < 1) )
    fprintf(stderr,"%s: Error: Signal %d was not properly sent to the remote "
	    "process.\n",progname,sig);
    
  
  if ( sig == SIGTSTP )
    fprintf(stderr,"%s: The stop signal does not translate correctly to all"
