...
/*
 * $Id: sioconf.h,v 1.2 1995/09/10 18:35:09 chuck Exp $
 */
 ...
 * OS-specific section.
 *
 * Features here use the flag HAS_<feature>.
 * List of flags (check the following for macros that can be overriden):
 *
 *			HAS_MMAP (overridable macros)
 *
 *			HAS_ATEXIT
 *			HAS_ONEXIT
 *			HAS_OTHER_FINALIZER	(must define macros)
 *
 *			HAS_MEMOPS
 *			HAS_BCOPY 			(HAS_MEMOPS will be preferred if both are defined)
 *
 ...
 * arguments).
 */
#if defined(HAS_ONEXIT) || defined(HAS_ATEXIT) || defined(HAS_FINALIZER)

#define HAS_FINALIZATION_FUNCTION

#if defined( HAS_ONEXIT ) && defined( HAS_ATEXIT )
#undef HAS_ONEXIT
#endif
...
#endif	/* HAS_ONEXIT || HAS_ATEXIT || HAS_FINALIZER */
 ...
 */
#if !defined(HAS_ISATTY) && !defined(HAS_BSDTTY) && !defined(HAS_SYSVTTY)
...
#if defined(HAS_BSDTTY) && defined(HAS_SYSVTTY)
ERROR HAS_BSDTTY_and_HAS_SYSVTTY_both_defined ;
#endif
