...
long32 value;
{
 return (relative_time(now-value));
}
...
aMsgClient *msgclient;
{
 ...
 strcat(mbuf, buf); strcat(mbuf, ctrlbuf); 
 strcat(mbuf, myitoa(now-msgclient->time)); 
...
void save_messages()
{
        ...
        if (msgclient->flags & FLAGS_DISPLAY_IF_DEST_REGISTER) {
            *mbuf = '\0';
            ...
            if (*get_msg(msgclient, 'n') == '-') {
              if (now > msgclient->time + t2)
                  msgclient->timeout = 0; 
            } else {
                     if (now > msgclient->time + t2 + atoi(mbuf))
                         msgclient->timeout = 0;
                } 
        }       
       t++;
 ...
 t = 1;
 w_code(Nltoa((long32)note_msm),fp);
 w_code(Nltoa((long32)note_mum),fp);
 w_code(Nltoa((long32)note_msw),fp);
 w_code(Nltoa((long32)note_muw),fp);
 w_code(Nltoa((long32)note_mst),fp);
 w_code(Nltoa((long32)note_msf),fp);
 ...
 while (fromname_index && t <= fromname_index) {
     ...
     w_code(Nltoa(msgclient->flags),fp);
     w_code(Nltoa(msgclient->timeout),fp);
     w_code(Nltoa(msgclient->time),fp);
...
aChannel *sptr_chn;
{
 ...
 if ((!secret || right_tonick) 
     ...
     strcat(mbuf, myitoa(now-msgclient->time)); strcat(mbuf, buf);
     if (*get_msg(msgclient, '2'))
        strcat(mbuf, get_msg(msgclient, '2')); 
      else strcat(mbuf, myitoa(now-msgclient->time));
     strcat(mbuf, buf);
...
while (send && qptr != sptr &&
        ...
        if (!right_tonick && secret && now-msgclient->time < SECONDS_WEEK)
       ...
       *gnew = 1;
       ...
       new(msgclient->passwd,"SERVER_RECEIVED","-","-",
           c ? c : msgclient->fromnick, msgclient->fromname,
           local_host(msgclient->fromhost), gflags,
	   note_mst*SECONDS_DAY+now, now,buf);
       break;
    }
 while (send && msgclient->flags & FLAGS_FIND_CORRECT_DEST_SEND_ONCE) {
        ...
        *gnew = 1;
        new(msgclient->passwd,msgclient->fromnick, msgclient->fromname,
            ...
            sptr->user->host,gflags,msgclient->timeout,now,"");
        break;
   }
...
char mode;
{
 ...
 if (!file_inited) return;
 if (now > saved_clock+note_msf) save_messages();
    ...
    while (last && t <= last) {
           ...
           if (!(msgclient->flags & FLAGS_KEY_TO_OPEN_OPER_LOCKS)
               ...
               if (message) {
                   ...
                   else if (!is_silenced(qptr, sptr)) sendto_one(sptr,
...
char *param;
{
  ...
  if (t <= last && now < msgclient->timeout) {
     ...
     if (!matches(toname, msgclient->toname) 
      ...
      /* If old request matches new... */
		 ...
		 /* If it's distributing, and there is no root access:
		    ...
		    && (!(msgclient->flags & FLAGS_SERVER_GENERATED) || 
		       (now - msgclient->time < SECONDS_SERVER_DISTRIBUTE)))
		   return;
		 ...
		 if (strcmp(msgclient->fromname, fromname))
			...
			create_new = 0;
		        msgclient->time = now;
			if (msgclient->timeout < now + seconds) 
			  msgclient->timeout = now + seconds;
  ...
  if (create_new) {
	  ...
	  tohost, gflags, now + seconds, now, message);
    }
#endif
just_distribute:
...
int seconds;
{
 ...
 if (IsOperHere(sptr)) {
    fast_distribute_timeout = now + seconds;
...
static void request_distributor()
{
  ...
  if (now - last_count_distribute > 1800) {
      last_count_distribute = now; f = number_distribute();
      if (!f) return;
  ...
  if (now < fast_distribute_timeout) delay = 1;
  if (!fromname_index || (now - last_call < delay)) return;
  last_call = now;
  for (count = 0; count < (fromname_index >> 3) || count < 10; count++) {
	  ...
	  f = 0;
	  if (msgclient->flags & FLAGS_SERVER_GENERATED) {
	      if (now - msgclient->time < SECONDS_SERVER_DISTRIBUTE) 
		  continue;
...
time_t *delay;
{
 ...
 request_distributor();
 if (now - last_call < MIN_DELAY) return;
 last_call = now; 
 garbage_collector();
}
...
char *passwd, *flag_s, *timeout_s, *name, *message;
{
 ...
 while (last && t <= last) {
        ...
        if (!mycmp(sptr->name, msgclient->fromnick)
            ...
            msgclient->timeout = timeout*3600+now;
            join = 1;
	  }
        t++;
    }
 ...
 if (!join) {
     ...
     msgclient = new(passwd,sptr->name, UserName(sptr),  
                     ...
                     timeout*3600+now, now, clean_spychar(message));
...
char *arg, *passwd, *flag_s, *id_s, *name, *time_s;
{
 ...
 while (last && t <= last) {
	...
	msgclient->id = ++id_count;
        flags = msgclient->flags;
        if (now > msgclient->timeout || channel 
           && !(msgclient->flags & FLAGS_CHANNEL) 
           && !(msgclient->flags & FLAGS_DENY)) { 
           t++; continue; 
        }
        ...
        if (local_check(sptr,msgclient,passwd,flags,
		    ...
		    sprintf(mbuf,"[%s!%s@%s] %s", msgclient->fromnick,
			    msgclient->fromname, msgclient->fromhost, dibuf);
                    ...
                    sprintf(buf,"for %s", 
                           relative_time(msgclient->timeout-now));
 ...
 if (users) {
    ...
    for (t = 1; t <= fromname_index; t++) {
         ...
         do next_msgclient = t1 < fromname_index ? FromNameList[++t1] : 0;
          while (next_msgclient && next_msgclient->timeout <= now);
         if (msgclient->timeout > now
             && (!time_l || msgclient->time >= time_l 
                 && msgclient->time < time_l+SECONDS_DAY)
                 ...
                 || !(host_check(next_msgclient->fromhost,
                  ...
                  sendto_one(sptr,"NOTICE %s :### %s[%d] %s (%s@%s) @%s",
                             ...
                             *delete ? 1 : count+1,
                             ...
                             local_host(msgclient->fromhost),
                             msgclient->fromhost);
                    if (*delete) msgclient->timeout = now-1;
                 count = 0; number = 1;
	     } else count++;
	 }
     }
