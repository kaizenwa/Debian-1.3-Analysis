    ...
    char **argv;
{
    ...
    fname = ckalloc (strlen(TKINEDLIB)+30);
    strcpy (fname, TKINEDLIB);
    strcat (fname, "/tkined.defaults");
    ReadDefaultFile (editor, interp, findfile (fname));
    strcpy (fname, TKINEDLIB);
    strcat (fname, "/site/tkined.defaults");
    ReadDefaultFile (editor, interp, findfile (fname));
    ckfree (fname);

    ReadDefaultFile (editor, interp, findfile ("~/.tkined/tkined.defaults"));
    ReadDefaultFile (editor, interp, findfile ("tkined.defaults"));
