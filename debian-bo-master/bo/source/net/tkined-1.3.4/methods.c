...
m_graph_create        _ANSI_ARGS_((Tcl_Interp *interp, Tki_Object *object,
				    int argc, char **argv));
static int
m_html_create	      _ANSI_ARGS_((Tcl_Interp *interp, Tki_Object *object,
				    int argc, char **argv));
static int
...
m_graph_retrieve      _ANSI_ARGS_((Tcl_Interp* interp, Tki_Object *object,
				   int argc, char** argv));
static int
m_html_retrieve	      _ANSI_ARGS_((Tcl_Interp *interp, Tki_Object *object,
				    int argc, char **argv));
static int
...
static int
m_html_dump        _ANSI_ARGS_((Tcl_Interp *interp, Tki_Object *object));
     ...
     ClientData clientData;
{
	...
	Tcl_VarEval (interp, type_to_string (elem->object->type),
		     "::resize ", elem->object->id, (char *) NULL);
    ...
    char **argv;
{
    ...
    trace (object->editor, (Tki_Object *) NULL, 
    ...
    char **argv;
{
    ...
    if ((file = findfile (largv[0])) == (char *) NULL) {
    ...
    pargv = (char **) ckalloc ((largc + 4) * sizeof (char *));
    memset ((char *) pargv, 0, (largc + 4) * sizeof (char *));
	...
	*p = 0;
    ...
    num = Tcl_CreatePipeline (interp, pargc, pargv, &pidArray, 
			      &object->xv[1], &object->xv[0], NULL);
...
/*
 * Create a HTML object (window). Initialize the id and name fields.
 */

static int
m_html_create (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
{
    ...
    trace (object->editor, (Tki_Object *) NULL, 
	   "ined create HTML", argc, argv, object->id);
...
static int
m_data_create (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
...
static int
m_event_create (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
    ...
    char **argv;
{
 ...
 case TKINED_HTML:
	return m_html_create (interp, object, argc, argv);
 case TKINED_DATA:
...
/*
 * Retrieve the external representation of a HTML object.
 */

static int
m_html_retrieve (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
{
    Tcl_AppendElement (interp, "HTML");
    Tcl_AppendElement (interp, object->id);
    Tcl_AppendElement (interp, object->name);
    Tcl_AppendElement (interp, object->address);
...
static int
m_data_retrieve (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
...
static int
m_event_retrieve (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
    ...
    char **argv;
{
 ...
 case TKINED_HTML:
	return m_html_retrieve (interp, object, argc, argv);
 case TKINED_DATA:
    ...
    char **argv;
{
	...
	if (object->type == TKINED_GROUP && !object->collapsed) {
	    ...
	    if (object->member) {
	        int i;
	        for (i = 0; object->member[i]; i++) {
		    notrace (m_move, interp, object->member[i], argc, argv);
		}
	    }
	}
...
/*
 * Return a string of ined commands that rebuilds the html object.
 */

static int
m_html_dump (interp, object)
    Tcl_Interp *interp;
    Tki_Object *object;
{
    Tcl_AppendResult (interp, "set ", object->id, 
		      " [ ined -noupdate create HTML ]\n", (char *) NULL);
...
/*
 * Append some text to the simple HTML viewer.
 */

#if 1

int
m_html_append (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
{
    Tcl_DString taggedList;

    Tcl_DStringInit (&taggedList);

    if (argc != 1) {
	return TCL_OK;
    }

    /* the left over tags should be a member of the object */

    if (HTML_Parse (argv[0], NULL, &taggedList)) {
        fprintf (stderr, "XXX fix me -- some tags left open\n");
    }

    Tcl_VarEval (interp, type_to_string (object->type),
		 "::html ", object->id, " ", Tcl_DStringValue (&taggedList),
		 (char *) NULL);

    Tcl_DStringFree (&taggedList);
    ...
    char **argv;
{
	...
	{
	    int i, c, size = (argc + 1) * sizeof (Tki_Object *);
...
int
m_bell (interp, object, argc, argv)
    Tcl_Interp *interp;
    Tki_Object *object;
    int argc;
    char **argv;
{
    Tcl_VarEval (interp, type_to_string (object->type),
		 "::bell ", object->id, (char *) NULL);

    trace (object->editor, object, "ined bell", argc, argv, "");
    ...
    char **argv;
{
	...
	while (object->member[0]) {
	    notrace (m_delete, interp, object->member[0], 0, (char **) NULL);
	    Tcl_ResetResult (interp);
	}
	ckfree ((char *) object->member);
    }
