...
proc Diagram::create { editor } {
    ...
    $w.menu.diag.m add command -label "Limit Axis" \
	    -command "Diagram::SetLimits $editor"
    $w.menu.diag.m add command -label "PostScript" \
	    -command "Diagram::PostScript $editor"
...
proc Diagram::Color { editor color } {
    set w [$editor graph]
    foreach graph [$w.blt element names] {
	if {[$graph selected]} {
	    $graph color $color
...
proc Diagram::SetLimits { editor } {
    ...
    set result [Dialog::request $w "Set the limits on the axis:" [list \
...
proc Diagram::PostScript { editor } {
	...
	if {![file writable $fname]} {
	    Dialog::acknowledge $w "Can not write $fname"
	    return
	}
	set res [Dialog::confirm $w "Replace file $fname?" \
		 [list replace cancel]]
	if {$res == "cancel"} return
    }

    if {[catch {open $fname w} f]} {
	Dialog::acknowledge $w "Can not open $fname: $f"
	return
    }

    if {[catch {puts $f [$w.blt postscript]} err]} {
	Dialog::acknowledge $w "Failed to write $fname: $err"
    }
