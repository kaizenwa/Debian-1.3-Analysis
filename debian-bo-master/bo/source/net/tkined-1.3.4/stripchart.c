...
static int
CreateStripchart(interp, canvas, itemPtr, argc, argv)
    ...
    char **argv;			/* Arguments describing stripchart. */
{
    ...
    if (StripchartCoords(interp, canvas, itemPtr, i, argv) != TCL_OK) {
	goto error;
    }

    if (ConfigureStripchart(interp, canvas, itemPtr, argc-i, argv+i, 0)
	   == TCL_OK) {
	return TCL_OK;
    }
...
static int
StripchartCoords(interp, canvas, itemPtr, argc, argv)
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc == 0) {
	Tcl_PrintDouble(interp, stripPtr->bbox[0], c0);
        Tcl_PrintDouble(interp, stripPtr->bbox[1], c1);
        Tcl_PrintDouble(interp, stripPtr->bbox[2], c2);
        Tcl_PrintDouble(interp, stripPtr->bbox[3], c3);
...
static int
ConfigureStripchart(interp, canvas, itemPtr, argc, argv, flags)
    Tcl_Interp *interp;         /* Interpreter for error reporting. */
					 ...
					 * recomputed. */
{
    ...
    stripPtr->header.x2 = stripPtr->bbox[2] + 1;
    stripPtr->header.y2 = stripPtr->bbox[3] + 1;
...
#if 0
    if (stripPtr->textGC != None) {
	XDrawString(display, drawable, stripPtr->textGC, x2 - x1, y2 + 10,
		    "nase", 4);
    }
#endif
}
