...
#ifndef lint
static	char	rcsid[] = "@(#)$Id: hook.c,v 1.31 1995/11/14 05:59:17 scottr Exp $";
#endif
...
static	int	Add_Remove_Check _((Hook *, char *));
		...
		quiet;
{
	...
	if (nick)
	{
		setup_struct((server == -1) ? -1 : (server & ~HS_NOGENERIC),
			sernum-1, sernum, 0);
		if ((tmp = (Hook *) remove_from_list_ext((List **) &hook_functions[which].list, nick, (int (*) _((List *, char *))) Add_Remove_Check)) != NULL)
		{
			if (!quiet)
