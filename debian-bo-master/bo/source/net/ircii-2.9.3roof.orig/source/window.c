...
#ifndef lint
static	char	rcsid[] = "@(#)$Id: window.c,v 1.64.2.2 1996/07/20 19:14:55 mrg Exp $";
#endif
	...
	int	delete;
{
	...
	if (found && delete)
	{
		...
		{
			/* if the channel has a window, _don't_ change it */
			if (!my_stricmp(chan->channel, channel) || chan->window)
				continue;

			if (!get_int_var(SAME_WINDOW_ONLY_VAR) && !is_bound(chan->channel, server) && (chan->window != found_window))
			{
				int	is_current = 0,
				...
				while (tmp = traverse_all_windows(&flag))
				{
					if (tmp->server != server)
						continue;
					if (tmp->current_channel && !my_stricmp(chan->channel, tmp->current_channel))
						is_current = 1;
				}
				if (!is_current)
					possible = chan;
		...
		if (!get_int_var(SAME_WINDOW_ONLY_VAR) && possible)
			set_channel_by_refnum(found_window->refnum, possible->channel);
	}
	return (found);
}
	...
	int	misc;
{
	...
	else
	{
		...
		{
#if 0	/* this does the Wrong Thing! */
			realloc_channels(window);
#endif
		...
		*subargs;
{
	...
	while ((arg = next_arg(args, &args)) != NULL)
	{
		...
		else if (strncmp("CHANNEL", cmd, len) == 0)
		{
			...
			else
			{
				...
				add_to_join_list(arg, from_server,
					window->refnum);
				from_server = server;
			}
				}
	...
	if (no_args)
	{
		...
		if (window->server_group)
			say("\tServer Group is (%d) %s", window->server_group, server_group_list[window->server_group - 1].name);
		if (window->nicks)
		{
			NickList *tmp;
	...
	char	*args;
{
	...
	if (arg)
	{
		...
		else if ((i = parse_server_index(arg)) != -1)
			port_num = server_list[i].port;
	}
	else
	{
