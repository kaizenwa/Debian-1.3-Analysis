  ...
  if (setup_stream(q, M_PARSE))
    {
      ...
      if (!notice)
	{
	  cmn_err(CE_CONT, "?%s: Copyright (c) 1993-1996, Frank Kardel\n", modlstrmod.strmod_linkinfo);
	  notice = 1;
	}
...
static int parserput(queue_t *q, mblk_t *imp)
{
  ...
  switch (type = mp->b_datap->db_type)
	...
	ctime.tv.tv_sec  = hrestime.tv_sec;
	ctime.tv.tv_usec = hrestime.tv_nsec / 1000;
  ...
  if (za->za_rr0 ^ (cdstate = zsstatus & ZSRR0_CD))
    {
       ...
       */
      gethrestime(&hrestime);
      cdevent.tv.tv_sec  = hrestime.tv_sec;
      cdevent.tv.tv_usec = hrestime.tv_nsec / 1000;
 ...
 *
 * Revision 3.24  1996/12/01 13:00:04  kardel
 * first working auto-configure version
 *
 * Revision 3.23  1996/11/24 23:16:33  kardel
 * checkpoint - partial autoconfigure update for parse modules
 *
 * Revision 3.22  1996/11/16 18:28:32  kardel
 * cleaned up version of parsesolaris.c
 * currently untested
 * remaining issues to be solved:
 *   how to de-install a zs interrupt
 *   safely not using splhigh()
 *
 * Revision 3.21  1996/10/05 13:30:23  kardel
 * general update
 *
 * Revision 3.20  1995/08/06  19:15:20  kardel
 * parsesolaris seems to be stable now
 *
 * Revision 3.19  1995/08/05  07:17:37  kardel
 * removed sign extension trap
 *
 * Revision 3.18  1995/06/18  12:10:51  kardel
 * removed dispersion calulation from parse subsystem
 *
 * Revision 3.17  1994/07/09  21:31:38  kardel
 * use gethrtime instead of uniqtime(which calls mutex_enter/exit->fatal in ISRs
 *
