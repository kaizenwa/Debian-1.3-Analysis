...
#define	CONFIG_AUTHDELAY	10		/* (not used) */
	...
	char *argv[];
{
	...
	while ((c = ntp_getopt(argc, argv, xntp_options)) != EOF)
		...
		case 'd':
		case 'D':
		  msyslog(LOG_ERR, "xntpd not compiled with -DDEBUG option - no DEBUG support");
