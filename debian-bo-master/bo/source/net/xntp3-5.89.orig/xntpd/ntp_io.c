...
# ifdef STREAMS_TLI
  ...
  if(ioctl(vs, I_STR, &ioc) < 0 ||
    ...
    {
      msyslog(LOG_ERR, "get interface configuration: %m");
      exit(1);
    }
...
#  endif /* SYS_WINNT */
    {
      msyslog(LOG_ERR, "get interface configuration: %m - exiting");
      exit(1);
    }
...
#  ifdef STREAMS_TLI
	...
	{
	  msyslog(LOG_ERR, "Failed to get interface flags: %m");
	  continue;
	}
#  else /* not STREAMS_TLI */
...
#  ifdef STREAMS_TLI
	  ...
	  if(ioctl(vs, I_STR, &ioc))
	    {
	      msyslog(LOG_ERR, "SIOCGIFBRDADDR fails");
	      exit(1);
	    }
#  else /* not STREAMS_TLI */
	  if (ioctl(vs, SIOCGIFBRDADDR, (char *)&ifreq) < 0)
	    {
	      msyslog(LOG_ERR, "SIOCGIFBRDADDR fails: %m");
	      exit(1);
	    }
#  endif /* not STREAMS_TLI */
...
#  ifdef STREAMS_TLI
      ...
      if(ioctl(vs, I_STR, &ioc))
	{
	  msyslog(LOG_ERR, "SIOCGIFNETMASK fails");
	  exit(1);
	}
#  else /* not STREAMS_TLI */
...
#if 0	/* GMS */
		  rb->recv_length =
		    recv(fd, (char *)&rb->recv_space, i, 0);
#else
# ifndef SYS_WINNT				
		  rb->recv_length =
