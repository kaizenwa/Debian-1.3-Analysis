...
int
CMIP_Action(cmiph, interp, argc, argv)
    CMIP_Handle	*cmiph;
    Tcl_Interp  *interp;
    int          argc;
    char       **argv;
{
    ...
    char	 *strActionType	= NULLCP;
    char	 *strActionValue= NULLCP;
    ...
    MName	  obj_inst;
    /*
     * A christmas game: Find out what ava, Ava, AVA and aVa are.
     */
    Ava		 *aVa      = NULLAVA;
    ...
    /* extract action-type and perhaps -value */
    if (Tcl_SplitList(interp, argv[4], &listArgc, &listArgv) != TCL_OK) {
	return TCL_ERROR;
    }
    if (listArgc == 1) {
	strActionType = ckstrdup(listArgv[0]);
    } else if (listArgc == 2) {
	strActionType = ckstrdup(listArgv[0]);
	strActionValue = ckstrdup(listArgv[1]);
    } else { /* not correct */
    ...
    if (strActionType) {
      fprintf(stderr, " {%s", strActionType);
      if (strActionValue) {
	fprintf(stderr, " %s", strActionValue);
      }
      fprintf(stderr, "}");
    }
    if (strScope)
    ...
    if ((aVa = str2ava(strActionType, strActionValue)) == NULLAVA) {
	Tcl_AppendResult (interp, "no such action type \"", strActionType,
			  "\" or value \"", strActionValue, "\"",
			  (char *)NULL);
	return TCL_ERROR;
    }
    actionType.mid_type   = MID_GLOBAL;
    actionType.mid_global = aVa->ava_oid;
    actionValue		  = aVa->ava_value;
    ...
    /* perform the M-ACTION request */
    (cmiph->req_nr)++;
    if (nonconfirmed) {
	status = M_Action(cmiph->msd, cmiph->req_nr, &obj_class, &obj_inst,
			  scope, filter, NULLMACCESS, sync,
			  &actionType, aVa->ava_value, &mi);
    } else {
	status = M_ActionConf(cmiph->msd, cmiph->req_nr, &obj_class, &obj_inst,
			      scope, filter, NULLMACCESS, sync,
			      &actionType, aVa->ava_value, &mi);
    }
    if (status == NOTOK) {
    ...
    if (!nonconfirmed) {
	status = CMIP_WaitResponse(interp, cmiph, M_ACTION, callback);
    }
    ...
    if (strActionType) {
	ckfree((char*) strActionType);
    }
    if (strActionValue) {
	ckfree((char*) strActionValue);
    }
