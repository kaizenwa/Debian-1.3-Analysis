...
##
## Get the current date. We do not use extend tcl's getclock command
## to remain independend of extended tcl.
##
...
proc discover_nodes {network} {
	...
	writeln "$count nodes found on network $network in [expr {[getclock]-$start}] seconds."
	flush stdout
    }
...
proc trace_route {iplist} {
	...
	set i 0
	set new_todo ""
	foreach aa [icmp trace $ttl $todo] {
	    set ip [lindex $todo $i]
	    set trace_ip [lindex $aa 0]
	    set trace_time [lindex $aa 1]
	    if {$trace_time < 0} {
                lappend routetable($ip) ""
            } else {
...
proc discover_traces {} {
    ...
    writeln "$count routes traced in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_masks {} {
    ...
    writeln "$count netmasks queried in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_snmp {} {
    ...
    writeln "$count snmp agents queried in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_fips {} {
    ...
    writeln "$count ip addresses queried in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_networks {} {
    ...
    writeln "$count networks discovered in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_gateways {} {
    ...
    writeln "$count gateways discovered in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc discover_links {} {
    ...
    writeln "$count links discovered in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
debug "--   checking $ip for name"
    ...
    writeln "$count nodes merged in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc talk_to_ined {} {
    ...
    writeln "$count tkined objects created in [expr {[getclock]-$start}] seconds."
    flush stdout
}
...
proc "Discover IP Network" {list} {
    ...
    writeln "Discover finished in [expr {[getclock]-$start}] seconds"
    writeln
}
...
proc "Show TCP Server" {list} {
    ...
    foreach comp $list {
	    ...
	    if {![catch {tcp connect $ip $port} f]} {
		tcp shutdown $f all
		tcp close $f
		write   [format "%-32s \[" $host ]
		write   $ip "IpFlash $ip"
...
proc test_user_host {user host level} {
    ...
    if {![catch {dns ptr [nslook [exec hostname]]} hostname]} {
