...
proc MoJoCheckIds { cmd ids } {
    ...
    if {$new_ids != $ids} {
	    ...
	    [job current] command [list $cmd $new_ids]
        }
    }
...
proc MoJoSelect { action } {
	...
	set hosts [lindex [$j command] 0]
	foreach id [lindex [$j command] 1] {
...
proc MoJoModify {} {
	...
	if {$jobid == $job} {
	    set jobcmd  [$job command]
	    set jobitv  [expr {[$job interval] / 1000.0}]
	    set jobstat [$job status]
	}
    }
    ...
    if {$jobstat == "active"}  { $jobid status waiting }
    if {$jobstat == "suspend"} { $jobid status suspended }
...
proc MoJoAction { id msg } {
	...
	if {$jobid != ""} {
	    set secs [expr {[$jobid interval] / 1000}]
	} else {
	    set secs 2
	}
	ined flash $id $secs
...
proc SnmpParameter {} {
	...
	switch [lindex $res 0] {
		...
		if {[catch {snmp session -alias $alias} s]} {
		    ined acknowledge $s
		} else {
...
proc SnmpOpen { id ip } {
	...
	if {[lindex $res 0] == "edit"} {
	    ...
	    return [snmp session -alias $alias]
	}
    }
	...
	set s [snmp session -timeout $snmp_timeout -retries $snmp_retries]
