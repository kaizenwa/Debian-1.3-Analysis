...
#else
extern 	long		find_item (array, find)
	an_array     	array;
	char *		find;
#endif
{
...
#else
extern 	void          	insert_index (index, size, newIndex)
	long **		index;
	long *		size;
	long 		newIndex;
#endif
{
	...
	if (*size)
		*index = (long *)new_realloc((char *)*index, (*size + 1) * sizeof(long));
	else
	{
...
#else
extern	void         	move_index(array, oldindex, newindex)
	an_array *	array;
	long		oldindex;
	long		newindex;
#endif
...
#else
extern	long         	find_index (array, item)
	an_array *	array;
	long         	item;
#endif
{
...
#else
extern	an_array *	get_array(name)
	char *		name;
#endif
{
	long index;
...
#endif
{
        ...
        if (array_info.size > 1)
        {
                ...
                array_info.item = (char**)new_realloc((char *)array_info.item, sizeof(char *) * array_info.size);
                array_info.index = (long *)new_realloc((char *)array_info.index, sizeof(long) * array_info.size);
                array_array = (an_array *)new_realloc((char *)array_array, sizeof(an_array) * array_info.size);
        }
        else
        {
...
#else 
#define BUILT_IN_FUNCTION(x, y) char * x ( y ) char * y; 
#endif
...
/*CDE Why?						new_free(&array->item[item]);*/
						malloc_strcpy(&array->item[item], input);
						result = 0;
					}
					else if (item == array->size)
					{
						array->item = (char **)new_realloc((char *)array->item, (array->size + 1) * sizeof(char*));
				...
				else
				{
					...
					{
						if (array_info.size)
							array_array = (an_array*)new_realloc((char *)array_array, (array_info.size + 1) * sizeof(an_array));
						else
						...
						if (array_info.size)
							array_info.item = (char **)new_realloc((char *)array_info.item, (array_info.size + 1) * sizeof(char *));
						else
                        ...
                        if (item >= 0 && item < array->size)
			{
				...
				else
				{
					...
					for(strptr=&(array->item[item]), cnt=item; cnt < array->size; cnt++, strptr++)
						*strptr = *(strptr + 1);
					array->item = (char**)new_realloc((char *)array->item, sizeof(char *) * array->size);
					array->index = (long*)new_realloc((char *)array->index, sizeof(long) * array->size);
				}
				found = 0;
                        }
                }
