...
aConfItem *make_conf _((void))
{
	return (aConfItem *) new_malloc(sizeof(aConfItem));
}

void report_conf_links(NickList *sptr, aConfList *List, int numeric, char c)
{
	...
	for (current = 0; current < List->length; current++)
	{
		...
		for(;ptr;ptr=ptr->next)
		{
			...
			if (!(tmp=ptr->conf))
				continue;
			host = !tmp->host ? null : tmp->host;
			pass = !tmp->passwd ? null : tmp->passwd;
			name = !tmp->name ? null : tmp->name;
			port = (int)tmp->port;
...
char	*my_string;
{
	...
	rev = (char *) new_malloc(strlen(my_string)+1);
...
aConfItem	*my_conf;
{
	...
	if ((ptr = strchr(my_conf->host, '@')) != NULL)
		...
		ptr = my_conf->host;
	tmp = (char *) new_malloc(strlen(ptr)+1);
	strcpy(tmp, ptr);
...
aConfList	*my_list;
{
	...
	if ((length % 5) == 0)	/* Re-alloc only after 5 entries -Sol */
	{
		...
		new = (aConfEntry *) new_malloc((length+100)*sizeof(aConfEntry));
		if (base)
		{
...
char		*(*cmp_field)();
{
	...
	while (lower != upper)
	{
		pos = my_stricmp(field, base[compare].pattern);
	...
	if (!pos)
	{
		...
		new_free((char **)&field);
		new = (aConfEntry *) new_malloc(sizeof(aConfEntry));
		new->pattern = NULL;
		new->conf = my_conf;
...
/*				memset(base[compare].sub, 0, sizeof(aConfList));*/
		...
		if (match(field, base[compare].pattern))
		{
			...
			/* Create sublist -Sol */
			new = (aConfList *) new_malloc(sizeof(aConfList));
			new->length = top-bottom+1;
			new->conf_list = (aConfEntry *)
				new_malloc((new->length/5+1)*5*
					sizeof(aConfEntry));
			...
			if (ptr->conf)
				free_conf(ptr->conf);
                        while (ptr->next)
                        {
                                ...
                                if (ptr->next->pattern)
                                        new_free((char **)&ptr->next->pattern);
				if (ptr->next->sub)
				{
					clear_conf_list(ptr->next->sub);
					new_free((char **)&ptr->next->sub);
				}
				if (ptr->next->conf)
					free_conf(ptr->next->conf);
                               	new_free((char **)&ptr->next);
                                ptr->next = tmp;
                        }
                        if (ptr->pattern)
                                new_free((char **)&ptr->pattern);
                }
                new_free((char **)&my_list->conf_list);
	}
...
char		*tomatch;
{
	...
	if (my_list && tomatch)
	{
		...
		while (lower != upper)
		{
			...
			if (match(base[compare].pattern, name) || match(name, base[compare].pattern))
			{
				matching = offset = &base[compare];
				return base[compare].conf;
			}

			pos = my_stricmp(name, base[compare].pattern);
...
char		*tomatch;
{
	...
	while (current < length)
	{
		if (!base[current].pattern)
			...
			if (!match(base[current].pattern, name))
