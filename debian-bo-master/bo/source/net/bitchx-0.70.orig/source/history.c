 ...
 * there. 
 */
static	char *history_match _((char *match))
{
	char	*ptr;
	char	*match_str = NULL;
	...
	if (get_int_var(HISTORY_VAR))
	{
		...
		{
			ptr = tmp->stuff;
			while (ptr && index(get_string_var(CMDCHARS_VAR), *ptr))
				ptr++;
...
void history(char *command, char *args, char *subargs)
{
	int	cnt,
		max;
	char	*value;
 ...
 */
char	*do_history _((char *com, char *rest))
{
	History *tmp;
	int	hist_num;
	char	*ptr, *ret = NULL;
