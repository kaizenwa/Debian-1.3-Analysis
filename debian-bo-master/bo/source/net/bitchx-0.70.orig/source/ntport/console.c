...
//
//
// These are hacks to do various cursor movement/attribute things
//
...
void NT_MoveToLineOrChar(int where,int line) {
	...
	dprintf("movetolineorchar relative called\n");
	if (hStdout == INVALID_HANDLE_VALUE){
		make_err_str(GetLastError(),errbuf,128);
...
char *tgoto(char *str,int hpos, int vpos) {


	NT_MoveToLineOrCharAbs(hpos,0);
	NT_MoveToLineOrCharAbs(vpos,1);
	return str;
}
