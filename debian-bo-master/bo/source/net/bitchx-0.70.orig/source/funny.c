	...
	else
	{
		if (channel)
		{
			...
			if (do_hook(current_numeric, "%s %s %s", from, channel, mode))
				put_it("%s", convert_output_format(get_string_var(FORMAT_MODE_CHANNEL_VAR), "%s %s %s %s %s", update_clock(GET_TIME), from, *FromUserHost ? FromUserHost:"ÿ", channel, mode));
		}
		else
		{
			if (do_hook(current_numeric, "%s %s", from, mode))
				put_it("%s", convert_output_format(get_string_var(FORMAT_MODE_CHANNEL_VAR), "%s %s %s %s %s", update_clock(GET_TIME), from, *FromUserHost ? FromUserHost:"ÿ", "", mode, ""));
		}
	}
}
