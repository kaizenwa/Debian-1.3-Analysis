...
#ifdef HAVE_STDARG_H
void	typed_add_to_whois_queue _((int, char *, void (*)(WhoisStuff *, char *, char *), char *, ...));
#else
void	typed_add_to_whois_queue();
#endif
...
void
set_beep_on_msg(win, str, unused)
	Window	*win;
	char	*str;
	int	unused;
{
 ...
 */
static	char	*whois_queue_head _((int server_index))
{
...
static	int whois_type_head _((int server_index))
{
	if ((WQ_head = (WhoisQueue *) get_server_qhead(server_index)) != NULL)
		return (WQ_head->type);
	else
...
static	void (*whois_func_head _((int server_index))) ()
{
	if ((WQ_head = (WhoisQueue *) get_server_qhead(server_index)) != NULL)
 ...
 * the calling routine 
 */
static	WhoisQueue *remove_from_whois_queue _((int server_index))
{
	WhoisQueue *new;
	...
	char	**ArgList;
{
	...
	else
	{
		...
		if (do_hook(current_numeric, "%s %s %s %s %s %s", from, nick,
			...
			UserList *tmp = NULL;
			ShitList *tmp1 = NULL;
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_HEADER_VAR), NULL));
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_NICK_VAR),"%s %s %s", nick, user, nsl?nsl:host));
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_NAME_VAR),"%s", name));

			malloc_sprintf(&userhost, "%s@%s", user, nsl ? nsl : host);
			if ((tmp = lookup_userlevelc(nick, userhost, "*", Bot_list)))					
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_BOT_VAR),"%d %d %s %s", tmp->aop, tmp->prot, tmp->channels, tmp->host));
			else if ((tmp = lookup_userlevelc("*", userhost, "*", user_list)))
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_FRIEND_VAR), "%d %d %d %s", tmp->level, tmp->aop, tmp->prot, tmp->host));
			if ((tmp1 = nickinshit(nick, userhost)))
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_SHIT_VAR),"%d %s %s %s", tmp1->level, tmp1->channels, tmp1->filter, tmp1->reason));
			if (tmp || tmp1)
			{
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_CHANNELS_VAR),"%s %s", tmp?"Allowed:":"Not-Allowed:", tmp?tmp->channels:tmp1->channels));
			}
			new_free(&userhost);
		}
	}
	...
	char	**ArgList;
{
	...
	else
	{
		...
		{
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOWAS_HEADER_VAR),NULL));
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOWAS_NICK_VAR),"%s %s %s", nick, user, host));
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_NAME_VAR),"%s", name));
		}
	}
	...
	char	**ArgList;
{
	...
	else
	{
		...
		if (do_hook(current_numeric, "%s %s", from, line))
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_CHANNELS_VAR),"%s", line));
	}
}
	...
	char	**ArgList;
{
	...
	else
	{
		...
		if (do_hook(current_numeric, "%s %s %s", from, server, line))
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_SERVER_VAR),"%s %s", server, line));
	}
}
	...
	else
	{
		...
		if ((nick = ArgList[0]) != NULL)
		{
			...
			if (do_hook(current_numeric, "%s %s %s", from, nick, ArgList[1])) 
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_OPER_VAR),"%s %s", nick, " (is \002NOT\002 an IRC warrior)"));
		}
	...
	char	**ArgList;
{
	if (!ignore_whois_crap)
	{
					...
					nick, idle_str, ArgList[2]))
		{
			...
			seconds = secs % 60;
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_IDLE_VAR),"%d %d %d",hours, minutes, seconds));
		}
	}
}
	...
	char	**ArgList;
{
	...
	if ((nick = ArgList[0]) != NULL)
	{
		...
		PasteArgs(ArgList, 0);
		message_from(NULL, LOG_CRAP);
		if (do_hook(current_numeric, "%s %s", from, ArgList[0]) && get_string_var(FORMAT_WHOIS_FOOTER_VAR))
			put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_FOOTER_VAR), NULL));
	}
}
		...
		**ArgList;
{
	...
	if (ptr && (whois_type_head(from_server) == WHOIS_ISON2) &&
	    !strcmp(ptr, nick))
	{
		WhoisQueue *thing;
	...
	message_from(NULL, LOG_CRAP);
	if (do_hook(current_numeric, "%s %s %s", from, nick, ArgList[1]))
		put_it("%s", convert_output_format(get_string_var(FORMAT_NONICK_VAR), "%s %s %s %s",update_clock(GET_TIME), nick, from, ArgList[1]));
	...
	if ((who = ArgList[0]) && (message = ArgList[1]))
	{
		...
		else
		{
			...
			if (do_hook(current_numeric, "%s %s", who, message))
				put_it("%s", convert_output_format(get_string_var(FORMAT_WHOIS_AWAY_VAR),"%s %s",who, message));
		}
		eat_away = 0;
	}
}
		...
		if (sed == 1 && !do_hook(ENCRYPTED_PRIVMSG_LIST,"%s %s", stuff->nick,text))
			...
			if (do_hook(MSG_LIST, "%s %s", stuff->nick, text))
			{
				if (away_set)
				{
					...
					t = time(NULL);
					put_it("%s", convert_output_format(get_string_var(FORMAT_IGNORE_MSG_AWAY_VAR), "%s %s %s", update_clock(GET_TIME), stuff->nick, msg));
					beep_em(get_int_var(BEEP_WHEN_AWAY_VAR));
				}
				else
				{
					put_it("%s", convert_output_format(get_string_var(FORMAT_IGNORE_MSG_VAR), "%s %s %s", update_clock(GET_TIME), stuff->nick, text));
	...
	if (stuff)
	{
		...
		if (is_ignored(ptr, IGNORE_NOTICES) != IGNORED)
		{
			...
			if (do_hook(NOTICE_LIST, "%s %s", stuff->nick, text))
				put_it("%s", convert_output_format(get_string_var(FORMAT_IGNORE_NOTICE_VAR), "%s %s %s", update_clock(GET_TIME), stuff->nick, text));
			set_lastlog_msg_level(level);
			sed = 0;
		}
		new_free(&ptr);
	...
	if (stuff)
	{
		...
		if (is_ignored(ptr, IGNORE_INVITES) != IGNORED)
		{
			if (do_hook(INVITE_LIST, "%s %s", stuff->nick, text))
				put_it("%s", convert_output_format(get_string_var(FORMAT_IGNORE_INVITE_VAR),"%s %s",stuff->nick, text));
			malloc_strcpy(&invite_channel, text);
			if (cget_int_var(AUTO_REJOIN_VAR, invite_channel))
	...
	if (stuff)
	{
		...
		if (is_ignored(ptr, IGNORE_WALLS) != IGNORED)
		{
			if (do_hook(WALL_LIST, "%s %s", stuff->nick, text))
				put_it("%s", convert_output_format(get_string_var(FORMAT_IGNORE_WALL_VAR), "%s %s %s", update_clock(GET_TIME), stuff->nick, text));
...
extern void got_userhost (WhoisStuff *stuff, char *nick, char *userhost)
{
	hard_uh_notify(stuff->not_on ? 0 : 1, nick, stuff->user, stuff->host);
	notify_mark(nick, stuff->not_on ? 0 : 1, 1);
	return;
}
	...
	char	*text;
{
	...
	if (flag != IGNORED)
	{
		...
		auto = check_autoreply(text);
		if (stuff)
		{
			if (do_hook(WALLOP_LIST, "%s %s %s", nick,
					(stuff->oper ? "+" : "-"), text))
				put_it("%s", convert_output_format(get_string_var(auto? FORMAT_WALLOP_AR_VAR:FORMAT_WALLOP_VAR),"%s %s %s",nick, stuff->oper? "+" : "-", text));
		}
		else
		{
...
void
#ifdef HAVE_STDARG_H
add_to_userhost_queue(char *nick, void (*func) _((WhoisStuff *, char *, char *)), char *format, ...)
#else
add_to_userhost_queue(nick, func, format, arg1, arg2,
   arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10)
void    (*func) ();
char    *format,
        *nick;
...
#else
	        typed_add_to_whois_queue(WHOIS_USERHOST, nick, func, format, arg1,
...
#if defined(__STDC__) && defined(HAVE_STDARG_H)
void	typed_add_to_whois_queue(int type, char *nick, void (*func) _((WhoisStuff *, char *, char *)), char *format, ...)
#else
