...
static	void	login_to_server _((int, int));
...
#endif
{
	...
	for (i = min; i < max; i++)
	{
		...
		if (server_list[i].pid != -1)
		{
			...
			server_list[i].pid = -1;
#else
			printf("kill server in server.c :170\n");
#endif
		}
	}
}
...
static	time_t	last_timeout = 0;
	...
	for (i = 0; i < number_of_servers; i++)
	{
		if ((des = server_list[i].write) != -1 && !(server_list[i].flags & LOGGED_IN))
		{
#ifdef NON_BLOCKING_CONNECTS
...
put_it("%s", convert_output_format(get_string_var(FORMAT_DISCONNECT_VAR), "%s %s", update_clock(GET_TIME), "No connection"));
				...
				default:
				{
					...
					if (x_debug & DEBUG_INBOUND)
						yell("[%d] <- [%s]", server_list[i].read, buffer);
					parse_server(buffer);
...
static void login_to_server(int refnum, int c_server)
{
	...
	doing_who = 0;
	who_on_join = 0;
	server_list[from_server].connected = 1;
	*server_list[from_server].umode = 0;
	server_list[from_server].operator = 0;
       	server_list[from_server].flags |= LOGGED_IN;  
	do_umode(from_server);
	add_delay_notify(from_server);
}
...
put_it("%s", convert_output_format(get_string_var(FORMAT_DISCONNECT_VAR), "%s %s", update_clock(GET_TIME), "You are not connected to a server. Use /SERVER to connect."));
	}
}
 ...
 */
int read_server_file _((char *servers_file))
{
	FILE *fp;
	int some = 0;
	char format[11];
...
/*
	...
	if (flags & USER_MODE_D)
		*c++ = 'd';
	if (flags & USER_MODE_E)
		*c++ = 'e';
	if (flags & USER_MODE_F)
		*c++ = 'f';
	if (flags & USER_MODE_G)
		*c++ = 'g';
	if (flags & USER_MODE_H)
		*c++ = 'h';
	if (flags & USER_MODE_I)
		*c++ = 'i';
	if (flags & USER_MODE_J)
		*c++ = 'j';
	if (flags & USER_MODE_K)
		*c++ = 'k';
	if (flags & USER_MODE_L)
		*c++ = 'l';
	if (flags & USER_MODE_M)
		*c++ = 'm';
	if (flags & USER_MODE_N)
		*c++ = 'n';
	if (get_server_operator(du_index))
		*c++ = 'o';
	if (flags & USER_MODE_P)
		*c++ = 'p';
	if (flags & USER_MODE_Q)
		*c++ = 'q';
	if (flags & USER_MODE_R)
		*c++ = 'r';
	if (flags & USER_MODE_S)
		*c++ = 's';
	if (flags & USER_MODE_T)
		*c++ = 't';
	if (flags & USER_MODE_U)
		*c++ = 'u';
	if (flags & USER_MODE_V)
		*c++ = 'v';
	if (flags & USER_MODE_W)
		*c++ = 'w';
	if (flags & USER_MODE_X)
		*c++ = 'x';
	if (flags & USER_MODE_Y)
		*c++ = 'y';
	if (flags & USER_MODE_Z)
		*c++ = 'z';
	*c = '\0';
*/
...
/* send_to_server: sends the given info the the server */
void 	my_send_to_server (int server, char *format, ...)
{
	char	buffer[BIG_BUFFER_SIZE + 1];	/* make this buffer *much*
						 * bigger than needed */
	char	*buf = buffer;
