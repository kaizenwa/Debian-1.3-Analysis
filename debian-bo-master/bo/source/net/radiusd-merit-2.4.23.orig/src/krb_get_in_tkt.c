...
/*
 * $Source: /usr/local/mnetauth/radius/RCS/krb_get_in_tkt.c,v $
 * $Author: web $
 *
...
"$Id: krb_get_in_tkt.c,v 1.3 1995/02/02 15:08:33 web Exp $";
...
char           *arg;
{
	...
	*(pkt->dat + (pkt->length)++) = (char) life;
	...
	switch (pkt_msg_type (rpkt) & ~1)
	{
		...
		/* Don't let ill formed error reply fake us out! */
		/* (This is a bug in MIT release of this module) */
		if ((kerror = (int) rep_err_code) == INTK_OK)
			kerror = INTK_BADPW;
		return (kerror);
	    default:
		return (INTK_PROT);
...
#endif				/* #if defined(A_KERB) || defined(M_KERB) */
