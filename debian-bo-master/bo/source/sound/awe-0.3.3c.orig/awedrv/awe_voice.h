...
/*#define _AWE_SET_GUS_BANK		0x09*/	/* obsolete */
...
/*#define _AWE_GET_CURRENT_MODE		0x10*/ /* removed */
...
{struct seq_event_rec tmp; tmp.arr[0] = SEQ_PRIVATE; tmp.arr[1] = dev;\
 tmp.arr[2] = _AWE_MODE_FLAG|(cmd); tmp.arr[3] = voice;\
 *(unsigned short*)&tmp.arr[4] = p1;\
 *(unsigned short*)&tmp.arr[6] = p2;\
...
/* set channel playing mode; mode=0/1 */
...
/* misc mode types */
enum {
/* 0*/	AWE_MD_EXCLUSIVE_OFF,	/* for compatibility; always off */
/* 1*/	AWE_MD_EXCLUSIVE_ON,	/* for compatibility; always on */
/* 2*/	AWE_MD_VERSION,		/* read only */
/* 3*/	AWE_MD_EXCLUSIVE_SOUND,	/* 0 or 1(default) */
/* 4*/	AWE_MD_REALTIME_PAN,	/* 0 or 1(default) */
/* 5*/	AWE_MD_GUS_BANK,	/* bank number (default 0) */
/* 6*/	AWE_MD_KEEP_EFFECT,	/* 0(default) or 1 */
	AWE_MD_END,
};
