...
char *Cirrus_Logic_c = "$Id: Cirrus_Logic.c,v 1.10 1997/02/01 08:31:13 marc Exp $";
...
int Cirrus_Logic_init (void)
     ...
     sprintf(buffer, "AT#VRL=%.0f", rec_range.min + ((rec_range.max -
      rec_range.min) * cvd.speaker_answer_volume.d.i/100.0));
     ...
     sprintf(buffer, "AT#VPL=%.0f", play_range.min + ((play_range.max -
      play_range.min) * cvd.speaker_play_volume.d.i/100.0));
     ...
     sprintf(buffer, "AT#VSST=%1u", cvd.rec_silence_threshold.d.i * 3 / 100);
...
static int Cirrus_Logic_set_compression ()
     {
