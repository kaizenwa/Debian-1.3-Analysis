...
/* Thus, searching the tree consists of running down the 'n' list */
 /* until the first character is matched, and then following the 'l' */
...
int     compress (un_char *outpath, int maxlen, int prefix) {
  int     out_fd = 0;
  int     suffix;
  int	  input_count = 0, output_count;
  int     max_bytes = 2048;	/* If you have better than 2048:maxlen */
				/* compression your packet size will be */
				/* smaller than maxlen. */
  char    *kludge;
  NODE * prfix, *pt;
  int     i,
  ...
  kludge = (char *) outpath;	/* this relies on 4 byte int's */
  for (i = 0; i < 260; ++i)	/* clear the output buffer (need to do */
  ...
  while (out_fd < lim && max_bytes >= 0) { /* while we have output less then */
	...
	pt -> num = comp_table_size++; /* Set the token number. */
				...
				/* Seeing as we are using explicit */
	...
	c_token_bits ++;
      ...
      put_token (outpath, &out_fd, (unsigned) prfix -> num); /* we output the */
						  /* token as well. */
  ...
  put_token (outpath, &out_fd, (unsigned) prfix -> num); /* Put the last token we */
					      /* had out. */
  output_count = (out_fd+BYTE_WIDTH_OUT - 1) / BYTE_WIDTH_OUT;
  stat_comp_out += output_count;
  return (output_count > input_count) ? -out_fd : out_fd;
}
