...
static print_header __APROTO((void));
static l_re_query __APROTO((char *pr, group_header *gh));
static merged_header __APROTO((void));
static disp_group __APROTO((group_header *gh));
...
print_header()
{
    ...
    if (current_group->group_flag & G_MERGED) tprintf("    MERGED");
...
fct_type menu;
{
    ...
    int status, was_unread, unread_at_reentry = 0, menu_cmd, o_killed;
    int o_only_unread;
