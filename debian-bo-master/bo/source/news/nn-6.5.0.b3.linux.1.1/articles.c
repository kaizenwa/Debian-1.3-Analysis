...
export int  ignore_re = 0;
export int  body_search_header = 0;
...
char *mask;
{
...
#ifdef ART_GREP
	if (s_keyboard || s_hangup) break; /* add to allow interrupt of GREP */
#endif
	...
	if (rexp != NULL) {
		...
		if (grep_article(gh, ah, (char *)rexp, regexec_cf)) goto match_ok;
#endif /* ART_GREP */
	    continue;
	} else
	if (mask != NULL) {
...
#ifdef ART_GREP
	    if (flags & (ACC_ON_GREP_UNREAD | ACC_ON_GREP_ALL))
		if (grep_article(gh, ah, mask, strmatch_cf)) goto match_ok;
#endif /* ART_GREP */
	    continue;
	}
     match_ok:
	...
	int nexta, roota, killa, newa;
        for (nexta = killa = newa = roota = 0; nexta < n_articles; nexta++) {
            ah = articles[nexta];
            if (ah->flag & A_ROOT_ART) roota = ah->replies & 127 ;
            if (roota && ah->replies && !auto_select_article(ah,1)) killa++;
            else articles[newa++] = ah;
        }
        n_articles -= killa;  
...
static int
grep_article(gh, ah, pattern, fcn)
  group_header *gh;
  article_header *ah;
  char *pattern;
  int (*fcn)();
{
	...
	buf[size] = 0; /* make buf a giant string so strchr below terminates */
	line = buf;
