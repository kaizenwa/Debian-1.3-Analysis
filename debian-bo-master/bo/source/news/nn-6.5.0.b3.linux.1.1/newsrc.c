...
static dump_file __APROTO((char *path, int mode));
static void dump_newsrc __APROTO((void));
static void dump_select __APROTO((void));
static time_t get_last_new __APROTO((void));
static void update_last_new __APROTO((group_header *lastg));
static article_number get_last_article __APROTO((group_header *gh));
static void init_rctest __APROTO((group_header *gh, register struct rc_info *r));
static attr_type rctest __APROTO((register article_header *ah, register struct rc_info *r));
static void append __APROTO(());
static void append_range __APROTO((int pp, int delim, article_number rmin, article_number rmax));
static void begin_rc_update __APROTO((register group_header *gh));
static void end_rc_update __APROTO((register group_header *gh));
static void mark_article __APROTO((register article_header *ah, int how));
...
static void
append_range(pp, delim, rmin, rmax)
int pp;
char delim;
...
register group_header *gh;
{
    ...
    if (!rc_merged_groups_hack) junk_seen = 0;
...
do_tidy_newsrc()
{
	...
	changed = 1;
     ...
     report:
	if (!silent) tprintf("%s%s\n", why, gh->group_name);
    }

    if (no_update) {
	if (changed) tprintf("Files were NOT updated\n");
	return;
    }
...
do_goback()
{
    ...
    if (goback_interact) {
	if (no_update) tprintf("Warning: changes will not be saved\n");
	init_term(1);
	nn_raw();
    }
	...
	if (count > 0) {
	    if (goback_interact) {
		tprintf("%s + %ld ?  (y) ", gh->group_name, (long)count); fl;
		y = yes(0);
		tputc(CR); tputc(NL);
		switch (y) {
		 case 1:
 ...
 out:
    ...
    if (no_update) tprintf("No files were updated\n");
}
