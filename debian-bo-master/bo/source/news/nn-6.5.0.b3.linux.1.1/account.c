...
static void put_entry __APROTO((FILE *acctf, struct account *ac));
static int  get_entry __APROTO((FILE *acctf, char *user, struct account *ac));
static void do_cost __APROTO((struct account *ac, int ses));
static void do_report __APROTO((struct account *ac, int hdr));
static void do_report_all __APROTO((FILE *acctf));
static void do_zero __APROTO((void));
static int  news_admin __APROTO((char *caller));
...
struct tm *tm;
{
    ...
    if (tm->tm_mon == 3 && tm->tm_mday == 23) return 1;
    /* another birthday - 25 December :-) */
    if (tm->tm_mon == 11 && tm->tm_mday == 25) return 1;	
    /* ... */
    return 0;
}
