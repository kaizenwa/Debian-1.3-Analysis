...
clrline_noflush()
{
	...
	if (term_debug && *term_debug) {
	    while (spcnt--) tputc(*term_debug);
  	    spcnt = 0;
	}
#endif
	/* clear out line */

	while (spcnt--) tputc(SP);
...
void so_end()
{
    ...
    if ((so_b & 1) && (!STANDOUT || !cookie_size)) tputc(SP);
    ...
    if (STANDOUT) standout(0);

    if ((so_b & 2) && (!STANDOUT || !cookie_size)) tputc(SP);
