...
rcstuff_init()
{
...
#ifdef USE_NNTP
    if (getenv("NEWSRC"))
	printf("NEWSRC environment variable ignored -- use DOTDIR to set the directory.\n") FLUSH;
#endif
    ...
    for (nextrcline = 0;
	(some_buf = get_a_line(buf,LBUFLEN,FALSE,rcfp)) != Nullch;
...
NG_NUM ngnum;
{
	...
	while ((some_buf = get_a_line(buf,LBUFLEN,FALSE,rcfp)) != Nullch) {
...
",stdout) FLUSH;
	...
	if (filestat.st_size == 0
	 && stat(rcbname,&filestat) >= 0 && filestat.st_size > 0) {
