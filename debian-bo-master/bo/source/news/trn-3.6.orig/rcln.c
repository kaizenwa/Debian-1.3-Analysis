...
int leave_count;
{
	...
	IF(verbose)
	    printf("\nMarking all but %d articles in %s as read.\n",
		   leave_count,rcline[ngx]) FLUSH;
	ELSE
#endif
#ifdef TERSE
	    printf("\nAll but %d marked as read.\n",leave_count) FLUSH;
#endif
	checkexpired(ngx, getngsize(ngx) - leave_count + 1);
    }
    else {
#ifdef VERBOSE
	IF(verbose)
...
register NG_NUM ngnum;
{
	...
	if (strNE(tmpbuf,rcline[ngnum]+rcnums[ngnum]))
	    checkexpired(ngnum,abs1st[ngnum]);	/* this might realloc rcline */
    }
