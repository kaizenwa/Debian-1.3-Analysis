...
# Project:      INN -- innfeed
# File:         procbatch.pl
# RCSId:        $Id: procbatch.pl,v 1.5 1996/08/26 21:35:37 brister Exp $
#
...
$spoolTop = "/var/spool/news" ;
$destDir = "/tmp" ;
$tapeDir = "/var/tmp/innfeed/tapes" ;
...
$usage = "$0 [ -q ][ -v ][ -u ][ -e host ][ -d dir ][ -c [ -s dir ]][ -m [-t dir ]] inn-batchfile\n
  ...
  -d dir\tto put the output file(s) in that directory
  -c\t\tto check pathnames of articles before storing them
  -s dir\tto specify the top of the news spool tree ($spoolTop)
  -m\t\tto have $0 move the new files to the tape directory.
  -t dir\tto specify the tape directory ($tapeDir)
  -u\t\tto unlink the input files when finished
  -v\t\tfor verbosity
  -q\t\tquiet mode; only display error messages. Good for cron jobs.

$0 will take inn-batchfile (normally a file in /var/spool/news/out.going),
...
that is the same as innfeed's 'tapefile' format. Simply rename these files
to peer1 peer2 peer3 in a running innfeed' tape directory and they will be
