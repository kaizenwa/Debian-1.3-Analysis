 ...
 * Project:     INN (innfeed)
 * File:        connection.c
 * RCSId:       $Id: connection.c,v 1.27 1996/12/07 01:31:46 brister Exp $
...
#if ! defined (lint)
static const char *rcsid = "$Id: connection.c,v 1.27 1996/12/07 01:31:46 brister Exp $" ;
                          ...
                          double lowPassLow)
{
  ...
  if (articleReceiptTimeout > MAX_MAXART_TOUT)
    {
      syslog (LOG_ERR, MAX_ARTTOUT_BIG, peerName, id, articleReceiptTimeout) ;
      croak = true ;
    }

  if (respTimeout > MAX_RESP_TOUT)
    {
      syslog (LOG_ERR, MAX_RESPTOUT_BIG, peerName, id, respTimeout) ;
      croak = true ;
    }
  ...
  cxn->sleepTimerId = 0 ;
  cxn->sleepTimeout = INITIAL_REESTABLISHMENT_PERIOD ;
      ...
      switch (code)
        {
          ...
          case 502:
            syslog (LOG_NOTICE,NO_TALK_NNRPD,peerName,cxn->ident,code,p) ;
            cxnSleep (cxn) ;
            hostCxnBlocked (cxn->myHost, cxn, rest) ;
            break ;
...
static void prepareReopenCbk (Connection cxn)
{
  ...
  cxn->sleepTimeout *= 2 ;
  if (cxn->sleepTimeout > MAX_REESTABLISHMENT_PERIOD)
    cxn->sleepTimeout = MAX_REESTABLISHMENT_PERIOD ;
}
...
static void issueIHAVEBody (Connection cxn)
{
  ...
  if (writeArray != NULL)
    ...
    {
      syslog (LOG_ERR,MISSING_ART_IHAVE_BODY,hostPeerName (cxn->myHost),
              cxn->ident) ;
      cxnWait (cxn) ;
    }
