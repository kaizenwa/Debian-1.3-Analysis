 ...
 * Project:     INN (innfeed)
 * File:        endpoint.c
 * RCSId:       $Id: endpoint.c,v 1.17 1996/12/07 02:28:42 brister Exp $
 *
...
#if ! defined (lint)
static char rcsid [] = "$Id: endpoint.c,v 1.17 1996/12/07 02:28:42 brister Exp $" ;
...
extern void addPointerFreedOnExit (char *pointerToFree) ;
              ...
              if (readyCount > 0 && ep != NULL) 
                {
                  ...
                  /* Every SELECT_RATIO times around this loop we check to
                     see if the mainEndPoint fd is ready to read. If so we
                     process it and do the current endpoint next time
                     around */
                  if (idx > 0 && mainEndPoint != NULL
                      && ep != mainEndPoint &&
                      (timesThrough % SELECT_RATIO) == 0)
                    {
                      fd_set trSet ;
                      struct timeval tw ;
  ...
  if (bCount > 0)
    {
      ...
      if (i > 0)
        ...
        rval = IoEOF ;
      else                   /* i < 0 && errno == EINTR || errno == EAGAIN */
        rval = IoIncomplete ;
