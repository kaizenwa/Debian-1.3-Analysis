...
time_t now;
{
	...
	if (fprintf(fp, "%s%c%ld%c%s%c", msgid, FIELDSEP,
	    (long)now, SUBFIELDSEP, expiry, SUBFIELDSEP) == EOF)
		fulldisk(art, filename);
	if (art->a_charswritten > 0 &&
	    fprintf(fp, "%ld", (long)art->a_charswritten) == EOF)
		fulldisk(art, filename);
...
boolean startlog;
{
	...
	if (startlog) {
		...
		now = time(&now);
	if (!openhist())
		persistent(art, '\0', "can't open history", "");
	else if (!writable)
		persistent(art, 'f', "no write permission on `%s'", filename);
	else if (fseek(fp, 0L, SEEK_END) == -1)
		/* could avoid fseek if still at EOF */
		persistent(art, 'f', "can't seek to end of `%s'", filename);
	else
		mkhistent(art, msgid, now, expiry);
	free(msgid);
	free(expiry);
...
struct article *art;
{
	transient(art, '\0', "article is a turkey", "");
	if (!opts.okrefusal)
...
closehist()
{
	...
	if (fp != NULL) {
		...
		if (dbmclose() < 0) {
			persistent(NOART, 'f', "error closing dbm history file",
				"");
			status |= ST_DROPPED;
		}
		if (nfclose(fp) == EOF) {
			persistent(NOART, 'f', "error closing history file",
				"");
			status |= ST_DROPPED;
		}
