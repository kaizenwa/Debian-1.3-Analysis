...
struct newsconf {
	boolean	nc_link;	/* link() works */
	boolean	nc_symlink;	/* symlink() works */
};
extern struct newsconf newsconf;
...
#define ST_DROPPED	(1<<4)	/* article was accidentally dropped - serious */
#define ST_DISKFULL	(1<<5)	/* disk full - give up fast */
...
#include "alloc.h"				/* ugh */
#define NOART ((struct article *)NULL)
