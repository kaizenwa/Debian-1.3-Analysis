# This Makefile is for the CGI extension to perl.
#
...
INST_PM = $(INST_LIBDIR)/CGI/Base.pm \
	$(INST_LIBDIR)/CGI/BasePlus.pm \
	$(INST_LIBDIR)/CGI/Carp.pm \
	$(INST_LIBDIR)/CGI/Form.pm \
	$(INST_LIBDIR)/CGI/MiniSvr.pm \
	$(INST_LIBDIR)/CGI/Request.pm \
	$(INST_LIBDIR)/CGI/test.pl


# --- MakeMaker const_loadlibs section:

# CGI might depend on some other libraries:
...
CCCMD = $(CC) -c -m486 -D__USE_BSD_SIGNAL -Dbool=char -DHAS_BOOL -O2
...
# installpm: CGI/Base.pm => $(INST_LIBDIR)/CGI/Base.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/Base.pm: CGI/Base.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/Base.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
$(INST_LIBDIR)/CGI/.exists :: $(PERL)
	@ $(MKPATH) $(INST_LIBDIR)/CGI
	@ $(TOUCH) $(INST_LIBDIR)/CGI/.exists


# installpm: CGI/BasePlus.pm => $(INST_LIBDIR)/CGI/BasePlus.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/BasePlus.pm: CGI/BasePlus.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/BasePlus.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
# installpm: CGI/Carp.pm => $(INST_LIBDIR)/CGI/Carp.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/Carp.pm: CGI/Carp.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/Carp.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
# installpm: CGI/Form.pm => $(INST_LIBDIR)/CGI/Form.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/Form.pm: CGI/Form.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/Form.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
# installpm: CGI/MiniSvr.pm => $(INST_LIBDIR)/CGI/MiniSvr.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/MiniSvr.pm: CGI/MiniSvr.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/MiniSvr.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
# installpm: CGI/Request.pm => $(INST_LIBDIR)/CGI/Request.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/Request.pm: CGI/Request.pm Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/Request.pm $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
# installpm: CGI/test.pl => $(INST_LIBDIR)/CGI/test.pl, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/test.pl: CGI/test.pl Makefile $(INST_LIBDIR)/CGI/.exists
	@ rm -f $@
	cp CGI/test.pl $@
	$(CHMOD) 644 $@
	@echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
MAP_TARGET    = perl
FULLPERL      = /usr/local/bin/perl
MAP_LINKCMD   = $(CC) -rdynamic
MAP_PERLINC   = -I./blib -I./blib -I/usr/lib/perl5 -I/usr/lib/perl5
MAP_STATIC    = ./blib/auto/CGI/CGI.a /usr/lib/perl5/auto/DynaLoader/DynaLoader.a
MAP_PRELIBS   = -lgdbm -ldbm -ldb -ldl -lm -lc -lbsd 

MAP_LIBPERL = /usr/lib/perl5/CORE/libperl.a

extralibs.ld: ./blib/auto/CGI/extralibs.ld /usr/lib/perl5/auto/DynaLoader/extralibs.ld
	@ rm -f $@
	@ $(TOUCH) $@
	cat ./blib/auto/CGI/extralibs.ld >> $@
...
./perlmain.o: ./perlmain.c
	cd . && $(CC) -I/usr/lib/perl5/CORE -c -m486 -D__USE_BSD_SIGNAL -Dbool=char -DHAS_BOOL -O2 perlmain.c
