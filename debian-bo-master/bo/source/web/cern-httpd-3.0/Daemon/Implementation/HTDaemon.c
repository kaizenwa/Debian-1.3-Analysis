...
#ifdef DEBIAN
/* This must be done outside of the ifndef, because the
   common Makefile overrides */
#define RULE_FILE               "/etc/cern-httpd.conf"
#endif /* not DEBIAN */
                             ...
                             CONST char *,      msg)
{
    ...
    if (req && req->output_stream) {
        HTStream * sink = req->output_stream;
        char * buf;
        int len = 500;
...
#ifdef DEBIAN
    /* Keep the pid file from being world writable.
       Based on the recommendation of Marek Michalkiewicz 
       <marekm@i17linuxb.ists.pwr.wroc.pl>, re Debian Bug #1655.
       CERN apparently agrees with this.
...
#ifdef LINUX_FSSTND
#ifdef DEBIAN
            StrAllocCat(fn, sc.pid_file ? sc.pid_file : "cern-httpd.pid");
#else
