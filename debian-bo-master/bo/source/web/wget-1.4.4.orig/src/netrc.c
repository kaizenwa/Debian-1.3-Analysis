/* Read and parse the .netrc file to get hosts, accounts, and passwords.
   ...
   Hrvoje Niksic <hniksic@srce.hr>, 1996
...
/* $Id: netrc.c,v 1.1.1.1.2.2 1997/02/17 20:18:05 hniksic Exp $ */
	     ...
	     int slack_default)
{
   ...
   if (!processed_netrc)
   {
      ...
      if (home)
      {
	 char *path = nmalloc(strlen(home) + 1 + strlen(NETRC_FILE_NAME) + 1);
      ...
      while (*p && last_token != tok_macdef)
	{
	  ...
	  switch (last_token)
	    ...
	    case tok_machine:
	      ...
	      current->host = nstrdup (tok);
	      break;

	    case tok_password:
	      if (current)
		current->passwd = nstrdup (tok);
	      else
	  ...
	  if (premature_token)
	    {
	      fprintf (stderr, "%s:%d: warning: \"%s\" token appears before any machine name\n",
