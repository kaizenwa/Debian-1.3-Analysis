...
image( )
{
	...
	if ( (fp = fopen( this_rp->filepath, "r")) == (FILE *) NULL ) {
		logerr( ERRMSG1, this_rp->filepath);
		wn_exit( 2);
	}
	...
	if ( *default_url && !res )
		dolocation( default_url, this_rp);
	else if ( !res) {
		if ( !coords_ok)
 ...
 * It returns a pointer to the char after n2.
 */
 ...
 */
	if ( segment( x - xclick, y - yclick) == HIT_IT) {
		dolocation( url, this_rp);  /* clicked on a vertex */
		return TRUE;
	}
