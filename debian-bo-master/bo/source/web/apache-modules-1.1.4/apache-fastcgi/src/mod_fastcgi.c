...
/*
 * Incoporate Tcl dynamic string library.
 *
...
int BufferWrite(Buffer *bufPtr, int fd)
{
    ...
    alarm(globalInfoPtr->reqPtr->server->timeout);
    BufferCheck(bufPtr);
...
{
  FcgiCleanUp(infoPtr);
  log_reason(msg, infoPtr->reqPtr->filename, infoPtr->reqPtr);
  longjmp(jmp_env, FCGI_ERROR);
  return SERVER_ERROR;
}

/*
...
int OS_CreateLocalIpcFd(OS_IpcAddress ipcAddress, int listenQueueDepth)
{
    ...
    chown(DStringValue(&ipcAddrPtr->bindPath), user_id, group_id);
    return listenSock;
		       ...
		       char *programName, char **envPtr)
{
    ...
    *childPid = fork();
    ...
    if(*childPid == 0) {
	    ...
	    fprintf(errorLogFd, "mod_fastcgi: unable to change uid\n");
	    return SERVER_ERROR;
	}
...
int StartFastCGIManager(FastCgiServerInfo *serverInfoPtr, cmd_parms *cmd)
{
    pid_t procManager;
      
    procManager = spawn_child(cmd->pool, RunFastCGIManager, serverInfoPtr, 
...

{
    ...
    */
    ...
    free(argv);
    argv = NULL;
    if (StartFastCGIManager(serverInfoPtr, cmd) != 0) {
        goto APP_CLASS_ERROR;
    }
...
void MakeExtraEnvStr(WS_Request *reqPtr)
{
  ...
  table_set(reqPtr->subprocess_env,"SERVER_ADDR",
		 inet_ntoa(reqPtr->server->host_addr));
}
void SetErrorLogFd(void *inp, int type)
{
  if (type) {
    cmd_parms *cmd = (cmd_parms *) inp;
    if (cmd && cmd->server && cmd->server->error_log)
      errorLogFd = cmd->server->error_log;
    else
      errorLogFd = stderr;
  }
  else {
    WS_Request *reqPtr = (WS_Request *) inp;
    if (reqPtr && reqPtr->server && reqPtr->server->error_log)
      errorLogFd = reqPtr->server->error_log;
    else
      errorLogFd = stderr;
  }
}
