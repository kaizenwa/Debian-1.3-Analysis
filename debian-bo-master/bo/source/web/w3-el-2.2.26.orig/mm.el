...
;;; Variables, etc
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defconst mm-version (let ((x "1.88"))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;(if (boundp 'MULE)
;;    (defun mm-insert-file-contents (filename &rest args)
;;      (let ((file-coding-system-for-read *noconv*))
;;        (insert-file-contents filename args)))
(fset 'mm-insert-file-contents 'insert-file-contents)
...
(defun mm-parse-args (st &optional nd allow-math)
  ...
  (let (
	...
	(while (not (eobp))
	  ...
	  (setq name (buffer-substring name-pos (point)))
	  (skip-chars-forward " \t\n")
	  (if (if (not allow-math)
		  (/= (or (char-after (point)) 0)  ?=) ; There is no value
		(not (memq (char-after (point)) '(?= ?* ?+ ?- ?/))))
	      (setq value nil)
	    (setq math (and allow-math
		  ...
		  (cond
		   ...
		   (t
		    (buffer-substring val-pos
					...
					(skip-chars-backward " \t")
					(point)))))))
	  (setq results (cons (if math
				  (cons name (cons math value))
				(cons name value)) results))
	  (skip-chars-forward "; \n\t"))
	results))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
