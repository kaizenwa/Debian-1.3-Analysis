...
void
cgi_url_hack(linebuf)
     char linebuf[];
{
  if (!strncmp(linebuf, "       url:", 11))
    {
      char urlbuf[256], *ptr1;

      strcpy(urlbuf, linebuf+12);
      /* remove trailing whitespace */
      for(ptr1=urlbuf+strlen(urlbuf)-1;
	  isspace(*ptr1) && ptr1 > urlbuf;
	  ptr1--) *ptr1 = '\0';

      printf("%11.11s <a href=\"%s\">%s</a>\n", linebuf, urlbuf, urlbuf);
    }
  else puts(linebuf);
  return;
}
...
cgi_standard_cso()
{
  ...
  aliasbuf[256], titlebuf[256], yearbuf[256], emailbuf[256], urlbuf[256];
  ...
  if (!ptr)
      ...
      while ((ptr=cgi_next_line_in_string(ptr, linebuf, 256)) != (char *)0)
	{
	  ...
	  /* make hyperlink in url: field */
	  if (cgi_cso_field(linebuf, "url", urlbuf, 256))
	    {
	      printf("%10s: <A HREF=\"%s\">%s</A>\n", "url", urlbuf, urlbuf);
	      continue;
	    }
