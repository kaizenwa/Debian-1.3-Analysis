...
static char rcsid[]="$Header: /mit/wwwdev/src/cgi/RCS/csoget.c,v 1.6 1996/10/03 13:36:34 brlewis Exp $";
     ...
     int origbufsize;
{
  ...
  /* Map underscores to spaces in query. */
  for (idx=query; *idx; idx++)
    if (*idx == '_') *idx = ' ';
  ...
  for (portnum=108; portnum >= 105; portnum -= 3)
    {
      ...
      /* connect */
      if (connect(sock, &sname, sizeof(sname)) < 0) continue;
     ...
     int bufsize;
{
  ...
  /* Remove leading trash */
  if (!strncmp(buf, "-200:1:motd:", 12))
    strcpy(buf, buf+12);
      ...
      do
	{
	  ...
	  if (!strncmp(colon1-3, "200", 3)) /* query results */
	      ...
	      if (!colon2 || *colon2 == '\012')	/* "Ok." or "Bye!" */
		{
		  ...
		  if (textline2)
		    {
		      /* Remove everything but the newline */
		      strcpy(textline1, textline2);
