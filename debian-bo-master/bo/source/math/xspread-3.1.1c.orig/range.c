...
int is_range;
{
	...
	scerror("Error: range name already defined");
	scxfree(name);
	return;
    }

    if (strlen(name) <= (unsigned)2) {
	scerror("Invalid range name - too short");
	scxfree(name);
	return;
    }
	...
	if (!((isalpha(*p) && (len<=2)) ||
	    ...
	    scerror("Invalid range name - illegal combination");
	    scxfree(name);
	    return;
	}
...
int row, col;
{
    ...
    v = lookat(row, col);
    if ((r = find_range((char *)0, 0, v, v)) != NULL) {
	return(r->r_name);
    } else {
...
int r1, c1, r2, c2;
{
    ...
    v1 = lookat(r1, c1);
    v2 = lookat(r2, c2);
    if ((r = find_range((char *)0, 0, v1, v2)) != NULL) {
	return(r->r_name);
    } else {
