...
/* finacial */
%token K_FV	/* @fv(e1,e2,e3) computes the future value */
%token K_IRR	/* @irr(range)	 computes the internal rate of return */
%token K_PMT
%token K_PV

/* time/date */
%token K_HOUR	/* @hour(e)	number of hours since midnight: 0 to 23 */
%token K_MINUTE	/* @minute(e)	the number of minutes past the hour: 0 to 59 */
%token K_SECOND	/* @second(e)	the number of seconds past the minute: 0-59 */
%token K_MONTH	/* @month(e)	Return the month: 1 (Jan) to 12 (Dec) */
%token K_DAY	/* @day(e)	Return the day of the month: 1 to 31 */
%token K_YEAR	/* @year(e)	Return the year.  Valid years begin with 1970*/
%token K_DATE	/* @date(e)	Convert the time in seconds to a date */
%token K_DTS	/* @dts(m,d,y)	Return m/d/y encoded in seconds since 1970 */
%token K_NOW	/* @now		Return the time encoded in seconds since 1970*/
%token K_TTS	/* @tts(h,m,s)	Return h:m:s encoded in seconds since midnight*/

/* type conversion */
%token K_FMT	/* @fmt(se,e)	Convert a number to a string using sprintf(3)*/
%token K_STON	/* @ston(se)	Convert string expr se to a numeric */
...
/* range commands */
	|	S_COPY range var_or_range 
		...
		/* destination conforms to the source */
	|	S_COPY var range
				{ copy($2.vp,NULL, $3.left.vp,$3.right.vp); }
...
/* file handling commands */
	...
	|	S_PUT strarg	
				{ (void) writefile ($2, 0, 0, maxrow, maxcol);
			 	scxfree($2); }
	|	S_READ strarg
				{ (void) readstrfile($2, currow, curcol,
					-1, -1);
			 	scxfree($2); }
	|	S_READ strarg range
				{ (void) readstrfile($2, ($3.left.vp)->row,
					($3.left.vp)->col,
					($3.right.vp)->row,
					($3.right.vp)->col);
			 	scxfree($2); }
	|	S_READ strarg var
				{ (void) readstrfile($2, $3.vp->row,
					$3.vp->col,
					-1, -1); 
			 	scxfree($2); }
