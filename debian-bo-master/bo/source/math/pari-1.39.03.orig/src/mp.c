...
squi(GEN x)
{
  long i,j,lx=lgef(x),lz,p1,p2,j2,k;
  GEN z;
  
  if(!signe(x)) return gzero;
  lz=(lx<<1)-2;if(lz>LGBITS) err(muler1);
  z=cgeti(lz);z[1]=evallgef(lz)+evalsigne(1);
  p1=x[lx-1];z[lz-1]=hiremainder=0;
  for(i=lx-2;i>=2;i--) z[lx+i-2]=addmul(p1,x[i]);
  z[lx-1]=hiremainder;
  for(j=lx-2;j>=3;j--)
  {
  ...
  k=0;for(j=lz-2;j>=3;j--) {z[j]=shiftl(z[j],1)+k;k=hiremainder;}
  z[2]=k;overflow=0;
  for(j2=lz,j=lx-1;j>=2;j--)
  {
    p1=x[j];p2=mulll(p1,p1);j2--;
    z[j2]=addllx(z[j2],p2);j2--;z[j2]=addllx(z[j2],hiremainder);
  }
  if(!(z[2]))
  {
...
convi(GEN x)
{
  ...
  p1=absi(x);lx=lgef(p1);lz=((lx-2)*15)/DIVCONVI+3;z=cgeti(lz);z[1]= -1;
...
dvmdii(GEN x, GEN y, GEN *z)
{
  ...
  else
  {
	...
	overflow=0;k=hiremainder;
      ...
      if(!overflow)
      {
	k3=subll(mulll(qp,saux),p1[i+2]);k4=subllx(hiremainder,k);
	while((!overflow)&&k4) {qp--;k3=subll(k3,saux);k4=subllx(k4,si);}
      }
      hiremainder=0;
      for(j=ly-1;j>=2;j--)
      {
	p1[i+j-1]=subll(p1[i+j-1],addmul(qp,p2[j]));hiremainder+=overflow;
      }
