...
# If we're in GNU m4 we need to undefine `format' before we
# can process fortran code
undefine(`format')

# Name of system platform (for use in comments)
define(`M4__SYSTEM', f2c)
...
# ')
#
# Note: This (and the use of integer*1 as BYTE_INT below) requires
#       f2c later then June 1993! If the f2c used is dated prior
#       to that date, the above M4__FORTRAN_DEFINES must be
#       activated, and the definition of BYTE_INT below must be
#       changed to integer.
#

# transformation from fortran name to name of C module
define(`NAMEF',`ifelse(index(`$1',`_'),-1,`$1_',`$1__')')	# f2c-convention
