 ...
 */
static char mrcsid[] = "$Id: cdftest.c,v 1.43 1996/02/22 23:17:09 steve Exp $";
...
/* #define NOBUF */
#include <stdio.h>
#include <errno.h>	/* included to prevent VMS warnings */
#include <stdlib.h>	/* for exit() for assert() */
...
/*	printf("title 2 \"%s\"\n", new) ; */
	...
	createtestvars(id, testvars, NUM_TESTVARS) ; 
 	...
 	{
 	nclong lfill = -1 ; double dfill = -9999 ;
