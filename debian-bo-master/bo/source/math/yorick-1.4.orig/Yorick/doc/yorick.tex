...
Copyright @copyright{} 1994.  The Regents of the University of California.
			 All rights reserved.
...
@titlepage
@sp 10
@center @titlefont{Yorick: An Interpreted Language}
@sp 2
...
Fortran uses the symbol @kbd{**} for that operation).@refill
...
200 closely spaced points by straight lines.@refill
...
if you do this accidentally, @pxref{Starting}).@refill
...
@kbd{omega} in the calling environment.@refill
...
later (@pxref{Broadcasting}); usually you get what you expected.@refill
...
arithmetic operation produces an array as its result.@refill
...
function need not have an explicit @kbd{return} statement:@refill
...
file object disappears, the associated file automatically closes.@refill
...
@kbd{file}) if and only if its condition (@kbd{!is_stream(file)}) is
true.  Any scalar number may serve as a condition -- a non-zero
value is ``true'', and the value zero is ``false''.@refill

The @kbd{is_stream} function returns 1 (true) when its argument is a
file object (a ``data stream''), and 0 (false) otherwise.  In
particular, if @kbd{file} is a text string (like @kbd{"q.out"}),
@kbd{is_stream} returns 0.  The unary operator @kbd{!} is logical
negation, that is, ``not''.@refill

Hence, if the @kbd{file} argument is not already a file object, the
new @kbd{q_out} presumes it is the name of a file, which it creates,
redefining @kbd{file} as the associated file object.  Thus, after the
first line of the function body, @kbd{file} will be a file object,
even if a file name was passed into the function.  Furthermore, since
the parameter @kbd{file} is local to @kbd{q_out}, none of this hocus
pocus will have any effect outside @kbd{q_out}.@refill

The second trick in the new @kbd{q_out} is the reappearance of a
@kbd{return} statement.  The original calling sequence:@refill
@example
q_out, 3, "q.out"
@end example
@noindent
...
object can be saved, which keeps the file open:@refill
...
clause itself an @kbd{if} statement:@refill
...
nothing at all happens if none of the conditions is satisfied.@refill
...
requires two statements, and the @kbd{else} three, write this:@refill
...
to avoid this type of inefficiency:@refill
...
behavior of @kbd{q_out} intact:@refill
...
@kbd{Q}, so scalar @kbd{Q} works as before.@refill
...
@kbd{body_statement} never executes.@refill
...
this case, use the @kbd{do while} statement:@refill
...
@kbd{while} loop, but not in a @kbd{for} loop.@refill
...
and @kbd{j}, a loop might look like this:@refill
...
in loop increment expressions.@refill
...
or both levels of a doubly nested loop:@refill
...
@kbd{break} just before @kbd{more_statements_B}.@refill

Break and continue statements may be used to escape from @kbd{while}
or @kbd{do while} loops as well.@refill
...
statement (note the inversion of the sense of the condition):@refill
...
subsequent calls to @kbd{q_out}.@refill
...
@kbd{E}, @kbd{m}, and @kbd{c} in it, then close the file:@refill
...
such a function call inside a function:@refill
...
@kbd{write}, or @kbd{plg}, come equipped with such comments.@refill
...
discipline, you can regard each include file as a library of
...
all of the documentation she needs to be able to use your package.@refill
...
writes them into a file suitable for printing.@refill
...
directory.@refill
@item
@file{Y_SITE/include}, where @file{Y_SITE} is the directory where
...
in your @file{~/Yorick} directory.@refill


@node Customizing,  , Include path, Include
@subsubsection The @file{custom.i} file
...
@file{Y_SITE/include/custom.i}.  (Yorick's @kbd{help} command will tell
you the actual name of the @file{Y_SITE} directory at your site.)@refill
...
plotting style preferences.@refill
...
through a series of related topics.@refill
...
go to the file and read the complete definition of the function.@refill
...
Read the @kbd{README} file in the @file{doc} directory.@refill
...
the C language, which gives them a precise meaning.)@refill
...
will work under any operating system.@refill
...
the @kbd{damped_wave} function to handle the overdamped case.@refill
...
function describes them all.@refill
...
starting with @kbd{0.0} and ending with @kbd{2*pi}.@refill
...
representing coordinates of the unit circle are defined with:@refill
...
equally spaced integer values.  By default, the list starts with 1.@refill

@item spanl(start, stop, number)
Use @kbd{spanl} (``span logarithmically'') to generate a list of numbers
which increase or decrease by a constant ratio.@refill
@end table
...
constant -- the simplest possible extrapolation rule.@refill
...
@kbd{interp} will evaluate.@refill
...
integral of the semicircle can be computed by:@refill
...
parameters specifying which function.@refill
...
which branch of a multi-valued function should be returned.@refill
...
directly; its name is @kbd{digitize}.@refill
...
rank of the resulting array, just like a scalar integer index value.@refill
...
operations of differential and integral calculus.@refill
@end itemize
...
represents the two element subarray @kbd{[x(3), x(5)]}.@refill
...
is the same as @kbd{x(10:8:-1)}.@refill

A useful special case of the index range default rules is @kbd{x(::-1)},
which represents the array @kbd{x} in reverse order.@refill
...
array with a single element, @kbd{[x(3)]}.@refill
...
array, consisting of the elements greater than 3.5.@refill
...
two-by-two-by-four array.@refill
...
of two vectors @kbd{x} and @kbd{y}:@refill
...
transpose of the array @kbd{outer}, you would write @kbd{x(-,)*y}.@refill
...
of 50 copies of @kbd{span(-10, 10, 100)}.@refill

If only the result @kbd{gauss2d} were required, a single default
pseudo-index would have sufficed:@refill
...
required -- as input to plotting routines for example.@refill
...
difference) range functions}.)@refill
...
@kbd{brightness} itself is a 1-D, 2-D, or 3-D array.@refill
...
@kbd{x(i,j,..,k,l)} are also legal, albeit rarely necessary.@refill
...
dimension (which I know is present).''@refill
...
to yield a single scalar result.@refill
...
except that the leading 0 value in the result is omitted.@refill
...
inverse of the @kbd{cum} function.@refill
...
are represented by 1-D arrays @kbd{y} and @kbd{x} of equal length is:@refill
...
one fewer line segment (slope) than point.@refill
...
and vice-versa.  For example:@refill
...
@kbd{deriv_pc2}, and @kbd{deriv_pc3}, or some other expression.@refill
...
number of points as the @kbd{x} and @kbd{y} arrays:@refill
...
Yorick statements compute the detector response:@refill
...
as the final dimension of @kbd{brightness}.@refill
...
its third dimension, and so on.@refill
...
of its first argument -- the array to be transposed.@refill
...
permutations, as shown in the sixth example.@refill
...
the polynomial at the corresponding @kbd{y}.@refill
...
each @kbd{y} being the value of the corresponding polynomial.@refill

A binary operation is performed once for each element, so that
@kbd{a+b}, say, means@refill
...
error'' when the shapes of binary operands do not match.@refill
...
such unit length indices when necessary (@pxref{Pseudo-Index}).@refill
...
a complicated dimension list:@refill
...
@kbd{(x(N),y(N))}, when x and y are N element arrays.@refill
...
have any dimensionality, as long as all four are the same shape.@refill
...
quadrilateral.  The plm command:@refill
...
families of lines (those with constant first or second index).@refill
...
algorithm Yorick uses to find contour polylines:@refill
...
resolve the contour you are trying to draw.@refill
...
N array of image pixel values.  (Currently, Yorick only plots black and
white or pseudocolor images.  True color images with three or more color
components per pixel would require a higher dimensional z array.)  The
optional x0, y0, x1, y1 arguments default to 0, 0, M, N.@refill
...
order of the z (or n) array.@refill
...
become hideously distorted when you switch to log axis scaling.@refill
...
dramatically slower than ordinary unrotated text.@refill
...
do not need to escape a trailing exclamation point.@refill
...
of RGB values, rather than by reading a palette file.@refill
...
function which writes a palette in Yorick's standard format.@refill
...
other programs steal all your colors.@refill
...
@kbd{"yellow"}, @kbd{"fg"}, and @kbd{"bg"}.@refill
...
for the window.  I'll return to graphics styles later.@refill
...
available with X11R4, Yorick permits only the two scale factors.@refill
...
@file{myfile.ps}.  The name of the file also implicitly determines the
file type; names of the form @file{*.ps} will be PostScript files, while
...
call hcp_file with @kbd{dump=1}.@refill
...
to create an EPS file called @file{yowza.epsi} containing that picture.
...
can then run ps2epsi by hand to convert this to an EPS file.@refill
...
Yorick's predefined graphics styles usually suffice.@refill
...
styles give portrait orientation.@refill
...
to enable the style= keyword to find them.@refill
...
descriptive legend with the @kbd{legend=} keyword:@refill
...
legends even from those graphics styles where they normally appear.@refill
...
preferences differ from Yorick's defaults.@refill
...
lines in your @file{custom.i} file:@refill
...
set them to a new value.@refill
...
distribution to learn more about making movies.@refill
...
well as @file{gray.gp} for shading).@refill
...
dfferent values of n for different surfaces or parts of surfaces.@refill
...
the idea is to use the @file{Make-cxx} template instead of the standard
@file{Maketmpl} to build versions of Yorick which include C++
packages.@refill
...
In the following discussion, I refer to three directories: @file{Y_SITE}
is the directory where the architecture independent parts of Yorick
reside; everything Yorick needs at runtime is here.  @file{Y_HOME} is
the directory where the libraries and executables you need to build
custom versions are stored.  @file{Y_LAUNCH} is the directory containing
...
start yorick and type @kbd{Y_HOME} to print the name of that directory.
...
Next, you need to write a Yorick include file @file{my_start.i} which
...
connects @kbd{my_func} to a compiled function @kbd{Y_my_func} of type
BuiltIn, which you are responsible for writing.  See
@file{Y_HOME/ydata.h} for the definition of the BuiltIn function type.
This function must pop its arguments off of Yorick's interpreter stack
using routines declared in @file{Y_HOME/ydata.h}, and push its result
back onto the stack.  The functions @kbd{YGet*} grab things off the
stack; @kbd{sp} is the stack pointer.  The @kbd{Globalize} function
gives you access to variables by the names visible to the interpreter;
they are in @kbd{globTab}.  You can use @kbd{Push*} functions to push
values onto the stack; use @kbd{CheckStack} to be sure there is enough
stack space to do so.

Usually, you want to avoid all these details; codger can generate
@file{Y_my_func} for you automatically.  To do this, put PROTOTYPE
comments in your startup include file:@refill
...
command (see @file{Y_HOME/include/mkdoc.i}).
...
In this comment, the four keywords @kbd{SRCS}, @kbd{LIB}, @kbd{DEPLIBS},
and @kbd{NO-WRAPPERS} are recognized.  You can use @kbd{\} at the end of
a line to continue it on the next line.  Only the @kbd{SRCS} keyword is
mandatory; you can either omit the others, or precede them by a @kbd{#}
to comment them out, as with @kbd{NO-WRAPPERS} in the example.@refill

The @kbd{SRCS} is a space delimited list of the source files required to
...
@kbd{.f} or @kbd{.F} suffixes alert Yorick to load with any special
libraries necessary for Fortran.

The @kbd{LIB} is the name of the library to be built for your package.
In the example, the library will be @kbd{libfrob.a}.  If you do not
...
The @kbd{DEPLIBS} is a space delimited list of system libraries that
your package needs.  You should not list @kbd{m} (the libm math
library), @kbd{X11}, or any other library which Yorick routinely loads
...
to edit the Makefile by hand to insert the proper @kbd{-L} options to
...
The @kbd{NO-WRAPPERS} keyword must be present if this startup include
...
after @file{my_start.i}.  The name @kbd{my_prog} is what your custom
version of Yorick will be called.@refill

You can load packages you created previously into your new Yorick by
giving their names after a @kbd{+} on this command line:
...
@file{Y_SITE/startup} or @file{Y_SITE/contrib}.  (Or you can make
softlinks to them from the current directory.)  The @kbd{LIB} libraries
mentioned in those startup include files must be in @file{Y_HOME/lib} or
@file{Y_HOME/lib/contrib} (or again have softlinks from your current
directory).  By default, the packages @file{matrix.i} and @file{fft.i}
will be included.  To omit them, place @file{-matrix} or @file{-fft}

The @file{make.i} script produces a Makefile which you can use to build
...
@file{Y_HOME/Maketmpl}.  (You can switch to another Makefile template if
you need to; the @file{Make-cxx} and @file{Make-mpy} templates are
...
can move your startup include files into @file{Y_SITE/contrib} or
@file{Y_SITE/startup} and your library into @file{Y_HOME/lib} or
@file{Y_HOME/lib/contrib} to make them ``visible'' for other package
...
as they reside -- Yorick looks in @file{Y_LAUNCH} for them as well as in
the standard places.
