/*
   ...
   $Id: hash.c,v 1.1 1993/08/27 18:32:09 munro Exp $
 */
...
int HashAdd(HashTable *table, const char *name, long n)
{
  if (HashFind(table, name, n)) {
    ...
    *item= NewHashItem(*item, nItems);
    table->names[nItems]= n>0? StrNCpy(name, n) : StrCpy(name);
...
/* hashSlots  - HashTable->nSlots will always be one of these numbers,
   ...
   hashCutoff - when HashTable->maxItems reaches these values, the
		...
		search. */
static long hashSlots[]=  {3,  7, 17, 31, 67, 127,
		           257, 521, 1031, 2053, 4099,  8191};
static long hashCutoff[]= {5, 12, 24, 49, 97, 192,
			   389, 776, 1542, 3076, 6145, 12287};
#define N_SIZES 12
...
void HashInit(HashTable *table, int estSize)
{
  ...
  table->nItems= 0;
  table->items= items= Ymalloc(sizeof(HashItem *)*hashSlots[i]);
  table->names= Ymalloc(sizeof(char *)*hashCutoff[i]);
  n= hashSlots[i];
  while (n--) (*items++)= 0;
}
...
void HashShrink(HashTable *table)
{
  ...
  if (nItems<=0) {
	...
	table->names= Yrealloc(table->names, sizeof(char *)*nItems);
	hashRealloc= 1;
      } else {
	hashRealloc= 0;
      }
...
static void Rehash(HashTable *table)
{
  ...
  if (newSlots!=nSlots) {
    ...
    itemSlots= nSlots;
    items= Ymalloc(sizeof(HashItem *)*newSlots);
  ...
  table->names= Yrealloc(names, sizeof(char *)*maxItems);
...
static MemoryBlock hashBlock= {0, 0, sizeof(union HashBlock),
...
static long SlotsRequired(long nItems, long *nSlots)
{
  ...
  if (i<N_SIZES) {
    /* Tables with 12287 or fewer entries have a number of slots
       within a factor of about 1.5 of their number of items.  */
    maxItems= hashCutoff[i];
    *nSlots= hashSlots[i];

  } else {
    /* Tables larger than 12287 always use 8191 slots; each call to
       SlotsRequired returns a maxItems of twice the input nItems.
       Performance will suffer, but we're talking conspicuous
       consumption here anyway.  */
    maxItems= 2*nItems;
