/*
    ...
    $Id: cache.c,v 1.1 1993/08/27 18:32:09 munro Exp $
 */
...
/* FLUSH_BYTES macro is in yio.h -- only thing needed here... */
#include "yio.h"
...
int CopyFile(IOStream *dst, IOStream *src)
{
  long address;
  CacheBlock *srcBlock;
...
static CacheBlock *NewBlock(IOStream *file, long addr, long last)
{
  ...
  block= Ymalloc(CACHE_HEADER+last-addr);
