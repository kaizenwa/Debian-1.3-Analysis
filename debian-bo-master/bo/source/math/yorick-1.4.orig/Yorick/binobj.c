...
static MemoryBlock arrayBlock= {0, 0, sizeof(Array),
				   64*sizeof(Array)};
...
Array *NewArray(StructDef *base, Dimension *dims)
{
  ...
  Array *array= (number*size>2*sizeof(double))?
    Ymalloc(sizeof(Array)+number*size) : NextUnit(&arrayBlock);
  array->references= 0;
...
void FreeArray(void *v)  /* ******* Use Unref(array) ******* */
{
  ...
  if (number*size>2*sizeof(double)) Yfree(array);
...
static MemoryBlock sdefBlock= {0, 0, sizeof(StructDef),
			          16*sizeof(StructDef)};
...
static MemoryBlock fourBlock= {0, 0, sizeof(union FourLongs),
