/*
    ...
    $Id: parse.c,v 1.1 1993/08/27 18:32:09 munro Exp $
 */
...
extern char *YpLitName(Literal);  /* hook for ScanForFunc in yorick.y */
...
/* number of local variables and number of referenced but undefined
   goto target labels */
static int nLocal, nTarget, zeroTarget;
...
static void ClearParser(void *func)
{
  ...
  nPos= nKey= nLocal= nTarget= zeroTarget= 0;
  hasPosList= 0;
...
static int CheckCodeSpace(long n)
{
  if (nextPC+n > vmCodeSize) {
    ...
    vmCode= Yrealloc(vmCode, sizeof(Instruction)*newSize);
...
static int CheckConstSpace(void)
{
  if (nConstants >= maxConstants) {
    ...
    constantTable= Yrealloc(constantTable, sizeof(Symbol)*newSize);
...
static void RecordLineNumber(long pc)
{
  if ((nYpReList&0xff)==0) {
    ...
    ypReList= Yrealloc(ypReList, sizeof(long)*newSize);
  }
...
static int ConstantReference(long pc)
{
  if (nConstantRefs >= maxConstantRefs) {
    ...
    constantRefs= Yrealloc(constantRefs, sizeof(long)*newSize);
...
static int VariableReference(Literal name)
{
  if (nVariableRefs >= maxVariableRefs) {
    ...
    variableRefs= Yrealloc(variableRefs, sizeof(long)*newSize);
...
static int TargetReference(Literal label)
{
  if (nGotoTargets >= maxGotoTargets) {
    ...
    gotoTargets= Yrealloc(gotoTargets, sizeof(long)*newSize);
...
static void PushMarkStack(int pushing)
{
  if (nMatrixMarkers >= maxMatrixMarkers) {
    ...
    matrixMarkers= Yrealloc(matrixMarkers, sizeof(MatrixMarker)*newSize);
...
static void PushBreak(int isBreak, long pc)
{
  if (nextBSP >= breakStackSize) {
    breakStack= Yrealloc(breakStack, sizeof(BreakStack)*(breakStackSize+16));
    breakStackSize+= 16;
  }
...
void YpEndInc(CodeBlock inc)
{
  ...
  if (nextInc+n+nLineNums+6 > incCodeSize) {
    ...
    incCode= Yrealloc(incCode, newSize*sizeof(Instruction));
    incCodeSize= newSize;
  }
...
CodeBlock YpFor(CodeBlock init, CodeBlock test, CodeBlock body)
{
  ...
  if (nextInc && incCode[nextInc-1].count==loopDepth) {
	...
	ypReList= Yrealloc(ypReList, sizeof(long)*size);
      }
      nextInc-= nLineNums;
...
CodeBlock YpLabel(Literal label)
{
  ...
  if (!(literalType&L_LABEL)) literalTypes[label]|= L_LABEL;
  else if (targetPC || zeroTarget) YpError("duplicate statement label");
  else nTarget--;
  if (!nextPC) zeroTarget= 1;
...
void YpFunc(int isMain, int eol)
{
  ...
  if (nConstants) {
    constantTable= Yrealloc(constantTable, nConstants*sizeof(Symbol));
