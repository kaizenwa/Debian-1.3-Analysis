/*
    ...
    $Id: yio.c,v 1.1 1993/08/27 18:32:09 munro Exp $
 */
...
extern UnaryOp EvalFN, EvalBI;   /* required by Print */
...
char *Yfgets(char *s, int n, FILE *stream)
{
  if (stream) return fgets(s, n, stream);
  else return YgetsIn(s, n);
}

int Yfeof(FILE *stream)
{
  if (stream) return feof(stream);
  else return 0;
}

int Yferror(FILE *stream)
{
  if (stream) return ferror(stream);
  else return 0;
}

static int gets_err_count= 0;

char *YDgetsIn(char *s, int n)
{
  char *line;
  /* Allow other input to trigger actions which do not require the
     Yorick interpreter, but always wait until standard input is
     ready to deliver the next line.  */
  if (YDispatch) do YDispatch(); while (!yPendingIn);
  line= fgets(s, n, stdin);
  if (!line) {
    *s= '\0';
    clearerr(stdin);
    gets_err_count++;
    if (gets_err_count>3) YError("(FATAL) unable to read stdin");
  } else {
    gets_err_count= 0;
  }
  return line;
...
int YDPrompt(const char *s)
{
  /* Note <stdio.h> might not contain any prototype for fputs or fflush,
     or it might contain the correct ANSI prototype, or it might contain
...
int YDputsErr(const char *s)
{
  /* Note <stdio.h> might not contain any prototype for fputs or fflush,
     or it might contain the correct ANSI prototype, or it might contain
     a K+R style function declaration...  */
  int val= fputs(s, stderr);
  val|= fputs("\n", stderr);
  fflush(stderr);
  return val;
}

char *(*YgetsIn)(char *s, int n)= &YDgetsIn;
int (*YPrompt)(const char *s)= &YDPrompt;
int (*YputsOut)(const char *s)= &YDputsOut;
int (*YputsErr)(const char *s)= &YDputsErr;

int yPendingIn= 0;

void YDDispatch(void)
{
  extern int yBatchMode;
  if (yBatchMode) YError("attempt to read keyboard in batch mode");
  yPendingIn= YstdinNB(0);
}

void YDMaybeDispatch(void)
{
  yPendingIn= YstdinNB(1);
}

void (*YDispatch)(void)= &YDDispatch;
void (*YMaybeDispatch)(void)= &YDMaybeDispatch;
...
char *Ygets(YgetsLine *getsLine, FILE *file)
{
  char *prev, *line= getsLine->line;
  int n, maxChars= getsLine->max;

  if (maxChars<MIN_LINE || maxChars>BIG_LINE) {
    StrFree(line);
    getsLine->line= line= StrAlloc(MIN_LINE);
  ...
  if (Yfeof(file) ||
      !Yfgets(line, maxChars+1, file) ||
      Yferror(file)) goto abort;

  n= strlen(line);
  while (line[n-1]!='\n' && !Yfeof(file)) {
    if (maxChars>=MAX_LINE) goto abort;
    prev= line;
    getsLine->line= line= StrAlloc(maxChars+INC_LINE);
    strcpy(line, prev);
    StrFree(prev);
    ...
    /* EOF is possible, in which case Yfgets returns 0 (ignored here)
       ...
       to return 0, and the loop terminates on the Yfeof test above.  */
    Yfgets(&line[n], maxChars+1-n, file);
    if (Yferror(file)) goto abort;
 ...
 */
extern char *YpNextLine(int context);
#define NL_MAIN 0
#define NL_NOINPUT 1
...
extern char *(*YaltNextLine)(int context);
...
extern UnaryOp PrintXFN, PrintXBI, PrintX;
...
extern int PutsAsArray(const char *s);
...
void PrintInit(int (*puts)(const char *))
{
  RawPrinter= puts;
  if (lenPrintBuf<printLength || (printLength<=79 && lenPrintBuf>79)) {
    ...
    printBuf= StrAlloc(printLength+1);  /* allow for newline */
  ...
  if (nArgs < 1) {
    ...
    if (arg->ops) {
      ...
      *typeFormat[i]= typeDefault[i];
      if (string!=typeDefault[i]) StrFree(string);
      string= YGetString(arg++);
      nArgs--;
      if (string && strlen(string)) *typeFormat[i]= StrCpy(string);
    }
  } while (--nArgs);
}
...
void PrintIO(Operand *op)
{
  ...
  printText= StrCat("  In directory: ", tmpText);
  SafeFree(&tmpText);
  PrintFunc(printText);
  SafeFree(&printText);
  ForceNewline();
  ...
  if (file->history) {
    ...
    if (history->recNumber>=0) {
      ...
      printText= StrCat("  File of current record: ", tmpText);
      SafeFree(&tmpText);
      PrintFunc(printText);
      SafeFree(&printText);
      ForceNewline();
...
static MemoryBlock ioflBlock= {0, 0, sizeof(IOFileLink),
...
char *YpReparse(void *function)
{
  ...
  if (position>=0) {
    ...
    if (nYpIncludes && ypIncludes[nYpIncludes-1].file) {
      fclose(ypIncludes[nYpIncludes-1].file);
