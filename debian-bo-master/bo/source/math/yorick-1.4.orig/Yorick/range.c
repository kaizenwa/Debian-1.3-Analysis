/*
    ...
    $Id: range.c,v 1.1 1993/08/27 18:32:09 munro Exp $
...
int RFsum(Array *array, int index)
{
  ...
  int typeID= ops->typeID;
  Array *result;
  if (typeID > T_COMPLEX) YError("bad data type in sum: range function");
  ...
  if (typeID!=T_COMPLEX) RFLoop(RawSum[typeID], array, result);
  else RFLoopC(&RFsumD, array, result);
  return 1;
}

#define RF_SUM(rfname, type1, type2) \
static void rfname(void *in, void *out) \
{ type1 *inp= in;  type2 *res= out, cur= inp[0];  long i; \
  for (i=step ; i<number ; i+=step) cur+= inp[i]; \
  res[0]= cur; }

RF_SUM(RFsumC, unsigned char, long)
RF_SUM(RFsumS, short, long)
RF_SUM(RFsumI, int, long)
RF_SUM(RFsumL, long, long)
RF_SUM(RFsumF, float, float)
RF_SUM(RFsumD, double, double)
...
static void rfname(void *in, void *out) \
{ type1 *inp= in;  type2 *res= out, cur= inp[0];  long i, n= 1; \
...
{ type1 *inp= in;  type2 *res= out;  long i; \
  for (i=0 ; i<number-step ; i+=step) res[i]= \
    0.5*((type2)inp[i+step]+(type2)inp[i]); }
...
{ type1 *inp= in;  type2 *res= out;  long i; \
  ...
  for (i=step ; i<number ; i+=step) res[i]= \
    0.5*((type2)inp[i]+(type2)inp[i-step]); res[i]= inp[i-step]; }
...
static void rfname(void *in, void *out) \
{ type1 *inp= in;  type2 *res= out;  long i;  res[0]= inp[0]; \
  for (i=step ; i<number-step ; i+=step) res[i]= 2.0*inp[i]-res[i-step]; }
