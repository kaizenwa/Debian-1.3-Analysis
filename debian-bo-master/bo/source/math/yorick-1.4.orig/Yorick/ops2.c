/*
    ...
    $Id: ops2.c,v 1.1 1993/08/27 18:32:09 munro Exp $
 */
...
{ double *dst= BuildResult2(l, r);
  if (dst) {
    long i, n= l->type.number;
    double *lv= l->value, *rv= r->value;
    double lr, li, rr, ri, nrm;          /* watch out for dst==lv or rv */
    for (i=0 ; i<n ; i++) {
      lr= lv[2*i];  li= lv[2*i+1];
      rr= rv[2*i];  ri= rv[2*i+1];
      if ((rr>0?rr:-rr)>(ri>0?ri:-ri)) { /* be careful about overflow... */
	nrm=rr; rr=1.0; ri/=nrm; nrm*=1.0+ri*ri;
      } else {
	nrm=ri; ri=1.0; rr/=nrm; nrm*=1.0+rr*rr;
      }
      dst[2*i]= (lr*rr+li*ri)/nrm;
      dst[2*i+1]= (li*rr-lr*ri)/nrm;
    }
    PopTo(l->owner);
  } else DivideError();
}
...
{ typd *dst= BuildResult2(l, r); \
  if (dst) { \
    long i, n= l->type.number; \
    typd *lv= l->value, *rv= r->value; \
    for (i=0 ; i<n ; i++) dst[i]= RMOD(lv[i],rv[i]); \
    Popper(l->owner); \
  } else ModuloError(); }
...
void PowerID(void) { Symbol *spr= sp--;
  sp->value.d= pow(sp->value.i, spr->value.d); sp->ops= &doubleScalar; }
void PowerIB(void)
...
void PowerLD(void) { Symbol *spr= sp--;
  sp->value.d= pow(sp->value.l, spr->value.d); sp->ops= &doubleScalar; }
void PowerLB(void)
...
void PowerDD(void) { Symbol *spr= sp--;
...
{ typd *dst= BuildResult1(l, r); \
  if (dst) { \
    ...
    for (i=0 ; i<n ; i++) dst[i]= powLL(lv[i], rv[i]); \
    Popper(l->owner); \
  } else PowerError(); }
...
{ typd *dst= BuildResult1(l, r); \
  if (dst) { \
    long i, n= l->type.number; \
    typd *lv= l->value; long *rv= r->value; \
    for (i=0 ; i<n ; i++) dst[i]= powDL(lv[i], rv[i]); \
    Popper(l->owner); \
  } else PowerError(); }
...
{ typd *dst= BuildResult2(l, r); \
  if (dst) { \
    long i, n= l->type.number; \
    typd *lv= l->value, *rv= r->value; \
    for (i=0 ; i<n ; i++) dst[i]= pow(lv[i], rv[i]); \
    Popper(l->owner); \
  } else PowerError(); }
...
{ double *dst= BuildResult2(l, r);
  if (dst) {
    long i, n= 2*l->type.number;
    double *lv= l->value, *rv= r->value;
    for (i=0 ; i<n ; i+=2) powZZ(&dst[i], &lv[i], &rv[i]);
    PopTo(l->owner);
  } else PowerError();
}
