 ...
 * GIST.H
 *
 * $Id: gist.h,v 1.1 1993/08/27 17:08:43 munro Exp $
 *
...
/* Change these to produce alternative versions of GIST.  */
#ifndef SINGLE_P
...
struct GpFillAttribs {
  ...
  int pattern;    /* index of F_PATTERN or F_HATCH */
  GpReal sizeX, sizeY;   /* pattern size in WC */
  GpReal refpX, refpY;   /* pattern reference point in WC */
...
struct GpTextAttribs {
  ...
  int orient;          /* text paths given by T_RIGHT, etc.  */
...
extern Drawing *GdNewDrawing(char *gsFile);
extern void GdKillDrawing(Drawing *drawing);
...
extern int GdSetDrawing(Drawing *drawing);
...
extern int GdClear(Drawing *drawing);
...
/* If changesOnly is non-zero, only new elements or damaged areas
   ...
   The Drawing* is also recorded on each engine.  */
...
/* The elIndex required by GdSetElement is NOT the id number returned
   ...
   total number of coordinate systems defined in the current Drawing.
...
extern int GdReadStyle(Drawing *drawing, const char *gsFile);
...
extern void GdDetach(Drawing *drawing, Engine *engine);
...
#define BLUE_COLOR (-7)
#define CYAN_COLOR (-8)
#define MAGENTA_COLOR (-9)
#define YELLOW_COLOR (-10)
...
/* Gist calls GmMalloc and GmFree for all of its incidental memory
   management needs.  By default, these are just malloc and free,
   but pointers to alternative memory mangers may be inserted by the
   calling application.
