/*
   ...
   $Id: drat.i,v 1.1 1993/09/02 22:48:38 munro Exp $
 */
 ...
 */
{
  ...
  n= numberof(times);
  if (!drat_quiet)
    write, "Processing "+pr1(n)+" records, now on record:";
  for (i=1 ; i<=n ; i++) {
    if (!drat_quiet) {
      write, format=" %3ld", i;
 ...
 */
{
  ...
  z= abs(tn);
  x= 1.0/(1.0+0.5*z);
  /* note that the "parentheses first" order of polynomial evaluation
     uses far less temporary space than the usual "parentheses last" */
  erfc= x*exp(-z*z+
	      ((((((((0.17087277*x-0.82215223)*x+1.48851587)*x-
		    1.13520398)*x+0.27886807)*x-0.18628806)*x+
		 0.09678418)*x+0.37409196)*x+1.00002368)*x-1.26551223);
  x= sign(tn);
  erfc= (1.0-x)+x*erfc;
  return gauss_norm*erfc;
}
