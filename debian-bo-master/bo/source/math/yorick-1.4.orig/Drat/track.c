/*
   ...
   D. Munro, Lawrence Livermore National Laboratory, 15 June 1990

   $Id: track.c,v 1.1 1993/09/02 22:48:38 munro Exp $
 */
...
EntryPoint *FindEntryPoints(Boundary *boundary, Ray *rayin)
{
   ...
   for (izsym= boundary->zsym?2:1 ; izsym ; izsym--) {
      ...
      for (i=0 ; i<boundary->npoints-1 ; i++, z++, r++, zone++, side++) {
	    ...
	    if (!newentry) {
	      long n= NBLOCK_SIZE;
	      nextEntry= (EntryPoint *)Dmalloc(sizeof(EntryPoint)*n);
...
extern void RayTrackS(Mesh *mesh, Ray *ray, RayPath *path, double *sLimits)
{
  ...
  if (limits) {
    ...
    if (smax >= -path->ds[0]) j2= 2*ncuts-1;
    else if (smax <= 0.0) {
...
void ExtendRayPath(RayPath *path, long pathinc)
{
   ...
   n= i+pathinc;
   if (i) {
      path->zone= (long *)Drealloc((void *)path->zone,n*sizeof(long));
      path->ds= (double *)Drealloc((void *)path->ds,n*sizeof(double));
      path->pt1= (long *)Drealloc((void *)path->pt1,n*sizeof(long));
      path->pt2= (long *)Drealloc((void *)path->pt2,n*sizeof(long));
      path->f= (double *)Drealloc((void *)path->f,n*sizeof(double));
   } else {
      path->zone= (long *)Dmalloc(n*sizeof(long));
      path->ds= (double *)Dmalloc(n*sizeof(double));
      path->pt1= (long *)Dmalloc(n*sizeof(long));
      path->pt2= (long *)Dmalloc(n*sizeof(long));
      path->f= (double *)Dmalloc(n*sizeof(double));
   }
   path->maxcuts= n;
}
