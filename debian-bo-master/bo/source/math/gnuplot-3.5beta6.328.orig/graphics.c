#ifndef lint
static char *RCSid = "$Id: graphics.c,v 1.126 1997/03/09 23:49:54 drd Exp $";
#endif
...
#if defined(NEED_TIME_T) /* HBB: was defined(sun386)*/
...
{
    int ytlen;
    int xtic_textheight, x2tic_textheight, ytic_textlength, y2tic_textlength;
    int top_margin;
...
#define KEY_PANIC(x) if (x) { lkey=0; goto key_escape; }
	...
	/*}}}*/
        ...
        /*{{{  adjust top and bottom margins if ytics or y2tics are rotated
	...
	title_y = x2label_y = ytop + x2ticdelta + (int)((t->v_char)*1.5) + x2tic_textheight;
        if(*x2label.text)
                title_y += (int)((x2lablin+0.5)*(t->v_char));
...
/* preliminary count of points inside array */ 
    ...
    gl = (int *) alloc ( goodcount * sizeof(int), "histeps valid point mapping" );
    if (gl == NULL) return;
