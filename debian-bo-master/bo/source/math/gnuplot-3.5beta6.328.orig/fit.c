#ifndef lint
static char *RCSid = "$Id: fit.c,v 1.50 1997/03/09 23:49:50 drd Exp $";
#endif
 ...
 *
 *	Copyright of this module:  1993, 1996  Carsten Grammes
 *
...
#ifdef NO_TMPFILE
/* KLUDGE ALERT!
   provide an implementation for pre-ANSI
   platforms. Unlike the ANSI function
   tmpfile(), the temprary file generated
   here must be deleted explicitly. lh
 */

extern char *getcwd __PROTO((char *, size_t));
extern char *getwd __PROTO((char *));


#define TEMPNAME "gfttemp.$$$"                  /* used for msdos and os/2 */
static char tmpfilename[256];
 
FILE *
tmpfile __PROTO((void))
{
    char path[256];
    char *tmpn = NULL;
    ...
    if (GP_GETCWD(path, sizeof path) == NULL)
        return NULL;
    ...
    sprintf (tmpfilename, "%s%c%s", path, '\\', TEMPNAME);
#else
# ifdef HAVE_TEMPNAM
...
struct value data;
{
	...
	last->next_udv = udv_ptr = (struct udvt_entry *)
	  alloc ((unsigned int)sizeof(struct udvt_entry), "fit setvar");
...
#ifdef MSDOS
/* sigh, work around 8+3 file name limit. lh */
    sprintf (tmpname, "%.8s", pfile);
    {
    	char *tmpn;
      if ( (tmpn = strchr (tmpname, '.')) != NULL)
	    *tmpn = '_';
	 }
    strcat (tmpname, ".bak");
#else /* MSDOS */
    sprintf (tmpname, "%s.bak", pfile);
#endif /* MSDOS */

#ifdef WIN32
    if ( rename (pfile, tmpname) ) {
/* Try short file name in case WIN32s is being used */
	sprintf (tmpname, "%.8s", pfile);
	strcat (tmpname, ".bak");
	remove (tmpname);   /* remove old version */
	if ( rename (pfile, tmpname) )
			Eex3 ("unable to rename %s to %s", pfile, tmpname)
    }
#else
    if ( rename (pfile, tmpname) )
	Eex3 ("unable to rename %s to %s", pfile, tmpname)
    ...
    if ( !(of = fopen (pfile, "w+")) )
	Eex2 ("parameter file %s could not be read(2)", pfile)

    fflush (nf);
    rewind (nf);
    while ( fgets (sstr, sizeof sstr, nf) != NULL ) {
	if ( fputs (sstr, of) == EOF) {
	    Eex ("I/O error during update")
	    break;
	}
    }
...
#ifdef NO_TMPFILE
/* you don't have remove() (ANSI), use unlink or delete (VMS). lh */
    if ( remove (tmpfilename) )     /* implies write permission !!!! */
	Eex2 ("temporary file %s could not be deleted", tmpfilename)
#endif /* NO_TMPFILE */
}
...
/* first read in experimental data */
    ...
    a = vec (MAX_PARAMS);
    par_name = (fixstr *) alloc ((MAX_PARAMS+1)*sizeof(fixstr), "fit param");
    num_params = 0;
...
/* HBB: maybe it would be better to realloc a and v instead ? */
    ...
    par_name = (fixstr *) ralloc (par_name, (num_params+1)*sizeof(fixstr), "fit param");
