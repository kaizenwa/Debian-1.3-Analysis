#ifndef lint
static char *RCSid = "$Id: set.c,v 1.58 1997/03/06 06:04:12 drd Exp $";
#endif
...
set_label()
{
    ...
    /* get justification */
    if (!END_OF_COMMAND && !almost_equals(c_token, "rot$ate") && !almost_equals(c_token, "norot$ate")) {
	   if (set_just)
...
set_linestyle()
{
    ...
    if (this_linestyle != NULL && tag == this_linestyle->tag) {
	   ...
	   new_linestyle = (struct linestyle_def *) 
		alloc ( (unsigned long) sizeof(struct linestyle_def), "linestyle");
