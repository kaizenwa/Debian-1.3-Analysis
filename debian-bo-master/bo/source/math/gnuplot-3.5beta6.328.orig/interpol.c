#ifndef lint
static char *RCSid = "$Id: interpol.c,v 1.24 1997/03/09 23:50:04 drd Exp $";
#endif
...
int num_points;			/* to determine end in plot->points */
{
    ...
    sc = (spline_coeff *) alloc((num_points) * sizeof(spline_coeff),
				"spline matrix");
    ...
    m = (five_diag *) alloc((num_points - 2) * sizeof(five_diag), "spline help matrix");

    r = (double *) alloc((num_points - 2) * sizeof(double), "spline right side");
    x = (double *) alloc((num_points - 2) * sizeof(double), "spline solution vector");
    h = (double *) alloc((num_points - 1) * sizeof(double), "spline help vector");
...
int first_point, num_points;
{
    ...
    sc = (spline_coeff *) alloc((num_points) * sizeof(spline_coeff), "spline matrix");
    m = (tri_diag *) alloc((num_points - 2) * sizeof(tri_diag), "spline help matrix");
...
struct curve_points *plot;
{
    ...
    new_points = (struct coordinate *) alloc(
