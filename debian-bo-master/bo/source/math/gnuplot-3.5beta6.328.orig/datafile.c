#ifndef lint
static char    *RCSid = "$Id: datafile.c,v 1.35 1996/12/19 19:12:31 drd Exp $";
#endif


/* GNUPLOT - datafile.c */
/*
 * Copyright (C) 1995 - 1996   David Denholm
 * 
	...
	*cols = 0;
	...
	for (;;)
	{
		...
		*cols = c;
		...
		if (*rows >= max_rows)
		{
			matrix = ralloc(matrix, (max_rows += 10)*sizeof(float *), "df_matrix");
		}

		/* allocate a row and store data */
		{
			int i;
			float *row = matrix[*rows] = (float *)alloc(c*sizeof(float), "df_matrix row");
