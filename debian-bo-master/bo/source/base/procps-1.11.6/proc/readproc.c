...
PROCTAB* openproc(int flags, ...) {
    ...
    if (!Do(PID) && !(PT->procfs = opendir("/proc")))
    	return NULL;
    if (Do(REAL)) flags |= PROC_FILLSTATUS;
    PT->flags = flags;
    ...
    else if (Do(UID) || Do(NOTUID) || Do(GID) || Do(NOTGID)) {
...
void status2proc (char* S, proc_t* P, int fill) {
    ...
    sscanf (tmp, 
        ...
        "VmStk: %lu kB\nVmExe: %lu kB\nVmLib: %lu kB\n"
        "SigPnd: %lx\nSigBlk: %lx\nSigIgn: %lx\nSigCgt: %lx",
        &P->pid, &P->ppid, &P->ruid, &P->uid, &P->suid, &P->fsuid,
        &P->rgid, &P->gid, &P->sgid, &P->fsgid, &P->vm_size, &P->vm_lock,
	&P->vm_rss, &P->vm_data, &P->vm_stack, &P->vm_exe, &P->vm_lib,
...
void stat2proc(char* S, proc_t* P) {
    ...
    *tmp = '\0';			/* replace trailing ')' with NUL */
    ...
    sscanf(tmp + 2,			/* skip space after ')' too */
	   "%c %d %d %d %d %d %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld "
	   "%ld %lu %lu %ld %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
...
next_proc:				/* get next PID for consideration */
	...
	if (!ent || !ent->d_name)
	    return NULL;
        if (Do(NOTPID) && XinLN (pid_t, (pid_t) atoi(ent->d_name), 
                                 PT->pids, PT->nuid))
            goto next_proc;
    ...
    if (!Do(REAL)){
        ...
        if (Do(NOTUID) && XinLN(uid_t, sb.st_uid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
        if (Do(GID) && !XinLN(uid_t, sb.st_gid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
        if (Do(NOTGID) && XinLN(uid_t, sb.st_gid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
    }
    ...
    if (Do(FILLSTAT)) {
        ...
        if (Do(FILLSTATUS)) {
            if ((file2str(path, "status", sbuf, sizeof sbuf)) == -1)
                goto next_proc;		/* error reading /proc/#/status */
            status2proc(sbuf, p, 0);            /* parse /proc/#/status */
        }
    } else if (Do(FILLSTATUS)) {
            if ((file2str(path, "status", sbuf, sizeof sbuf)) == -1)
                goto next_proc;  
    ...
    if (!matched && Do(NOSLEAD) && p->session == p->pid)
        goto next_proc;
    
    if (!matched && Do(NOGLEAD) && p->pgrp == p->pid)
        goto next_proc;

    if (!matched && Do(REAL)){
        if (Do(UID)   && !XinLN(uid_t, p->ruid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
        if (Do(NOTUID) && XinLN(uid_t, p->ruid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
        if (Do(GID)   && !XinLN(uid_t, p->rgid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
        if (Do(NOTGID) && XinLN(uid_t, p->rgid, PT->uids, PT->nuid))
            goto next_proc;		/* not one of the requested uids */
    }
