 ...
 *
 * usage: ldconfig [-DvnNX] [-f conf] [-C cache] dir ...
 *        ldconfig -l [-Dv] lib ...
 *        ldconfig -p
 ...
 *        -C cache: us cache instead of /etc/ld.so.cache
...
char *is_shlib(char *dir, char *name, int *type, int *islink)
{
    ...
    if (strncmp(name, "lib", 3) == 0 && name[strlen(name)-1] != '~' &&
	(cp = strstr(name, ".so")))
    {
	...
	else
	{
	    ...
	    *islink = S_ISLNK(statbuf.st_mode);
		...
		else if (N_MAGIC(exec) != ZMAGIC && N_MAGIC(exec) != QMAGIC)
		{
		    ...
		    {
			if (*islink || (good = readsoname(file)) == NULL)
			    good = xstrdup(name);
			else
			{
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "DvnNXlpf:C:")) != EOF)
	switch (c)
	{
	case 'D':
	    debug = 1;		/* debug mode */
	...
	case 'C':
	    ...
	    break;
	default:
	    fprintf(stderr, "usage: %s [-DvnNX] [-f conf] [-C cache] dir ...\n", prog);
