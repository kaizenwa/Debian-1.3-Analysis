  ...
  if (n_ext_modules_used)
    {
      ...
      sec = obj_create_alloced_section(f, ".kmodtab", sizeof(void*),
  ...
  if (flag_export && !obj_find_section(f, "__ksymtab"))
    {
      ...
      sec = obj_create_alloced_section(f, "__ksymtab", sizeof(void*), 0);
      ...
      for (nsyms = i = 0; i < HASH_BUCKETS; ++i)
	{
	  ...
	  for (sym = f->symtab[i]; sym; sym = sym->next)
		...
		&& (sym->secidx >= SHN_LORESERVE
		    || loaded[sym->secidx]))
	      {
		ElfW(Addr) ofs = nsyms * 2*sizeof(void*);

		obj_symbol_patch(f, sec->idx, ofs, sym);
		obj_string_patch(f, sec->idx, ofs+sizeof(void*), sym->name);

		nsyms++;
	      }
	}

      obj_extend_section(sec, nsyms * 2 * sizeof(char*));
    }
