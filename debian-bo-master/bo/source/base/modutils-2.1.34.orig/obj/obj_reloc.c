...
int
obj_allocate_commons_and_check_undefineds(struct obj_file *f)
{
  struct common_entry
  {
  ...
  if (f->string_patches)
      ...
      for (p = f->string_patches; p ; p = p->next)
	{
	  ...
	  if (targsec->header.sh_type == SHT_NOBITS)
	    {
	      /* Convert the section to PROGBITS so we can write to it.  */
	      targsec->contents = memset(xmalloc(targsec->header.sh_size),
					 0, targsec->header.sh_size);
	      targsec->header.sh_type = SHT_PROGBITS;
	    }
