 ...
 *
 * Version:	@(#)shutdown  2.5  18-Apr-1997  MvS
 *
...
char *Version = "@(#) shutdown 2.5 18-Apr-1997 MvS";
...
void fastdown()
{
  ...
  for(i = 0; i < 3; i++) if (!isatty(i)) close(i);
...
char **argv;
{
  ...
  if (doself && down_level[0] != '0' && down_level[0] != '6') {
	fprintf(stderr, "shutdown: can only use \"-n\" for halt or reboot.\r\n");
	exit(1);
  }
  ...
  /*
   *	Ignore all signals, but catch SIGINT.
   */
  for(c = 1; c < _NSIG; c++)
	signal(c, SIG_IGN);
