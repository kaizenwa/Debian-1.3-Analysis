 ...
 *
 * Version:	@(#)last  2.4  07-Feb-1997  MvS
 *
...
char *Version = "@(#) last 2.4 07-Feb-1997 MvS";
...
void usage(char *s)
{
  fprintf(stderr, "Usage: %s [-num | -n num] [-f file] [-R] [-x] [username..] [tty..]\n", s);
  exit(1);
}
...
struct utmp *get_one(FILE *fp)
{
  ...
  if (off < pos) {
	/* Search back */
	sz = pos;
	if (sz > UBUFS) sz = UBUFS;
	off = pos - 1;
	pos -= sz;
	if (pos < 0 || sz == 0) return NULL;
...
int main(int argc, char **argv)
{
  ...
  if ((fp = fopen(ufile, "r")) == NULL) {
	...
	if (altufile == NULL && x == ENOENT) fprintf(stderr,
  ...
  while(!quit && (u = get_one(fp)) != NULL) {
	ut = *u;
#endif
	if (ut.ut_time) lastdate = ut.ut_time;
	if (lastb)
