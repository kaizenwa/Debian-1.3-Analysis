...
void InitPushButton(struct XObj *xobj)
{
 ...
 if (xobj->icon==NULL)
 {
  i=xobj->xfont->max_bounds.ascent+xobj->xfont->max_bounds.descent+12;
  if (xobj->height<i)
   xobj->height=i;
 ...
 else
 {
  ...
  if (xobj->width<i)
   xobj->width=i;
  i=xobj->icon_h+2*(xobj->xfont->max_bounds.ascent+xobj->xfont->max_bounds.descent+10);
  if (xobj->height<i)
   xobj->height=i;
 }
