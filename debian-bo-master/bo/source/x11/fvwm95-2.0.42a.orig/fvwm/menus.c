...
int do_menu (MenuRoot *menu, int style)
{
  ...
  int PrevMenuX = PrevActiveMenuX; /* 1 */
...
void PaintEntry(MenuRoot *mr, MenuItem *mi)
  ...
  if ((mi->state)&&(mi->func_type != F_TITLE)&&
    ...
    XFillRectangle(dpy, mr->w, Scr.ScratchGC1, 3, y_offset,
                   mr->width-6, y_height);
    TextGC = Scr.HiGC;
    }
  else
  ...
  if(mi->func_type == F_NOP && *mi->item==0) 
    {
    DrawSeparator(mr->w, ShadowGC, ReliefGC, 2, y_offset-1+(HEIGHT_SEPARATOR >> 1),
  ...
  if (*mi->item)
    if (check_allowed_function(mi))
      {
      XDrawString(dpy, mr->w, TextGC, mi->x, text_y, mi->item, mi->strlen);
      }
    else
      {
      /* should be a shaded out word, no just re-colored. */
      if (!mi->state)
        XDrawString(dpy, mr->w, ReliefGC, mi->x+1, text_y+1, mi->item, mi->strlen);
      XDrawString(dpy, mr->w, ShadowGC, mi->x, text_y, mi->item, mi->strlen);
      }

  if (mi->strlen2>0)
    if (check_allowed_function(mi))
      {
      XDrawString(dpy, mr->w, TextGC, mi->x2, text_y, mi->item2, mi->strlen2);
      }
    else
      {
      /* should be a shaded out word, no just re-colored. */
      if (!mi->state)
        XDrawString(dpy, mr->w, ReliefGC, mi->x2+1, text_y+1, mi->item2, mi->strlen2);
      XDrawString(dpy, mr->w, ShadowGC, mi->x2, text_y, mi->item2, mi->strlen2);
      }
  ...
  if (mi->hotkey > 0)
    if (check_allowed_function(mi))
      {
      DrawUnderline(mr->w, TextGC, mi->x, text_y, mi->item, mi->hotkey - 1);
      }
    else
      {
      /* should be a shaded out underline, no just re-colored. */
      if (!mi->state)
        DrawUnderline(mr->w, ReliefGC, mi->x+1, text_y+1, mi->item, mi->hotkey - 1);
      DrawUnderline(mr->w, ShadowGC, mi->x, text_y, mi->item, mi->hotkey - 1);
      }

  if (mi->hotkey < 0)
    if (check_allowed_function(mi))
      {
      DrawUnderline(mr->w, TextGC, mi->x2, text_y, mi->item2, -1 - mi->hotkey);
      }
    else
      {
      /* should be a shaded out underline, no just re-colored. */
      if (!mi->state)
        DrawUnderline(mr->w, ReliefGC, mi->x2+1, text_y+1, mi->item2, -1 - mi->hotkey);
      DrawUnderline(mr->w, ShadowGC, mi->x2, text_y, mi->item2, -1 - mi->hotkey);
      }
  ...
  if (y + menu->height > Scr.MyDisplayHeight-2) 
    {
      ...
      /* Not with MWM menus! at least only in this case */
      if (menu_on < 1) /* was 2 */ /* 1 */
