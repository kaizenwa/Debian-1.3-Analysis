		  ...
		  int *delta_x, int *delta_y,Bool Grab)
{
  ...
  *delta_y = 0;
  ...
  /* Windows obscured by the scroll region won't receive 
   * EnterNotify events.  Broadcast this event so
   * applications that care (FvwmTaskBar) can decide what to do.
   */
  Broadcast(M_SCROLLREGION, 2, *xl, *yt, 0,0,0,0,0);
...
void  changeDesks(int val1, int val2)
{
  ...
  if((FocusWin)&&(FocusWin->flags & ClickToFocus))
    ...
    SetFocus(FocusWin->w, FocusWin,1);
