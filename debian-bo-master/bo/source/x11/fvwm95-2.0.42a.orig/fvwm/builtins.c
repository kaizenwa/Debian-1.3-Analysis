	      ...
	      unsigned long context, char *action, int *Module)
{
  ...
  else
  {
    ...
    if(val1 >0)
    {
      new_width = val1*val1_unit/100; /*-2; why?*/
      new_x = 0;
    }
    if(val2 >0)
    {
      new_height = val2*val2_unit/100; /*-2; why?*/
      new_y = 0;
    }
    if((val1==0)&&(val2==0))
    {
      new_x = 0;
      new_y = 0;
      new_height = Scr.MyDisplayHeight; /*-2; why?*/
      new_width = Scr.MyDisplayWidth;   /*-2; why?*/
    }
    tmp_win->flags |= MAXIMIZED;
		   ...
		   unsigned long context,char *action, int *Module)
{
  ...
  if (mystrncasecmp(action, "exec", 4) != 0)
    {
...
void SetHiColor(XEvent *eventp,Window w,FvwmWindow *tmp_win,
  ...
  if(Scr.d_depth > 2)
    {
    if(hifore != NULL) Scr.HiColors.fore = GetColor(hifore);
    if(hiback != NULL) Scr.HiColors.back = GetColor(hiback);
    }
  else
    {
...
void SetStdColor(XEvent *eventp,Window w,FvwmWindow *tmp_win,
  ...
  if(Scr.d_depth > 2)
    ...
    if (stdfore != NULL) Scr.StdColors.fore = GetColor(stdfore);
    if (stdback != NULL) Scr.StdColors.back = GetColor(stdback);
    Scr.WinRelief.back = GetShadow(Scr.WinColors.back);
    Scr.WinRelief.fore = GetHilite(Scr.WinColors.back);
    }
  else
    {
    ...
    Scr.WinRelief.back = GetColor(black);
    Scr.WinRelief.fore = GetColor(white);
    }
  gcm = GCFunction|GCPlaneMask|GCGraphicsExposures|GCLineWidth|GCForeground|
  ...
  gcv.foreground = Scr.WinRelief.fore;
  gcv.background = Scr.WinRelief.back;
  if(Scr.WinReliefGC != NULL) XFreeGC(dpy, Scr.WinReliefGC);
  Scr.WinReliefGC = XCreateGC(dpy, Scr.Root, gcm, &gcv);  

  gcv.foreground = Scr.WinRelief.back;
  gcv.background = Scr.WinRelief.fore;
...
void SetStickyColor(XEvent *eventp,Window w,FvwmWindow *tmp_win,
  ...
  if(Scr.d_depth > 2)
    ...
    {
    Scr.StickyColors.back = Scr.StdColors.back;
    Scr.StickyColors.fore = Scr.StdColors.fore; 
    }
...
void SetMenuStyle(XEvent *eventp,Window w,FvwmWindow *tmp_win,
  ...
  if ((font == NULL)||
    ...
    {
    fprintf(stderr,
            "[SetMenuStyle]: ERROR -- Couldn't load font '%s' or 'fixed'\n",
            (font==NULL)?("NULL"):(font));
    exit(1);
    }
  ...
  gcm = GCFont;
  gcv.font = Scr.StdFont.font->fid;
  if (Scr.HiGC != NULL)  XChangeGC(dpy, Scr.HiGC,  gcm, &gcv);
  if (Scr.WinGC != NULL) XChangeGC(dpy, Scr.WinGC, gcm, &gcv);
  if (Scr.WinReliefGC != NULL) XChangeGC(dpy, Scr.WinReliefGC, gcm, &gcv);
  if (Scr.WinShadowGC != NULL) XChangeGC(dpy, Scr.WinShadowGC, gcm, &gcv);
