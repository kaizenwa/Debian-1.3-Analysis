...
void SmartPlacement(ASWindow *t, int width, int height, int *x, int *y)
{
  int temp_h,temp_w;
  ...
  while(((test_y + temp_h) < (Scr.MyDisplayHeight))&&(!loc_ok))
      ...
      while(((test_x + temp_w) < (Scr.MyDisplayWidth))&&(!loc_ok))
	{
	  ...
	  while((test_window != (ASWindow *)0)&&(loc_ok == True))
		...
		{
    	  if(StubbornPlacement && (test_window->flags&ICONIFIED)&&
        	 ...
        	 (test_window != t))
        	{
        	  ...
        	  if((tx<(test_x+width+1))&&((tx+tw+1) > test_x)&&
            	 (ty<(test_y+height+1))&&((ty+th +1)>test_y))
            	{
            	  loc_ok = False;
            	}
        	}
	  ...
	  if(xl < 0)
	      ...
	      if(GrabEm(POSITION))
		{
		  ...
		  DragWidth += 2*tmp_win->boundary_width;
                  /* modified 02/17/97 */
		  DragHeight += 
		    tmp_win->title_height + NS_HANDLE_HEIGHT;
...
void GetGravityOffsets (ASWindow *tmp,int *xp,int *yp)
{
