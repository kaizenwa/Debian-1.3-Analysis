 ...
 *
 ****************************************************************************/
void move_window(XEvent *eventp,Window w,ASWindow *tmp_win,int context,
 ...
 *
 ****************************************************************************/
void moveLoop(ASWindow *tmp_win, int XOffset, int YOffset, int Width,
	      ...
	      Bool AddWindow)
{
  ...
  while (!finished)
	...
	case MotionNotify:
...
#ifndef NO_PAGER
	  if((Scr.ASPager != NULL)&&
	     (xl < Scr.ASPager->frame_x + Scr.ASPager->frame_width)&&
	     (xl+Width >  Scr.ASPager->frame_x)&&
	     (yt < Scr.ASPager->frame_y + Scr.ASPager->frame_height)&&
	     (yt+ Height >  Scr.ASPager->frame_y)&&(!opaque_move))
	    MoveOutline(Scr.Root,0,0,0,0);
#endif
	  ...
	  if(Scr.ASPager)
	    {
	      pagerwidth = Scr.ASPager->frame_width - 
		2*Scr.ASPager->boundary_width;
	      pagerheight = Scr.ASPager->frame_height - 
		Scr.ASPager->title_height - 2*Scr.ASPager->boundary_width;
...
void DisplayPosition (ASWindow *tmp_win, int x, int y,int Init)
{
  char str [100];
  int offset;
...
void InteractiveMove(Window *win, ASWindow *tmp_win, int *FinalX, int *FinalY, XEvent *eventp)
{
