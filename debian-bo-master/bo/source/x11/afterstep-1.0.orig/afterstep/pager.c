...
{
#ifndef NO_PAGER
  if((Scr.ASPager)&&(EnablePagerRedraw))
    XClearArea(dpy,Scr.Pager_w, 0, 0, Scr.ASPager->frame_width,
	       Scr.ASPager->frame_height,True);
#endif
}
...
void RaiseWindow(ASWindow *t)
{
	  ...
	  if (top->next != t2)
	    {
	      ...
	      if (t2->next) t2->next->prev = t2->prev;
	      t2->next = top->next;
	      t2->prev = top;
	      top->next->prev = t2;
	      top->next = t2;
  ...
  if (ttop == NULL)  ttop = top;  /* where to insert a non-ontop window */
  ...
  if (t->flags & ONTOP)
    {
      ...
      t->prev = &Scr.ASRoot;
      if (Scr.ASRoot.next)  Scr.ASRoot.next->prev = t;
      Scr.ASRoot.next = t;
    }
  else
    {
...
void MoveResizePagerView(ASWindow *t)
{
#ifndef NO_PAGER
  unsigned int width,height;
  int ww,wh;
  ...
  if((!(t->flags & STICKY)&&
	...
	{	      
	  ...
	  ww = t->icon_p_width*(int)width/MaxW;
	  wh = t->icon_p_height*(int)height/MaxH;
	}
      else 
	{
	  /* show the actual window */
