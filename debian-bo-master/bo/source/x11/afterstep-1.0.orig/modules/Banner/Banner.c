 ...
 *                                                                           
 *  Show Banner 
 *  Modified by Robert Depenbrock (robert@eclipse.asta.uni-essen.de)
 *                                                                           
 ***************************************************************************/
...
int main(int argc, char **argv)
{
  ...
  {
    fprintf (stderr,
	     "%s version %s should only be executed by afterstep\n",
	     myName,
	     VERSION);
    exit(1);
  }
  ...
  if (!(dpy = XOpenDisplay(display_name))) 
    {
      fprintf(stderr,"Banner: can't open display %s",
