 ...
 * Modified slightly by Dan Weeks
 ***********************************************************************/
  ...
  if(window_w > 0)
    {
      ...
      Scr.VScale = (Scr.VxMax + Scr.MyDisplayWidth)/window_w;
    }
  if(window_h > 0)
    {
      window_h = ((window_h - m)/(m+1))*(m+1)+m;
      Scr.VScale = (Scr.VyMax + Scr.MyDisplayHeight)/(window_h- label_h);
    }
  if(window_w <= 0)
...
void DrawGrid(int i, int erase)
{
  ...
  while(x < MaxW)
    {
      x1 = x*desk_w/MaxW;
      XDrawLine(dpy,Desks[i].w,NormalGC,x1,y1,x1,y2);
      x += Scr.MyDisplayWidth;
    }
...
void DrawIconGrid(int erase)
{
  ...
  while(x < MaxW)
    {
      x1 = x*icon_w/MaxW;
      XDrawLine(dpy,icon_win,NormalGC,x1,y1,x1,y2);
      x += Scr.MyDisplayWidth;
    }
  ...
  while(y < MaxH)
    {
      y1 = y*(icon_h)/MaxH;
      XDrawLine(dpy,icon_win,NormalGC,x1,y1,x2,y1);
      y += Scr.MyDisplayHeight;
    }
  n1 = Scr.Vx/Scr.MyDisplayWidth;
  m1 = Scr.Vy/Scr.MyDisplayHeight;
...
void LabelWindow(PagerWindow *t)
{
  ...
  if(t->PagerView != None)
    {
      XClearWindow(dpy, t->PagerView);
      XDrawImageString (dpy, t->PagerView,tgc,2,windowFont->ascent+2 , 
