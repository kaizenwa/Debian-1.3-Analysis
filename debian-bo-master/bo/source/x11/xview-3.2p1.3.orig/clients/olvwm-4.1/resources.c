...
#define RI_IMMUTABLE    (1<<0)      /* cannot be updated any more */
#define RI_LOCALE_DEP   (1<<1)      /* is locale dependent */
#define RI_LAST_ITEM    (1<<2)      /* this is the last item in the table */
#define RI_IMMUTABLE_AFTER_SET (1<<3) /* cannot be updated once initialized */
    ...
    unsigned long flags;
{
...
#if 1
	/* Items that never can be set don't make too much sense, so allow
	 * setting them once (only during InitGlobals, not UpdateGlobals)
	 */
	if ((item->flags & RI_IMMUTABLE)
	    || ((flags & SR_UPDATING) && (item->flags & RI_IMMUTABLE_AFTER_SET)))
#else
	...
	if (flags & SR_UPDATING) {
		...
		if ((flags & SR_IMMUTABLE) || (item->flags & RI_IMMUTABLE_AFTER_SET))
