...
#ifdef EXTRASMENU_PATH
    /* When setting RedefineTextExtrasMenu in XView.cf, the Imakefile adds a
     * compiler-flag that defines EXTRASMENU to the value set in XView.tmpl.
     * This is bad, because EXTRASMENU gets redefined above. So instead, we
     * set EXTRASMENU_PATH in the Imakefile and use that value here.
     */
    strcpy(tmp, EXTRASMENU_PATH);
#else
