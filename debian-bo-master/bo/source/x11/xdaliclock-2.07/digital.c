 ...
 * Copyright (c) 1991, 1992, 1993, 1994, 1995 Jamie Zawinski <jwz@netscape.com>
 *
...
#ifdef __STDC__
static struct frame *image_to_frame (XImage *image);
static void flush_segment_buffer (Display *dpy, Drawable drawable, GC draw_gc);
static void set_current_scanlines (struct frame *into_frame,
				   struct frame *from_frame);
static void draw_frame (Display *dpy, struct frame *frame, Drawable drawable,
			GC draw_gc, int x_off);
static void one_step (struct frame *current_frame, struct frame *target_frame,
		      int tick);
static void load_font (Display *dpy, char *fn);
static void load_builtin_font (Display *dpy, Visual *visual, int which);
static void get_resources (Display *dpy);
static void initialize_digits (Display *dpy, Visual *visual);
static void initialize_window (Display *dpy, Window window);
static void fill_pig_cache (Display *dpy, Drawable drawable,
			    struct frame *work_frame);
static long fill_time_digits (void);
     ...
     Window window;
{
  ...
  if (window != xgwa.root &&
      window != RootWindowOfScreen (DefaultScreenOfDisplay (dpy)))
...
static void
fill_borders (dpy, window, gc, x, y, width, height)
     Display *dpy;
     Window window;
     GC gc;
