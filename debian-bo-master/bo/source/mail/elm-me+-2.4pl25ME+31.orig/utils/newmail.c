...
char *argv[];
{
	...
	if((ptr = get_full_name(username)) != NULL)
	  strfcpy(full_username, ptr, sizeof full_username);
	else
	...
	**/
	...
	while (mail_gets(buffer, SLEN, fd) != 0) {
...
#else
	      first_word(buffer, "From ") && real_from(buffer, &hdr);
#endif /* MMDF */
	    continue;
	  }
	  ...
	  if (istrcmp(fld_name, "To:") == 0) {
	    figure_out_addressee(fld_val, hdr.to, sizeof hdr.to);
	    continue;
	  }
	...
	*/
	...
	if (cp > name+1) {
	  ...
	  *cp++ = '\0';		/* null terminate the filename & get prefix */
	  ...
	  strfcpy(folders[current_folder].prefix, cp,
		  sizeof folders[current_folder].prefix);
	  print_prefix = 1;
	}
	...
	*/
	...
	if (metachar(name[0]))
	  expand_filename(name, folders[current_folder].foldername,
			  sizeof folders[current_folder].foldername);
	else if (access(name, 00) == -1) {
	  /* let's try it in the mail home directory */
	  sprintf(buf, "%s%s", mailhome, name);
	  if (access(buf, 00) != -1) 		/* aha! */
	    strfcpy(folders[current_folder].foldername, buf,
		    sizeof folders[current_folder].foldername);
	  else
	    strfcpy(folders[current_folder].foldername, name,
		    sizeof folders[current_folder].foldername);
	}
	else
	  strfcpy(folders[current_folder].foldername, name,
...
add_default_folder()
{
	...
	if ((cp = getenv("MAIL")) == NULL)
	  ...
	  strfcpy(folders[0].foldername, cp,
		  sizeof folders[0].foldername);
...
struct folder_struct *cur_folder;
{
	...
	if (hdr->from[0] == '\0')
	  strfcpy(hdr->from, getusername(), sizeof hdr->from);
	if (hdr->subject[0] == '\0')
	...
	**/
	...
	for (i=0; i < total_folders; i++)
	  for (j = strlen(folders[i].prefix); j < len; j++)
	    strfcat(folders[i].prefix, " ", sizeof folders[i].prefix);
}
