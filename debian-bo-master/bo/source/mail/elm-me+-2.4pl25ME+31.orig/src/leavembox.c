	...
	if (to_keep > 0) {
	  if (to_store > 0) {
		...
		MCsprintf(buffer, catgets(elm_msg_cat, ElmSet, ElmLeaveKeepDeletePlural,
	      ...
	      if (to_keep == 1)
		strfcpy(buffer, catgets(elm_msg_cat, ElmSet, ElmLeaveKeep,
					"[Keeping message.]"),
			sizeof buffer);
	      else
		strfcpy(buffer, catgets(elm_msg_cat, ElmSet, 
					ElmLeaveKeepPlural,
	  ...
	  if (to_delete > 0) {
	    ...
	    if (to_store == 1)
	      strfcpy(buffer, catgets(elm_msg_cat, ElmSet, ElmLeaveStore,
				      "[Storing message.]"),
		      sizeof buffer);
	    else
	      strfcpy(buffer, catgets(elm_msg_cat, ElmSet, ElmLeaveStorePlural,
				     "[Storing all messages.]"),
		      sizeof buffer);
	  }
	} else {
	  if (to_delete > 0)
	    strfcpy(buffer, catgets(elm_msg_cat, ElmSet, ElmLeaveDelete,
