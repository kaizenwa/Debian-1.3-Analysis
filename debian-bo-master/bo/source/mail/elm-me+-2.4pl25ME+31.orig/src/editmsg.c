	...
	if (sourcebuf != NULL) {
	  ...
	  if (*sourcebuf != '\0') {
	    ...
	    if (strlen(sourcebuf) > 0) {
	      ...
	      strfcat(buffer, sourcebuf, sizeof buffer);
	    }
	  }
	}

	optionally_enter(buffer, -1, -1, OE_APPEND_CURRENT,
			 sizeof buffer);  /* already data! */
...
char *filename;
{
	...
	**/
	...
	if (fsize(edit_fd) > 0L)
	  strfcpy(buf, catgets(elm_msg_cat, ElmSet, ElmContinueEntering,
			       "\n\rContinue entering message."),
		  sizeof buf);
	else
	  strfcpy(buf, catgets(elm_msg_cat, ElmSet, ElmEnterMessage,
			      "\n\rEnter message."),
		 sizeof buf);
	strfcat(buf, catgets(elm_msg_cat, ElmSet, ElmTypeElmCommands,
