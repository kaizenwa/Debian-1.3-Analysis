...
static int get_aliasname P_((char *, char *, int *, int));
...
static int get_aliasname(aliasname, buffer, duplicate, size)
     ...
     int size_first, size_last, size_comment, size_buffer;
{
  ...
  sprintf(buffer, catgets(elm_msg_cat, AliasesSet, AliasesEnterLastName,
	...
	if (strlen(lastname) == 0) {
	  if (strlen(firstname) == 0) {
	    strfcpy(lastname, aliasname, size_last);  
	  }
	  else {
 ...
 */
	...
	PutLine2(elm_LINES-2,0, catgets(elm_msg_cat, AliasesSet, 
					AliasesAddressTo,
					"New alias: %s is '%s'."), 
		 aliasname, buffer);
	CleartoEOLN();
