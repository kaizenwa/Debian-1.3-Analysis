...
int add_new_only;
{
	...
	**/
	...
	"\n\nCouldn't allocate enough memory! Message #%d.\n\n"),
		...
		c = buffer;
		while (*c && isspace(*c)) ++c;
		strfcat(current_header->to, " ", 
			sizeof current_header->to);		  
		strfcat(current_header->to, c,
			sizeof current_header->to);
		...
		c = buffer;
		while (*c && isspace(*c)) ++c;
		strfcat(current_header->cc, " ",
			sizeof current_header->cc);
		strfcat(current_header->cc, c,
			sizeof current_header->cc);
	      } else
		in_cc_list = FALSE;
	    }
