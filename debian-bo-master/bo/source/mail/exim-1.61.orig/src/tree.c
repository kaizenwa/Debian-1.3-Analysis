...
{
tree_node *node = store_malloc(sizeof(tree_node) + (int)strlen(s));
strcpy(node->name, pfr? s : address_prepare(s));
node->data.ptr = NULL;
if (!tree_insertnode(&tree_nonrecipients, node)) store_free(node);
}
...
strcpy(node->name, pfr? s : address_prepare(s));
node->data.ptr = NULL;
if (!tree_insertnode(&tree_duplicates, node)) store_free(node);
}
...
tree_node *node;
char s[256];
sprintf(s, "T:%s:%s", h->name, h->address);
node = store_malloc(sizeof(tree_node) + (int)strlen(s));
strcpy(node->name, s);
node->data.val = h->why;
if (!tree_insertnode(&tree_unusable, node)) store_free(node);
}
...
/*************************************************
*          Free store used by a tree             *
*************************************************/

/* This disposes of an entire tree. We never free parts, because we never
actually remove anything from a tree.

Argument: the root of the tree
Returns:  nothing
*/

void
tree_free(tree_node *p)
{
if (p != NULL)
  {
  if (p->left != NULL) tree_free(p->left);
  if (p->right != NULL) tree_free(p->right);
  store_free(p);
  }
}
