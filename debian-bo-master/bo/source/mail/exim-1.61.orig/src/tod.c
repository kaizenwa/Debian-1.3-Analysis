...
/* Other types require the GMT offset to be calculated */
  ...
  /* Format used in BSD inbox separator lines. Sort-of documented in
  RFC 976 ("UUCP Mail Interchange Format Standard") but only by example,
  not by explicit definition. The examples show no timezone offsets, and some
  MUAs appear to be sensitive to this, so Exim has been changed to remove
  the timezone offsets that originally appeared. */

  if (type == tod_bsdin)
    {
    int len = strftime(timebuf, sizeof(timebuf), "%a %b %d %H:%M:%S", &local);
    /****
    (void) sprintf(timebuf + len, " %+03d%02d", diff_hour, diff_min);
    len += strlen(timebuf + len);
    ****/
    strftime(timebuf + len, sizeof(timebuf) - len, " %Y", &local);
    }

  /* Format used in Received: headers and SMTP greeting. */

  else
    {
    int len = strftime(timebuf, sizeof(timebuf), "%a, ", &local);
    (void) sprintf(timebuf + len, "%d ", local.tm_mday);
    len += strlen(timebuf + len);
