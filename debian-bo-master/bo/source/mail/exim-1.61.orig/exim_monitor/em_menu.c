...
{
char buffer[256];
Widget text = text_create((char *)client_data, 200);
FILE *f;
sprintf(buffer, "%s/msglog/%s", spool_directory, (char *)client_data);
...
Widget text = text_create((char *)client_data, 200);
FILE *f;
sprintf(buffer, "%s/input/%s-D", spool_directory, (char *)client_data);
...
if (editing)
  ...
  if ((pid = fork()) == 0)
    {
    sprintf(buffer, "xterm -n %s -title %s -e %s %s %s", id, id,
      exim_path, action, id);
    system(buffer);
    _exit(0);
    }
...
sprintf(buffer, "%s %s %s %s%s%s%s%s", exim_path, action, id,
  quote, address_arg, at, qualify, quote);

/* If we know we are going to need the window, create it now. */

if (action_output || delivery)
  {
  text = text_create(id, 200);
  text_showf(text, "%s\n", buffer);
  }
...
if (!delivery)
  ...
  if (rc == 0 && strcmp(action + (int)strlen(action) - 4, "-Mes") == 0)
    {
    queue_item *q = find_queue(id, queue_noop);
    if (q != NULL) q->sender = string_copy(address_arg);
    }
...
if (pid < 0) text_showf(text, "Failed to fork: %s\n", strerror(pid)); else
  {
  pipe_item *p = (pipe_item *)malloc(sizeof(pipe_item));
...
{
char *s;
s = XawDialogGetValueString(dialog_widget);
...
char *sender;
strcpy(actioned_message, (char *)client_data);
q = find_queue(actioned_message, queue_noop);
...
char buffer[256];
header_line *h, *next;
Widget text = text_create((char *)client_data, 500);

sprintf(buffer, "%s-H", (char *)client_data);
if (spool_read_header(buffer, TRUE) != spool_read_OK)
  {
...
if (recipients_list != NULL)
  ...
  for (i = 0; i < recipients_count; i++)
    {
    text_showf(text, "  %s %s\n",
      (tree_search(tree_nonrecipients, recipients_list[i]) == NULL)? " ":"*",
        recipients_list[i]);
    free(recipients_list[i]);
    }
  text_show(text, "\n");
...
while (p != NULL)
  ...
  while (pp != NULL)
    {
    if (pp == client_data) { p->widget = NULL; return; }
    pp = XtParent(pp);
    }
  p = p->next;
  }
...
menu_trans = XtParseTranslationTable(
