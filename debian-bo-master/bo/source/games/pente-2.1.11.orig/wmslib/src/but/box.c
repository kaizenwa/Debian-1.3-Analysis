...
void  but_drawBox(ButWin *win, int x, int y, int w, int h, int bstate,
  ...
  if (angles & BUT_ALEFT)  {
    ...
    pnum += curve(x+rounding,y+  rounding, rounding-bw,  90,90, points+pnum);
    pnum += curve(x+rounding,y+h-rounding, rounding-bw, 180,45, points+pnum);
    pnum += curve(x+rounding,y+h-rounding, rounding, 225,-45, points+pnum);
    pnum += curve(x+rounding,y+  rounding, rounding, 180,-90, points+pnum);
  ...
  if (angles & BUT_ARIGHT)  {
    ...
    pnum += curve(x+w-rounding,y+rounding, rounding, 90,-45, points+pnum);
    pnum += curve(x+w-rounding,y+rounding, rounding-bw,  0,45, points+pnum);
  ...
  if (angles & BUT_ALEFT)  {
    ...
    pnum += curve(x+rounding,y+h-rounding, rounding-bw, 270,-45, points+pnum);
    pnum += curve(x+rounding,y+h-rounding, rounding, 225,45, points+pnum);
  } else  {  /* Square left edge. */
    points[pnum  ].x = x + bw - xOff;
    points[pnum++].y = y + h - bw - yOff;
  ...
  if (angles & BUT_ARIGHT)  {
    ...
    pnum += curve(x+w-rounding,y+h-rounding, rounding, 270,90, points+pnum);
    pnum += curve(x+w-rounding,y+rounding, rounding, 0,45, points+pnum);
    pnum += curve(x+w-rounding,y+rounding, rounding-bw, 45,-45, points+pnum);
    pnum += curve(x+w-rounding,y+h-rounding, rounding-bw, 0,-90, points+pnum);
  } else  {  /* Square right edge. */
    points[pnum  ].x = x + w - xOff;
    points[pnum++].y = y + h - yOff;
    points[pnum  ].x = x + w - xOff;
...
void  but_drawCtb(ButWin *win, int flags, int fgpic, int bgpic, int pbgpic,
  ...
  but_drawBox(win, x, y, w, h, flags & (BUT_PRESSED|BUT_NETPRESS), bw,
