...
static void  sdraw(But *but, int x,int y, int w,int h)  {
  ...
  if (m->child == NULL)  {
    ...
    but_drawCt(win, flags, BUT_FG,BUT_BG,BUT_PBG,
	       ...
	       sbw, m->options[m->cval].text, BUT_SLEFT|BUT_SRIGHT, 0);
  }
}
...
static But  *lcreate(But *smenu, bool local)  {
  ...
  for (i = 0;  i < m->nvals;  ++i)  {
    if (m->options[i].text == BUTMENU_OLBREAK)  {
  ...
  m->clickOpen = FALSE;
  if (local)
    XWarpPointer(env->dpy, None, win->win, 0,0,0,0,
		 but->x + but->w/2, m->options[m->cval].y - butbw + sh/2);
  m->skip_clicks = TRUE;
  but_init(but);
...
static ButOut  lmmove(But *but, int x, int y)  {
  ...
  for (msel = 0;  msel < m->nvals;  ++msel)  {
    if (m->options[msel].text != BUTMENU_OLBREAK)  {
...
static void  ldraw(But *but, int x,int y, int w,int h)  {
  ...
  for (i = 0;  i <= m->nvals;  ++i)  {
    if ((m->options[i].text == BUTMENU_OLBREAK) ||
	(m->options[i].text == BUTMENU_OLEND))  {
...
static ButOut  ldestroy(But *but)  {
  ...
  XWarpPointer(env->dpy, win->win, win->win,
	       but->x, but->y, but->w, but->h,
	       sbut->x + sbut->w/2, sbut->y + sbut->h/2);
  m->child = NULL;
  return(0);
}
...
static int  menu_width(But *but)  {
  ...
  for (i = 0;  m->options[i].text != BUTMENU_OLEND;  ++i)  {
    if (m->options[i].text != BUTMENU_OLBREAK)  {
      tw = butEnv_textWidth(env, m->options[i].text, 0);
      if (tw > maxw)
	maxw = tw;
    }
  }
