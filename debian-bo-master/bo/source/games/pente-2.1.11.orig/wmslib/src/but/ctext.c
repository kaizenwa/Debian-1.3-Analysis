...
static bool  mouse_over_but(But *but, int x, int y)  {
  ...
  int  butbw = (but->win->env->font0h + 3) / 6;

  if (x < but->x + but->w/2)  {
    if (angleflags & BUT_ALEFT)  {
      if ((y < but->y + butbw) || (y >= but->y + but->h - butbw))
	return(0);
      if (y < but->y + but->h/2)
	if (x <= but->x + butbw + (but->y + but->h/2 - y))
	  return(0);
      else
	return(x > but->x + butbw + (y - (but->y + but->h/2)));
    } else if (angleflags & BUT_SLEFT)
      return((x >= but->x + butbw) && (y >= but->y + butbw) &&
	     (y < but->y + but->h - butbw));
  } else  {
	...
	return(0);
      if (y < but->y + but->h/2)
	return(x < but->x + but->w - butbw - (but->y + but->h/2 - y));
      else
	return(x < but->x + but->w - butbw - (y - (but->y + but->h/2)));
    } else if (angleflags & BUT_SRIGHT)
      return((x < but->x + but->w - butbw) && (y >= but->y + butbw) &&
	     (y < but->y + but->h - butbw));
  }
  /* We're dealing with the tough case...rounded edges! */
  radius = but->h / 4;
  radlimit = (radius - butbw + 1);
  radlimit *= radlimit;
  if (y < but->y + radius)  {
    if (x < but->x + radius)  {
      dx = but->x + radius - x;
