...
void  butWin_activate(ButWin *win)  {
  ...
  if (win->isIcon)  {
    ...
    if (win->wRatio != 0)  {
      ...
      sizeHints->min_aspect.x = win->wRatio;
      sizeHints->min_aspect.y = win->hRatio;
      sizeHints->max_aspect.x = win->wRatio;
      sizeHints->max_aspect.y = win->hRatio;
    }
...
void  butWin_checkDims(ButWin *win)  {
  ...
  if (win->wRatio)  {
    if ((win->w + 1) * win->hRatio - 1 < win->h * win->wRatio)
      win->w = (win->h * win->wRatio) / win->hRatio;
    if ((win->h + 1) * win->wRatio - 1 < win->w * win->hRatio)
      win->h = (win->w * win->hRatio) / win->wRatio;
  }
}
