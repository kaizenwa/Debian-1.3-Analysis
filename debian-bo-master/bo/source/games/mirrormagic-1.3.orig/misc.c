	      ...
	      int dest_x, int dest_y, int pad_x, int pad_y)
{
  ...
  XCopyArea(display,pix[PIX_DB_BACK],pix[PIX_DB_DOOR],gc,
	    dest_x-pad_x,dest_y-pad_y,
	    width+2*pad_x,height+2*pad_y, buf_x,buf_y);
  XSetClipOrigin(display,clip_gc[PIX_BACK],
		 buf_x-src_x+pad_x,buf_y-src_y+pad_y);
  XCopyArea(display,pix[PIX_BACK],pix[PIX_DB_DOOR],clip_gc[PIX_BACK],
