 ...
 * XBoard $Id: gamelist.c,v 1.6 1996/12/19 22:42:52 mann Exp $
 *
    ...
    GameInfo *gameInfo;
{
  if (appData.debugMode)
    ...
    /* We init these to -1 to let us know that we haven't seen
       a new "Creating:" mesage from ICS yet */
    gameInfo->whiteRating = -1;
    gameInfo->blackRating = -1;
    gameInfo->keepRatings = 0;
}
    ...
    GameInfo *gameInfo;
{
  ...
  if ( appData.debugMode )
    printf("CLEAR-GAME-INFO; keep=%d\n", gameInfo->keepRatings );
    ...
    if ( gameInfo->keepRatings ) {
      w = gameInfo->whiteRating;
      b = gameInfo->blackRating;
      GameListInitGameInfo(gameInfo);
      gameInfo->whiteRating = w;
      gameInfo->blackRating = b;
    } else
    GameListInitGameInfo(gameInfo);
}
     ...
     GameInfo *gameInfo;
{
    ...
    int len = 10 + strlen(event) + 3 + strlen(white) + 1 + 
      strlen(black) + 3 + strlen(date) + 1;
    char *ret = (char *) malloc(len);
    sprintf(ret, "%d  %s / %s-%s / %s", number, event, white, black, date);
    return ret;
}
