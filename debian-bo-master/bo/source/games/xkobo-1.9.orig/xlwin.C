...
win::~win()
{
    if (null_cursor){
        DBGP(null_cursor);
        XFreeCursor(disp, null_cursor);
    }
    if (null_bm){
        DBGP(null_bm);
        XFreePixmap(disp, null_bm);
    }
    if (icon){
        DBGP(icon);
        XFreePixmap(disp, icon);
    }
    if (w){
        DBGP(w);
...
void win::make(win *back,int wx,int wy,int sizex,int sizey)
{
    ...
    DBGP(w = XCreateSimpleWindow(disp,bk,x,y,sx,sy,
    ...
    DBGP(icon = XCreateBitmapFromData(disp, w, icon_bitmap, 64, 64));
...
void win::erase_cursor()
{
    ...
    DBGP(null_bm = XCreateBitmapFromData(disp,w,bmdata,1,1));
    if (null_cursor){
        DBGP(null_cursor);
        XFreeCursor(disp, null_cursor);
    }
    DBGP(null_cursor = XCreatePixmapCursor(disp,null_bm,null_bm,&cc,&cc,0,0));
