...
/* set up initial search path for file loads */
{
	...
	do {
	    ...
	    {
		fprintf(stderr, "Too many pattern libraries\n");
		exit(1);
	    }
	    *dirp++ = cp;
	} while
...
int add_loadreq(loadqueue, patname, relpath, loadtime)
    LoadReq **loadqueue;
    char *patname;
    int relpath;
...
int do_loadfile(seenlist,loadqueue,ismain)
    LoadReq **seenlist;
    LoadReq **loadqueue;
    int ismain;
{
