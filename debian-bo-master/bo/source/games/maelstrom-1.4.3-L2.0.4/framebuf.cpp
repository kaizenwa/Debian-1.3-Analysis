...
FrameBuf:: ScreenDump(char *prefix, Rect *frame)
{
	...
	/* We can only handle 8 bits-per-pixel video at the moment */
	if ( video_bpp > 1 )
		return(-1);
	...
	if ( !Grab_Area(frame->left, frame->top, width-1, height-1, &data) )
		return(-1);
	len = (height*width);
...
"Warning: image has more than %d colors!  (Corrupt?)\n", NUM_COLORS);
			...
			new_colors[ncolors] |=(Color_Map[index].blue&0xFF00)>>8;
			new_colors[ncolors] |=(Color_Map[index].green&0xFF00);
			new_colors[ncolors] |=(Color_Map[index].red&0xFF00)<<8;
	...
	*dest = newpixels;
	...
	switch (image_bpp) {
		case 1: /* Easy: 8-bit image to anything */
			while ( len-- ) {
				memcpy(Data, &Pixel_colors[*data++], video_bpp);
				Data += video_bpp;
			}
			break;
		case 2:
...
FrameBuf:: Clear(int DoRefresh)
{
	...
	for ( int row=0; row<HEIGHT; ++row ) {
		for ( int col=0; col<VWIDTH; col += video_bpp )
			CopyPixel(&shared_mem[row*VWIDTH+col], Black);
	}
	UnlockVideo();
	for ( int i=0; i<shared_len; i += video_bpp )
		CopyPixel(&backbuf[i], Black);
...
{
	if ( ! LockVideo() ) return;
	CopyPixel(&shared_mem[(y*pitch)+x*video_bpp], color);
	UnlockVideo();
	if ( ! faded )
		RefreshArea(x, y, 1, 1);
	CopyPixel(&backbuf[(y*VWIDTH)+x*video_bpp], color);
}
...
{
	if ( ! LockVideo() ) return;
	memcpy(&shared_mem[y*pitch+x*video_bpp], &color, video_bpp);
	UnlockVideo();
}
	...
	if ( Y1 == Y2 )  {  /* Horizontal line */
		...
		if ( ((slope < 0) ? slope > -1 : slope < 1) ) {
			...
			for ( x=lo; x<hi; ++x ) {
				...
				CopyPixel(&shared_mem[(y*pitch)+x*video_bpp],
									color);
			}
		} else {
			lo = (Y1 < Y2 ? Y1 : Y2);
					...
					unsigned long color)
{
	...
	for ( x=X; x<=maxx; ++x )
		CopyPixel(&shared_mem[(Y*pitch)+x*video_bpp], color);
	for ( x=X; x<=maxx; ++x )
		CopyPixel(&shared_mem[(maxy*pitch)+x*video_bpp], color);
	for ( y=Y; y<=maxy; ++y )
		CopyPixel(&shared_mem[(y*pitch)+X*video_bpp], color);
	for ( y=Y; y<=maxy; ++y )
		CopyPixel(&shared_mem[(y*pitch)+maxx*video_bpp], color);
	UnlockVideo();
}
					...
					unsigned long color)
{
	...
	while ( height-- > 0 ) {
		for ( sideways=width; sideways; --sideways ) {
			memcpy(&shared_mem[offset], &color, video_bpp);
			offset += video_bpp;
		}
		offset += loffset;
	}
						...
						unsigned char *area)
{
	...
	if ( ! LockVideo() ) return;
	for ( row=0; row<height; ++row ) {
		memcpy(&shared_mem[(row+y)*pitch+x*video_bpp],
					&area[row*width*video_bpp],
							width*video_bpp);
	}
	UnlockVideo();
}
	...
	for ( row=y; row<(y+height); ++row ) {
		for ( col=x; col<(x+width); ++col ) {
			...
			if ((bdata[offset/8]>>(7-(offset%8))) & 0x01) {
				offset = ((row*pitch)+col*video_bpp);
				CopyPixel(&shared_mem[offset], color);
			}
		}
	}
	UnlockVideo();
						...
						unsigned char *bdata)
{
	...
	for ( row=y; row<(y+height); ++row ) {
		for ( col=x; col<(x+width); ++col ) {
			...
			if ((bdata[offset/8]>>(7-(offset%8))) & 0x01) {
				offset = ((row*pitch)+col*video_bpp);
				CopyPixel(&shared_mem[offset], Black);
			}
		}
	}
	UnlockVideo();
						...
						unsigned char *data)
{
	...
	vheight = height;
	while ( vheight-- > 0 ) {
		memcpy(&shared_mem[offset], data, vwidth);
		offset += pitch;
		data += vwidth;
	}
	UnlockVideo();
	...
	for ( row=y; row<(y+height); ++row ) {
		for ( col=x; col<(x+width); ++col, sdata += video_bpp ) {
			...
			if ((mdata[offset/8]>>(7-(offset%8))) & 0x01) {
				offset = ((row*pitch)+col*video_bpp);
				memcpy(&shared_mem[offset], sdata, video_bpp);
			}
		}
	}
	UnlockVideo();
			...
			unsigned char *sdata, unsigned char *mdata)
{
	...
	for ( row=y; row<maxy; ++row ) {
		for ( col=(x+xoff); col<maxx; ++col ) {
			...
			if ( mdata[doffset] ) {
				doffset *= video_bpp;
				soffset = ((row*pitch)+col*video_bpp);
				memcpy(&shared_mem[soffset], &sdata[doffset],
								video_bpp);
			}
		}
	...
	for ( row=y; row<maxy; ++row ) {
		for ( col=(x+xoff); col<maxx; ++col ) {
			...
			if ( mdata[doffset] ) {
				...
				soffset = ((row*pitch)+col*video_bpp);
				memcpy(&shared_mem[soffset], &backbuf[boffset],
								video_bpp);
			}
		}
		mdata += width;
	...
	for ( ; ; ) {
		switch (sprite->ops[ops_i++]) {
			...
			case OP_DRAW:
				...
				memcpy(&shared_mem[offset], &pixels[pixels_i],
									len);
				offset += len;
				pixels_i += len;
				break;
...
{
	static unsigned char *black_mem = NULL;
	static unsigned char *saved_mem = NULL;
	int x, y;

	/* Get the black background memory, if needed */
	if ( ! black_mem ) {
		black_mem = new unsigned char[shared_len];
		saved_mem = new unsigned char[shared_len];
		faded = 1;
		return;		// Assume first fade is to black from black
	}

	if ( state == XFADE_OUT ) {
		if ( ! LockVideo() ) return;
		for ( y=0; y<HEIGHT; ++y ) {
			memcpy(&saved_mem[y*VWIDTH],
						&shared_mem[y*pitch], VWIDTH);
		}
		UnlockVideo();
		for ( y=0; y<shared_len; y += video_bpp )
			CopyPixel(&black_mem[y], Black);
	} else {
		faded = 0;
		if ( ! LockVideo() ) return;
		for ( y=0; y<HEIGHT; ++y ) {
			memcpy(&black_mem[y*VWIDTH],
						&shared_mem[y*pitch], VWIDTH);
		}
		for ( y=0; y<HEIGHT; ++y ) {
			for ( x=0; x<VWIDTH; x += video_bpp )
				CopyPixel(&shared_mem[y*pitch+x], Black);
		}
		UnlockVideo();
		Refresh();
	}
	...
	if ( ! LockVideo() ) return;
        for ( y=0; y<HEIGHT; y += 4 ) {
                for ( x=0; x<VWIDTH; x += 4*video_bpp ) {
			memcpy(&shared_mem[(y*pitch)+x],
					&black_mem[(y*VWIDTH)+x], video_bpp);
                }
        } 
	UnlockVideo();
	Refresh();
	if ( ! LockVideo() ) return;
        for ( y=0; y<HEIGHT; y += 4 ) {
                for ( x=3*video_bpp; x<VWIDTH; x += 4*video_bpp ) {
			memcpy(&shared_mem[(y*pitch)+x],
					&black_mem[(y*VWIDTH)+x], video_bpp);
                }
        } 
	...
	if ( ! LockVideo() ) return;
        for ( y=1; y<HEIGHT; y += 4 ) {
                for ( x=1*video_bpp; x<VWIDTH; x += 4*video_bpp ) {
			memcpy(&shared_mem[(y*pitch)+x],
					&black_mem[(y*VWIDTH)+x], video_bpp);
                }
        } 
	...
	if ( ! LockVideo() ) return;
        for ( y=1; y<HEIGHT; y += 4 ) {
                for ( x=2*video_bpp; x<VWIDTH; x += 4*video_bpp ) {
			memcpy(&shared_mem[(y*pitch)+x],
					&black_mem[(y*VWIDTH)+x], video_bpp);
                }
        } 
	...
	if ( state == XFADE_OUT ) {
		if ( ! LockVideo() ) return;
		for ( y=1; y<HEIGHT; ++y ) {
			memcpy(&shared_mem[y*pitch],
					&saved_mem[y*VWIDTH], VWIDTH);
		}
		UnlockVideo();
		faded = 1;
	}
