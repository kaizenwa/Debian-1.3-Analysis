...
static char *add_esc_string PARAMS ((char *str));

static void ps_cook_imf PARAMS ((void));
static void ps_initialize_imf PARAMS ((PsImage *psim));
static char *ps_hex_dump PARAMS ((Obj *data, int w, int h));
static char *img_untile PARAMS ((char *tiled, int w, int h, int istile,
			 int dither, int scale, int gray));
static void misc_init PARAMS ((void));
static void ps_initialize PARAMS ((FILE *fp));
static void page_init PARAMS ((FILE *fp, int page, int i, int j, char *stime));
static int nearest_valid_x PARAMS ((int x, int y));
static int sideno_of_seen_unit_at PARAMS ((int x, int y, Side *side));
static char *name_of_seen_unit_at PARAMS ((int x, int y, Side *side));
static char *summary_of_seen_units_at PARAMS ((int x, int y, Side *side));
static int ps_string_width PARAMS ((char *str, int font));
static int print_unit_legends PARAMS ((FILE *fp, char *name, char *summary, char *m,
